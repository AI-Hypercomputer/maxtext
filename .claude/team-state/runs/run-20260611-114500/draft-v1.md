...
<ul>
  ...
  <li><code>micro_batch_size_to_train_on = per_device_batch_size * num_devices_per_pod = 24</code> (single-node)</li>
  <li><code>pipeline_microbatch_size = 24 / 64 = 0.375</code>... actually computed as <code>micro_batch_size_to_train_on / num_pipeline_microbatches</code></li>
</ul>
...
<div class="callout">
<strong>Why a proxy?</strong> The real DeepSeek-V2/V3 uses MLA (Multi-Latent Attention), a specialized decoder block, and 160+ layers. This proxy replaces those with standard global attention and the Mixtral decoder, keeping the MoE router and pipeline geometry identical while being cheaper to compile and profile.
</div>
...
<p>
<code>.lower()</code> converts the Jaxpr to an XLA <code>HloModule</code>. The SPMD partitioner runs during this phase, converting logical sharding annotations into physical device assignments. Key HLO ops that appear:
</p>
...
<p>
<code>.compile()</code> runs the full XLA compiler pipeline. The passes relevant to tmem:
</p>
...
<pre><code>checkpoint_size = num_stages * num_layers_per_stage * [micro_size, seq_len, emb_dim] * sizeof(bf16)
                = 8 * 4 * [pipeline_microbatch_size, 4096, 4096] * 2 bytes</code></pre>
...
<svg viewBox="0 0 850 280" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:850px;">
  ...
  <text x="430" y="172" fill="#f85149" font-size="10" font-weight="600">+9 GB tmem</text>
...
<p>
When <code>pipeline_save_decoder_layer_input = true</code> (the default), each of the <code>num_layers_per_pipeline_stage</code> layers per stage saves its input. With 4 layers per stage and 8 stages, that is 32 checkpoint buffers of shape <code>[micro_size, 4096, 4096]</code> in bf16. These must remain live across the full forward-then-backward lifetime of the scan, making them the dominant tmem contributor.
</p>
...
<ol>
  ...
  <li><strong><code>shard_map</code> with <code>check_vma=True</code></strong>: The <code>check_vma</code> (Verified Memory Annotation) flag forces the compiler to verify that the output sharding matches the declared <code>out_specs</code>. This often requires materializing an explicit copy of the output, adding another full-size buffer to tmem.</li>
</ol>
...
<p>
The key insight is that <code>lax.slice_in_dim</code> and <code>jnp.concatenate</code> along axis 0 (the stage axis) are <em>local operations on the full array</em>. XLA can often implement them as buffer views (pointer arithmetic) with zero copies, especially when the slice boundaries are statically known. Even when a copy is needed, it is a single local memcpy, not a multi-device collective with separate send/recv buffers.
</p>
...
<svg viewBox="0 0 850 400" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:850px;">
  ...
  <text x="20" y="187" fill="#8b949e" font-size="10">The full [num_stages, ...] array lives on each device (SPMD replicated along stage axis).</text>

  <!-- Single device view -->
  <rect x="20" y="200" width="415" height="55" rx="6" fill="#1a2a1a" stroke="#7ee787"/>
  <text x="30" y="218" fill="#7ee787" font-size="10" font-weight="600">Full array: [8, micro_size, seq_len, emb_dim]</text>
...
<h3>7.3 Removed meta.remove_axis on Weights</h3>

<p>
In the original circular pipeline path within <code>Pipeline</code> (for <code>num_pipeline_repeats > 1</code>), the weight preparation code stripped Flax's <code>LogicallyPartitioned</code> metadata:
</p>

<pre><code><span class="diff-del">- weights = meta.remove_axis(weights, 0, {</span>
<span class="diff-del">-     nn.PARTITION_NAME: "layers",</span>
<span class="diff-del">-     "sub_weight_split_dims_mapping": (None,),</span>
<span class="diff-del">-     "is_initializing": self.is_initializing(),</span>
<span class="diff-del">-     "x_times": self.num_stages,</span>
<span class="diff-del">- })</span></code></pre>

<p>
<code>meta.remove_axis</code> was needed in the old <code>shard_map</code>-based approach because <code>shard_map</code> operates on raw arrays without Flax metadata. With the local-operation approach, the vmapped body receives weights already correctly partitioned from the enclosing context. The metadata strip created a copy of the weight pytree in tmem for no benefit.
</p>
...
per_stage = num_layers_per_pipeline_stage * activation_size

# These are live across the full scan (all iterations):
# The scan carries them as part of the remat checkpoint
total_decoder_layer_checkpoints = per_stage * total_iterations_active</code></pre>

<p>
The exact savings depend on the buffer assignment's ability to share memory across iterations, but empirically this accounts for approximately 7&ndash;9 GB of the 9.5 GB total tmem reduction.
</p>
...
<p>
The float32 accumulation buffer is a separate allocation from the bf16 expert outputs, adding approximately 2 GB of tmem per device for <code>emb_dim=4096</code>. Setting <code>float32_weight_sum = false</code> keeps everything in bf16, using the same buffer for accumulation and output.
</p>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td style="color:#7ee787;">~5&ndash;7 GB</td>
    </tr>
    <tr>
      ...
      <td style="color:#7ee787;">~1&ndash;2 GB</td>
    </tr>
    <tr>
      <td>Remove <code>meta.remove_axis</code> on weights</td>
      <td>Eliminates weight pytree copy in tmem</td>
      <td style="color:#7ee787;">~0.5 GB</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td style="color:#7ee787;">~7&ndash;9 GB</td>
    </tr>
    <tr>
      <td><code>float32_weight_sum = false</code></td>
      <td>Eliminates fp32 accumulation buffer in MoE</td>
      <td style="color:#7ee787;">~2 GB</td>
    </tr>
    <tr>
      <td><code>skip_trivial_specs = True</code></td>
      <td>Eliminates spurious no-op sharding copies</td>
      <td style="color:#7ee787;">~0.1 GB</td>
    </tr>
    <tr style="font-weight:700; border-top: 2px solid var(--accent);">
      <td>Total</td>
      <td></td>
      <td style="color:#58a6ff;">~29.9 GB &rarr; ~20.4 GB (32% reduction)</td>
    </tr>
  </tbody>
</table>

<div class="callout-ok callout">
<strong>Note on estimates:</strong> The per-optimization savings do not add up exactly because XLA's buffer assignment is a global optimization. Removing one buffer may allow another buffer to share its memory slot, or it may not (depending on liveness overlap). The individual estimates are based on analytical reasoning about buffer sizes; the total is the empirically measured difference.
</div>
...
<svg viewBox="0 0 850 520" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:850px;">
  ...
  <text x="140" y="132" text-anchor="middle" fill="#f85149" font-size="9">decoder_layer_input checkpoints (~9 GB)</text>

  <rect x="230" y="115" width="115" height="25" rx="3" fill="#3a1a2a" stroke="#d2a8ff" stroke-width="0.8"/>
  <text x="287" y="132" text-anchor="middle" fill="#d2a8ff" font-size="9">ppermute buffers (~6 GB)</text>

  <rect x="350" y="115" width="50" height="25" rx="3" fill="#2a2a1a" stroke="#f0883e" stroke-width="0.8"/>
  <text x="375" y="132" text-anchor="middle" fill="#f0883e" font-size="8">fp32 (~2G)</text>

  <rect x="55" y="145" width="90" height="25" rx="3" fill="#1a2a2a" stroke="#58a6ff" stroke-width="0.8"/>
  <text x="100" y="162" text-anchor="middle" fill="#58a6ff" font-size="9">sharding copies (~2 GB)</text>

  <rect x="150" y="145" width="80" height="25" rx="3" fill="#1a1a3a" stroke="#8b949e" stroke-width="0.8"/>
  <text x="190" y="162" text-anchor="middle" fill="#8b949e" font-size="9">gather+bcast (~1 GB)</text>
  ...
  <text x="485" y="279" fill="#c9d1d9" font-size="10">decoder_layer_input checkpoints (~9 GB)</text>
  ...
  <text x="485" y="311" fill="#c9d1d9" font-size="10">ppermute collective buffers (~6 GB)</text>
  ...
  <text x="485" y="343" fill="#c9d1d9" font-size="10">fp32 MoE accumulator (~2 GB)</text>
  ...
  <text x="485" y="375" fill="#c9d1d9" font-size="10">Sharding copies + gather buffers (~2.5 GB)</text>
...
<ul>
  <li><strong>slice+concat replacing ppermute</strong>: Both implement the same permutation <code>[0,1,...,N-1] &rarr; [N-1,0,...,N-2]</code>. The local version operates on the full replicated array; the collective version operates on per-device shards. The result is bit-identical.</li>
  ...
  <li><strong>Removing meta.remove_axis</strong>: This only strips Flax metadata; it does not modify array values.</li>
...
float32_weight_sum: false                 # ~2 GB savings, slight precision change
