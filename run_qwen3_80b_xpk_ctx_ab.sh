...
# A/B harness: isolates the `context` remat policy at per_device_batch_size=8.
#
#   usage:  bash run_qwen3_80b_xpk_ctx_ab.sh offload    # arm A (new baseline)
#           bash run_qwen3_80b_xpk_ctx_ab.sh remat      # arm B
#
# Both arms are byte-identical except for `context=`. Everything else is
# inherited from run_qwen3_80b_xpk.sh with three deliberate deltas:
#
#   1. num_moe_token_chunks 2 -> 4
#      Required. With chunks=2 the `context=remat` arm OOMs at compile time:
#        RESOURCE_EXHAUSTED: HLO temporaries (31.72G) exceeds available HBM (31.24G)
#      i.e. 480 MB short. `offload` frees a tensor outright, `remat` still needs
#      transient HBM to hold the recomputed value, so remat costs MORE HBM than
#      offload -- context=offload was load-bearing for fitting pdb=8, not just a
#      latency choice. chunks=4 halves the MoE ragged buffers
#      (bf16[98304,3072] 604MB -> bf16[49152,3072] 302MB), freeing ~900 MB.
#      Applied to BOTH arms so it cancels out of the comparison.
#
#   2. profiler_steps 2 -> 1, tpu_num_sparse_core_tiles_to_trace 1 -> 0
#      Profiler-only, no effect on model math. The run-0812030226 capture was
#      TRUNCATED at 69% of the step because SparseCore TEC tile tracing emitted
#      4.82M of 5.0M trace events. Dropping tile tracing keeps the
#      OFFLOAD_* breakdown (cheap, separate thread) and loses only TEC noise.
#
#   3. Reuses the image from run-0812030226 -- no code change, only CLI args.

CONTEXT_MODE="${1:-}"
case "$CONTEXT_MODE" in
  offload) SHORT=ctxoff ;;
  remat)   SHORT=ctxrem ;;
  *) echo "usage: $0 <offload|remat>" >&2; exit 2 ;;
esac
...
export WORKLOAD_NAME="mohit-qn80b-${SHORT}-${TIMESTAMP}"
export DEVICE_TYPE="v6e-256"
export NUM_SLICES=1
export PRIORITY="very-high"
export MAX_RESTARTS=0
export MODEL_NAME="qwen3-next-80b-a3b"
export BASE_OUTPUT_DIR="gs://runner-maxtext-logs/qwen3-next-80b-profiles/${SHORT}-chunks4-${TIMESTAMP}"

echo "========================================================================"
echo "ARM: context=${CONTEXT_MODE}  (num_moe_token_chunks=4, pdb=8)"
...
echo "ARM_DONE context=${CONTEXT_MODE} WORKLOAD=${WORKLOAD_NAME} PROFILE=${BASE_OUTPUT_DIR}/param-3/tensorboard/plugins/profile/"
