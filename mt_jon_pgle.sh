...
# export WORKLOAD_NAME=$USER-pgle-fa-7b-2n-jb1107-${RANDOM:0:2}
# export LOCAL_IMAGE_NAME=us-west1-docker.pkg.dev/supercomputer-testing/lancewang/lance-1107-nv

export WORKLOAD_NAME=$USER-pgle-7b-2n-jb1119-${RANDOM:0:2}
...
python ../xpk/xpk.py  workload create --device-type h100-mega-80gb-8 --project supercomputer-testing --zone australia-southeast1 --cluster a3plus-benchmark \
  ...
  --command 'export LD_LIBRARY_PATH=/usr/local/cuda-12.6/compat:$LD_LIBRARY_PATH;'"python3 MaxText/train.py MaxText/configs/models/gpu/llama2_7b.yml run_name=maxtext-$MODEL_NAME model_name=llama2-7b use_iota_embed=true skip_first_n_steps_for_profiler=5 profiler=xplane steps=10 hardware=gpu enable_checkpointing=false dataset_type=synthetic base_output_directory=gs://lancewang-dev-supercomputer-testing/maxtext_gpu attention=cudnn_flash_te remat_policy=full logits_dot_in_fp32=false dcn_fsdp_parallelism=2 ici_tensor_parallelism=8 max_target_length=4096 weight_dtype=bfloat16 per_device_batch_size=1 "  \
  --num-nodes 2 \
