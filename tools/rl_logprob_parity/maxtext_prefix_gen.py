...
OUT = "/mnt/disks/persist/pr4925_repro"; S = 512; B = 8; NL = int(os.environ.get("NL", "8")); L3 = 3
LENS = sorted(set([4, 8, NL])); REC = sorted(set([2, 3, 7, NL - 1])); MODES = os.environ.get("MODES", "own,replay").split(",")
...
def make_fn(n_layers, replay):
  ...
  def fn(st, tokens_b, pos, seg, replay_idx):
    ...
    with nn.logical_axis_rules(cfg.logical_axis_rules):
      ...
      if os.environ.get("FULLCALL") == "1":   # whole-model __call__ (needed for qwix-quantized models); logprobs only at NL
    ...
    REPLAY["queue"] = None
    return {k: outs[k].astype(jnp.float32) for k in REC if k in outs}, lens
  return fn
...
with mesh, nn.logical_axis_rules(cfg.logical_axis_rules):
  ...
  for mode, replay in [(m, m == "replay") for m in MODES]:
    ...
    log(f"{mode}: prefix done; " + "; ".join(f"lens{k} logp={res[f'{mode}_lens{k}_logp_actual'].mean():.3f} top1={np.mean(res[f'{mode}_lens{k}_top1']==np.roll(tokens.reshape(-1),-1)):.3f}" for k in LENS if k in lens))
...
np.savez(f"{OUT}/maxtext_prefix_nl{NL}" + os.environ.get("SUFFIX", "") + ".npz", **res); log("done")
