...
OUT = "/mnt/disks/persist/pr4925_repro"; MODEL = "Qwen/Qwen3.5-397B-A17B-FP8"; NL = 8; L3 = 3
...
tokens = np.load(f"{OUT}/real35b_L3.npz")["tokens"]; h_np = np.load(f"{OUT}/maxtext397_prefix_own.npz")["own_h_after3"]; B, S, D = h_np.shape; T = B * S
...
llm = LLM(model=MODEL, dtype="bfloat16", tensor_parallel_size=8, enable_expert_parallel=True,
          ...
          gpu_memory_utilization=0.85, language_model_only=True, limit_mm_per_prompt={"image": 0, "video": 0},
...
np.savez(f"{OUT}/torchax397_prefix_{tag}.npz", **res)
