...
OUT = "/mnt/disks/persist/pr4925_repro"; MODEL = os.environ.get("MODEL", "Qwen/Qwen3.5-35B-A3B"); NL = int(os.environ.get("NL", "8")); L3 = 3
LENS = sorted(set([4, 8, NL])); REC = sorted(set([2, 3, 7, NL - 1]))
...
def _prefix(self, input_ids, positions, n):
  ...
  for i in range(n):
    ...
    outs[i] = h
  return outs
HEADS = [x for x in os.environ.get("HEADS", "").split(",") if x]  # extra head precisions at depth NL: fp32 (fp32 dot, HIGHEST) | f32acc (bf16 inputs, fp32 accumulate, unrounded logits)
def _lens(self, h, head=""):  # logit lens: final norm + lm_head (+ vLLM logits processor)
  x = self.language_model.model.norm(h)
  if not head:
    return self.language_model.compute_logits(x)
  xj = jax_view(x); wj = jax_view(self.language_model.lm_head.weight)
  if head == "fp32": lg = jnp.dot(xj.astype(jnp.float32), wj.astype(jnp.float32).T, precision=jax.lax.Precision.HIGHEST)
  elif head == "f32acc": lg = jnp.dot(xj, wj.T, preferred_element_type=jnp.float32)
  else: raise ValueError(head)
  return torch_view(lg[:, :getattr(self.language_model.lm_head, "org_vocab_size", lg.shape[-1])])
...
def prefix_fn(st, kv, ids, positions, attn_md):
  ...
  with c1, c2, c3:
    ...
    for head in HEADS:
      logits = jax_view(call(st, {"call_method": "run_lens", "call_kwargs": {"h": torch_view(outs[NL - 1]), "head": head}}))
      hlens[head] = lens_stats(logits, ids)
  return {k: outs[k].astype(jnp.float32) for k in REC}, lens, experts, hlens
...
with jax.set_mesh(mesh):
  ...
  outs, lens, experts, hlens = prefix_fn(state, kv_caches, ids, pos, grouped); jax.block_until_ready(outs[7])
...
tag = f"dp{sc.attn_dp_size}tp{sc.tp_size}_ep1_nl{NL}" + ("_fp8" if "FP8" in MODEL else "") + ("_fp32gate" if FP32GATE else "") + ("_nofr" if os.environ.get("NO_FUSED_RESIDUAL") == "1" else "") + ("_reshard" if os.environ.get("RESHARD") == "1" else "") + ("_heads" if HEADS else "")
res = {f"h_after{k+1}": f32(outs[k]).reshape(B, S, D) for k in REC}; res.update({
...
for head in HEADS:
  lp_act, top1, lp_full = hlens[head]
  res[f"{head}_lens{NL}_logp_actual"] = f32(lp_act); res[f"{head}_lens{NL}_top1"] = np.asarray(jax.device_get(top1)).astype(np.int32); res[f"{head}_lens{NL}_logprobs_2seq"] = f32(lp_full)
  log(f"head {head}: lens{NL} logp={res[f'{head}_lens{NL}_logp_actual'].mean():.3f} top1={np.mean(res[f'{head}_lens{NL}_top1']==np.roll(tokens.reshape(-1),-1)):.3f}")
np.savez(f"{OUT}/torchax_prefix_{tag}.npz", **res)
log(f"saved ({tag}); " + "; ".join(f"lens{k} logp={res[f'lens{k}_logp_actual'].mean():.3f} top1={np.mean(res[f'lens{k}_top1']==np.roll(tokens.reshape(-1),-1)):.3f}" for k in LENS))
log("done")
