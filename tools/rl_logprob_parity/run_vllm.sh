#!/bin/bash
# usage: [ADAPTER=1] run_vllm.sh <script.py> [args]  (vLLM/tpu-inference env with the production serving flags; ADAPTER=1 = MaxText-in-vLLM: MODEL_IMPL_TYPE=flax_nnx + MaxText tree on PYTHONPATH)
...
if [ "$ADAPTER" = "1" ]; then export PYTHONPATH=$PYDEPS:$TPUINF_WT:$VLLM_WT:$MAXTEXT_WT/src; else export PYTHONPATH=$PYDEPS:$TPUINF_WT:$VLLM_WT; fi
...
[ "$ADAPTER" = "1" ] && export MODEL_IMPL_TYPE=flax_nnx
cd "$(dirname "$0")"
exec python "$(pwd)/$1" "${@:2}"
