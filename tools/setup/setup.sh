...
if [[ "$MODE" == "nightly" ]]; then
    ...
    python3 -m uv pip install --no-cache-dir -U -r "$nightly_txt" \
                                                -r "${dep_name%/*}"'/extra_deps_from_github.txt'
    rm -fv -- "$nightly_txt"
else
    ...
    if [ -z "${requirements_txt}" ]; then
      ...
      python3 -m uv pip install --resolution=lowest -r "$requirements_txt" \
                                                    -r "${requirements_txt%/*}"'/extra_deps_from_github.txt'
    fi
fi
...
if [ -f 'pyproject.toml' ]; then
  ...
  python3 -m uv pip install --resolution=lowest -r 'dependencies/requirements/extra_deps_from_github.txt'
fi
