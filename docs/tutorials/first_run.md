...
python3 -m MaxText.train MaxText/configs/base.yml \
  ...
  steps=10
```
Next, you can try training on a HugginFace dataset, see [Data Input Pipeline](data-input-pipeline) for data input options.
...
1. [GKE, recommended] [Running Maxtext with xpk](run-xpk) - Quick Experimentation and Production support
2. [GCE] [Running Maxtext with Multihost Jobs](run-multihost-job) - Long Running Production Jobs with Queued Resources
3. [GCE] [Running Maxtext with Multihost Runner](run-multihost-runner) - Fast experiments via multiple ssh connections.
