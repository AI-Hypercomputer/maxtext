...
Google's quantization library for JAX, providing tools for QAT.

**Qwix:**
Recommended successor to AQT with a non-intrusive API for quantization in JAX.
...
Gradient approximation technique for non-differentiable quantization operations in backward pass.
...
**Remat Policy:**
Strategy for which activations to save vs recompute (options: `full`, `minimal`, `minimal_flash`, `minimal_offloaded_flash`, `save_dot_except_mlp`, `save_qkv_proj`, etc.).
...
**Checkpoint:**
Saved model state including parameters, optimizer state, and training step count.
...
**Orbax:**
JAX checkpointing library used by MaxText, providing flexible and efficient checkpoint management.
...
**ArrayRecord:**
Random-access data format optimized for distributed training, recommended for multi-host setups.
...
**JAX:**
Python library for high-performance numerical computing with composable function transformations (grad, jit, vmap, pmap).
...
**Pallas:**
JAX extension for writing custom TPU/GPU kernels with fine-grained control over memory and computation.
...
**All-Reduce:**
Sum values across all devices and broadcast the result to all devices.

**All-Gather:**
Gather sharded data from all devices and concatenate.

**Reduce-Scatter:**
Reduce across devices then shard the result.

**All-to-All:**
General permutation communication pattern for resharding data.
...
**Padding Batch:**
Empty batch generated when some hosts run out of data before others.

**Data Sharding:**
Splitting dataset across multiple hosts/workers to enable parallel data loading.

**Global Shuffle:**
Shuffling data across all hosts for better randomization.

**Hierarchical Shuffle:**
Multi-level shuffling strategy with different shuffle buffers at different levels.
...
**Dataset Path:**
Location of training data (GCS path, local path, or HuggingFace dataset name).
...
**Worker:**
Process running on a device in distributed training.

**Replica:**
Copy of model/computation in data parallel setup.

**Rank:**
ID of worker in distributed system (0 to num_workers-1).
...
**SentencePiece:**
Tokenizer library supporting subword tokenization, used in Llama and Gemma models.

**Tiktoken:**
OpenAI's tokenizer library, used in GPT models.
...
**Flax:**
Neural network library built on JAX, providing high-level abstractions.

**NNX:**
New Flax API with object-oriented interface and improved ergonomics.

**Linen:**
Original Flax API, being gradually replaced by NNX.

**Optax:**
JAX optimization library providing various optimizers and learning rate schedules.

**Tunix:**
JAX library for post-training techniques (SFT, GRPO) integrated with MaxText.

**MoBA (Mixture of Block Attention):**
Technique combining different attention patterns (full, sliding window) with learned routing for efficiency.
...
**NNX:**
New Flax API with object-oriented interface, replacing the older Linen API.
