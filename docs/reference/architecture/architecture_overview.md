...
- GKE with Cluster Toolkit (recommended for production): This is the most scalable and robust method for running MaxText on GKE. Cluster Toolkit (`gcluster`) standardizes cluster provisioning and workload submission for large-scale ML jobs, decoupling compute provisioning from training orchestration while keeping the deployment reproducible and easier to operate. This approach is recommended for production-grade training and serving due to its scalability, fault tolerance, and integration with the broader Google Cloud ecosystem. Legacy XPK flow remains available only for older environments.
