...
`ragged_buffer_factor`: A scalar multiplier for the active MoE backend's balanced receive capacity. Effective only when `sparse_matmul` is True.

- Value > 0: Uses an explicit buffer size which may drop tokens when this size is exceeded. TransformerEngine MoEBlock requires positive values to be at least `1.0` and applies its required per-expert alignment before scaling.
- Value <= 0: Uses the active backend's calculated worst-case buffer size, which is guaranteed not to drop tokens.
