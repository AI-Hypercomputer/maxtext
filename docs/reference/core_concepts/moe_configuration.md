...
`retry_when_tokens_dropped`: If enabled alongside `ragged_buffer_factor > 0`, retries the entire training step with a worst-case (dropless) ragged buffer size whenever tokens would otherwise be dropped due to buffer overflow. This allows training with a smaller tuned buffer for speed/memory efficiency while guaranteeing no token dropping if an overflow occurs. The retry is memory-neutral: the primary step discards its own update in-graph when an overflow is detected, so the state stays donated (input/output aliased) and the replay runs from the state the step returned rather than from a second copy. Not supported with `enable_diloco`, which has no dropless replay executable.
