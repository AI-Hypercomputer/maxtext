...
In high-performance computing, there's a trade-off between memory usage and computation time. **Tiling** (also known as chunking) is an optimization technique that prioritizes reducing memory usage, but sometimes at a cost to running time.
...
Tiling is also implemented at a lower level inside **Pallas kernels**, such as in Splash Attention. This is often necessary to manage data movement between the large HBM and the much smaller, faster VMEM. For more details, see the [Pallas Kernels Performance Guide](https://maxtext.readthedocs.io/en/latest/guides/pallas_kernels_performance.html).

## Tiling vs. Sharding

**Tiling** and **sharding** are independent concepts that do not conflict; in fact, they are often used together. Sharding distributes a tensor across multiple devices, while tiling processes a tensor in chunks on the same device. For example, pipeline parallelism combines sharding (splitting layers across devices) with gradient accumulation (a form of tiling) on each device.
