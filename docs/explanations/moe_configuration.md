...
* [Megablox](../../src/MaxText/kernels/megablox): Enabled by setting `sparse_matmul=True, use_tokamax_gmm=False, megablox=True`. 
...
### Routing Mechanism
`use_random_routing`: If enabled, ignores the gate logits and routes tokens to random experts. Primarily used for debugging and performance testing.
