...
`shard_mode` (`src/maxtext/configs/base.yml:616`) selects how MaxText expresses sharding:
...
plus mesh construction at `src/maxtext/utils/maxtext_utils.py:2263-2268`, which stamps *every* mesh
...
07. **One headline number in the matrix is a harness artifact, not a model property.** gemma3's
    ...
    `len(GEMMA3_ATTENTION_PATTERN) == 6`, so `scan_length = 4 // 6 = 0` (`nnx_decoders.py:801`) and the
...
09. **Explicit forfeits capabilities**: `check_vma` (which `base.yml:783` calls a performance win) is
...
5. **The persistent compilation cache silently suppresses HLO dumps.** MaxText defaults
   `jax_cache_dir: "~/jax_cache"` (`base.yml:605`). A cache hit skips compilation, so
...
MaxText builds the unembedding as a separate `DenseGeneral` at
`src/maxtext/layers/nnx_decoders.py:561-573`, whose `kernel_axes` come from
`sharding.lm_head_logical_axes`; in the default orientation that is `("embed_vocab", "vocab")`, so
the parameter is `[emb, V]`. `base.yml:632` maps `embed_vocab → fsdp` and `:631` maps `vocab → [tensor, ...]`; at
...
`src/maxtext/layers/nnx_decoders.py:1673-1677` computes an `out_sharding` for the logits
...
`head_out_axes` is `("activation_embed_and_logits_batch", "activation_length", "activation_vocab")`
in the default orientation. It is passed into the head at `nnx_decoders.py:1703`.
`src/maxtext/layers/linears.py:493-495` is where it becomes explicit-only:
...
(`nnx_decoders.py:1680-1689`) and **never reaches `logits_dense` at all**. There is no separate vocab
...
`src/maxtext/layers/nnx_decoders.py:801` computes `scan_length = num_layers // 6`. At
`base_num_decoder_layers=4` that is **0**, the main scan is skipped entirely, and all four layers
fall into the unscanned `layers_remainder` block (`nnx_decoders.py:817-824`). Every parameter in the
...
1. **Remat stops being CSE'd.** `remat_policy` defaults to `'full'` (`base.yml:435`). When the block
...
- **Not multimodal.** `use_multimodal` defaults false (`base.yml:1385`). Neither dump contains a
...
module spans on `/device:TPU:0`, three reps per cell. Rep spread is 0.014% at the median cell and
under 0.05% in 29 of the 32 cells; the exceptions are one cell at 0.25% and two near 0.09%, all
driven by a single slow rep that the median absorbs. Treat anything under ~0.05% as a tie.

Both tables below were re-measured end to end on a later main (2026-09-21, same TPU7x host, same
JAX 0.11.1) after this branch was rebased across ~195 commits. All 32 cells landed within 0.13% of
the absolute figures printed here, every delta kept its sign, and the two medians came back at
−0.066% and −0.435% against the −0.063% and −0.435% recorded below.
...
±0.001. Re-running the whole check on 2026-09-21 reproduced every one of those per-step lists except
deepseek2-16b, which came back **identical** at both sequence lengths; which steps a given model
differs on is therefore not stable across sessions, while the ±0.001 bound and the absence of drift
are. Table C's SparseCore arms give the same answer on a different partition of the models —
...
Every matrix / parallelism / scale run used `gradient_accumulation_steps: 1` (`base.yml:1106`) and a
...
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| `check_vma=True`                                 | **rejected by MaxText** (`types.py:3601-3602`) although JAX 0.11.1 accepts it on an Explicit mesh               | `base.yml:783` calls it "recommended for improved performance" |
| `fused_qkv=True`                                 | **trace-time `ShardingTypeError`** — `attentions.py:1254` is the one projection that forwards no `out_sharding` | no validator catches it; you get a raw JAX error               |
...
| partial adoption                                 | **impossible** — mesh axis types are all-or-nothing (`maxtext_utils.py:2263-2268`)                              |                                                                |

`check_vma` is the most consequential: it gates the varying-manual-axes fast path in megablox
(`ops.py:860`, `ops.py:1008`, `backend.py:528`, `backend.py:789`) on exactly the MoE models that are
...
- **`with_sharding_constraint` becomes a zero-HLO no-op assert** under an all-Explicit mesh. Any
  ...
  `maxtext_utils_nnx.py:175` (init-time), `attention_op.py:2904-2916` and `:2956-3030`
  (decode/prefill), `diloco_sharding.py:78`. Training is unaffected (those branches are gated on
  non-train model modes), but decode and prefill under explicit are running without their sharding
  hints. Two sites this section originally listed have since been cleaned up independently:
  `qwen3.py` now reshards the `mixed_qkvz` projection through `maybe_shard_with_name`, and the
  constraints in `kvcache.py` are gone (the file also moved to `src/maxtext/inference/`).
...
The golden suites run **auto only**. Nothing under `tests/` sets `shard_mode` at all — not the case
table at `tests/utils/sharding_dump.py:39-82`, nor the generator at
`tests/utils/run_sharding_dump.py:66-88`. (The sharding-comparison test that used to consume those
goldens, `tests/unit/sharding_compare_test.py`, was emptied by the NNX-only migration and is now a
docstring with no tests in it, so the goldens currently have no consumer either.) The HLO regression test
...
02. **Ship `dense_weight_grad_in_kernel_order` — on by default under explicit (§4.8).** The same
    ...
    (`linears.py:737,759,780`), the four attention projections (`attentions.py:693,732,775,832`) and
