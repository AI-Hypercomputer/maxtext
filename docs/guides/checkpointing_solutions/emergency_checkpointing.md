...
1. **Set up environment variables:**
   ...
   ```bash
   # If you skipped the cluster setup section above, also export PROJECT_ID, CLUSTER_NAME, CLUSTER_LOCATION, COMPUTE_TYPE, TOPOLOGY, and GCS_BUCKET
   export RAMDISK_DIRECTORY=<RAMDISK_DIRECTORY> # example: /tmp/ramdisk
   export WORKLOAD_NAME=<RUN_NAME>
   export NUM_SLICES=<NUM_SLICES> # example: 2
