...
1. Grain currently supports two data formats: [ArrayRecord](https://google-grain.readthedocs.io/en/stable/tutorials/data_sources/arrayrecord_data_source_tutorial.html) (random access) and [Parquet](https://google-grain.readthedocs.io/en/stable/tutorials/data_sources/parquet_dataset_tutorial.html) (partial random-access through row groups). Only the ArrayRecord format supports the global shuffle mentioned above. For converting a dataset into ArrayRecord, see [Apache Beam Integration for ArrayRecord](https://github.com/google/array_record/tree/main/beam). Additionally, other random access data sources can be supported via a custom [data source](https://google-grain.readthedocs.io/en/latest/data_sources.html) class.
...
3. Set `dataset_type=grain`, `grain_file_type={arrayrecord|parquet}`, `grain_train_files` **(where?)** to match the file pattern on the mounted local path.
...
When setting eval_interval > 0, evaluation will be run with a specified eval dataset. Example config (set in [`MaxText/configs/base.yml`](https://github.com/AI-Hypercomputer/maxtext/blob/main/MaxText/configs/base.yml) or through command line):
