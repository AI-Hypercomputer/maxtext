...
1.  **Modify Base Requirements**: Update the desired dependencies in `requirements.txt` or the hardware-specific files (`base_requirements/tpu-base-requirements.txt`, `base_requirements/gpu-base-requirements.txt`).
2.  **Generate New Files**: Run the `seed-env` CLI tool to generate new, fully-pinned `requirements.txt` files based on your changes.
...
First, you need to install the `seed-env` command-line tool. Follow the instructions in the
...
You can find the latest commit hashes on the [JAX nightly build page](https://github.com/jax-ml/jax/commits/main/build). Choose a recent, successful build and copy its full commit hash.
...
2.  **Update `extra_deps_from_github.txt` (if necessary):**
    The `seed-env` tool may not be able to resolve dependencies that are installed directly from GitHub. If any such dependencies were present in the base requirements files, you may need to manually add them to `src/install_maxtext_extra_deps/extra_deps_from_github.txt`.
