...
```bash
python3 -m maxtext.trainers.pre_train.train base_output_directory=<GCS_BUCKET> \
  dataset_path=<DATASET_PATH> run_name=goodput-test-run steps=200 goodput_upload_interval_seconds=30
```
...
```bash
python3 -m maxtext.trainers.pre_train.train base_output_directory=<GCS_BUCKET> \
  dataset_path=<DATASET_PATH> run_name=goodput-test-run steps=200 step_deviation_interval_seconds=30
```
...
```bash
python3 -m maxtext.trainers.pre_train.train base_output_directory=<GCS_BUCKET> dataset_path=<DATASET_PATH> \
