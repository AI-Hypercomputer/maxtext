...
Cluster Toolkit simplifies MaxText's Vertex AI Tensorboard integration when used with `gcluster job submit`. A Vertex Tensorboard instance and Experiment can be created as part of the cluster or job environment, and the required environment variables can be supplied by the runtime. Set `use_vertex_tensorboard=False` to avoid setting up Vertex Tensorboard again in MaxText if the job environment already handles it. This is the recommended configuration for new MaxText deployments on GKE:
...
The above configuration will upload logs in `config.tensorboard_dir` to the Vertex TensorBoard instance configured by the Cluster Toolkit job environment.
