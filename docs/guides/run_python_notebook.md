...
- **`qwen3_native_lora_demo.ipynb`** → Qwen3-0.6B PEFT training and evaluation with native LoRA and QLoRA. Includes both SFT training on GSM8K and pre-training. Runs successfully on free-tier Google Colab TPUs.
- **`gemma4_native_lora_demo.ipynb`** → Gemma4-e2b PEFT training and evaluation with native LoRA and QLoRA, demonstrating multi-query attention (MQA) support under Flax NNX. We recommend running this on a v5p-8 TPU VM.
