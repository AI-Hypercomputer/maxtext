...
Every image pipeline run pushes `gcr.io/<project>/<image>:<run_id>` **before** the
tests run, then runs the CI test suite against that tag and, on success, adds the
tag `verified-ci-<run_id>`. A failed test run does not delete the image, so it can
be reused by the modes below.
...
- `image_suffix`: pushes `<image>_<suffix>` instead of `<image>`, which keeps
  experiments away from the images CI depends on and disables the failure
  notification. In fast-rebuild mode the source run must have used the same suffix.
...
- The source run must have built the same image names (same `image_suffix`).
