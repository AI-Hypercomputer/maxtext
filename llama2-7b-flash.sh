...
set_nccl_gpudirect_tcpx_specific_configuration() {
  if [[ "$USE_GPUDIRECT_TCPX" == "yes" ]]; then
    ...
    export LD_LIBRARY_PATH="/usr/local/tcpx/lib64"
...
python3 MaxText/train.py MaxText/configs/base.yml dataset_path=gs://maxtext-dataset \
  ...
  attention=cudnn_flash_te async_checkpointing=False base_output_directory=gs://runner-maxtext-logs steps=30 hardware=gpu
