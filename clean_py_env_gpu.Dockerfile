...
# `docker build --build-arg DEVICE=gpu -t <docker_image_name>:<tag> -f clean_py_env_gpu.Dockerfile .`
# Or
# `docker build --build-arg PYTHON_VERSION=3.11 --build-arg DEVICE=gpu -t <docker_image_name>:<tag> -f clean_py_env_gpu.Dockerfile .`
#
# How to upload the image to Google Container Registry (GCR):
# e.g., DEVICE=gpu and PYTHON_VERSION=3.12
#.  gcloud init
#   gcloud auth configure-docker
#   docker tag <docker_image_name>:<tag> gcr.io/tpu-prod-env-multipod/maxtext-unit-test-gpu:py312
#   docker push gcr.io/tpu-prod-env-multipod/maxtext-unit-test-gpu:py312
