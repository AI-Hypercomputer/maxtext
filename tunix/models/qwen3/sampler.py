...
class Sampler:
  ...
  def init_sample_state(
    ...
    input_mask = input_mask.at[:, :num_input_tokens].set(
        all_input_ids != self.vocab.pad_token_id
    )
  ...
  def tokenize(self, input_string: str) -> jax.Array:
    ...
    input_ids = self.vocab.encode(input_string)
    input_ids = jnp.array(
        [self.vocab.pad_token_id] + input_ids, dtype=jnp.int32
    )
    return input_ids

  def mask_tokens_after_eos_ids(self, token_buffer):
    """Mask token IDs after the EOS token with the padding ID."""
    eos_id = self.vocab.eos_token_id
    ...
    masked_token_buffer = token_buffer * mask + self.vocab.pad_token_id * (
        1 - mask
    )
  ...
  def _prefill_fn(
    ...
    input_mask = tokens != self.vocab.pad_token_id
    ...
    if sampler_state.logits_buffer is not None:
      ...
      logits_buffer = sampler_state.logits_buffer
        ...
        token_buffer[:, decoding_step + 1], self.vocab.eos_token_id
    )
  ...
  def _sample_step(
    ...
    input_mask = sampler_state.token_buffer == self.vocab.pad_token_id
  ...
  def __call__(
    ...
    all_input_ids = jnp.array([
        pad_to_length(
            ...
            pad_value=self.vocab.pad_token_id,
            left=True,
        )
        for x in tokens
    ])
    ...
    # TODO: Add masking after EOS token.
    # masked_token_buffer = self.mask_tokens_after_eos_ids(
    #     sampling_state.token_buffer
    # )
    masked_token_buffer = sampling_state.token_buffer

    out_tokens = []
    out_logits = []
    for i, token_buffer in enumerate(masked_token_buffer):
      start_idx = (
          find_first_non_pad_idx(token_buffer, self.vocab.pad_token_id)
          if echo
    ...
    decoded_outputs = [
        self.vocab.decode(tokens.tolist()) for tokens in out_tokens
    ]
