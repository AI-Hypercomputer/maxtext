...
RUN apt-get update && apt-get install -y curl gnupg gcc git numactl
...
RUN echo "Running command: bash cpu_setup.sh LIBTPU_GCS_PATH=${ENV_LIBTPU_GCS_PATH} DEVICE=${ENV_DEVICE}"
RUN bash cpu_setup.sh LIBTPU_GCS_PATH=${ENV_LIBTPU_GCS_PATH} DEVICE=${ENV_DEVICE}
