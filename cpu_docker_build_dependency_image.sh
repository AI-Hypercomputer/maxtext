...
if [[ -z ${LIBTPU_GCS_PATH+x} ]] ; then
  ...
  echo "============================="
  echo "docker build --network host --build-arg MODE=${MODE} --build-arg JAX_VERSION=$JAX_VERSION --build-arg LIBTPU_GCS_PATH=$LIBTPU_GCS_PATH --build-arg DEVICE=$DEVICE -f ./cpu_maxtext_dependencies.Dockerfile -t ${LOCAL_IMAGE_NAME} ."
  docker build --network host --build-arg MODE=${MODE} --build-arg JAX_VERSION=$JAX_VERSION --build-arg LIBTPU_GCS_PATH=$LIBTPU_GCS_PATH --build-arg DEVICE=$DEVICE -f ./cpu_maxtext_dependencies.Dockerfile -t ${LOCAL_IMAGE_NAME} .
fi
