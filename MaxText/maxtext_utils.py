...
def initialize_platform(config, devices=None):
  """Initializes the foundational platform components: mesh, and RNG."""
  init_rng = jax.random.PRNGKey(config.init_weights_seed)
  # Mesh definition
  devices_array = create_device_mesh(config, devices)
  mesh = Mesh(devices_array, config.mesh_axes)

  return mesh, init_rng
...
def create_training_tools(config, model, mesh, init_rng):
  """Creates the optimizer, learning rate schedule, and checkpoint manager."""
  ...
  return checkpoint_manager, learning_rate_schedule, tx