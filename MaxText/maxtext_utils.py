...
def calculate_mla_qkv_tflops_per_device(config):
  """Calculate Multi-Head Latent Attention QKV TFLOP"""
...
def calculate_sparse_ffn_tflops_per_device(config):
  """Helper function to calculate sparse ffn TFLOP"""
...
def calculate_tflops_training_per_device(config, log=True):
  ...
  if config.num_experts > 1:
    total_ffn_flops = calculate_sparse_ffn_tflops_per_device(config)
  else:
    total_ffn_flops = calculate_ffn_mamtul_tflops_per_device(config, config.mlp_dim)

  # Attention flops
  if config.attention_type == "mla":
    qkv_flops = calculate_mla_qkv_tflops_per_device(config)
  else:
    qkv_flops = (
        2
        * config.per_device_batch_size
