...
def compute_times(convos, prefill_bucket_size_to_ms, system_time_per_decode_token_ms, verbose=False):
  ...
  total_time_seconds = total_prefill_time_seconds + total_generate_time_seconds

  print(f"\nTotal time {total_time_seconds:.3f} seconds: "
        f"\n\tPrefill time: {total_prefill_time_seconds:.3f} seconds"
        f"\n\tGenerate time: {total_generate_time_seconds:.3f} seconds")
  return total_time_seconds, total_prefill_time_seconds, total_generate_time_seconds
...
if __name__ == "__main__":
  ...
  kept_convos = get_conversations_from_file(args.convo_file, MAX_INPUT_TOKENS, MAX_OUTPUT_TOKENS)
  total_input_tokens, total_output_tokens = get_num_tokens_in_convos(kept_convos)
  prefill_time_ms_buckets, generate_time_ms = get_prefill_and_generate_times(filename=args.mb_timing_file)
  total_time_seconds, _, _ = compute_times(kept_convos, prefill_time_ms_buckets, generate_time_ms, args.verbose)
