...
def data_load_loop(config, state=None):
  ...
  if jax.process_index() == 0:
    max_logging.log(
        f"STANDALONE DATALOADER : First step completed in {time_to_load_first_batch} seconds, on host 0"
    )
  ...
  if jax.process_index() == 0:
    max_logging.log(
        f"STANDALONE DATALOADER : {config.steps} batches loaded in {end-start} seconds, on host 0"
    )
  return state
