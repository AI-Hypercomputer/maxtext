...
def _convert_huggingface_to_jax_weights(base_model_path: str, model_size: str, model_params: dict, mem_info: psutil.Process):
  ...
  if is_llama4_model:
    ...
    for block_idx in range(layer_cycle_interval):
      ...
      if is_dense_layer:
        # mlp_dict
        jax_weights["decoder"][layer_name]["mlp"] = {
            "wi_0": {"kernel": None},
            "wi_1": {"kernel": None},
            "wo": {"kernel": None},
        }
        ...
        # moe_dict
        jax_weights["decoder"][layer_name]["Llama4MoEBlock_0"] = {
            "MoeBlock_0": {
                "wi_0": None,
                "wi_1": None,
                "wo": None,
  ...
  # debug
  print(jax_weights["decoder"]["decoder_norm"]["scale"].shape)
  ...
  print(jax_weights["decoder"]["logits_dense"]["kernel"].shape)
  ...
  # debug
  print(jax_weights["token_embedder"]["embedding"].shape)
  # breakpoint()
  ...
  for layer_idx in tqdm(range(base_num_decoder_layers), desc="layers", leave=False):
    if is_llama4_model:
      ...
      self_attention = jax_weights["decoder"][f"layers_{block_idx}"]["self_attention"]
    else:
  ...
  self_attention_list = (
      ...
      else [jax_weights["decoder"][f"layers_{block_idx}"]["self_attention"] for block_idx in range(layer_cycle_interval)]
  )
  ...
  for i in range(layer_cycle_interval):
    print(
        f"layers_{i}.self_attention.query.kernel",
        jax_weights["decoder"][f"layers_{i}"]["self_attention"]["query"]["kernel"].shape,
    )
    print(
        f"layers_{i}.self_attention.key.kernel",
        jax_weights["decoder"][f"layers_{i}"]["self_attention"]["key"]["kernel"].shape,
    )
    print(
        f"layers_{i}.self_attention.value.kernel",
        jax_weights["decoder"][f"layers_{i}"]["self_attention"]["value"]["kernel"].shape,
    )
    print(
        f"layers_{i}.self_attention.out.kernel",
        jax_weights["decoder"][f"layers_{i}"]["self_attention"]["out"]["kernel"].shape,
    )
    # breakpoint()
  ...
  for layer_idx in tqdm(range(base_num_decoder_layers), desc="layers", leave=False):
    if is_llama4_model:
      # e.g., interval=4, layer 11 is sublayer 2 in block 3
      block_layer_idx, block_idx = divmod(layer_idx, layer_cycle_interval)
      layer_weight = jax_weights["decoder"][f"layers_{block_idx}"]
      stack_shape = (base_num_decoder_layers // layer_cycle_interval,)
    else:
      block_layer_idx = layer_idx
      stack_shape = (base_num_decoder_layers,)
  ...
  layer_weight_list = (
      ...
      if not is_llama4_model
      else [jax_weights["decoder"][f"layers_{block_idx}"] for block_idx in range(layer_cycle_interval)]
  )
  ...
  for i in range(layer_cycle_interval):
    print(
        f"layers_{i}.pre_self_attention_layer_norm.scale",
        jax_weights["decoder"][f"layers_{i}"]["pre_self_attention_layer_norm"]["scale"].shape,
    )
    print(
        f"layers_{i}.post_self_attention_layer_norm.scale",
        jax_weights["decoder"][f"layers_{i}"]["post_self_attention_layer_norm"]["scale"].shape,
    )
  # breakpoint()
  ...
  for layer_idx in tqdm(range(base_num_decoder_layers), desc="layers", leave=False):
    ...
    if is_dense_layer:
      ...
      layer_weight["Llama4MoEBlock_0"]["MoeBlock_0"]["gate"]["kernel"][layer_idx, ...] = gate
      ...
      # 3 shared experts: Llama4MoEBlock_0.shared_experts.wi_0.kernel, Llama4MoEBlock_0.shared_experts.wi_1.kernel, Llama4MoEBlock_0.shared_experts.wo.kernel
      wi_0 = (
  ...
  if not is_llama4_model:
    ...
    for block_idx in range(layer_cycle_interval):
      ...
      if is_dense_layer:
        ...
        layer_weight["mlp"]["wi_1"]["kernel"] = np.transpose(layer_weight["mlp"]["wi_1"]["kernel"], axes=(1, 0, 2))
        layer_weight["mlp"]["wo"]["kernel"] = np.transpose(layer_weight["mlp"]["wo"]["kernel"], axes=(1, 0, 2))
        print(f"layers_{block_idx}.mlp.wi_0.kernel", layer_weight["mlp"]["wi_0"]["kernel"].shape)
        print(f"layers_{block_idx}.mlp.wi_1.kernel", layer_weight["mlp"]["wi_1"]["kernel"].shape)
        print(f"layers_{block_idx}.mlp.wo.kernel", layer_weight["mlp"]["wo"]["kernel"].shape)
      else:
        ...
        layer_weight["Llama4MoEBlock_0"]["MoeBlock_0"]["wi_0"] = np.transpose(
            layer_weight["Llama4MoEBlock_0"]["MoeBlock_0"]["wi_0"], axes=(1, 0, 2)
        )
        layer_weight["Llama4MoEBlock_0"]["MoeBlock_0"]["wi_1"] = np.transpose(
            layer_weight["Llama4MoEBlock_0"]["MoeBlock_0"]["wi_1"], axes=(1, 0, 2)
        )
        layer_weight["Llama4MoEBlock_0"]["MoeBlock_0"]["wo"] = np.transpose(
            layer_weight["Llama4MoEBlock_0"]["MoeBlock_0"]["wo"], axes=(1, 0, 2)
        )
        ...
        print(
            f"layers_{block_idx}.Llama4MoEBlock_0.MoeBlock_0.gate.kernel",
            layer_weight["Llama4MoEBlock_0"]["MoeBlock_0"]["gate"]["kernel"].shape,
        )
        print(
            f"layers_{block_idx}.Llama4MoEBlock_0.MoeBlock_0.wi_0",
            layer_weight["Llama4MoEBlock_0"]["MoeBlock_0"]["wi_0"].shape,
        )
        print(
            f"layers_{block_idx}.Llama4MoEBlock_0.MoeBlock_0.wi_1",
            layer_weight["Llama4MoEBlock_0"]["MoeBlock_0"]["wi_1"].shape,
        )
        print(
            f"layers_{block_idx}.Llama4MoEBlock_0.MoeBlock_0.wo", layer_weight["Llama4MoEBlock_0"]["MoeBlock_0"]["wo"].shape
        )
        print(
            f"layers_{block_idx}.Llama4MoEBlock_0.shared_experts.wi_0.kernel",
            layer_weight["Llama4MoEBlock_0"]["shared_experts"]["wi_0"]["kernel"].shape,
        )
        print(
            f"layers_{block_idx}.Llama4MoEBlock_0.shared_experts.wi_1.kernel",
            layer_weight["Llama4MoEBlock_0"]["shared_experts"]["wi_1"]["kernel"].shape,
        )
        print(
            f"layers_{block_idx}.Llama4MoEBlock_0.shared_experts.wo.kernel",
            layer_weight["Llama4MoEBlock_0"]["shared_experts"]["wo"]["kernel"].shape,
        )
