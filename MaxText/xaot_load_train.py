...
def train_loop(config, state=None):
  ...
  # Mesh definition
  devices_array = max_utils.create_device_mesh(config, jax.devices())
...
# Define compiled top-level functions.
    ...
    try:
        with open("x_in_tree_train.pickle", "rb") as f:
            in_tree = pickle.load(f)
    except:
        print("Failed to pickle read in_tree")
        in_tree = None
    try:
        with open("x_out_tree_train.pickle", "rb") as f:
            out_tree = pickle.load(f)
    except:
        print("Failed to pickle read out_tree")
        out_tree = None
    p_train_step = deserialize_and_load(serialized_compiled, in_tree, out_tree)
    print("Loaded the serialized compiled train step!!!")
