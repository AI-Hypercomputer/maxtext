...
def summarize_pytree_data(params, name="Params"):
  ...
  (
      ...
      total_param_size,
      avg_param_size,
  ) = max_utils.summarize_size_from_pytree(params)
...
def prefill_benchmark(
  ...
  profile_name = (
      f"prefill_{tokens.size}" if profile_name == "" else profile_name
  )
  ...
  total_prefill_tflops, _, _ = maxtext_utils.calculate_tflops_prefill(
      num_model_params, tokens.size, config
  )
  tflops_per_sec_per_device = (
      total_prefill_tflops / jax.device_count() / prefill_average_ms * 1000.0
  )
  print(
...
def ar_benchmark_loop(
    config, engine, decode_state, params, iters, profile_name=""
):
...
def ar_benchmark(
  ...
  if cache_size is None:
    _, cache_size, _ = summarize_pytree_data(
        decode_state["cache"], name="Cache"
    )
  ...
  total_throughput = (
      jax.device_count() * config.per_device_batch_size / seconds_per_step
  )
...
def collate_results(
  ...
  if incl_config:
    ...
    for k, v in dict(config.get_keys()).items():
      results["config"][k] = (
          str(v) if k == "dtype" else v
      )  # json fails with original dtype
  return results
...
def print_results_for_analyze(results):
  ...
  print(
      f"SYSTEM_TIME_PER_DECODE_TOKEN_MS = {results['AutoRegressive']['ar_step_in_ms_per_seq']}"
  )
...
def main(config):
  ...
  results = collate_results(
      config, benchmark_results, model_size, cache_size, num_model_params
  )
