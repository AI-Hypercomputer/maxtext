...
def prefill_benchmark_loop(config, engine, decode_state, params, tokens, true_length, iters, profile_name=""):
  ...
  for i in range(iters):
    ...
    _, _, _ = summarize_pytree_data(prefill_result, name="Prefill Result")
...
def prefill_benchmark(config, engine, params, decode_state, tokens, true_length,
  ...
  prefill_tflops_per_sec_per_device = prefill_tflops_per_device / prefill_avg_seconds
  print(f"\tPrefill step average time: {prefill_average_ms:.3f}ms\n"
        f"\tPrefill total TFLOPs: {prefill_tflops_per_device:.3f}\n"
        f"\tPrefill TFLOPs/sec/device: {prefill_tflops_per_sec_per_device:.3f}\n\n\n\n")
  result_dict = {
                  "prefill_time_in_ms": prefill_average_ms,
                  "prefill_total_tflops": prefill_tflops_per_device,
                  "prefill_tflops_per_sec_per_device": prefill_tflops_per_sec_per_device,
