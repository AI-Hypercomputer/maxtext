...
class RayTpuManager:
    ...
    def remote(cls,
               ...
               *args, **kwargs) -> List[Union[ray.actor.ActorHandle, ray._raylet.ObjectRef]]:
        ...
        if multislice:
            ...
            for tpu in tpus:
                # Schedule on the lead worker first to consume the HEAD resource
                handles += [actor_or_fn.options(resources={"TPU": 4, tpu.name: 1, f"TPU-{tpu.topology}-head": 1}).remote(*args, **kwargs)]
                time.sleep(1)
                handles += [actor_or_fn.options(resources={"TPU": 4, tpu.name: 1}).remote(*args, **kwargs) for _ in range(tpu.num_hosts - 1)]
        return handles
