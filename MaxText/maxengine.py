...
class MaxEngine(engine_api.Engine):
  ...
  def load_params(self, *args, **kwargs) -> Params:
    ...
    self.kv_cache_annotations = max_utils.get_kv_cache_annotations(
        self.model, self.config, self.rng, self._mesh
    )
    self.kv_cache_shardings = jax.tree_map(
    ...
    if not self.model.quant:
      ...
      def model_apply(_p, _rng):
        return self.model.apply(
            _p | {"aqt": {}},
            jnp.ones(
                (1, self.config.max_prefill_predict_length), dtype=jnp.int32
            ),
            jnp.ones(
                (1, self.config.max_prefill_predict_length), dtype=jnp.int32
            ),
            decoder_segment_ids=jnp.zeros(
                (1, self.config.max_prefill_predict_length), dtype=jnp.int32
            ),
      ...
      params["params"] = quantizations.remove_quantized_params(
          state.params["params"], new_vars["aqt"]
      )
  ...
  def prefill(
    ...
    one_d_output = (
        ones_to_keep * common_types.DECODING_ACTIVE_SEQUENCE_INDICATOR
    )
  ...
  def get_tokenizer(self) -> tokenizer_pb2.TokenizerParameters:
    ...
    return tokenizer_pb2.TokenizerParameters(
        path=self.config.tokenizer_path, extra_ids=0
    )
  ...
  def init_decode_state(self, *args, **kwargs) -> DecodeState:
    ...
    def init(abstract_params):
      ...
      _, cache = self.model.apply(
          ...
          x,
          x,
          decoder_segment_ids=jnp.zeros(x.shape, dtype=jnp.int32)
    ...
    shardings = jax.tree_map(
        lambda mesh_annotation: jax.sharding.NamedSharding(
            self._mesh, mesh_annotation
        ),
        mesh_annotations,
    )
