...
class MaxEngine(engine_api.Engine):
  ...
  def prefill(
    ...
    return {"logits" : selected_logits, "cache" : new_vars['cache'], "next_pos" : next_pos, "generated_tokens" : generated_tokens}
  ...
  def generate(
    ...
    return {"logits" : out_logits, "cache" : new_cache, "next_pos" : decode_state["next_pos"]+1, "generated_tokens" : decode_state["generated_tokens"]+1}, result
  ...
  def insert(
    ...
    inserted_generated_tokens = jax.lax.dynamic_update_index_in_dim(decode_state['generated_tokens'], unboxed_prefix['generated_tokens'], slot, 0)
