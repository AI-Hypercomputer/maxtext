...
class MaxEngine(engine_api.Engine):
  ...
  def get_tokenizer(self) -> TokenizerParameters:
    ...
    try:
      ...
      return TokenizerParameters(path=self.config.tokenizer_path, tokenizer_type=tokenizer_type_val,
                                 access_token=self.config.tokenizer_access_token, extra_ids=0)
    except KeyError:
      raise KeyError(f"Unsupported tokenizer type: {self.config.tokenizer_type}")
