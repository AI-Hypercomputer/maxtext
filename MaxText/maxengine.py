...
class MaxEngine(engine_api.Engine):
  ...
  def load_params(self, *args, **kwargs) -> Params:
    ...
    self.abstract_params = jax.tree_util.tree_map(lambda x: jax.ShapeDtypeStruct(shape=x.shape, dtype=x.dtype, sharding=x.sharding),
                                        state.params)
  ...
  def init_decode_state(self, *args, **kwargs) -> DecodeState:
    ...
    shardings = jax.tree_util.tree_map(lambda mesh_annotation : jax.sharding.NamedSharding(self._mesh, mesh_annotation),
                             mesh_annotations)
