...
class MaxEngine(engine_api.Engine):
  ...
  def init_decode_state(self, *args, **kwargs) -> DecodeState:
    ...
    def init(abstract_params):
      input_shape = (int(self.config.per_device_batch_size * jax.device_count()), self.config.max_prefill_predict_length)
      model_mode = common_types.MODEL_MODE_PREFILL
      decoder_segment_ids=jnp.zeros(input_shape, dtype=jnp.int32) + common_types.DECODING_ACTIVE_SEQUENCE_INDICATOR
      if self.config.attention == "paged":
        input_shape = (int(self.config.per_device_batch_size * jax.device_count()), 1)
        model_mode=common_types.MODEL_MODE_AUTOREGRESSIVE
