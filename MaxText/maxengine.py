...
class MaxEngine(engine_api.Engine):
  ...
  def _prefill_jit(
    ...
    if self.config.use_multimodal:
      input_images = jnp.expand_dims(images, 0)  # Add batch dimension [B, H, W, C]
      input_images = jnp.expand_dims(input_images, 1)  # Add sequence dimension [B, N, H, W, C]
  ...
  def init_decode_state(
    ...
    def init(abstract_params, page_state):
      ...
      dummy_image = jnp.ones((int(self.config.per_device_batch_size * jax.device_count()), 1, self.config.image_size_for_vit, self.config.image_size_for_vit, 3), )
