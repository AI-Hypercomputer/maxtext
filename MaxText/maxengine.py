...
class MaxEngine(engine_api.Engine):
  ...
  def prefill_insert(  # pylint: disable=too-many-positional-arguments
    ...
    if self.page_manager is not None and self.page_state is not None:
      self.page_state = self.page_manager.update_prefill_pages(
          page_state=self.page_state, request_id=slot, true_length=true_length
      )
  ...
  def insert(self, prefix: Prefix, decode_state: DecodeState, slot: int) -> DecodeState:
  ...
  def release_pages(self, slot: int):
    ...
    if self.page_manager is not None and self.page_state is not None:
      self.page_state = self.page_manager.release_pages(page_state=self.page_state, request_id=slot)
      return True
    return False
