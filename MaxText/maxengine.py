...
class MaxEngine(engine_api.Engine):
  ...
  def release_pages(self, slot: int):
    ...
    print(f"Engine: Releasing pages for slot {slot}.")
    new_page_state = self.page_manager.release_pages(page_state=self.page_state, page_group_id=slot)

    self.page_state = new_page_state
    jax.block_until_ready(self.page_state)
    print(f"Engine: Pages released for slot {slot}.")
