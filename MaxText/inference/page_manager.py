...
def _update_prefill_pages_for_group(
  ...
  def handle_zero_length(_unused_operand) -> PageState:
    """Handles the case where true_length is 0."""
    return released_state

  def handle_normal_case(_unused_operand) -> PageState:
    """Handles the case where true_length > 0."""
    ...
    has_enough_resources = (int(num_free_pages) >= num_pages_needed) and group_has_capacity

    def allocate_and_update_state(initial_state_tuple: Tuple[PagesInt1d, GroupsPagesInt2d, GroupsInt1d]) -> PageState:
      """Allocates pages if resources are sufficient."""

      def allocate_one_page(
          page_idx_in_group: int, loop_state_tuple: Tuple[PagesInt1d, GroupsPagesInt2d, GroupsInt1d]
        ...
        new_loop_status = jax.lax.cond(
            page_allocated, lambda s: s.at[next_free_page_global].set(1), lambda s: s, current_loop_status
        )
        ...
        new_loop_num_used = jax.lax.cond(
            page_allocated, lambda n: n.at[page_group_id].add(1), lambda n: n, current_loop_num_used
        )
      ...
      final_page_status, final_page_map, final_num_pages_used = jax.lax.fori_loop(
          0, num_pages_needed, allocate_one_page, initial_state_tuple
      )
    ...
    if has_enough_resources:
      return allocate_and_update_state((current_page_status, current_page_map, current_num_pages_used))
    else:
      return released_state

  if true_length == 0:
    return handle_zero_length(None)
  else:
...
def _update_decode_pages_global(
  ...
  def allocate_for_group_if_needed(group_idx: int, current_state: PageState) -> PageState:
...
class PageManager:
  ...
  def __init__(self, config: Config):
    ...
    self.max_page_groups: int = config.pagedattn_max_page_groups
  ...
  def update_prefill_pages(self, page_state: PageState, page_group_id: int, true_length: int) -> PageState:
    ...
    Example:
      ...
      ```
    """
    is_valid_group = (page_group_id >= 0) and (page_group_id < self.max_page_groups)
    is_valid_length = (true_length >= 0) and (true_length <= self.max_target_length)
    is_valid_input = is_valid_group and is_valid_length
    ...
    if is_valid_input:
      ...
      return page_state  # Return unchanged state if input is invalid
  ...
  def release_pages(self, page_state: PageState, page_group_id: int) -> PageState:
    ...
    Example:
      ...
      ```
    """
    is_valid_input = (page_group_id >= 0) and (page_group_id < self.max_page_groups)
