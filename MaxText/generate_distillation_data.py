...
Example command:
  ...
  steps=100 decode_sampling_strategy=weighted ici_tensor_parallelism=4 \
...
async def send_request(config, parser_config, request, stub, progress_bar):  # pylint: disable=redefined-outer-name
  ...
  completion_tokens = [[] for _ in range(parser_config.num_generations)]
...
async def run_inference(config, parser_config, requests, step):  # pylint: disable=redefined-outer-name
  """Asynchronously runs inference on JetStream server."""
  progress_bar = tqdm(total=len(requests))
  progress_bar.set_description(f"Step {step} - Running inference on {len(requests)} prompts")
...
def generate_completions(config, parser_config, data, step):  # pylint: disable=redefined-outer-name
...
def upload_data_to_hf(distillation_data, hf_repo_id, hf_access_token, batch_num):
  ...
  try:
    distillation_dataset.to_parquet(f"train-{batch_num}.parquet")
    upload_file(
        repo_id=full_repo_name,
        repo_type="dataset",
        path_or_fileobj=f"train-{batch_num}.parquet",
        path_in_repo=f"data/train-{batch_num}.parquet",
...
def generate_data(config, parser_config):  # pylint: disable=redefined-outer-name
  ...
  for step in range(config.steps):
    ...
    prompts_completions = generate_completions(config, parser_config, data_batch, step)
    distillation_data.extend(prompts_completions)
    if step % config.checkpoint_period == 0:
      upload_data_to_hf(distillation_data, parser_config.hf_repo_id, config.hf_access_token, batch_num)
      batch_num += 1
      distillation_data = []
  return distillation_data, batch_num
...
def main(config, parser_config):  # pylint: disable=redefined-outer-name
  if config.decode_sampling_strategy == "greedy":
    max_logging.log("Only one sample will be generated per prompt when decode_sampling_strategy=greedy.")
    parser_config.num_generations = 1

  jetstream_server = None
  try:
    jetstream_server = run_jetstream_server(config, parser_config)
    distillation_data, batch_num = generate_data(config, parser_config)
