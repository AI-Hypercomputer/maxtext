...
def main(raw_args=None) -> None:
  ...
  hidden_dim, embed_dim = params["transformer"]["layer_0"]["mlp"]["linear"][
      "w"
  ].shape
  ...
  for layer_idx in range(num_layers):
    ...
    if args.model_size == "2b":  # MQA
      self_attention["query"]["kernel"].append(
          params["transformer"][in_layer_name]["attn"]["q_einsum"][
              "w"
          ].transpose((1, 0, 2))
          * head_dim**-0.5
      )
      self_attention["key"]["kernel"].append(
          params["transformer"][in_layer_name]["attn"]["kv_einsum"]["w"][
              0
          ].transpose((1, 0, 2))
      )
      ...
      self_attention["query"]["kernel"].append(
          params["transformer"][in_layer_name]["attn"]["qkv_einsum"]["w"][
              0
          ].transpose((1, 0, 2))
          * head_dim**-0.5
      )
    ...
    self_attention["out"]["kernel"].append(
        params["transformer"][in_layer_name]["attn"]["attn_vec_einsum"]["w"]
    )
    # mlp
    layer_weight["mlp"]["wi_0"]["kernel"].append(
        params["transformer"][in_layer_name]["mlp"]["gating_einsum"]["w"][0]
    )
    layer_weight["mlp"]["wi_1"]["kernel"].append(
        params["transformer"][in_layer_name]["mlp"]["gating_einsum"]["w"][1]
    )
    layer_weight["mlp"]["wo"]["kernel"].append(
        params["transformer"][in_layer_name]["mlp"]["linear"]["w"]
    )
    layer_weight["pre_self_attention_norm"]["scale"].append(
        params["transformer"][in_layer_name]["pre_attention_norm"]["scale"] + 1
    )
    layer_weight["pre_ffw_norm"]["scale"].append(
        params["transformer"][in_layer_name]["pre_ffw_norm"]["scale"] + 1
    )
  ...
  self_attention["out"]["kernel"] = np.array(
  ...
  layer_weight["pre_self_attention_norm"]["scale"] = np.array(
      layer_weight["pre_self_attention_norm"]["scale"]
  ).transpose((1, 0))
  layer_weight["pre_ffw_norm"]["scale"] = np.array(
      layer_weight["pre_ffw_norm"]["scale"]
  ).transpose((1, 0))
