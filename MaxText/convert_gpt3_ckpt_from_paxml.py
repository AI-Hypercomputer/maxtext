...
r"""Convert weights from a paxml gpt3 model to a MaxText one.

Test cmd for gpt3-dummy:
python convert_gpt3_ckpt_from_paxml.py \
  --paxml-ckpt-path=gs://lizhiyu-multipods/lizhiyu/colab/ckpt_test/paxml/checkpoints/checkpoint_00000000/state \
  --maxtext-model-name=gpt3-dummy \
  --run-name=ckpt_test \
  --base-output-directory=gs://lizhiyu-multipods/lizhiyu/colab
...
python convert_gpt3_ckpt_from_paxml.py \
  ...
  --base-output-directory=gs://lizhiyu-multipods/mlperf
"""
...
def convert(paxml_ckpt_path, maxtext_model_name, base_output_directory, run_name):
  ...
  keystr_map = {
    ...
    "['position_embedder']['embedding']": (".params.lm.position_emb.emb_var", None),
    "['decoder']['decoder']['pre_self_attention_layer_norm']['scale']": (".params.lm.transformer.repeat.sub.x_layers_0.layer_norm.scale", lambda x: np.moveaxis(x, 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['pre_self_attention_layer_norm']['bias']": (".params.lm.transformer.repeat.sub.x_layers_0.layer_norm.bias", lambda x: np.moveaxis(x, 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['self_attention']['query']['kernel']": (".params.lm.transformer.repeat.sub.x_layers_0.self_attention.combined_qkv.w", lambda x: np.moveaxis(x[:,0], 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['self_attention']['query']['bias']": (".params.lm.transformer.repeat.sub.x_layers_0.self_attention.combined_qkv.b", lambda x: np.moveaxis(x[:,0], 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['self_attention']['key']['kernel']": (".params.lm.transformer.repeat.sub.x_layers_0.self_attention.combined_qkv.w", lambda x: np.moveaxis(x[:,1], 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['self_attention']['key']['bias']": (".params.lm.transformer.repeat.sub.x_layers_0.self_attention.combined_qkv.b", lambda x: np.moveaxis(x[:,1], 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['self_attention']['value']['kernel']": (".params.lm.transformer.repeat.sub.x_layers_0.self_attention.combined_qkv.w", lambda x: np.moveaxis(x[:,2], 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['self_attention']['value']['bias']": (".params.lm.transformer.repeat.sub.x_layers_0.self_attention.combined_qkv.b", lambda x: np.moveaxis(x[:,2], 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['self_attention']['combined_qkv']['kernel']": (".params.lm.transformer.repeat.sub.x_layers_0.self_attention.combined_qkv.w", lambda x: np.moveaxis(x, [0, 1], [cfg.param_scan_axis, -1])),
    "['decoder']['decoder']['self_attention']['combined_qkv']['bias']": (".params.lm.transformer.repeat.sub.x_layers_0.self_attention.combined_qkv.b", lambda x: np.moveaxis(x, [0, 1], [cfg.param_scan_axis, -1])),
    "['decoder']['decoder']['self_attention']['out']['kernel']": (".params.lm.transformer.repeat.sub.x_layers_0.self_attention.post.w", lambda x: np.moveaxis(x, [0, 1], [cfg.param_scan_axis, -1])),
    "['decoder']['decoder']['self_attention']['out']['bias']": (".params.lm.transformer.repeat.sub.x_layers_0.self_attention.post.b", lambda x: np.moveaxis(x, 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['mlp']['mlp_layer_norm']['scale']": (".params.lm.transformer.repeat.sub.x_layers_0.ff_layer.layer_norm.scale", lambda x: np.moveaxis(x, 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['mlp']['mlp_layer_norm']['bias']": (".params.lm.transformer.repeat.sub.x_layers_0.ff_layer.layer_norm.bias", lambda x: np.moveaxis(x, 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['mlp']['wi']['kernel']": (".params.lm.transformer.repeat.sub.x_layers_0.ff_layer.ffn_layer1.linear.w", lambda x: np.moveaxis(x, 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['mlp']['wi']['bias']": (".params.lm.transformer.repeat.sub.x_layers_0.ff_layer.ffn_layer1.bias.b", lambda x: np.moveaxis(x, 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['mlp']['wo']['kernel']": (".params.lm.transformer.repeat.sub.x_layers_0.ff_layer.ffn_layer2.linear.w", lambda x: np.moveaxis(x, 0, cfg.param_scan_axis)),
    "['decoder']['decoder']['mlp']['wo']['bias']": (".params.lm.transformer.repeat.sub.x_layers_0.ff_layer.ffn_layer2.bias.b", lambda x: np.moveaxis(x, 0, cfg.param_scan_axis)),
  ...
  memory_metrics = {'max_cpu_bytes': 0, 'max_per_device_tpu_bytes': 0}
  ...
  def map_fn(key_path, value):
    ...
    max_cpu_bytes, max_per_device_tpu_bytes = check_memory()
    memory_metrics["max_cpu_bytes"] = max(max_cpu_bytes, memory_metrics["max_cpu_bytes"])
    memory_metrics["max_per_device_tpu_bytes"] = max(max_per_device_tpu_bytes, memory_metrics["max_per_device_tpu_bytes"])
    return result
  ...
  max_logging.log(f"Peak tpu memory in a single device: {fmt_size(memory_metrics['max_per_device_tpu_bytes'])}")
  max_logging.log("checkpoint converted and saved successfully.")

if __name__ == "__main__":
  parser = argparse.ArgumentParser()
  parser.add_argument('--paxml-ckpt-path', type=str, default="gs://mlperf-llm-public2/gpt3_spmd1x64x24_tpuv4-3072_v84_20221101/checkpoints/checkpoint_00004000", required=True)
