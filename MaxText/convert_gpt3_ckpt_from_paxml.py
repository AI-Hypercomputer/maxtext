...
def check_memory():
  ...
  for d in jax.local_devices():
    ...
    max_logging.log(
        f"tpu memory: Using {fmt_size(used)} / {fmt_size(limit)} ({used/limit:%}) on {d}"
    )


def convert(
    paxml_ckpt_path, maxtext_model_name, base_output_directory, run_name
):
  """convert ckpt."""
  ...
  state, _, _ = max_utils.setup_training_state(
      model, None, tx, cfg, init_rng, mesh, checkpoint_manager
  )
  ...
  keystr_map = {
      ...
      "['decoder']['position_embedder']['embedding']": (
          ".params.lm.position_emb.emb_var",
          None,
      ),
  ...
  def map_fn(key_path, value):
    ...
    # log peak cpu memory
    cpu_bytes = Process().memory_info().rss
    memory_metrics["max_cpu_bytes"] = max(
        cpu_bytes, memory_metrics["max_cpu_bytes"]
    )
  ...
  max_logging.log(
      f"Peak cpu memory in a single process: {fmt_size(memory_metrics['max_cpu_bytes'])}"
  )
...
if __name__ == "__main__":
  ...
  if not args.paxml_ckpt_path.startswith("gs://"):
    raise ValueError(
        "--paxml-ckpt-path should be a gcs path starting with gs://"
    )
