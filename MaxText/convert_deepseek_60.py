      ...
      mapping.update(
          {
              ...
              f"model.layers.60.mlp.experts.{expert_idx}.gate_proj.weight": f"mtp_block.mtp_layer_1.mtp_1_transformer_layer.DeepSeekMoeBlock_0.MoeBlock_0.{expert_idx}.wi_0",
              f"model.layers.60.mlp.experts.{expert_idx}.up_proj.weight": f"mtp_block.mtp_layer_1.mtp_1_transformer_layer.DeepSeekMoeBlock_0.MoeBlock_0.{expert_idx}.wi_1",
          }
      )
    mapping.update(
        {
...
def _convert_huggingface_to_jax_weights(base_model_path, model_params, mem_info) -> dict:
  ...
  # =========================================================================================
  # ❗ NEW LOGIC: Manually create the MTP block by copying from layer 60 and loading from layer 61
  # =========================================================================================
  max_logging.log("Creating MTP block by copying weights from layer 60...")

  # Step 1: Load the unique MTP head weights from chkpt_vars (source is layer 61)
  ...
  # Step 2: Manually copy the transformer block weights from the already-processed layer 60

  # The source MoE layer index for the original model's layer 60
  # For deepseek3-671b, this will be 60 - 3 = 57
  source_moe_idx = base_num_decoder_layers - first_num_dense_layers - 1

  # Get references to the source and destination dictionaries for easier access
  mtp_transformer_layer = jax_weights["mtp_block"]["mtp_layer_1"]["mtp_1_transformer_layer"]
  source_moe_layers = jax_weights["decoder"]["moe_layers"]

  # Copy Layer Norms
  # The layer index is the first dimension [0] for these transposed arrays
  mtp_transformer_layer["pre_self_attention_layer_norm"]["scale"] = source_moe_layers["pre_self_attention_layer_norm"][
      "scale"
  ][source_moe_idx, :]
  mtp_transformer_layer["post_self_attention_layer_norm"]["scale"] = source_moe_layers["post_self_attention_layer_norm"][
      "scale"
  ][source_moe_idx, :]

  # Copy Attention weights
  mtp_attn_block = mtp_transformer_layer["self_attention"]
  source_attn_block = source_moe_layers["self_attention"]

  # The layer index is the second dimension [1] for most of these transposed arrays
  mtp_attn_block["kv_norm"]["scale"] = source_attn_block["kv_norm"]["scale"][source_moe_idx, :]
  mtp_attn_block["wkv_a"]["kernel"] = source_attn_block["wkv_a"]["kernel"][:, source_moe_idx, :]
  mtp_attn_block["wkv_b"]["kernel"] = source_attn_block["wkv_b"]["kernel"][:, source_moe_idx, :, :]
  mtp_attn_block["out"]["kernel"] = source_attn_block["out"]["kernel"][:, source_moe_idx, :, :]

  if q_lora_rank != 0:
    mtp_attn_block.update({"q_norm": {"scale": None}, "wq_a": {"kernel": None}, "wq_b": {"kernel": None}})
    mtp_attn_block["q_norm"]["scale"] = source_attn_block["q_norm"]["scale"][source_moe_idx, :]
    mtp_attn_block["wq_a"]["kernel"] = source_attn_block["wq_a"]["kernel"][:, source_moe_idx, :]
    mtp_attn_block["wq_b"]["kernel"] = source_attn_block["wq_b"]["kernel"][:, source_moe_idx, :, :]
  else:
    mtp_attn_block.update({"query": {"kernel": None}})
    mtp_attn_block["query"]["kernel"] = source_attn_block["query"]["kernel"][:, source_moe_idx, :, :]

  # Copy MoE weights
  mtp_moe_block = mtp_transformer_layer["DeepSeekMoeBlock_0"]
  source_moe_block = source_moe_layers["DeepSeekMoeBlock_0"]

  # Copy shared experts and gate
  mtp_moe_block["shared_experts"]["wi_0"]["kernel"] = source_moe_block["shared_experts"]["wi_0"]["kernel"][
      :, source_moe_idx, :
  ]
  mtp_moe_block["shared_experts"]["wi_1"]["kernel"] = source_moe_block["shared_experts"]["wi_1"]["kernel"][
      :, source_moe_idx, :
  ]
  mtp_moe_block["shared_experts"]["wo"]["kernel"] = source_moe_block["shared_experts"]["wo"]["kernel"][:, source_moe_idx, :]
  mtp_moe_block["MoeBlock_0"]["gate"]["kernel"] = source_moe_block["MoeBlock_0"]["gate"]["kernel"][:, source_moe_idx, :]
  if q_lora_rank != 0:
    mtp_moe_block["MoeBlock_0"]["gate"]["bias"] = source_moe_block["MoeBlock_0"]["gate"]["bias"][source_moe_idx, :]

  # Copy expert weights
  mtp_moe_block["MoeBlock_0"]["wi_0"] = source_moe_block["MoeBlock_0"]["wi_0"][:, source_moe_idx, :, :]
  mtp_moe_block["MoeBlock_0"]["wi_1"] = source_moe_block["MoeBlock_0"]["wi_1"][:, source_moe_idx, :, :]
  mtp_moe_block["MoeBlock_0"]["wo"] = source_moe_block["MoeBlock_0"]["wo"][:, source_moe_idx, :, :]

  max_logging.log("Finished creating MTP block.")
