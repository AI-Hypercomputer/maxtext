...
class PrefillHelper:
  ...
  def __init__(self, type: str, engine: MaxEngine):
    ...
    if type == "default":
      self._processor = {}
    elif type == "prefill_packing":
      self._processor = prefill_packing.PrefillProcessor(engine)
    elif type == "dummy":
  ...
  def aot_compile(self,
    ...
    if self._type == "default":
      ...
      for bucket in buckets:
        log.info(f"compile prefill process({bucket})")
        self._processor[bucket] = (
          jax.jit(
              self._default_process,
              in_shardings=(params_layout, None, None, None, decode_state_layout),
              out_shardings=(None, decode_state_layout,),
              donate_argnames=("decode_state"),
          )
          .lower(
              params,
              jax.ShapeDtypeStruct((bucket,), jnp.dtype("int32")),
    ...
    elif self._type == "prefill_packing":
      ...
      for bucket in buckets:
        for num_prompts in range(1, 2 * max_length // bucket):
          self._processor.aot_compile(params, bucket, max_length, num_prompts)
    else:
  ...
  def process(self,
    ...
    if self._type == "default":
      assert padded_length in self._processor, f"padded length: {padded_length} supported length: {self._processor.keys()}"
      first_token, decode_state = self._processor[padded_length](
        model_params,
        input_tokens_padded,
        decode_slot,
        input_true_length,
  ...
  def finalize(self,
    ...
    if self._type == "default":
      ...
      self._processor.flush(model_params, decode_state, prefill_done)
    else:
      assert self._type == "dummy", f"type: {self._type}"

  def _default_process(self, params, tokens, slot, true_length, decode_state):
...
class OfflineInference:
  ...
  def __init__(self, engine: engine_api.Engine, params, base_engine: engine_api.Engine, enable_batch_prefill: bool):
    ...
    if self.dummy:
      ...
      self.prefill = PrefillHelper("prefill_packing", self.engine)
    else:
  ...
  def batch_inference_with_callback(
    ...
    log.info(f"summary-{desc}-prefills-{counter.prefill}-decodes-{counter.decode} completed.")
