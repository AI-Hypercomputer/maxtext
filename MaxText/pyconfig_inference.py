...
class __HyperParameters:
  ...
  def __init__(self, argv: list[str], **kwargs):
    ...
    max_logging.log(f"Pyconfig_inference: Updating keys from env and command line: {keys_from_env_and_command_line}")
    keys_from_model = _HyperParameters.update_model_vars(argv[1], raw_keys, config_name)
    max_logging.log(f"Pyconfig_inference: Updating keys from model: {keys_from_model}")
