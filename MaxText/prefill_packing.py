...
class PrefillBucket:
  ...
  def try_add(self, slot: int, id: int, prompt: jax.Array) -> bool:
    ...
    if len(prompt) > self.capacity:
      raise ValueError(f"Prefill length exceeds capacity. prefill length: {len(prompt)}, capacity: {self.capacity}")
    if len(prompt) > self.unallocated():
      return False
  ...
  def add(self, slot: int, id: int, prompt: jax.Array) -> None:
    """Adds a prefill prompt to bucket.

    Raise ValueError if fails.
    """
    if not self.try_add(slot, id, prompt):
      raise ValueError(f"Not enough space. prefill length: {len(prompt)}, unallocated length: {self.capacity - self.length}")
...
class PrefillProcessor:
  """An extra layer upon MaxEngine prefill API, provides prefill packing feature."""
  ...
  def process(
    ...
    if len(input_prompt) > bucket.unallocated():
      ...
      if prefill_done:
        prefill_done(prefill_result, bucket.ids, decode_state)
      bucket.clear()
  ...
  def _process_bucket(
    ...
    lengths = jnp.array([len(prompt) for prompt in bucket.prompts])
    tokens = jnp.concat(bucket.prompts)
    slots = jnp.array(bucket.slots)
    positions = jnp.concat([jnp.arange(0, length) for length in lengths])
    prefill_masks = jnp.concat([jnp.full(lengths[i], i * 2 + 1, dtype=int) for i in range(len(lengths))])
    start_pos = jnp.cumsum(jnp.insert(lengths, 0, 0))[:-1]

    # Prepare padding functions.
    # Use padding below to keep static shape of jitted function input.
    nulls_fn = functools.partial(jnp.zeros, dtype=jnp.dtype("int32"))
    zeros_fn = functools.partial(jnp.zeros, dtype=int)
    range_fn = functools.partial(jnp.arange, dtype=int)
    def padded(input: jax.Array, padding: int, fn: Any):
      if len(input) < padding:
        input = jnp.append(input, fn(padding - len(input)))
      return input
    ...
    first_tokens, decode_state = prefill_fn(
        model_params,
        padded(tokens,        bucket.capacity, nulls_fn),
        padded(slots,         16,              zeros_fn),
        padded(positions,     bucket.capacity, range_fn),
        padded(prefill_masks, bucket.capacity, zeros_fn),
        padded(start_pos,     16,              zeros_fn),
        padded(lengths,       16,              zeros_fn),
        decode_state,
    )
