...
class PageManager(nn.Module):
    ...
    def _validate_page_group(self, page_group_id: int):
        ...
        _ = jax.lax.cond(
            ...
            lambda _: jax.debug.callback(raise_page_group_error, page_group_id, self.max_page_groups), # Use callback to raise error
    ...
    def _reserve_single_page(self, page_group_id: int, i: int,
    ...
    def reserve_prefill_page_group_pages(
        ...
        def release_scan(carry, page_idx_in_group):
            ps, pm = carry
        ...
        (page_status, page_map), _ = jax.lax.scan(
            ...
            jnp.arange(self.max_pages_per_group)
        )
        ...
        cur_page = jnp.where(num_pages_needed > 0,
                            page_map[page_group_id, num_pages_needed - 1],
                            -1)
    ...
    def reserve_decode_step_pages(self, page_status, page_map, sequence_lengths, num_pages_used, current_page, current_page_position) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        ...
        (page_status, page_map, current_page, num_pages_used), _ = jax.lax.scan(
            ...
            jnp.arange(self.max_page_groups)
        )
    ...
    def __call__(self, model_mode: Optional[str] = None, page_group_id: Optional[int] = None,
        ...
        if model_mode == "prefill":
            ...
            page_status, page_map, sequence_lengths, num_pages_used, current_page, current_page_position = (
                self.reserve_prefill_page_group_pages(
                    page_group_id, true_length, page_status, page_map,
