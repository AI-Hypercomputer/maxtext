...
def ar_benchmark(config, engine, params, tokens, true_length, steps=10, cache_size=None, model_size=None): 
  decode_state = engine.init_decode_state()
  if cache_size == None:
...
def main(config):
  ...
  results = {"config": {}, 
             "sizes": {
                "model_size_in_GB": model_size / 2**30,
                "cache_size_in_GB": cache_size / 2**30,
  ...
  for prefill_length in prefill_lengths:
    ...
    ar_results = ar_benchmark(config, engine, params, tokens, true_length, num_steps, cache_size, model_size)
