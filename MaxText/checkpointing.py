...
def process_replicator_error_file(error_file: str):
...
def setup_checkpoint_logger(config):  # pytype: disable=attribute-error
...
def maybe_save_checkpoint(checkpoint_manager, state, config, data_iterator, step=None):
  ...
  if save_checkpoint(checkpoint_manager, actual_step, state, config, data_iterator, force_ckpt_save):
