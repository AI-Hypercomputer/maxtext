...
def broadcast_one_slice_to_all(
  ...
  out_tree = ()
  N = len(in_tree)
  for chunck_idx in range(num_passes):
    in_tree_chunck = in_tree[chunck_idx*N//num_passes: (chunck_idx+1)*N//num_passes]
    out_sharding_chunk = jax.tree_map(
      lambda x: jax.sharding.NamedSharding(
          global_mesh, jax.sharding.PartitionSpec(*x.sharding.spec)
      ),
      in_tree_chunck
      )
    in_tree_sharded = jax.tree_map(pre_jit, in_tree_chunck,
                                  per_slice_shardings[chunck_idx*N//num_passes: (chunck_idx+1)*N//num_passes])
    out_tree_chunck = jax.jit(_sum, out_shardings=out_sharding_chunk)(in_tree_sharded)
    out_tree += out_tree_chunck
