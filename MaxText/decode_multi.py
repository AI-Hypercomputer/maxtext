...
def _validate_config(config):
  ...
  assert config.attention == "paged", "This script requires paged attention."
  assert (
...
def main(argv: Sequence[str]) -> None:
  ...
  max_utils.print_mem_stats("After decode state init")
  ...
  for slot_idx, prefill_result in prefill_results_to_insert.items():
    ...
    max_utils.print_mem_stats(f"After inserting stream {slot_idx}")
  ...
  for step in range(total_steps):
    ...
    if active_stream_indices:
      ...
      max_utils.print_mem_stats(f"  After generate step {step+1}")
    ...
    if can_prefill_more and available_slots > 0:
      try:
        ...
        streams_prefilled_count += 1
        max_utils.print_mem_stats(f"  After prefill new stream {next_available_slot}")
        ...
        max_utils.print_mem_stats(f"  After inserting new stream {next_available_slot}")
