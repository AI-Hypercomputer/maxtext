...
def visualize_page_allocation(page_state, num_layers, num_pages, step, output_dir="./debug_output"):
    ...
    for layer in range(num_layers):
        ...
        if current_page >= 0:  # Only show valid pages
            ...
            plt.annotate(f"pos: {current_pos}", 
                        (current_page, layer),
    ...
    for layer in range(num_layers):
        ...
        if valid_pages:
            plt.plot(valid_pages, [layer] * len(valid_pages), markers[slot % len(markers)], 
                     markersize=4, label=f'Slot {slot}' if layer == 0 else "")
...
def main(argv: Sequence[str]) -> None:
    ...
    # Load model parameters using standard method (no patching needed)
    ...
    # Debug: Check initial page state if using paged attention
    ...
    # Insert
    print(f"\n🔄 Inserting prefill results into decode state...")
    ...
    for i in range(num_tokens):
        ...
        # Debug: Check page state after every few tokens
...
def validate_config(config):
    ...
    if config.attention == "paged":
        ...
        assert hasattr(config, 'pagedattn_max_pages_per_group'), "Config must include pagedattn_max_pages_per_group for paged attention"
