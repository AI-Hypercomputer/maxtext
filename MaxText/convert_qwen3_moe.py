...
def convert_hf_to_maxtext(base_model_path: str, model_params: dict) -> dict:
    ...
    maxtext_weights = {
        "decoder": {
            "layers": {
                ...
                "self_attention": {
                    "query": {"kernel": None}, "key": {"kernel": None}, "value": {"kernel": None},
                    "out": {"kernel": None}, "query_norm": {"scale": None}, "key_norm": {"scale": None},
                },
                "moe_block": { "gate": {"kernel": None}, "wi_0": None, "wi_1": None, "wo": None, },
            },
            "decoder_norm": {"scale": None}, "logits_dense": {"kernel": None},
        },
    ...
    maxtext_weights["decoder"]["logits_dense"]["kernel"] = chkpt_vars["decoder.logits_dense.kernel"].to(torch.float16).numpy().transpose()
    ...
    for l in tqdm(range(num_layers), desc="Stacking layer weights"):
        ln["pre_self_attention_layer_norm"]["scale"][l, :] = chkpt_vars[f"decoder.layers.{l}.pre_self_attention_layer_norm.scale"].to(torch.float16).numpy()
        ln["post_self_attention_layer_norm"]["scale"][l, :] = chkpt_vars[f"decoder.layers.{l}.post_self_attention_layer_norm.scale"].to(torch.float16).numpy()

        s_attn["query"]["kernel"][l, ...] = chkpt_vars[f"decoder.layers.{l}.self_attention.query.kernel"].to(torch.float16).numpy().transpose().reshape(hidden_size, num_heads, head_dim)
        s_attn["key"]["kernel"][l, ...] = chkpt_vars[f"decoder.layers.{l}.self_attention.key.kernel"].to(torch.float16).numpy().transpose().reshape(hidden_size, num_kv_heads, head_dim)
        s_attn["value"]["kernel"][l, ...] = chkpt_vars[f"decoder.layers.{l}.self_attention.value.kernel"].to(torch.float16).numpy().transpose().reshape(hidden_size, num_kv_heads, head_dim)
        s_attn["out"]["kernel"][l, ...] = chkpt_vars[f"decoder.layers.{l}.self_attention.out.kernel"].to(torch.float16).numpy().transpose().reshape(num_heads, head_dim, hidden_size)

        s_attn["query_norm"]["scale"][l, ...] = chkpt_vars[f"decoder.layers.{l}.self_attention.query_norm.scale"].to(torch.float16).numpy()
        s_attn["key_norm"]["scale"][l, ...] = chkpt_vars[f"decoder.layers.{l}.self_attention.key_norm.scale"].to(torch.float16).numpy()
        
        moe["gate"]["kernel"][l, ...] = chkpt_vars[f"decoder.layers.{l}.moe_block.gate.kernel"].to(torch.float16).numpy().transpose()
        for i in range(num_experts):
