...
def checkpoint_loop(config, state=None):
  ...
  Returns:
  """
  init_rng, _, checkpoint_manager, mesh, model, _, tx = setup_mesh_and_model(
      config
  )
  ...
  if state is not None:  # Checkpoint was available for restore
    if jax.process_index() == 0:
      max_logging.log(
          f"STANDALONE CHECKPOINTER : Checkpoint restored in : {checkpoint_load_end - checkpoint_load_start}"
      )
...
def add_entropy_to_checkpoint(state):
  ...
  opt_0 = opt_0._replace(
      mu=jax.tree_util.tree_map(
          lambda x: jax.random.normal(create_random_keys(x), shape=x.shape),
          state.params,
      )
  )
  opt_0 = opt_0._replace(
