...
def create_maxengine(
    devices: config_lib.Devices, config: Any
) -> engine_api.Engine:
  del devices
...
def get_server_config(
    config_str: str, config: Any
) -> Type[config_lib.ServerConfig]:
    ...
    case "MaxtextInterleavedServer":
      server_config = config_lib.ServerConfig(
          ...
          interleaved_engine_create_fns=(
              functools.partial(create_maxengine, config=config),
          ),
      )
    case _:
