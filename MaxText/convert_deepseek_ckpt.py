...
def _convert_huggingface_to_jax_weights(base_model_path, model_params, mem_info) -> dict:
  ...
  jax_weights["mtp_block"]["mtp_layer_1"]["mtp_1_embedding_norm"]["scale"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_embedding_norm.scale"].to(torch.float16).numpy()
  jax_weights["mtp_block"]["mtp_layer_1"]["mtp_1_hidden_state_norm"]["scale"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_hidden_state_norm.scale"].to(torch.float16).numpy()
  jax_weights["mtp_block"]["mtp_layer_1"]["mtp_1_projection"]["kernel"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_projection.kernel"].to(torch.float16).numpy().transpose()
  ...
  mtp_transformer_layer["pre_self_attention_layer_norm"]["scale"] = (chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.pre_self_attention_layer_norm.scale"].to(torch.float16).numpy())
  mtp_transformer_layer["post_self_attention_layer_norm"]["scale"] = (chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.post_self_attention_layer_norm.scale"].to(torch.float16).numpy())

  mtp_attn_block = mtp_transformer_layer["self_attention"]
  mtp_attn_block["kv_norm"]["scale"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.self_attention.kv_norm.scale"].to(torch.float16).numpy().transpose()
  mtp_attn_block["wkv_a"]["kernel"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.self_attention.wkv_a.kernel"].to(torch.float16).numpy().transpose()
  wkv_b = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.self_attention.wkv_b.kernel"].to(torch.float16).numpy().transpose()
  out = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.self_attention.out.kernel"].to(torch.float16).numpy().transpose()
  mtp_attn_block["wkv_b"]["kernel"] = np.reshape(wkv_b, [kv_lora_rank, base_num_query_heads, (qk_nope_head_dim + v_head_dim)])
  ...
  if q_lora_rank != 0:
      mtp_attn_block["q_norm"]["scale"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.self_attention.q_norm.scale"].to(torch.float16).numpy()
      mtp_attn_block["wq_a"]["kernel"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.self_attention.wq_a.kernel"].to(torch.float16).numpy().transpose()
      wq_b = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.self_attention.wq_b.kernel"].to(torch.float16).numpy().transpose()
      mtp_attn_block["wq_b"]["kernel"] = np.reshape(wq_b, [q_lora_rank, base_num_query_heads, (qk_nope_head_dim + qk_rope_head_dim)])
  else:
      query = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.self_attention.query.kernel"].to(torch.float16).numpy().transpose()
      mtp_attn_block["query"]["kernel"] = np.reshape(query, [base_emb_dim, base_num_query_heads, (qk_nope_head_dim + qk_rope_head_dim)])

  # MTP internal transformer layer - MoE Block
  moe = mtp_transformer_layer["DeepSeekMoeBlock_0"]
  moe["MoeBlock_0"]["gate"]["kernel"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.DeepSeekMoeBlock_0.MoeBlock_0.gate.kernel"].to(torch.float16).numpy().transpose()
  moe["shared_experts"]["wi_0"]["kernel"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.DeepSeekMoeBlock_0.shared_experts.wi_0.kernel"].to(torch.float16).numpy().transpose()
  moe["shared_experts"]["wi_1"]["kernel"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.DeepSeekMoeBlock_0.shared_experts.wi_1.kernel"].to(torch.float16).numpy().transpose()
  moe["shared_experts"]["wo"]["kernel"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.DeepSeekMoeBlock_0.shared_experts.wo.kernel"].to(torch.float16).numpy().transpose()
  if q_lora_rank != 0:
      moe["MoeBlock_0"]["gate"]["bias"] = chkpt_vars["mtp_block.mtp_layer_1.mtp_1_transformer_layer.DeepSeekMoeBlock_0.MoeBlock_0.gate.bias"].to(torch.float16).numpy().transpose()

  experts_wi0, experts_wi1, experts_wo = [], [], []
  for k in range(num_experts):
      experts_wi0.append(chkpt_vars[f"mtp_block.mtp_layer_1.mtp_1_transformer_layer.DeepSeekMoeBlock_0.MoeBlock_0.{k}.wi_0"].to(torch.float16).numpy().transpose())
      experts_wi1.append(chkpt_vars[f"mtp_block.mtp_layer_1.mtp_1_transformer_layer.DeepSeekMoeBlock_0.MoeBlock_0.{k}.wi_1"].to(torch.float16).numpy().transpose())
      experts_wo.append(chkpt_vars[f"mtp_block.mtp_layer_1.mtp_1_transformer_layer.DeepSeekMoeBlock_0.MoeBlock_0.{k}.wo"].to(torch.float16).numpy().transpose())
