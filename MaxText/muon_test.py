...
def _compute_muon_reshape(x: jax.Array, spec: MuonWeightSpec) -> tuple[ReshapeFn, ReshapeFn]:
  ...
  # Correct way: Get dimension sizes from x.shape and then compute their product
  get_dim_size = lambda axes: math.prod([x.shape[i] for i in axes])
  flat_shape = tuple(map(get_dim_size, [batch_axes, reduction_axes, output_axes]))
  reshape_fn = lambda x: x.transpose(transpose).reshape(flat_shape)
  # revision 2
  # inverse_fn = lambda x: x.reshape(batch_axes + output_axes + reduction_axes
  #  ).transpose(inv_transpose)
  # The shape of the array after the initial transpose, but before the flatten/reshape
  original_transposed_shape = (
      tuple(x.shape[i] for i in batch_axes)
      + tuple(x.shape[i] for i in output_axes)
      + tuple(x.shape[i] for i in reduction_axes)
  )
  # Now, use this correct shape in your lambda function
  inverse_fn = lambda y: y.reshape(original_transposed_shape).transpose(inv_transpose)
...
def orthogonalize_via_newton_schulz(
  ...
  if ns_coeffs.ndim > 2 or ns_coeffs.shape[-1] != 3:
    raise ValueError("Newton-Schulz coefficients must have shape (3,) or (n, 3), " f"got {ns_coeffs.shape}")
...
def scale_by_muon(
  ...
  def update_fn(updates, state, params=None):
    ...
    if weight_specs is not None:
      ...
      # updates = jax.tree.map(
      #     lambda x, spec: orthogonalize_via_newton_schulz(x, state.ns_coeffs, ns_steps, eps, spec), mu_hat, weight_specs, is_leaf=lambda x: isinstance(x, MuonWeightSpec) or x is None,
      # )
    else:
      print("mu_hat", mu_hat)
...
if __name__ == "__main__":
  ...
  # c = test_update(params, muon_weight_mask=None, muon_weight_specs=spec)
