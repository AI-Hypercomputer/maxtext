...
class TrainState(train_state.TrainState):
  ...
  def apply_to_slice(self, grads, params, stacked_opt_state, index):
    slice_opt_state = jax.tree.map(
        lambda x: jax.lax.dynamic_index_in_dim(x, index, 0, keepdims=False),
        stacked_opt_state,
    )
  ...
  def apply_gradients(self, *, grads, **kwargs):
    ...
    if stacked_params:
      ...
      def apply_to_slice(carry, xs):
        del carry
        grads, params, index = xs
        new_params, new_opt_state = self.apply_to_slice(
            grads, params, self.opt_state.stacked_opt_state, index
        )
      ...
      new_stacked_params, new_stacked_opt_state = jax.lax.scan(
          apply_to_slice,
          None,
          (stacked_grads, stacked_params, jnp.arange(0, stack_length)),
      )[1]
  ...
  def create(cls, *, apply_fn, params, tx, optimizer_memory_host_offload, **kwargs):
    ...
    if stacked_params:
      def init_slice(carry, params):
        del carry
        return None, tx.init(_add_dummy_stacking(params))
