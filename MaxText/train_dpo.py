...
def loss_fn(model, config, data, dropout_rng, params, reference_params, is_train=True):
...
def train_step(model, config, dpo_state, data, dropout_rng):
  """
  ...
  """
  state = dpo_state["train_state"]  
  reference_params = dpo_state["reference_params"]

  if config.gradient_accumulation_steps > 1:

    def accumulate_gradient(acc_grad_and_loss, data):
      grad_func = jax.value_and_grad(loss_fn, argnums=4, has_aux=True)
      (_, aux), cur_batch_gradient = grad_func(model, config, data, dropout_rng, state.params, reference_params, is_train=True)
    ...
    grad_func = jax.value_and_grad(loss_fn, argnums=4, has_aux=True)
    (loss, aux), raw_grads = grad_func(model, config, data, dropout_rng, state.params, reference_params, is_train=True)
  ...
  new_dpo_state = {"train_state": new_state, "reference_params": reference_params}
  metrics = {
      "scalar": {
          "learning/loss": loss,
          ...
          "learning/reward_accuracy": aux["reward_accuracy"],
      },
      "scalars": {},
  }
  ...
  return new_dpo_state, metrics


def eval_step(model, config, dpo_state, data, dropout_rng):
  """eval_step no backprop and new state compared with train_step."""
  state = dpo_state["train_state"]
  reference_params = dpo_state["reference_params"]
  eval_loss_fn = functools.partial(loss_fn, model, config, data, dropout_rng, reference_params, is_train=False)
...
def setup_train_loop(config):
  ...
  dpo_state = {"train_state": state, "reference_params": state.params}
  dpo_state_mesh_annotations = {"train_state": state_mesh_annotations, 
                                "reference_params": state_mesh_annotations.params}
...
def train_loop(config):
  ...
  # to properly allow donating argnums
  dpo_state["reference_params"] = jax.tree.map(jnp.copy, dpo_state["reference_params"])
  state = dpo_state["train_state"]
  # pylint: disable=line-too-long
  (
      functional_train,
  ...
  ) = maxtext_utils.get_functional_train_with_signature(train_step, mesh, dpo_state_mesh_annotations, model, config)
  ...
  if eval_data_iterator:
    ...
    (
        ...
        static_argnums_eval,
        donate_argnums_eval,
    ) = maxtext_utils.get_functional_eval_with_signature(eval_step, mesh, dpo_state_mesh_annotations, model, config)
  ...
  if config.compiled_trainstep_file != "":
    ...
    p_train_step = maxtext_utils.load_compiled(config, functional_train, dpo_state)
  ...
  for step in np.arange(start_step, config.steps):
    ...
    with jax.profiler.StepTraceAnnotation("train", step_num=step):
      ...
      with mesh, nn_partitioning.axis_rules(config.logical_axis_rules):
        dpo_state, metrics = p_train_step(dpo_state, example_batch, nextrng)
    ...
    if checkpoint_manager is not None:
      if save_checkpoint(checkpoint_manager, int(step), state["train_state"], config.dataset_type, data_iterator, config):
    ...
    if config.eval_interval > 0 and step > start_step and (step + 1) % config.eval_interval == 0:
      ...
      for eval_batch in eval_data_iterator:
        if config.eval_steps > 0 and eval_step_count >= config.eval_steps:
          break
        with mesh, nn_partitioning.axis_rules(config.logical_axis_rules):
          eval_metrics = p_eval_step(dpo_state, eval_batch, nextrng)
