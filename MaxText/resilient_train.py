...
class MaxtextTrainer(ray_cluster.ResilientWorker):
  ...
  def _train_loop(self, state=None):
    ...
    ) = maxtext_utils.get_functional_train_with_signature(train_step, mesh, state_mesh_annotations, model, self.config)
    ...
    if eval_data_iterator:
      ...
      (
          ...
          static_argnums_eval,
          donate_argnums_eval,
      ) = maxtext_utils.get_functional_eval_with_signature(eval_step, mesh, state_mesh_annotations, model, self.config)
    ...
    for step in np.arange(start_step, self.config.steps):
      with self.EnableHeartbeat():
        if step == first_profiling_step:
          if self.config.profile_cleanly:
        ...
        if checkpoint_manager is not None:
          if save_checkpoint(checkpoint_manager, int(step), state, self.config.dataset_type, data_iterator, self.config):
