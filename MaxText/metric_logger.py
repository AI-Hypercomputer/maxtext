...
class MetricLogger:
  ...
  def __init__(self, config, learning_rate_schedule):
    ...
    self.last_step_completion = datetime.datetime.now()
  ...
  def write_metrics(self, metrics, step, is_training=True):
    ...
    if metrics_to_write:
      ...
      if self.config.gcs_metrics and jax.process_index() == 0:
        self.write_metrics_for_gcs(metrics_to_write, steps_to_write, is_training)
  ...
  def record_train_metrics(self, metrics, step):
    ...
    step_time_delta = current_time - self.last_step_completion
    self.last_step_completion = current_time
  ...
  def record_eval_metrics(self, metrics, eval_step_count, step):
    ...
    if eval_step_count:
      ...
      max_logging.log(
          f"average loss after {step=}: {eval_step_count=},"
          f" eval_loss={self.cumulative_eval_metrics['scalar']['eval/avg_loss']},"
          f" total_weights={self.cumulative_eval_metrics['scalar']['eval/total_weights']}"
      )

  def flush_metrics_and_cleanup(self):
    """Writes all buffered metrics to TensorBoard and GCS, then closes resources.
    ...
    The internal metrics buffer is cleared after a successful flush.
    """
    self.write_metrics(None, self.config.steps - 1)
