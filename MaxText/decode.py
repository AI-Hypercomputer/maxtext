...
def main(argv: Sequence[str]) -> None:
  ...
  for iter in range(_NUM_ITERS):
    ...
    first_token_list = []
    sampled_tokens_list = []
    slot_id_base = (iter * _NUM_STREAMS) % batch_size
    # Prefill
    rng, rng_prefill = jax.random.split(rng)  # Split RNG before calling prefill
    for i in range(_NUM_STREAMS):
      prefill_result, first_token = engine.prefill(
          params=params, padded_tokens=tokens, true_length=true_length, rng=rng_prefill, slot=slot_id_base+i
      )
    ...
    for i in range(_NUM_STREAMS):
      decode_state = engine.insert(prefill_result_list[i], decode_state, slot=slot_id_base+i)
    ...
    # Get results
    for i in range(_NUM_STREAMS):
      results = [t.get_result_at_slot(slot_id_base+i).tokens.item() for t in sampled_tokens_list]
