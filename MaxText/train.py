...
def write_metrics(
  ...
  if _buffered_metrics is not None:
    ...
    write_metrics_to_tensorboard(
        writer, _buffered_metrics, _buffered_step, config
    )
  ...
  if config.scan_layers:
    ...
    for layer_num in range(config.num_decoder_layers):
      output_metrics["scalar"][
          f"activ_fraction_zero/layer_{layer_num:03d}"
      ] = metrics_dict["activation_fraction_zero"][0][layer_num]
      output_metrics["scalar"][
          f"activ_mean/layer_{layer_num:03d}"
      ] = metrics_dict["activation_mean"][0][layer_num]
      output_metrics["scalar"][
          f"activ_stdev/layer_{layer_num:03d}"
...
def loss_fn(model, config, data, dropout_rng, params, is_train=True):
  ...
  xent = nn.with_logical_constraint(
      xent, ("activation_batch", "activation_length")
  )
...
def setup_train_loop(config):
  ...
  data_iterator, eval_data_iterator, _ = create_data_iterator_with_tokenizer(
      config, mesh
  )
...
def train_loop(config, state=None):
  ...
  for step in np.arange(start_step, config.steps):
    ...
    if checkpoint_manager is not None:
      if save_checkpoint(
          checkpoint_manager, step, state, config.dataset_type, data_iterator
      ):
