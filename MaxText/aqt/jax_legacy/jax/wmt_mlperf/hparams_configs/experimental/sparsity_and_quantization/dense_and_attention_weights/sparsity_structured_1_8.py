...
"""Small-sized model with 1:8 sparsity for dense/attention weights only."""
...
def get_config(quant_target=base_config.QuantTarget.NONE):
  """Returns configuration for a small transformer model with 1:8 sparsity for dense/attention weights only."""
  ...
  config.attention.dense_kqv.weight_sparsity.prune_rate = (1, 8)
  config.attention.dense_out.weight_sparsity.type = 'STRUCTURED_NM'
  config.attention.dense_out.weight_sparsity.prune_rate = (1, 8)
  config.metadata.hyper_str = 'mlp_attn_weights_sparsity_structured_1_8'
  return config
