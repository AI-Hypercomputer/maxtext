...
def _possibly_unroll_params(
    config, training_state, training_state_annotations, mesh
):
  ...
  training_state_annotations_layers = (
    training_state_annotations.params["params"]["decoder"]["layers"]
  )
  ...
  new_per_layer_state_annotation = jax.tree_map(
      new_pspec, training_state_annotations_layers
  )
  new_per_layer_state_sharding = jax.tree_map(
      lambda x: jax.sharding.NamedSharding(mesh, x),
      new_per_layer_state_annotation,
  )
  ...
  for i in range(config.num_decoder_layers):
    ...
    def slice_ith(input_layers):
      return jax.tree_map(
          lambda x: jax.numpy.take(x, i, axis=config.param_scan_axis),
          input_layers,
      )

    new_layer = jax.jit(slice_ith, out_shardings=new_per_layer_state_sharding)(
        training_state_layers
    )

    training_state.params["params"]["decoder"][f"layers_{i}"] = new_layer
    training_state_annotations.params["params"]["decoder"][
        f"layers_{i}"
    ] = new_per_layer_state_annotation
...
def _save_decode_checkpoint(config, state, checkpoint_manager):
  ...
  with jax.spmd_mode("allow_all"):
    decode_state = max_utils.init_decode_state(
        None, jax.tree_map(lambda x: x.astype(jax.numpy.bfloat16), state.params)
    )
...
def generate_decode_checkpoint(config):
  ...
  max_logging.log(
      f"Read training checkpoint from: {config.load_full_state_path}"
  )
  training_state, training_state_annotations = _read_train_checkpoint(
      config, checkpoint_manager, mesh
  )
  assert (
  ...
  max_logging.log(
      f"Successfully generated decode checkpoint at: {config.checkpoint_dir}0/items"
  )
  return True
