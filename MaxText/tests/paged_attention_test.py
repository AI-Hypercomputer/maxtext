...
class PagedAttentionTest(unittest.TestCase):
  ...
  def test_update_prefill_step_pages(self):
    ...
    self.assertEqual(
        ...
        (
            ...
            self.cfg["max_prefill_predict_length"] // self.cfg["page_size"],
            self.cfg["page_size"],
            self.cfg["head_dim"],
        ),
    )
  ...
  def test_update_decode_step_pages(self):
    ...
    self.assertEqual(
        ...
        (self.cfg["num_kv_heads"], self.cfg["num_pages"], self.cfg["page_size"], self.cfg["head_dim"]),
    )

    # Instead of trying to extract logical axes from the value,
    # verify against the attention op's configured axis names
    self.assertEqual(
        self.attention_op.kv_pages_axis_names, ("paged_kv_heads", "num_pages", "page_size", "paged_kv_head_dim_size")
    )
