...
class PagedAttentionTest(unittest.TestCase):
  ...
  def test_update_prefill_step_pages_state_management(self):
    ...
    page_state = PageState(
        ...
        page_map=jnp.full((batch_size, self.config.max_target_length // self.config.tokens_per_page), -1, dtype=jnp.int32),
    ...
    variables = self.attention_op.init(
        self.rng, key, key, value, None, "prefill", page_state  # decoder_segment_ids  # model_mode
    )

    # First apply - should initialize state
    output_tuple, vars = self.attention_op.apply(
        variables, key, key, value, None, "prefill", page_state, mutable=["cache"]  # decoder_segment_ids  # model_mode
    )
    ...
    for b in range(batch_size):
      for p in range(expected_pages_per_seq):
        ...
        np.testing.assert_array_equal(
            key_pages[:, page_idx], key[b, start_pos:end_pos].transpose(1, 0, 2), f"Key mismatch batch {b} page {p}"
        )

        # Check value storage
        np.testing.assert_array_equal(
            value_pages[:, page_idx], value[b, start_pos:end_pos].transpose(1, 0, 2), f"Value mismatch batch {b} page {p}"
        )
  ...
  def test_decode_step_pages_state_management(self):
    ...
    def determine_read_location(
        step: int, page_map, current_page, current_page_position, sequence_lengths
      ...
      page_indices, position = jax.lax.cond(
          is_first_step, _first_step_fn, lambda: jax.lax.cond(is_end_of_page, _end_of_page_fn, _within_page_fn)
      )
    ...
    for step in range(num_steps):
      ...
      output_tuple, vars = self.attention_op.apply(
          variables, step_key, step_key, step_value_v, None, "autoregressive", page_state, mutable=["cache"]
      )
      ...
      read_pages, read_pos = determine_read_location(
          step, new_page_map, new_current_page, new_current_page_position, new_sequence_lengths
      )
    ...
    for step in range(num_steps):
      for b in range(batch_size):
        ...
        np.testing.assert_array_almost_equal(all_stored_keys[step][b], all_step_keys[step], decimal=5)
