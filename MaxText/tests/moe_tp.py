...
def main(config, test_args):
  ...
  config.__setattr__("ici_fsdp_parallelism", 2)
  config.__setattr__("ici_tensor_parallelism", 2)
  config.__setattr__("per_device_batch_size", 2)
  (
      _,
      _,
      _,
  ...
  for golden_data_index in range(len(golden_data)):
    ...
    config.__setattr__("ici_fsdp_parallelism", 4)
    config.__setattr__("ici_tensor_parallelism", 1)
    config.__setattr__("per_device_batch_size", 4)
    ids, decoder_segment_ids, decoder_positions, _ = get_data(golden_data,golden_data_index,config)
    ...
    config.__setattr__("ici_tensor_parallelism", 2)
    config.__setattr__("per_device_batch_size", 2)
    ids2, decoder_segment_ids2, decoder_positions2, _ = get_data(golden_data,golden_data_index,config)
    full_train_logits2 = model2.apply(
        state.params,
        ids2,
        decoder_positions2,
        decoder_segment_ids2,
        enable_dropout=False,
        rngs={"aqt": init_rng},
    )
    ...
    assert jax.numpy.allclose(
            full_train_logits[0, :token_size, :], full_train_logits2[0, :token_size, :], rtol=float(test_args.rtol), atol=float(test_args.atol), equal_nan=False
        )
