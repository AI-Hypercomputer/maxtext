...
class TrainCompile(unittest.TestCase):
  ...
  @pytest.mark.cpu_only
  def test_save_compiled_v7x(self):
    temp_dir = gettempdir()
    compiled_trainstep_file = os.path.join(temp_dir, "test_compiled_v7x.pickle")
    train_compile_main(
        (
            None,
            os.path.join(PKG_DIR, "configs", "base.yml"),
            f"compiled_trainstep_file={compiled_trainstep_file}",
            "compile_topology=v7x-16",
            "compile_topology_num_slices=1",
            "base_emb_dim=256",
            "base_mlp_dim=256",
            "base_num_decoder_layers=2",
  ...
  @pytest.mark.cpu_only
  def test_save_compiled_v7x_two_slices(self):
    temp_dir = gettempdir()
    compiled_trainstep_file = os.path.join(temp_dir, "test_compiled_v7x_two_slices.pickle")
    train_compile_main(
        (
            None,
            os.path.join(PKG_DIR, "configs", "base.yml"),
            f"compiled_trainstep_file={compiled_trainstep_file}",
            "compile_topology=v7x-8",
            "compile_topology_num_slices=2",
            "base_emb_dim=256",
            "base_mlp_dim=256",
            "base_num_decoder_layers=2",
