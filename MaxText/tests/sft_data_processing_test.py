...
class SFTDataProcessingTest(unittest.TestCase):
  ...
  def get_train_iterator(self, train_ds, data_columns):
    print("dataloading_host_index: ", self.process_indices.index(jax.process_index()))
    print("dataloading_host_count: ", len(self.process_indices))
    self.train_iter = _hf_data_processing.preprocessing_pipeline(
      ...
      sft_train_on_completion=self.config.sft_train_on_completion,
  ...
  def test_sft_format_with_messages(self):
    ...
    self.assertEqual(self.tokenizer.decode(np.where(batch['inputs_segmentation'][0]>0, batch['inputs'][0], 0)), truncated_exp1_inputs)
    self.assertEqual(self.tokenizer.decode(np.where(batch['targets_segmentation'][0]>0, batch['targets'][0], 0)), truncated_exp1_targets_predictable)
    ...
    self.assertEqual(self.tokenizer.decode(np.where(batch['inputs_segmentation'][1]>0, batch['inputs'][1], 0)), packed_exp2_inputs)
    self.assertEqual(self.tokenizer.decode(np.where(batch['targets_segmentation'][1]>0, batch['targets'][1], 0)), packed_exp2_targets_predictable)
