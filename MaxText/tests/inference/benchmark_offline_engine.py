...
def init_pyconfig(**kwargs):
    init_kwargs = {
        ...
        "ici_data_parallelism": 8, # <=== set ici_data_parallelism to dp
...
data = [np.arange(128) for _ in range(config.global_batch_size_to_train_on)]
...
def run(
    ...
    [inference_engine.batch_inference(input_data[:num_input]) for _ in range(4)]
...
run(
    num_input=32,
    profile=True,
    profile_path=f"gs://runner-maxtext-logs/mohitkhatwani_offline_benchmark/baseline/0904/{time.strftime('%Y%m%d-%H%M%S')}/",
)
