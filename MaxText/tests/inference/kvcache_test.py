...
class MlaKVCacheTest(unittest.TestCase):
  ...
  def test_update_kv_cache(self):
    test_module = kvcache.MlaKVCache(
        self.prefill_len, self.target_len, self.dtype
    )
    low_rank_main = jnp.ones(
        (self.batchsize, self.prefill_len, self.kv_lora_rank), dtype=self.dtype
    ) * 0.02
    key_rope = jnp.ones(
    ...
    ar_low_rank_main = jnp.transpose(
        new_vars["cache"]["cached_ar_key"].value, test_module.key_axis_order
    )
    ar_key_rope = jnp.transpose(
        new_vars["cache"]["cached_ar_value"].value, test_module.key_axis_order
    )
    ...
    self.assertEqual(
        ...
        (self.batchsize, (self.target_len - self.prefill_len), 1,
         self.kv_lora_rank),
    )
    self.assertEqual(
        ar_key_rope.shape,
        (self.batchsize, (self.target_len - self.prefill_len), 1,
         self.k_rope_head_dim),
    )
    ...
    low_rank_main_1 = (
        jnp.ones((self.batchsize, 1, self.kv_lora_rank), dtype=self.dtype)
        * 0.04
    )
    key_rope_1 = (
        jnp.ones((self.batchsize, 1, 1, self.k_rope_head_dim), dtype=self.dtype)
        * 0.05
    )
    _, new_vars = test_module.apply(
        new_vars,
