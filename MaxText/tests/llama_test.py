...
class RoPETest(unittest.TestCase):
  ...
  def test_rope(self):
    ...
    key_proj = embeddings.RotaryEmbedding(embedding_dims=dim_per_head)(
    ...
    self.assertTrue(
        jax.numpy.allclose(
            permute_to_match_maxtext_rope(llama_output[0]),
            query_proj,
            rtol=1e-01,
            atol=1e-04,
            equal_nan=False,
    ...
    self.assertTrue(
        jax.numpy.allclose(
            permute_to_match_maxtext_rope(llama_output[1]),
            key_proj,
            rtol=1e-01,
            atol=1e-04,
            equal_nan=False,
  ...
  def test_scaling_rope(self):
    ...
    query_proj_1 = embeddings.RotaryEmbedding(embedding_dims=dim_per_head)(
    ...
    query_proj_2 = embeddings.RotaryEmbedding(embedding_dims=dim_per_head)(
