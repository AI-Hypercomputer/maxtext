...
class RoPETest(unittest.TestCase):
  ...
  def test_rope(self):
    ...
    key_proj = embeddings.RotaryEmbedding(min_timescale=1, max_timescale=10_000, embedding_dims=dim_per_head)(
        permute_to_match_maxtext_rope(x_k), position=position
    )

    # Compare results
    self.assertTrue(
        jax.numpy.allclose(
            permute_to_match_maxtext_rope(llama_output[0]), query_proj, rtol=1e-01, atol=1e-04, equal_nan=False
        )
    )
    self.assertTrue(
        jax.numpy.allclose(permute_to_match_maxtext_rope(llama_output[1]), key_proj, rtol=1e-01, atol=1e-04, equal_nan=False)
    )
  ...
  def test_scaling_rope(self):
    ...
    query_proj_1 = embeddings.RotaryEmbedding(min_timescale=1, max_timescale=10_000, embedding_dims=dim_per_head)(
        x_q, position=position
    )
    query_proj_1 = query_proj_1 * (dim_per_head**-0.5)

    # scale first and then apply RoPE
    query_proj_2 = x_q * (dim_per_head**-0.5)
    query_proj_2 = embeddings.RotaryEmbedding(min_timescale=1, max_timescale=10_000, embedding_dims=dim_per_head)(
        query_proj_2, position=position
    )
