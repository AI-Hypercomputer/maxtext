...
class GradientAccumulationTest(unittest.TestCase):
  ...
  def test_grad_accumulate_same_loss(self):
    ...
    tmp_dir = gettempdir()
    run_accumulate_metrics_file = os.path.join(tmp_dir, f"runner_grad_accumulate_{random_suffix}.txt")
    print(f"{run_accumulate_metrics_file=}")
    run_regular_metrics_file = os.path.join(tmp_dir, f"runner_regular_{random_suffix}.txt")
