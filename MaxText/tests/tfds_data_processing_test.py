...
class TfdsDataProcessingTest(unittest.TestCase):
  ...
  def _get_preprocessed_datasets(self):
    ...
    mesh = Mesh(
        mesh_utils.create_device_mesh(mesh_shape_1d), self.config.mesh_axes
    )
    sp_tokenizer = input_pipeline_interface.get_tokenizer(
  ...
  def test_batch_determinism(self):
    ...
    self.assertTrue(
        tf.reduce_all(tf.equal(batch1["targets"], batch2["targets"]))
    )
    self.assertTrue(
        tf.reduce_all(
            tf.equal(
                batch1["inputs_segmentation"], batch2["inputs_segmentation"]
            )
        )
    )
    self.assertTrue(
        tf.reduce_all(
            tf.equal(
                batch1["targets_segmentation"], batch2["targets_segmentation"]
            )
        )
    )
    self.assertTrue(
        tf.reduce_all(
            tf.equal(batch1["inputs_position"], batch2["inputs_position"])
        )
    )
    self.assertTrue(
        tf.reduce_all(
            tf.equal(batch1["targets_position"], batch2["targets_position"])
        )
    )
