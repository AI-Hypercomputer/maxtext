...
class MoeTest(unittest.TestCase):
  ...
  def test_moe_block(self):
    ...
    with self.mesh, nn_partitioning.axis_rules(self.cfg.logical_axis_rules):
      ...
      import numpy as np
      print(f"diff: {(np.abs(expected_output-actual_output))}")
      # for i in range(expected_output.shape[0]):
      #    for j in range(expected_output.shape[1]):
      #       for k in range(expected_output.shape[2]):
      #         if abs(expected_output[i,j,k]-actual_output[i,j,k]) > 0.1:
      #            print('location %d,%d,%d -> %.4f v.s. %.4f, diff %.4f' % (i, j, k, expected_output[i,j,k], actual_output[i,j,k],expected_output[i,j,k]- actual_output[i,j,k]) )
