...
def init_random_model_vars(model, rng, example_batch):
  ...
  def _replace_initialization(key, value):
    ...
    if "scale" in keystr or "bias" in keystr:
      value = jax.nn.initializers.normal(1.0)(
          rng, value.shape, dtype=value.dtype
      )
    return value

  model_vars = jax.tree_util.tree_map_with_path(
      _replace_initialization, model_vars
  )
  return model_vars
...
class GPT3(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.model_vars = init_random_model_vars(
        self.model, self.rng, self.example_batch
    )
  ...
  def test_logits_numerically(self):
    ...
    per_example_xent_truth = jnp.array(
        [[31.976467, 25.806253, 17.311134, 45.362663, 0.0]], dtype=jnp.float32
    )
    ...
    one_hot_targets = jax.nn.one_hot(
        self.example_batch["targets"], self.cfg.vocab_size
    )
    per_example_xent = -jnp.sum(
        jax.nn.log_softmax(logits) * one_hot_targets, axis=-1, dtype=jnp.float32
    )
    # Mask out paddings at the end of each example.
    per_example_xent = per_example_xent * (
        self.example_batch["targets_segmentation"] != 0
    )
