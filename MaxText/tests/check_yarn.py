...
class TestRotaryEmbeddingEquivalence(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.yarn_embedding_as_gpt_oss = YarnRotaryEmbedding(
            ...
            decoder_block=DecoderBlockType.GPT_OSS,
            truncate=False,
        )
        ...
        self.inputs = jax.random.normal(
            key,
            (self.batch_size, self.seq_len, self.num_heads, self.embedding_dims),
            dtype=jnp.float32
        )
    ...
    def test_frequency_calculation_equivalence(self):
        ...
        self.assertTrue(
            jnp.allclose(sin_ref, sin_yarn_scaled, atol=1e-5, rtol=1e-5),
            "Sin values from frequency calculations do not match."
        )
        self.assertTrue(
            jnp.allclose(cos_ref, cos_yarn_scaled, atol=1e-5, rtol=1e-5),
            "Cos values from frequency calculations do not match."
        )
