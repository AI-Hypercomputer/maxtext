...
""" Tests for Page Manager (Hybrid Global State Version)."""
import sys
import unittest
import jax
import jax.numpy as jnp
# Assuming the new hybrid code is saved as page_manager.py
...
class TestPageManager(unittest.TestCase):
  ...
  def setUp(self):
    ...
    # num_layers is no longer directly used by PageManager's state, but might be in config
    self.num_layers_config = 2

    # Initialize configuration for PageManager
    ...
    config = pyconfig.initialize(
        ...
        # Pass the number of layers if config requires it, though PM ignores it
        base_num_decoder_layers=self.num_layers_config,
        # These are the crucial keys for the new PageManager
    ...
    # Initialize JAX distributed system and mesh (if needed for context, though PM is often local)
    ...
    # Instantiate the PageManager with the global state logic
  ...
  def test_jit_compatibility(self):
    ...
    concrete_page_group_id = 0
    concrete_true_length = 12

    initial_state = self.pm.get_initial_page_state()

    # JIT the prefill function
    @jax.jit
    def jitted_prefill(page_state, page_group_id, true_length):
    ...
    self.assertEqual(int(updated_state.sequence_lengths[concrete_page_group_id]), concrete_true_length)

    # JIT the decode function
    @jax.jit
    def jitted_decode(page_state):
      return self.pm.update_decode_pages(page_state=page_state)

    # Call the JITted decode function
    decode_state = jitted_decode(page_state=updated_state)
    # Check sequence length increment (no layer index)
    self.assertEqual(int(decode_state.sequence_lengths[concrete_page_group_id]), concrete_true_length + 1)
  ...
  def test_reserve_prefill_group(self):
    ...
    expected_pos = (true_length - 1) % self.tokens_per_page if true_length > 0 else 0
  ...
  def test_reserve_prefill_edge_cases(self):
    """Tests update_prefill_pages with edge cases: zero length, exact page multiple, partial page (global state)."""
    initial_state = self.pm.get_initial_page_state()

    # Test case 1: Zero length - no pages should be allocated, group remains inactive.
    state_zero = self.pm.update_prefill_pages(page_state=initial_state, page_group_id=0, true_length=0)
    self.assertEqual(int(state_zero.sequence_lengths[0]), 0)
    self.assertEqual(int(state_zero.num_pages_used[0]), 0)
    self.assertFalse(bool(state_zero.has_active_page[0]))
    self.assertEqual(int(state_zero.active_page_position[0]), 0)
    ...
    # Position on boundary is last index
    self.assertEqual(int(state_multiple.active_page_position[1]), self.tokens_per_page - 1)
    ...
    self.assertEqual(int(state_partial.active_page_position[2]), length_partial - 1)
  ...
  def test_update_decode_pages(self):
    ...
    self.assertEqual(int(prefill_state_boundary.active_page_position[0]), self.tokens_per_page - 1)
    ...
    self.assertEqual(int(prefill_state_partial.active_page_position[1]), 4)
  ...
  def test_state_consistency(self):
    ...
    initial_state = self.pm.get_initial_page_state()
    # Initially, sum of status and used should be 0.
    ...
    self.assertEqual(total_allocated_pages, total_mapped_pages, "Sum of page_status != sum of num_pages_used")
    ...
    self.assertEqual(total_allocated_pages_d, total_mapped_pages_d, "Consistency failed after decode step")
  ...
  def test_multiple_group_interactions(self):
    ...
    for i in range(num_test_groups):
      ...
      # Basic verification after each allocation
      ...
      # Check global status reflects total pages used so far
      self.assertEqual(int(jnp.sum(current_state.page_status)), int(jnp.sum(current_state.num_pages_used)))
  ...
  def test_invalid_page_group_id(self):
    ...
    # Try operations with invalid page_group_id (-1)
    updated_state_neg = self.pm.update_prefill_pages(page_state=initial_state, page_group_id=-1, true_length=10)
    self._assert_page_state_equal(initial_state, updated_state_neg, "Prefill with ID -1 failed")

    # Try operations with invalid page_group_id (>= max_page_groups)
    updated_state_over = self.pm.update_prefill_pages(
        page_state=initial_state, page_group_id=self.max_page_groups, true_length=10
    )
    self._assert_page_state_equal(initial_state, updated_state_over, "Prefill with ID >= max failed")

    # First allocate some pages to a valid group
    valid_state = self.pm.update_prefill_pages(page_state=initial_state, page_group_id=0, true_length=10)

    # Then try to release invalid groups
    released_state_neg = self.pm.release_pages(page_state=valid_state, page_group_id=-1)
    self._assert_page_state_equal(valid_state, released_state_neg, "Release with ID -1 failed")

    released_state_over = self.pm.release_pages(page_state=valid_state, page_group_id=self.max_page_groups)
    self._assert_page_state_equal(valid_state, released_state_over, "Release with ID >= max failed")

  def test_invalid_length(self):
    """Tests that operations with invalid length have no effect (global state)."""
    initial_state = self.pm.get_initial_page_state()

    # Try prefill with negative length
    updated_state_neg = self.pm.update_prefill_pages(page_state=initial_state, page_group_id=0, true_length=-1)
    self._assert_page_state_equal(initial_state, updated_state_neg, "Prefill with length -1 failed")

    # Try prefill with length exceeding max_target_length
    updated_state_over = self.pm.update_prefill_pages(
        page_state=initial_state, page_group_id=0, true_length=self.max_target_length + 1
    )
    self._assert_page_state_equal(initial_state, updated_state_over, "Prefill with length > max failed")
