...
class TestPageManager(unittest.TestCase):
  ...
  def _validate_state_shapes(self, state: PageState):
    ...
    expected_shapes = {
        ...
        "page_map": (self.config.pagedattn_max_page_groups, self.max_pages_per_group),
        "sequence_lengths": (self.config.pagedattn_max_page_groups,),
        "num_pages_used": (self.config.pagedattn_max_page_groups,),
        "active_page": (self.config.pagedattn_max_page_groups,),
        "has_active_page": (self.config.pagedattn_max_page_groups,),
        "active_page_position": (self.config.pagedattn_max_page_groups,),
    }
  ...
  def test_page_group_id_boundaries(self):
    ...
    max_group_idx = self.config.pagedattn_max_page_groups - 1
  ...
  def test_multiple_group_interactions(self):
    ...
    num_test_groups = min(3, self.config.pagedattn_max_page_groups)
  ...
  def test_invalid_page_group_id(self):
    ...
    updated_state_over = self.pm.update_prefill_pages(
        page_state=initial_state, page_group_id=self.config.pagedattn_max_page_groups, true_length=10
    )
    ...
    released_state_over = self.pm.release_pages(page_state=valid_state, page_group_id=self.config.pagedattn_max_page_groups)
