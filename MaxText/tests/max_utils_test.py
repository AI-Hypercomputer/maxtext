...
class MaxUtilsSummaryStats(unittest.TestCase):
  ...
  def test_l2norm_pytree(self):
    ...
    self.assertTrue(
        jax.numpy.allclose(
            pytree_l2_norm, 7, rtol=1e-05, atol=1e-08, equal_nan=False
        )
    )
...
class MaxUtilsInitState(unittest.TestCase):
  ...
  def test_init_training_state(self):
    state = max_utils.init_training_state(
...
class MaxUtilsInitStateWithMultipleCollections(unittest.TestCase):
  ...
  def _test_init_initial_state_driver(self, is_training):
    state_under_test = max_utils.init_initial_state(
        self.model, self.tx, self.config, is_training, self.key3
    )
...
class MaxUtilsT5XCrossEntropy(unittest.TestCase):
  ...
  def test_t5x_cross_entropy(self):
    ...
    targets = jax.random.randint(
        key, shape=(48, 2048), dtype=jax.numpy.int32, minval=1, maxval=10
    )
    logits = jax.random.uniform(
        key, shape=(48, 2048, 4096), dtype=jax.numpy.float32
    )
    ...
    self.assertTrue(
        jax.numpy.allclose(
            optax_xent, t5x_xent, rtol=1e-05, atol=1e-08, equal_nan=False
        )
    )
