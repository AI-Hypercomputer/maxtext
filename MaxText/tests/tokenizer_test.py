...
class TokenizerTest(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.dataset = train_ds_builder.as_dataset(
  ...
  def test_tokenize(self):
    ...
    self.assertTrue(
        (
            np.asarray(source_tokenizer.tokenize(text))
            & np.asarray(test_tokenizer.tokenize(text))
        ).all()
    )
  ...
  def test_detokenize(self):
    ...
    self.assertEqual(
        np.asarray(source_tokenizer.detokenize(tokens)),
        np.asarray(test_tokenizer.detokenize(tokens)),
    )
