...
class GrainDataProcessingTest(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    if exit_code != 0:
      raise ValueError(
          f"Running setup_gcsfuse.sh failed with exit code: {exit_code}"
      )
  ...
  def setUp(self):
    ...
    self.mesh = Mesh(
        mesh_utils.create_device_mesh(self.mesh_shape_1d), self.config.mesh_axes
    )
    ...
    (
        ...
        self.eval_iter,
        self.predict_iter,
    ) = self._get_preprocessed_datasets()

  def _get_datasets(self):
    print("Sharding dataset in ", jax.process_count(), " shards")
    train_ds, eval_ds = _grain_data_processing.get_datasets(config=self.config)
  ...
  def _get_preprocessed_datasets(self):
    ...
    (
        ...
        eval_iter,
        test_iter,
    ) = _grain_data_processing.preprocess_dataset(
        self.config,
  ...
  def test_batch_determinism(self):
    ...
    self.assertTrue(
        (batch1["inputs_segmentation"] == batch2["inputs_segmentation"]).all()
    )
    self.assertTrue(
        (batch1["targets_segmentation"] == batch2["targets_segmentation"]).all()
    )
    self.assertTrue(
        (batch1["inputs_position"] == batch2["inputs_position"]).all()
    )
    self.assertTrue(
        (batch1["targets_position"] == batch2["targets_position"]).all()
    )
