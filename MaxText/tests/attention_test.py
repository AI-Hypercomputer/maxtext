...
class ChunkedCausalMaskTest(unittest.TestCase):
    ...
    def test_basic_chunking(self):
        ...
        # pylint: disable=protected-access
        actual_mask = attentions._generate_chunk_attention_mask(
...
class AttentionTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        devices_array_cp = maxtext_utils.create_device_mesh_with_maxtextconfig(
            self.cfg_cp
        )  # for context parallelism
        self.mesh_cp = Mesh(
            devices_array_cp, self.cfg_cp.mesh_axes
        )  # for context parallelism
    ...
    def _dot_product_attention(
        ...
        attention_w_layout_prefill, attention_w_layout_output_cache = (
            attention_w_layout.apply(
                attention_w_layout_variable,
                lnx_prefill,
                lnx_prefill,
                decoder_segment_ids=decoder_segment_ids_prefill,
        ...
        for idx in range(prefill_length, decode_total_length):
            ...
            attention_w_layout_idx, attention_w_layout_output_cache = (
                attention_w_layout.apply(
                    attention_w_layout_variable,
                    lnx_idx,
                    lnx_idx,
    ...
    def _dot_product_attention_reshape_q(self, compute_axis_order):
        ...
        attention_wo_reshape_q_prefill, attention_wo_reshape_q_output_cache = (
            attention_wo_reshape_q.apply(
                attention_wo_reshape_q_variable,
                lnx_prefill,
                lnx_prefill,
                decoder_segment_ids=decoder_segment_ids_prefill,
        ...
        attention_w_reshape_q_prefill, attention_w_reshape_q_output_cache = (
            attention_w_reshape_q.apply(
                attention_w_reshape_q_variable,
                lnx_prefill,
                lnx_prefill,
                decoder_segment_ids=decoder_segment_ids_prefill,
        ...
        for idx in range(prefill_length, decode_total_length):
            ...
            attention_wo_reshape_q_idx, attention_wo_reshape_q_output_cache = (
                attention_wo_reshape_q.apply(
                    attention_wo_reshape_q_variable,
                    lnx_idx,
                    lnx_idx,
            ...
            attention_w_reshape_q_idx, attention_w_reshape_q_output_cache = (
                attention_w_reshape_q.apply(
                    attention_w_reshape_q_variable,
                    lnx_idx,
                    lnx_idx,
...
class MLATest(parameterized.TestCase):
    ...
    def get_data(self, cfg, rng, dtype):
        ...
        decoder_segment_ids = jax.random.randint(
            rng, (cfg.global_batch_size_to_train_on, cfg.max_target_length), 0, 4
        )
        decoder_positions = jax.random.randint(
            rng,
    ...
    def get_structured_data(self, cfg, rng, dtype):
        ...
        decoder_segment_ids = (
            jax.numpy.zeros((cfg.per_device_batch_size, cfg.max_target_length))
            + DECODING_ACTIVE_SEQUENCE_INDICATOR
        )
