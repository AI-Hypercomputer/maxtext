...
class TestSamplingFunctions(unittest.TestCase):
  ...
  def setUp(self):
    ...
    # Descending order of indices by logit value: [5, 7, 3, 4, 8, 1, 9, 6, 2, 0]

  def test_topk_filtering(self):
    """Tests that sampling is restricted to the top-k tokens."""
    topk = 3
    # The indices with the 3 highest logits are {5, 7, 3}
    top_k_indices = {5, 7, 3}
  ...
  def test_topp_filtering(self):
    ...
    # With nucleus_topp=0.8, the valid set of indices is {5, 7, 3, 4, 8, 1}.
    nucleus_topp = 0.8
    top_p_indices = {5, 7, 3, 4, 8, 1}
    rngs = jax.random.split(self.rng, 100)
  ...
  def test_combined_filtering(self):
    ...
    # First, filter to top_k=5. Indices are {5, 7, 3, 4, 8}.
    # Then, apply nucleus_topp=0.8 to this smaller set.
    # The renormalized probabilities lead to a nucleus of the top 3 tokens,
    # which are indices {5, 7, 3}.
    topk = 5
    nucleus_topp = 0.8
    # CORRECTED: The valid set of indices is {5, 7, 3}, not {5, 7}.
    valid_indices = {5, 7, 3}
    rngs = jax.random.split(self.rng, 100)
  ...
  def test_low_temperature_is_greedy(self):
    ...
    low_temp = 1e-6
    greedy_token_index = 5  # Index of the highest logit
