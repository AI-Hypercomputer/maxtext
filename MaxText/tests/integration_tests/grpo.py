...
class GRPOTest(unittest.TestCase):
  ...
  #     input_position = jnp.tile(jnp.arange(input_ids.shape[1])+1, (4, 1))
  ...
  #   self.assertTrue(jax.numpy.allclose(hf_logits, logits[:,:,:-128], rtol=1e-3, atol=7e-1, equal_nan=False))
  #   # grpo_trainer.compute_log_probs(
  ...
  def test_logps(self):
    def _prepare_maxtext_inputs():
      ...
      input_ids = jnp.pad(jnp.tile(jnp.concat([jnp.array(prompt),jnp.array(prompt)], axis = -1), (4, 1)), ((0, 0),(0, 4)), constant_values=0) # pad some tokens at the end of input prompt
      input_segmentation = (input_ids > 0).astype(jnp.int32)
      input_position = jnp.where(input_segmentation, jnp.arange(input_segmentation.shape[1])+1, 0)
    ...
    def _prepare_trl_inputs():
      ...
      input_ids = F.pad(torch.cat([prompt, prompt], axis = -1), (0, 4))
      attention_mask = (input_ids > 0).int()
      breakpoint()
      return input_ids, attention_mask, logits_to_keep

    hf_input_ids, attention_mask, logits_to_keep = _prepare_trl_inputs()
    hf_logits, hf_per_token_logps = self.trainer._get_per_token_logps(self.hf_model, hf_input_ids, attention_mask, logits_to_keep)
    
    input_ids, input_segmentation, input_position, completion_segmentation = _prepare_maxtext_inputs()
    maxtext_per_token_logps, maxtext_logits, _ = compute_log_probs(self.model, self.state.params, input_ids, input_position, input_segmentation, self.cfg, is_train=False)
    self.assertEqual(hf_input_ids[0].tolist(), input_ids[0].tolist())
    self.assertEqual(attention_mask[0].tolist(), input_segmentation[0].tolist())
    print(f"Max Diff {np.max(np.abs(np.asarray(maxtext_logits)[0,:,:-128] - hf_logits.detach().numpy()[0]))}")
    breakpoint()
