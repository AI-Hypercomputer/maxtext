...
def run_checkpointing(hardware, attention_type):
  ...
  train_main(
      get_checkpointing_command(
          run_date, hardware=hardware, steps=5, metrics_file="saved_metrics.txt", attention_type=attention_type
      )
      + grain_command
  )
  ...
  train_main(
      get_checkpointing_command(
          run_date, hardware=hardware, steps=10, metrics_file="restored_metrics.txt", attention_type=attention_type
      )
      + grain_command
  )
