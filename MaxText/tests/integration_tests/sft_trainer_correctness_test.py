...
def get_input_ids(data, tokenizer, config):
  ...
  return {
      "input_ids": torch.stack(
          [torch.tensor(input_ids[: config.max_target_length], dtype=torch.long).clone() for _ in range(global_batch_size)]
      ),
      "labels": torch.stack(
          [torch.tensor(labels[: config.max_target_length], dtype=torch.long).clone() for _ in range(global_batch_size)]
      ),
      "attention_mask": torch.stack(
          [
              torch.tensor(attention_mask[: config.max_target_length], dtype=torch.long).clone()
...
def prepare_maxtext_inputs(config):
  ...
  tokenized_data = _input_pipeline_utils.tokenization(
      data, hf_tokenizer=tokenizer, truncation=False, max_length=config.max_target_length, column_names=["messages"]
  )
