...
def setup_sft_trainer(config, data):
    fsdp_config = {
...
def get_input_ids(data, tokenizer, config):
    input_ids = []
    attention_mask = []
    for d in data:
    ...
    global_batch_size = int(jax.device_count() * config.per_device_batch_size * config.gradient_accumulation_steps)
    return {
        "input_ids": torch.stack([torch.tensor(input_ids[:config.max_target_length], dtype=torch.long).clone() for _ in range(global_batch_size)]),
        "labels": torch.stack([torch.tensor(labels[:config.max_target_length], dtype=torch.long).clone() for _ in range(global_batch_size)]),
        "attention_mask": torch.stack([torch.tensor(attention_mask[:config.max_target_length], dtype=torch.long).clone() for _ in range(global_batch_size)]),
    }  

def prepare_trl_inputs(config):
    trl_data = DATA.copy()
    data = apply_chat_template(trl_data)
    tokenizer = get_tokenizer(config)
    tokenized_data = [tokenizer(d) for d in data]
    processed_data = get_input_ids(tokenized_data, tokenizer, config)
    return processed_data

def prepare_maxtext_inputs(config):
    ...
    inputs_position = jnp.stack([np.arange(masked_inputs["inputs"].shape[0], dtype=np.int32) for _ in range(global_batch_size)])
    return inputs, inputs_segmentation, inputs_position

def get_maxtext_logits(config, inputs, inputs_position, inputs_segmentation):
...
def test_maxtext_with_sft_in_trl():
    config = init_pyconfig(run_name="maxtext-with-sft-in-trl")
    inputs, inputs_segmentation, inputs_position = prepare_maxtext_inputs(config)
    trl_data =  prepare_trl_inputs(config)
    
    assert trl_data["input_ids"][0].tolist() == inputs[0].tolist()
    assert trl_data["attention_mask"][0].tolist() == inputs_segmentation[0].tolist()

    maxtext_logits = get_maxtext_logits(config, inputs, inputs_position, inputs_segmentation)
    
    hf_model = get_hf_model(config)
    trl_trainer = setup_sft_trainer(config, trl_data)
    trl_loss, trl_outputs = trl_trainer.compute_loss(hf_model, trl_data, return_outputs=True)
...
@pytest.mark.integration_test
def test_maxtext_without_sft_in_trl():
    config = init_pyconfig(run_name="maxtext-without-sft")
    inputs, inputs_segmentation, inputs_position = prepare_maxtext_inputs(config)
    trl_data =  prepare_trl_inputs(config)
    ...
    hf_model = get_hf_model(config)
    hf_outputs = hf_model(input_ids=trl_data["input_ids"], attention_mask=trl_data["attention_mask"])
    ...
    kl_div = get_kl_div(maxtext_logits, hf_logits)
    assert jax.numpy.all(kl_div < 7e-5)
