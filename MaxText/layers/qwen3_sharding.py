    ...
    for axis in axes:
      match tensor_name, axis, tensor_type, mode:
        case _, "embed_and_logits_batch", TT.Activation, _:
          mesh_axes.append((dp, pp, fsdp, fsdp_t))
        ...
        case _, "embed", TT.Weight, _:
          ...
          mesh_axes.append((tp, tp_t) if not context_ar and tensor_name not in ("query", "kv", "qkv", "out") else ())
        case _, "q_heads", TT.Weight, _:
          mesh_axes.append((tp, tp_t, tp_s, ar) if not context_ar and tensor_name not in ("query", "kv", "qkv", "out") else ())
