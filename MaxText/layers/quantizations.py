...
from aqt.jax.v2.config import set_bits
import common_types
...
class AqtQuantization:
  ...
  def einsum_act(self, mesh_axes: Tuple[str, ...] = (), mode=""):
    """Returns einsum configured with aqt params."""
    quant_dg = _get_activation_only_quant_config(lhs_bits=8, rhs_bits=None)
    
    if isinstance(self.quant_dg, dict):
      quant_dg, is_tiled, tiling_fn = self._get_mixed_precision_cfg()
    else:
      quant_dg, is_tiled, tiling_fn = quant_dg, False, None

    rhs_axis_metadata_wrapper = self._get_rhs_axis_metadata_wrapper(mesh_axes, is_tiled)
    # set_bits(quant_dg, fwd_lhs_bit=8, )
    aqt_einsum = functools.partial(
        aqt_flax.AqtEinsum(
            cfg=quant_dg,
            lhs_freeze_mode=aqt_flax.FreezerMode.NONE,
...
def _get_activation_only_quant_config(lhs_bits=None, rhs_bits=None):
  return aqt_config.dot_general_make(lhs_bits=lhs_bits, rhs_bits=None)
