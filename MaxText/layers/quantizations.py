...
def match_aqt_and_unquantized_param(aqt_params, params):
  aqt_param_flat, aqt_tree_def = jax.tree_util.tree_flatten_with_path(aqt_params, is_leaf=lambda x: isinstance(x, aqt_tensor.QTensor))
...
#     pruned_keys = []
#     for d in list(k):
#       if "AqtDotGeneral" in d.key or "AqtEinsum" in d.key:
#         # import pdb
#         # pdb.set_trace()
#         pruned_keys.append(jax.tree_util.DictKey(key="kernel"))
#         break
#       # if "AqtEinsum" in d.key:
#       #   # continue
#       #   # import pdb
#       #   # pdb.set_trace()
#       #   pruned_keys.append(jax.tree_util.DictKey(key="kernel"))
