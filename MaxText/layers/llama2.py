...
class LlamaDecoderLayer(nnx.Module):
  ...
  def __init__(
    ...
    #batch_size = 1 if model_mode == MODEL_MODE_PREFILL else config.global_batch_size_to_train_on


    #batch_size = 1 if model_mode == MODEL_MODE_PREFILL else config.per_device_batch_size

    #batch_size = 1 if model_mode == MODEL_MODE_PREFILL else 2

    #batch_size = 2
    
    batch_size = config.micro_batch_size_to_train_on
    #batch_size = config.global_batch_size_to_train_on
    #dummy_inputs_shape = (int(batch_size), int(config.max_target_length), int(config.emb_dim))
    #dummy_inputs_shape = (int(batch_size), int(config.max_prefill_predict_length), int(config.emb_dim))
    ...
    #dummy_inputs_shape = (int(batch_size), int(1), int(config.emb_dim))

    #dummy_inputs_shape = (int(batch_size), int(seq_len), int(config.emb_dim))
    dummy_inputs_shape = (batch_size, seq_len, config.emb_dim)

    # print('config.emb_dim: ' + str(config.emb_dim))
    # print('config.base_emb_dim: ' + str(config.base_emb_dim))
