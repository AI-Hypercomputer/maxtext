...
def apply_mask_to_logits(logits: Array, mask: Array):
  ...
  """
  return jnp.where(
      (mask >= DEFAULT_MASK_VALUE * 0.5), logits, DEFAULT_MASK_VALUE
  )
...
class AttentionOp(nn.Module):
  ...
  def check_attention_inputs(
      self, query: Array, key: Array, value: Array
  ) -> None:
  ...
  def generate_attention_mask(
      self, query, key, decoder_segment_ids: Array | None, model_mode: str
  ) -> Array | None:
    mask = None
    if model_mode == common_types.MODEL_MODE_AUTOREGRESSIVE:
      mask = (
          decoder_segment_ids[:, None, None, None, :]
    ...
    return (
        jnp.where(output_mask, 0.0, DEFAULT_MASK_VALUE)
  ...
  def apply_attention(
    ...
    if (
      ...
      return self.apply_attention_dot(
        ...
        self.attention_kernel == "flash"
        or self.attention_kernel == "autoselected"
    ):
      if model_mode == common_types.MODEL_MODE_AUTOREGRESSIVE:
        raise ValueError(
      ...
      return (
          self.tpu_flash_attention(query, key, value, decoder_segment_ids),
          None,
          None,
      )
  ...
  def compute_local_attention(
      self, attn_weights: Array, value: Array
  ...
  def _get_prefill_cache(self, batch, heads, kv_head_size, quantize_kvcache):
    ...
    cached_segment_id = self.variable(
        ...
        nn.with_logical_partitioning(
            jnp.zeros, ("cache_batch", "cache_sequence")
        ),
  ...
  def kv_cache_prefill(
    ...
    (
        ...
        cached_prefill_segment_id,
    ) = self._get_prefill_cache(
    ...
    if self.quantize_kvcache:
      key_shaped_for_cache, key_scale = quantizations.quantize_kv(
          key_shaped_for_cache
      )
      value_shaped_for_cache, value_scale = quantizations.quantize_kv(
  ...
  def update_ar_key_value(
    ...
    if self.quantize_kvcache:
      one_token_key, one_token_key_scale = quantizations.quantize_kv(
          one_token_key
      )
      one_token_value, one_token_value_scale = quantizations.quantize_kv(
          one_token_value
      )
    ...
    ar_key = jax.lax.dynamic_update_index_in_dim(
        ar_key, one_token_key, jnp.squeeze(one_hot_indices), 0
    )
    ...
    ar_value = jax.lax.dynamic_update_index_in_dim(
        ar_value, one_token_value, jnp.squeeze(one_hot_indices), 0
    )
    ...
    if self.quantize_kvcache:
      ...
      ar_key = quantizations.unquantize_kv(
          cached_key_var.value, cached_key_scale_var.value, one_token_key.dtype
      )
      ar_value = quantizations.unquantize_kv(
          cached_value_var.value,
          cached_value_scale_var.value,
          one_token_value.dtype,
      )
  ...
  def prefill_cache_var_model_var(self, cache_var, target_dtype):
    if not self.quantize_kvcache:
      ...
      raw_cache_unquantized = quantizations.unquantize_kv(
          raw_cache.value, quant_scale.value, target_dtype
      )
  ...
  def kv_cache_autoregressive(
    ...
    if sequence != 1:
      raise ValueError(
          f"Sequence length should be 1 during autoregression, got {sequence=}"
      )
    is_initialized = self.has_variable("cache", "cache_ar_index")
    if not is_initialized:
      raise ValueError(
          "Error, we can't do autoregression if we haven't seeded the KV Cache."
      )

    (
        cached_ar_key_var,
        cached_ar_value_var,
        cached_ar_segment_id,
        cache_ar_index,
    ...
    ar_key, ar_value = self.update_ar_key_value(
        key, value, cached_ar_key_var, cached_ar_value_var, cache_ar_index.value
    )
    active_indicator = (
        jnp.zeros((batch, 1), dtype=jnp.int32)
        + common_types.DECODING_ACTIVE_SEQUENCE_INDICATOR
    )
    cached_ar_segment_id.value = jax.lax.dynamic_update_index_in_dim(
        cached_ar_segment_id.value,
        active_indicator,
        jnp.squeeze(cache_ar_index.value),
        1,
    )
    cache_ar_index.value = jnp.mod(
        cache_ar_index.value + 1,
  ...
  def kv_cache(
    ...
    if key.shape != value.shape:
      raise ValueError(
          f"Can't KV cache with mismatched shapes {key.shape=}, {value.shape=}"
      )
  ...
  def __call__(self, query, key, value, decoder_segment_ids, model_mode):
    prefill_kv_cache, ar_kv_cache = self.kv_cache(
        key, value, decoder_segment_ids, model_mode
    )

    (
        prefill_unnormalized_output,
        prefill_exponentials_max,
    ...
    return self.normalize_attention(
        unnormalized_outputs, exponentials_maxes, exponentials_sums
    )
...
class Attention(nn.Module):
  ...
  def key_rotary(self, key: Array, inputs_positions: Array):
    ...
    key = RotaryEmbedding(embedding_dims=self.head_dim, name="key_rotary")(
