...
class AttentionOp(nn.Module):
  ...
  def swap(self, items: tuple[Any, Any, Any, Any], axis_order: AxisIdxes) -> tuple[Any, Any, Any, Any]:
    return tuple([items[i] for i in axis_order])

  def _get_prefill_cache_vars(self, batch, heads, kv_head_size):

    dtype = jnp.int8 if self.quantize_kvcache else jnp.bfloat16
    cache_logical_shape = (batch, self.max_prefill_predict_length, heads, kv_head_size)

    cache_axis_names = self.swap(self.cache_logical_axis_names, self.prefill_cache_axis_order)
    cache_shape = self.swap(cache_logical_shape, self.prefill_cache_axis_order)
    ...
    if self.quantize_kvcache:
      ...
      if self.kv_quant_axis in ("d", "d-b"):
        cache_scale_logical_shape = (batch, self.max_prefill_predict_length, heads, 1)
      elif self.kv_quant_axis == "hd":
        cache_scale_logical_shape = (batch, self.max_prefill_predict_length, 1, 1)

      cache_scale_axis_names = self.swap(self.cache_scale_logical_axis_names, self.prefill_cache_axis_order)
      cache_scale_shape = self.swap(cache_scale_logical_shape, self.prefill_cache_axis_order)
  ...
  def _get_ar_cache_vars(self, batch, heads, kv_head_size):
    ...
    cache_axis_names = self.swap(self.cache_logical_axis_names, self.ar_cache_axis_order)
    cache_shape = self.swap(cache_logical_shape, self.ar_cache_axis_order)
    ...
    if self.quantize_kvcache:
      ...
      cache_scale_axis_names = self.swap(self.cache_scale_logical_axis_names, self.ar_cache_axis_order)
      cache_scale_shape = self.swap(cache_scale_logical_shape, self.ar_cache_axis_order)
  ...
  def kv_cache_prefill(
    ...
    if self.quantize_kvcache:
      prefill_key_axis_names = self.swap(self.cache_logical_axis_names, self.prefill_cache_axis_order)
  ...
  def update_ar_key_value(
    ...
    if self.quantize_kvcache:
      ar_cache_scale_axis_names = self.swap(self.cache_scale_logical_axis_names, self.ar_cache_axis_order)
  ...
  def get_cached_values(self, cache_vars, target_dtype, cache_axis_order):
    ...
    if cache_scale_var is not None:
      ...
      scale_value_broadcast_shape = None
      if self.kv_quant_axis == "b-d":
        value_broadcast_axis = cache_var.names.index(CACHE_KV)
        value_broadcast_size = cached_value.shape[value_broadcast_axis]
        
        scale_value_broadcast_axis = cache_scale_var.names.index(CACHE_SCALE_KV)
        scale_value_broadcast_shape = cached_scale_value.shape[:scale_value_broadcast_axis] + [value_broadcast_size] + cached_scale_value.shape[scale_value_broadcast_axis+1:]
      cached_value = quantizations.unquantize_kv(cached_value, cached_scale_value, target_dtype, scale_value_broadcast_shape)
