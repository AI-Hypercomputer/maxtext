...
def extract_constructor_fields(nnx_class: type[M]) -> dict[str, tuple[tp.Any, tp.Any]]:
  ...
  for name, param in sig.parameters.items():
    ...
    if name == "self" or param.kind in (param.VAR_POSITIONAL, param.VAR_KEYWORD):
      continue
    annotation = param.annotation if param.annotation is not inspect.Parameter.empty else tp.Any
    default = param.default if param.default is not inspect.Parameter.empty else dataclasses.MISSING
...
def to_linen_class(
  ...
  for name, (annotation, default) in constructor_fields.items():
    ...
    if default is dataclasses.MISSING:
      ...
      dataclass_fields.append((name, annotation, default))

  dataclass_fields.append(("extra_kwargs", tp.Optional[dict], None))
