...
class Pipeline(nn.Module):
    ...
    * circ_storage_mover: assigned to rotated output and pushed into circ_storage on the next iteration
    ...
    if self.use_circ_storage:
      ...
      def _rotate_right_and_update(circ_storage_mover_in, circ_storage_in):
        ...
        # The offset is the previous iterations microbatch ID of the last stage.
        offset = (
  ...
  def __call__(
    ...
    inputs = inputs.reshape(
        (self.config.num_pipeline_microbatches, self.microbatch_size, self.config.max_target_length, self.config.emb_dim)
    )
    ...
    final_output = jnp.reshape(
        final_output, (self.config.global_batch_size_to_train_on, self.config.max_target_length, self.config.emb_dim)
    )
