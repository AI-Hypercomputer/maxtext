...
class MixtralDecoderLayer(nnx.Module):
  ...
  def __init__(
    ...
    batch_size = 1 if model_mode == MODEL_MODE_PREFILL else config.global_batch_size_to_load
  ...
  def __call__(
    ...
    attention_lnx = nn.with_logical_constraint(
        attention_lnx, activation_axis_names
    )
    ...
    hidden_states = nn.with_logical_constraint(
        hidden_states, activation_axis_names
    )
