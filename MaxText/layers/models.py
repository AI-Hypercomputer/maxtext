...
class Decoder(nn.Module):
  ...
  def __call__(self,
    ...
    if cfg.scan_layers:
      ...
      )(config=cfg, mesh=mesh, quant=self.quant, name='layers')(
          y,
          decoder_segment_ids,
          decoder_positions,
          deterministic,
      ...
      for lyr in range(cfg.num_decoder_layers):
        y = BlockLayer(config=cfg, mesh=mesh, quant=self.quant, name=f'layers_{lyr}')(
            y,
            decoder_segment_ids,
            decoder_positions,
            deterministic,
...
class Transformer(nn.Module):
  ...
  def setup(self):
    ...
    self.decoder = Decoder(
        config=cfg, shared_embedding=self.shared_embedding, mesh=mesh, quant=self.quant
    )
