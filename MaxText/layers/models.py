...
class DecoderLayer(nn.Module):
  ...
  def __call__(
    ...
    if model_mode == MODEL_MODE_PREFILL:
      attention_lnx = nn.with_logical_constraint(attention_lnx, ("activation_batch", "prefill_activation_length", "activation_embed"))
    else:
    ...
    if model_mode == MODEL_MODE_PREFILL:
      print("before constraint", layer_output.sharding)
      layer_output = nn.with_logical_constraint(
          layer_output,
          ("activation_batch", "prefill_activation_length", "activation_embed"),
      )
      print("after constraint", layer_output.sharding)
    else:
      print("here?")
      layer_output = nn.with_logical_constraint(
          layer_output,
