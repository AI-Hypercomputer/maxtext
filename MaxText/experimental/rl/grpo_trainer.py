...
def setup_train_loop(config, config_inference):
  ...
  _, _, inference_state_mesh_shardings = maxtext_utils.get_abstract_state(model, tx, config, init_rng, inference_mesh, is_training=False)
...
def train_loop(config, config_inference, state=None):
  ...
  inference_params = transfer_data({'params':state.params['params']}, inference_state_mesh_shardings.params)
