...
def summarize_logits(logits, choice):
  print(f"{choice} -- Argmax = {jax.numpy.argmax(logits, axis = 2)}")
...
def decode_loop(model, model_vars, sp_tokenizer, rng, prompts):
  ...
  summarize_logits(next_logit, "First Logit")
  new_position, new_cache, next_logit, selected_id = decode_ar_one_step(
    ...
    kv_cache_mesh_shardings, replicated_sharding)
  summarize_logits(next_logit, "Second Logit")
