...
block_size = 128
profiling_out_dir = f"/mnt/disks/persist/maxtext/profiling/"
...
def simple_timeit(f, warmups=5, tries=5, verbose=True):
...
class RaggedAttentionTest(absltest.TestCase):
  ...
  def test_ragged_mqa(self):
    ...
    jax.profiler.start_trace(f"/mnt/disks/persist/maxtext/profiling/dummy")
    jax.profiler.stop_trace()
    ...
    vmap_mqa_ref = jax.vmap(mqa_reference, in_axes=[None, 2, 2, None])
    ...
    vmap_ragged = jax.vmap(ragged_mqa, in_axes=[None, 1, 1, None])
