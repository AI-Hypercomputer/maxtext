...
class VertexTensorboardManager:
  ...
  def setup(self):
    ...
    if not vertex_tensorboard_project or not vertex_tensorboard_region:
      max_logging.log(
          "Either config.vertex_tensorboard_project or config.vertex_tensorboard_region is not set."
      )
      return None
  ...
  def upload_data(self, tensorboard_dir):
    ...
    if (
        not tensorboard_project
        or not tensorboard_region
        or not tensorboard_name
        or not experiment_name
    ):
      max_logging.log(
          "Vertex Tensorboard configurations are not set. Data will not be uploaded to Vertex AI."
      )
      self.uploader_flag = False
  ...
  def configure_vertex_tensorboard(self, config):
    ...
    if jax.process_index() == 0:
      ...
      if not os.environ.get("TENSORBOARD_NAME"):
        ...
        os.environ[
            "TENSORBOARD_NAME"
        ] = f"{vertex_tensorboard_project}-tb-instance"
      ...
      if config.use_vertex_tensorboard:  # running MaxText on GCE
        ...
        if tensorboard_url is None:
          raise ValueError(
              "Unable to create Tensorboard and Experiment in Vertex AI."
          )
        max_logging.log(
            f"View your Vertex AI Tensorboard at: {tensorboard_url}"
        )
        self.upload_data(config.tensorboard_dir)
      elif os.environ.get(
          "UPLOAD_DATA_TO_TENSORBOARD"
      ):  # running MaxText via XPK
        self.upload_data(config.tensorboard_dir)
