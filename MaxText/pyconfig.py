...
def set_and_validate_pipeline_config(raw_keys):
  if using_pipeline_parallelism(raw_keys):
    ...
    if raw_keys["pipeline_parallel_layers"] == -1:
      ...
      assert (raw_keys["pipeline_parallel_layers"] < raw_keys["num_decoder_layers"]), f"You can only pipeline a subset of decoder layers, but you requested to pipeline {raw_keys['pipeline_parallel_layers']} but there are only {raw_keys['num_decoder_layers']} layers "
    if raw_keys["num_pipeline_repeats"] == -1:
      ...
      assert (
          not remainder
      ), f"The number of layers per stage ({raw_keys['num_layers_per_pipeline_stage']}) times the number of stages ({num_stages}) must divide the number of pipeline_parallel_layers defaulting to decoder layers  ({raw_keys['pipeline_parallel_layers']}) "
    ...
    ), f"The product of pipeline stages ({num_stages}), repeats ({raw_keys['num_pipeline_repeats']}), and layers per stage ({raw_keys['num_layers_per_pipeline_stage']}) must be equal to pipeline_parallel_layers defaulting to decoder layers ({raw_keys['pipeline_parallel_layers']})"
