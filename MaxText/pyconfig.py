...
def validate_attention_type(s: str) -> None:
  ...
  if s not in valid_attention_types:  # currently supported attention
    raise ValueError(
        "Invalid attention type was passed. Valid options ",
        valid_attention_types,
    )
...
class _HyperParameters:
  ...
  def _update_from_env_and_command_line(
    ...
    for k in raw_data_from_yaml:
      ...
      if (
          not k in raw_data_from_cmd_line
          and not yaml_key_to_env_key(k) in os.environ
      ):
  ...
  def user_init(raw_keys):
    ...
    ] = get_quantization_local_shard_count(raw_keys)
