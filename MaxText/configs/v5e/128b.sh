...
# bash MaxText/configs/v5e/128b.sh EXECUTABLE=train_compile.py M_COMPILE_TOPOLOGY=v5e-256 M_COMPILE_TOPOLOGY_NUM_SLICES=2
...
python3 MaxText/$EXECUTABLE MaxText/configs/base.yml\
    steps=10 per_device_batch_size=1 enable_checkpointing=false\
    enable_profiler=true remat_policy=qkv_proj_offloaded global_parameter_scale=128\
    dcn_fsdp_parallelism=2 ici_fsdp_parallelism=16 ici_tensor_parallelism=16\
    max_target_length=2048 base_output_directory=gs://runner-maxtext-logs\
    use_iota_embed=true dataset_type=synthetic gcs_metrics=true\
