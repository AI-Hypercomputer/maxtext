    ...
    for USE_INT8_TRAINING in ${USE_INT8_TRAINING_ARRAY[@]}; do
        for DTYPE in ${DTYPE_ARRAY[@]}; do
            for PRNG_KEY in ${PRNG_KEY_ARRAY[@]}; do
                ...
                python3 multihost_job.py --BUCKET_NAME="mattdavidow-maxtext-br" --RUN_NAME=${RUN_NAME} --TPU_TYPE=v5litepod-256 --NUM_SLICES=1 --VERSION=v2-alpha-tpuv5-lite --COMMAND="bash setup.sh && bash MaxText/configs/8b-experiments-fwd-7-25-23.sh ${REMAT_POLICY} ${USE_INT8_TRAINING} ${DTYPE} ${PRNG_KEY} ${RUN_NAME}" --CQR_EXTRA_ARGS="--reserved --metadata priority-group=medium" --ZONE=us-west4-a
            done        
        done
    done
done