...
class RunInfo(BaseModel):
    ...
    sharding_strategy: None | str = Field(
        None,
...
class Checkpointing(BaseModel):
    ...
    source_checkpoint_layout: str = Field(
        "orbax", description="Optional checkpoint context to use for loading."
    )
...
class EmergencyCheckpointing(BaseModel):
    ...
    enable_emergency_checkpoint: bool = Field(
        False,
        description="Enables the experimental Orbax emergency checkpointing feature.",
    )
...
class MoEKernels(BaseModel):
    ...
    tile_batch_seq: int = Field(
        512, description="Tunable tiling dimension for batch/sequence in Megablox."
    )
    tile_activation_dim: int = Field(
        1024, description="Tunable tiling dimension for activation in Megablox."
    )
    tile_weight_dim: int = Field(
        1024, description="Tunable tiling dimension for weight in Megablox."
    )
...
class DatasetGeneral(BaseModel):
    ...
    train_data_columns: list[str] = Field(
        ["text"], description="Columns to use from the training data."
    )
    train_image_column: str = Field(
        "image", description="Column name for images in the training data."
    )
    eval_data_columns: list[str] = Field(
        ["text"], description="Columns to use from the evaluation data."
    )
    eval_image_column: str = Field(
        "image", description="Column name for images in evaluation data."
    )
    packing: bool = Field(
        True,
    ...
    expansion_factor_real_data: int = Field(
        -1, description="Factor for partial data loading on hosts."
    )
...
class FineTuning(BaseModel):
    ...
    num_generations: None | int = Field(
...
class Optimizer(BaseModel):
    ...
    learning_rate_schedule_steps: int = Field(
        -1, description="Total steps for the LR schedule. -1 defaults to `steps`."
    )
...
class InferenceServer(BaseModel):
    ...
    prefill_slice: str = Field(
        "v5e-16", description="Slice for prefill in disaggregation mode."
    )
    generate_slice: str = Field(
        "v5e-16", description="Slice for generation in disaggregation mode."
    )
...
class Metrics(BaseModel):
    ...
    metrics_file: PathStr = Field(
        "", description="Local file to store scalar metrics for testing."
    )
...
class MultimodalGeneral(BaseModel):
    ...
    remat_policy_for_vit: str = Field(
        "minimal", description="Rematerialization policy for the vision encoder."
    )
...
class DerivedValues(BaseModel):
    ...
    model_fsdp_ag_once: None | bool = Field(
        None,
    ...
    def set_derived_values(self) -> "MaxTextConfig":
        ...
        try:
            if self.compile_topology:
                compile_topology_spec = (
                    accelerator_to_spec_map.get_system_characteristics(
                ...
                subslice_shape_tuple = tuple(
                    int(x) for x in self.subslice_shape.split(",")
                )
        ...
        self.global_batch_size_to_load = int(
            micro_batch_size_to_load * self.gradient_accumulation_steps
        )
        self.global_batch_size_to_train_on = int(
            micro_batch_size_to_train_on * self.gradient_accumulation_steps
        )
