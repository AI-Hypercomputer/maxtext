...
class RunConfig(BaseModel):
    ...
    run_name: str = Field(
        default="", description="Name of the run. Auto-populated if empty."
    )
    base_output_directory: str = Field(
        default=os.path.join(gettempdir(), "maxtext"),
        description="Base directory for all outputs.",
    )
    metrics_file: Optional[str] = Field(
        default="",
        description="Local file for scalar metrics; empty means no local file.",
    )
    ...
    log_period: PositiveInt = Field(
        default=100, description="Frequency of TensorBoard flushes."
    )
    steps: int = Field(
        default=150_001,
        description="Total training steps. -1 to use learning_rate_schedule_steps.",
    )
    enable_tensorboard: bool = Field(
        default=True, description="Enable TensorBoard logging."
    )
    use_vertex_tensorboard: bool = Field(
        default=False, description="Use Vertex AI TensorBoard."
    )
    vertex_tensorboard_project: Optional[str] = Field(
        default="", description="GCP project for Vertex AI TensorBoard."
    )
    vertex_tensorboard_region: Optional[str] = Field(
        default="", description="Region for Vertex AI TensorBoard."
    )
...
class CheckpointLoadingConfig(BaseModel):
    ...
    load_parameters_path: Optional[str] = Field(
        default="", description="Path to load parameters-only checkpoint."
    )
    lora_input_adapters_path: Optional[str] = Field(
        default="", description="GCS path for LoRA adapters directory."
    )
    load_full_state_path: Optional[str] = Field(
        default="", description="Path to load full training state checkpoint."
    )
    checkpoint_is_quantized: bool = Field(
        default=False, description="True if loading a quantized checkpoint."
    )
...
class CheckpointSavingConfig(BaseModel):
    ...
    async_checkpointing: bool = Field(
        default=True, description="Use asynchronous checkpointing."
    )
    checkpoint_period: NonNegativeInt = Field(
        default=10_000, description="Checkpoint saving frequency in steps."
    )
    force_unroll: bool = Field(
        default=False,
        description="Force unroll loop for generate_param_only_checkpoint.",
    )
    save_quantized_params_path: Optional[str] = Field(
        default="", description="Path to save on-the-fly quantized params."
    )
...
class CheckpointStorageConfig(BaseModel):
    ...
    checkpoint_storage_target_data_file_size_bytes: int = Field(
        default=2147483648,
        description="Target file size for Orbax checkpoint sharding.",
    )
    checkpoint_storage_use_ocdbt: bool = Field(
        default=True, description="Use OCDBT for checkpointing."
    )
    checkpoint_storage_use_zarr3: bool = Field(
        default=True, description="Use Zarr3 for checkpointing."
    )
    checkpoint_storage_concurrent_gb: int = Field(
        default=96, description="Concurrent GB for checkpoint I/O."
    )
...
class EmergencyCheckpointConfig(BaseModel):
    ...
    enable_emergency_checkpoint: bool = Field(
        default=False, description="Enable Orbax emergency checkpointing."
    )
    local_checkpoint_directory: Optional[str] = Field(
        default="", description="Local directory for emergency checkpoints."
    )
    local_checkpoint_period: NonNegativeInt = Field(
        default=0, description="Frequency for local emergency checkpoints."
    )
    use_replicator_service: bool = Field(
        default=False, description="Use emergency checkpoint with replicator service."
    )
...
class CheckpointLoggingMiscConfig(BaseModel):
    ...
    enable_checkpoint_cloud_logger: bool = Field(
        default=False, description="Enable checkpoint cloud logger."
    )
...
class CheckpointConfig(BaseModel):
    ...
    emergency: EmergencyCheckpointConfig = Field(
        default_factory=EmergencyCheckpointConfig
    )
    logging_misc: CheckpointLoggingMiscConfig = Field(
        default_factory=CheckpointLoggingMiscConfig
    )
...
class ModelIdentityConfig(BaseModel):
    ...
    model_name: str = Field(
        default="default", description="Name of the model configuration to use."
    )
    override_model_config: bool = Field(
        default=False, description="Allow overriding model params via CLI."
    )
...
class ModelCoreConfig(BaseModel):
    ...
    decoder_block: DecoderBlockType = Field(
        default=DecoderBlockType.LLAMA2, description="Type of decoder block to use."
    )
    global_parameter_scale: int = Field(
        default=1, description="Global parameter scale (power of 2)."
    )
    weight_dtype: str = Field(
        default="float32", description="Data type for model weights."
    )
    normalization_layer_epsilon: float = Field(
        default=1.0e-05, description="Epsilon for normalization layers."
    )
    model_call_mode: ModelCallMode = Field(
        default=ModelCallMode.TRAIN,
        description="Mode for model execution ('train', 'inference').",
    )
    param_scan_axis: int = Field(
        default=1, description="Axis for parameter scanning if scan_layers is true."
    )
...
class ModelArchitectureConfig(BaseModel):
    ...
    base_emb_dim: PositiveInt = Field(
        default=2048, description="Base embedding dimension."
    )
    base_num_query_heads: PositiveInt = Field(
        default=16, description="Base number of query heads."
    )
    base_num_kv_heads: PositiveInt = Field(
        default=16, description="Base number of key/value heads."
    )
    base_mlp_dim: PositiveInt = Field(default=7168, description="Base MLP dimension.")
    base_num_decoder_layers: PositiveInt = Field(
        default=16, description="Base number of decoder layers."
    )
    head_dim: Optional[PositiveInt] = Field(
        default=128, description="Dimension of each attention head."
    )
...
class ModelActivationConfig(BaseModel):
    ...
    mlp_activations: List[str] = Field(
        default_factory=lambda: ["silu", "linear"],
        description="MLP activation functions.",
    )
    dropout_rate: NonNegativeFloat = Field(default=0.0, description="Dropout rate.")
    logits_via_embedding: bool = Field(
        default=False, description="Compute logits via embedding layer transpose."
    )
    normalize_embedding_logits: bool = Field(
        default=True,
        description="Normalize pre-softmax logits if logits_via_embedding is true.",
    )
    logits_dot_in_fp32: bool = Field(
        default=False, description="Use fp32 for logits dot product for stability."
    )
    cast_logits_to_fp32: bool = Field(
        default=True, description="Cast final logits to fp32."
    )
    float32_qk_product: bool = Field(
        default=False, description="Use fp32 for QK product in attention."
    )
    ...
    activations_in_float32: bool = Field(
        default=False, description="Cast activations to float32 before nonlinearity."
    )
...
class ModelMiscBehaviorConfig(BaseModel):
    ...
    record_internal_nn_metrics: NonNegativeInt = Field(
        default=0, description="Log internal NN metrics if > 0."
    )
    use_iota_embed: bool = Field(
        default=False, description="Use iota operator in Embed layer."
    )
    use_untrainable_positional_embedding: bool = Field(
        default=False, description="Use untrainable positional embeddings."
    )
    trainable_position_size: int = Field(
        default=-1,
        description="Enable GPT3-style trainable positional embeddings if > 0.",
    )
...
class QuantizationConfig(BaseModel):
    ...
    quantization: Optional[str] = Field(
        default="", description="Quantization type (e.g., 'int8', 'fp8')."
    )
    matmul_precision: MatMulPrecision = Field(
        default=MatMulPrecision.DEFAULT, description="Precision for matmul operations."
    )
    replicate_quant_scale: bool = Field(
        default=False, description="Replicate quantization scale for 2D sharding."
    )
    quant_cfg_path: Optional[str] = Field(
        default="", description="Path to quantization config for 'intmp'."
    )
    quantize_kvcache: bool = Field(
        default=False, description="Quantize KV Cache values."
    )
    kv_quant_axis: str = Field(
        default="heads_and_dkv", description="Axis for KV cache quantization."
    )
    kv_quant_dtype: str = Field(
        default="int8", description="Data type for KV cache quantization."
    )
    quantization_local_shard_count: int = Field(
        default=-1, description="Local shard count for quantization range finding."
    )
    ...
    def validate_kv_axis(cls, v, values):
        if values.get("quantize_kvcache") and v == "":
            raise ValueError(
                "kv_quant_axis cannot be empty if quantize_kvcache is True"
            )
        return v
...
class MoEConfig(BaseModel):
    ...
    num_experts_per_tok: PositiveInt = Field(
        default=1, description="Number of experts per token."
    )
    ...
    capacity_factor: float = Field(
        default=-1.0, description="Expert capacity factor for token dropping."
    )
    load_balance_loss_weight: NonNegativeFloat = Field(
        default=0.01, description="Weight for load balance loss."
    )
    use_random_routing: bool = Field(
        default=False, description="Use random routing for debug/test."
    )
    tile_batch_seq: Optional[PositiveInt] = Field(
        default=512, description="Tunable tiling dimension for Megablox."
    )
    tile_activation_dim: Optional[PositiveInt] = Field(
        default=1024, description="Tunable tiling dimension for Megablox."
    )
    tile_weight_dim: Optional[PositiveInt] = Field(
        default=1024, description="Tunable tiling dimension for Megablox."
    )
...
class DeepSeekMoEConfig(BaseModel):
    ...
    base_moe_mlp_dim: PositiveInt = Field(
        default=7168, description="Intermediate dimension at MoE layer for DeepSeek."
    )
    first_num_dense_layers: NonNegativeInt = Field(
        default=0, description="Number of initial dense layers for DeepSeek."
    )
    shared_experts: PositiveInt = Field(
        default=1, description="Number of shared experts for DeepSeek."
    )
    routed_scaling_factor: float = Field(
        default=1.0, description="Scaling factor for routing scores for DeepSeek."
    )
    routed_score_func: Optional[str] = Field(
        default="", description="Scoring function for routing for DeepSeek."
    )
    routed_bias: bool = Field(
        default=False, description="Add bias term for routing for DeepSeek."
    )
    n_routing_groups: int = Field(
        default=-1, description="Number of groups for routing for DeepSeek."
    )
    topk_routing_group: int = Field(
        default=-1, description="Number of top groups to route inputs for DeepSeek."
    )
...
class PipelineParallelConfig(BaseModel):
    ...
    num_pipeline_microbatches: int = Field(
        default=-1, description="Auto-computed if -1."
    )
...
class RematConfig(BaseModel):
    ...
    decoder_layer_input: RematTensorConfigValue = Field(
        default=RematTensorConfigValue.DEVICE
    )
...
class OptimizerConfig(BaseModel):
    ...
    mu_dtype: Optional[str] = Field(
        default="",
        description="Data type for AdamW 'mu'. Inherits from weight_dtype if unset.",
    )
...
class RoPEConfig(BaseModel):
    ...
    local_rope_max_timescale: int = Field(
        default=-1, description="Use rope_max_timescale if -1."
    )
...
class DecodeAlgoConfig(BaseModel):
    ...
    decode_sampling_nucleus_p: float = Field(
        default=-1.0, ge=-1.0, le=1.0, description="Allow -1 for 'not set'."
    )
...
class MaxEngineRunConfig(BaseModel):
    ...
    inference_server: InferenceServerType = Field(
        default=InferenceServerType.MAXTEXT_INTERLEAVED
    )
    prefill_slice: Optional[str] = Field(
        default="v5e-16", description="Slice for prefill in disaggregated server."
    )
    generate_slice: Optional[str] = Field(
        default="v5e-16", description="Slice for generation in disaggregated server."
    )
...
class MultimodalRunConfig(BaseModel):
    ...
    image_size_for_vit: PositiveInt = Field(
        default=896, description="Default for Gemma3."
    )
...
class InferenceMicrobenchmarkConfig(BaseModel):
    ...
    inference_microbenchmark_num_samples: List[PositiveInt] = Field(
        default_factory=lambda: [1, 2, 3, 4, 5]
    )
    inference_metadata_file: Optional[str] = Field(
        default=""
    )  # Path to JSON, not in base.yml
...
class MaxTextConfig(BaseModel):
    ...
    model_identity_config: ModelIdentityConfig = Field(
        default_factory=ModelIdentityConfig
    )
    model_core_config: ModelCoreConfig = Field(default_factory=ModelCoreConfig)
    model_architecture_config: ModelArchitectureConfig = Field(
        default_factory=ModelArchitectureConfig
    )
    model_activation_config: ModelActivationConfig = Field(
        default_factory=ModelActivationConfig
    )
    model_misc_behavior_config: ModelMiscBehaviorConfig = Field(
        default_factory=ModelMiscBehaviorConfig
    )
    ...
    pipeline_parallel_config: PipelineParallelConfig = Field(
        default_factory=PipelineParallelConfig
    )
    remat_config: RematConfig = Field(default_factory=RematConfig)

    attention_mechanism_config: AttentionMechanismConfig = Field(
        default_factory=AttentionMechanismConfig
    )
    attention_behavior_config: AttentionBehaviorConfig = Field(
        default_factory=AttentionBehaviorConfig
    )
    ...
    dataset_nesting_config: DatasetNestingConfig = Field(
        default_factory=DatasetNestingConfig
    )

    basic_training_config: BasicTrainingConfig = Field(
        default_factory=BasicTrainingConfig
    )
    ...
    max_engine_run_config: MaxEngineRunConfig = Field(
        default_factory=MaxEngineRunConfig
    )
    splash_attention_run_config: SplashAttentionRunConfig = Field(
        default_factory=SplashAttentionRunConfig
    )
    paged_attention_run_config: PagedAttentionRunConfig = Field(
        default_factory=PagedAttentionRunConfig
    )
    chunked_prefill_run_config: ChunkedPrefillRunConfig = Field(
        default_factory=ChunkedPrefillRunConfig
    )
    prefix_caching_run_config: PrefixCachingRunConfig = Field(
        default_factory=PrefixCachingRunConfig
    )
    llama4_specific_config: Optional[Llama4SpecificConfig] = None
    multimodal_run_config: MultimodalRunConfig = Field(
        default_factory=MultimodalRunConfig
    )
    ...
    gcp_workload_monitor_config: GCPWorkloadMonitorConfig = Field(
        default_factory=GCPWorkloadMonitorConfig
    )
    inference_microbenchmark_config: InferenceMicrobenchmarkConfig = Field(
        default_factory=InferenceMicrobenchmarkConfig
    )

    rep: NonNegativeInt = Field(
        default=0, description="For testing TPU performance, repeat the same batch."
    )
    max_checkify: bool = Field(
        default=False,
        description="Enable extra checks using jax.checkify (affects performance).",
    )
