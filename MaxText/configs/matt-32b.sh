echo "Running repro-raymond32b.sh"
RUN_NAME=${1}
REMAT_POLICY=${2}
PROJ_LIST=${3}
BATCH_SIZE=${4}
...
python3 MaxText/train.py MaxText/configs/base.yml run_name=$RUN_NAME\
    steps=6 per_device_batch_size=${BATCH_SIZE} enable_checkpointing=false\
    enable_profiler=true remat_policy=${REMAT_POLICY} base_emb_dim=8192 base_mlp_dim=32768\
    base_num_heads=32 base_num_decoder_layers=40 head_dim=256\
    max_target_length=2048 metrics_file='metrics.txt'\
    log_period=150 proj_list=${PROJ_LIST}
