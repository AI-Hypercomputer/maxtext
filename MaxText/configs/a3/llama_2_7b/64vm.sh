...
# --device-type ${DEVICE_TYPE} --num-nodes 64 \
# --command "bash MaxText/configs/a3/llama_2_7b/64vm.sh"

# Stop execution if any command exits with error
set -e

export OUTPUT_PATH="gs://maxtext-experiments-multipod"
export RUN_NAME="llama2-7b-64vm-$(date +%Y-%m-%d-%H-%M)"
...
# 64 node, DCN_DP=64, ICI_FSDP=8
python MaxText/train.py MaxText/configs/base.yml hardware=gpu \
    steps=30 dcn_data_parallelism=64 ici_fsdp_parallelism=8 per_device_batch_size=4 max_target_length=4096 model_name=llama2-7b \
