...
def get_tokenizer(tokenizer_path, add_bos=True, add_eos=True):
  ...
  sp_tokenizer = tokenizer.load_tokenizer(
      tokenizer_path=tokenizer_path, add_bos=add_bos, add_eos=add_eos
  )
  return sp_tokenizer


def make_c4_mlperf_train_iterator_and_tokenizer(
    config, mesh, add_bos, add_eos, process_indices
):
...
def make_c4_train_iterator_and_tokenizer(
    config, mesh, add_bos, add_eos, process_indices
):
...
def make_grain_train_iterator_and_tokenizer(
    config, mesh, add_bos, add_eos, process_indices
):
  """Make train iterator and tokenizer for C4 dataset"""
...
class BadSyntheticDataIterator:
  ...
  def __init__(self, config, mesh):
    ...
    self.data_generator = jax.jit(
        BadSyntheticDataIterator.get_bad_synthetic_data,
        out_shardings=data_pspec_shardings,
        static_argnums=0,
    )
...
def make_mixed_train_iterator_and_tokenizer(config, mesh, add_bos, add_eos):
  ...
  if jax.process_index() in process_indices:
    if config.dataset_type == "c4":
      return make_c4_train_iterator_and_tokenizer(
          config, mesh, add_bos, add_eos, process_indices
      )
    elif config.dataset_type == "c4-array_record":
      return make_grain_train_iterator_and_tokenizer(
          config, mesh, add_bos, add_eos, process_indices
      )
    ...
    return (
        BadSyntheticDataIterator(config, mesh),
        None,
        get_tokenizer(config.tokenizer_path, add_bos, add_eos),
    )


def create_data_iterator_with_tokenizer(
    config, mesh, add_bos=True, add_eos=True
):
  if config.dataset_type == "synthetic":
    return (
        SyntheticDataIterator(config, mesh),
        None,
        get_tokenizer(config.tokenizer_path, add_bos, add_eos),
    )
  elif config.dataset_type in ("c4", "c4-array_record", "c4_mlperf"):
    return make_mixed_train_iterator_and_tokenizer(
        config, mesh, add_bos, add_eos
    )
  else:
