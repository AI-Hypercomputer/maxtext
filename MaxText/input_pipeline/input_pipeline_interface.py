...
def get_converted_jsonl_datasets(
  config: ml_collections.ConfigDict,
  read_config = None,
):
  """Load and return dataset of batched examples for use during training."""
  # Training dataset.
  # converted_jsonl_dataset_path = config.dataset_path
...
def make_converted_jsonl_train_iterator_and_tokenizer(config, mesh, add_bos, add_eos):
  """ Make train iterator and tokenizer for converted_jsonl dataset"""

  read_config = tfds.ReadConfig(
    shuffle_seed = config.data_shuffle_seed,
  )
  # train_ds, eval_ds = get_datasets(
  train_ds, eval_ds = get_converted_jsonl_datasets(
    config=config,
...
def create_data_iterator_with_tokenizer(config, mesh, add_bos = True, add_eos = True):
  ...
  elif config.dataset_type == "converted_jsonl":
    return make_converted_jsonl_train_iterator_and_tokenizer(config, mesh, add_bos, add_eos)
  else:
