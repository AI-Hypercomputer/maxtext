...
class SyntheticDataIterator:
  ...
  def __init__(self, config, mesh):
    self.mesh = mesh
    self.config = config
    data_pspec_shardings = maxtext_utils.get_input_data_sharding(mesh, config.logical_axis_rules)
...
class BadSyntheticDataIterator:
  ...
  def __init__(self, config, mesh):
    ...
    self.data_generator = multihost_dataloading.MultiHostDataLoadIterator(dataset, self.mesh, config.logical_axis_rules)
...
def create_data_iterator(config, mesh):
  ...
  input_data_sharding = maxtext_utils.get_input_data_sharding(mesh, config.logical_axis_rules)
