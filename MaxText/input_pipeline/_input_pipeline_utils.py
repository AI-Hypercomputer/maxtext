...
class HFDataSource(grain.RandomAccessDataSource):
  ...
  def _check_shard_count(self):
    if self.n_shards < (self.dataloading_host_count * self.num_threads):
      max_logging.log(f"WARNING: Inefficient dataloading. Your train or eval dataset contains {self.n_shards} shards, ", 
                      f"smaller than number of host loading data. This is known to lead to inefficient dataloading. ",
                      f"Please reshard the data, or use a subset of hosts for dataloading by setting expansion_factor_real_data.",
                      )
...
class RemainderPaddedSource(grain.RandomAccessDataSource):
  """Generate padding examples when this host has less real examples than other hosts"""
  def __init__(self, dataset: grain.ArrayRecordDataSource , dataloading_host_count: int, max_target_length: int):
    self.dataset = dataset
    self.dataloading_host_count = dataloading_host_count
    self.max_target_length = max_target_length
    self.padded_len = math.ceil(len(self.dataset)/self.dataloading_host_count) * self.dataloading_host_count
  ...
  def __getitem__(self, record_key: int):
    ...
    elif record_key < self.padded_len:
      return self._generate_padding_example()

  def _generate_padding_example(self):
    values = [0]*self.max_target_length
    feature = tf.train.Feature(int64_list=tf.train.Int64List(value=values))
    feature_dict = {'ids': feature}
