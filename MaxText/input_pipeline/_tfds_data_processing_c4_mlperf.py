...
def rekey(ds, key_map=None):
  ...
  return ds.map(
      functools.partial(_rekey, key_map=key_map), num_parallel_calls=AUTOTUNE
  )
...
def preprocess_dataset(
  ...
  train_ds = reduce_concat_tokens(
      train_ds, feature_key="targets", batch_size=4096
  )
