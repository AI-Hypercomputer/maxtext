...
def make_tfds_train_iterator(
  ...
  if not config.colocated_python_data_input:
    ...
    return multihost_dataloading.MultiHostDataLoadIterator(train_dataloader, global_mesh, config.logical_axis_rules)
  else:
...
def make_tfds_eval_iterator(
  ...
  if not config.colocated_python_data_input:
    ...
    return multihost_dataloading.MultiHostDataLoadIterator(eval_dataloader, global_mesh, config.logical_axis_rules)
  else:
    get_ds_fn = functools.partial(
        get_datasets,
        dataset_name=config.eval_dataset_name,
