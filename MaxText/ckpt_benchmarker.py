...
def create_big_query_table(table_id):
  ...
  client = bigquery.Client(project=TESS_PROJECT)
  ...
  gcs_source = os.path.join(args.gcs_metrics_dir, args.run_name, GCS_METRICS_FILE)
...
def aggregate_metrics():
  ...
  for n in range(args.num_processes):
    path = os.path.join(args.gcs_metrics_dir, args.run_name, f'{n}.csv')
...
def main() -> None:
  ...
  print("Pods finished running successfully!")
  if args.gcs_metrics_dir != '':
    # Aggregate csv files from each process and combine them into one.
    print("Aggregating metrics files, it can take a while if there are many processes.")
    aggregate_metrics()
    print("Successfully aggregated metrics files.")
    # Upload the combined metrics file to GCS.
    aggregated_metrics_path = os.path.join(args.gcs_metrics_dir, args.run_name, GCS_METRICS_FILE)
    upload_blob(aggregated_metrics_path, GCS_METRICS_FILE)
    print(f"Uploaded combined csv file to {aggregated_metrics_path}")
    if args.bq_dataset != '':
