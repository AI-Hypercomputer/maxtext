...
This guide focuses on **response-based knowledge distillation**, a technique where a smaller "student" model is trained to replicate the outputs and behaviors of a larger, more complex "teacher" model. Within response-based knowledge distillation, two primary methods are often employed:
...
hf_access_token=$HF_TOKEN scan_layers=False \
...
When `multi_sampling=True` (Step 3.a), the `--num-generations` parameter should correspond to the desired number of unique completions per prompt. The `--batch-size 1024` argument specifies that 1024 prompts from the input dataset will be processed by the JetStream server for inference. The `--num-batches 200` argument indicates that this process of fetching 1024 prompts, generating completions, and then uploading the resulting prompt-completion pairs will be repeated 200 times. It's important to note that some prompts may be filtered out by pre-processing logic before inference. Additionally, the generated dataset can also be uploaded to Google Cloud Storage (GCS) by setting `upload-to-gcs --gcs-bucket <gcs bucket name> --gcs-data-path <path in gcs bucket>`.
...
hf_path=${USERNAME_OR_ORG}/${HF_REPO_NAME} train_split='train' train_data_columns=['prompt','completion'] \
load_parameters_path=${BASE_DIRECTORY}/llama2-7b-chat/scanned/0/items model_name=llama2-7b \
per_device_batch_size=2 ici_expert_parallelism=-1 ici_fsdp_parallelism=4 \
max_target_length=2048 \
hf_access_token=$HF_TOKEN checkpoint_period=1000
```
...
base_output_directory=${BASE_DIRECTORY}/distillation/deepseek2-16b-distill-llama2-7b \
tokenizer_path=meta-llama/Llama-2-7b-chat-hf tokenizer_type=huggingface \
hf_path='HuggingFaceH4/ultrachat_200k' train_split='train_sft' train_data_columns=['messages'] \
load_parameters_path=${FINE_TUNED_MODEL_CKPT_PATH} model_name=llama2-7b \
per_device_batch_size=2 ici_expert_parallelism=-1 ici_fsdp_parallelism=4 \
max_target_length=2048 \
hf_access_token=$HF_TOKEN checkpoint_period=1000
```
