...
class TokenizerTest(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.source_tokenizer = _input_pipeline_utils.get_tokenizer(
        os.path.join(ASSETS_ROOT, "tokenizer"), "sentencepiece", add_bos=False, add_eos=False
    )
...
class TikTokenTest(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    dataset_path = "gs://maxtext-dataset"
    cls.source_tokenizer = _input_pipeline_utils.get_tokenizer(
        os.path.join(ASSETS_ROOT, "tokenizer_llama3.tiktoken"),
        "tiktoken",
        add_bos=False,
        add_eos=False,
    )
...
class HFTokenizerTest(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    destination = os.path.join(ASSETS_ROOT, "")
    subprocess.run(
        ["gcloud", "storage", "cp", "-R", source, destination],
        check=True,
    )
    cls.hf_tokenizer = _input_pipeline_utils.get_tokenizer(
        os.path.join(ASSETS_ROOT, "gemma2-2b"), "huggingface", add_bos=False, add_eos=False
    )
    cls.sp_tokenizer = _input_pipeline_utils.get_tokenizer(
        os.path.join(ASSETS_ROOT, "tokenizer.gemma"), "sentencepiece", add_bos=False, add_eos=False
    )
