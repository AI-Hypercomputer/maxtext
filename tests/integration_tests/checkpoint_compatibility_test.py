...
from MaxText.globals import REPO_ROOT
...
def run_checkpoint_compatibility(hardware, attention_type):
  ...
  train_main(
      get_checkpointing_command(
          ...
          hardware=hardware,
          steps=1,
          metrics_file=os.path.join(REPO_ROOT, "run_1_metrics.txt"),
  ...
  train_main(
      get_checkpointing_command(
          ...
          hardware=hardware,
          steps=2,
          metrics_file=os.path.join(REPO_ROOT, "run_2_metrics.txt"),
          attention_type=attention_type,
          dataset_type="tfds",
          dataset_path="/tmp/gcsfuse",
      )
  ...
  check_start_step(os.path.join(REPO_ROOT, "run_2_metrics.txt"), 1.0)
