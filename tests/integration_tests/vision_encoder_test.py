...
from MaxText.globals import PKG_DIR, TEST_ASSETS_ROOT, ASSETS_ROOT
...
class VisionEncoderEmbeddingTest(unittest.TestCase):
  ...
  CONFIGS = {
      "gemma3-4b": [  # tests decode with multimodal gemma-4b
          ...
          rf"tokenizer_path={os.path.join(ASSETS_ROOT, 'tokenizer.gemma3')}",
          "use_multimodal=True",
          ...
          rf"image_path={os.path.join(TEST_ASSETS_ROOT, 'test_image.jpg')}",
  ...
  def test_image_embedding_gemma3_4b_tpu(self):
    ...
    input_golden_data_path = os.path.join(TEST_ASSETS_ROOT, "golden_data_gemma3_vit.jsonl")
