...
How to run:
  ...
  2. Run the script (forcing JAX on CPU to ensure exact parity with PyTorch):
     $ JAX_PLATFORMS=cpu python3 -m tests.assets.logits_generation.generate_dpo_golden_data_and_compare_pytorch_logits
"""
...
class PyTorchSyncTrainingHooks(InterceptingTrainingHooks):
  ...
  @classmethod
  def cleanup(cls):
    """Resets the class-level state between generation scenarios."""
    super().cleanup()
    cls.torch_policy_model = None
    cls.torch_ref_model = None
...
def run_parity_and_generate_golden():
  ...
  if jax.default_backend() != "cpu":
    raise RuntimeError(
        "DPO golden data generation must run on CPU (please run with environment variable JAX_PLATFORMS=cpu), "
  ...
  # Instantiate a dummy base test class to reuse its config builder helper methods.
  # This ensures parity in the JAX configuration and setup between this golden data
  # generation script and the actual unit/integration correctness tests.
  base_test = DPOCorrectnessTestBase()
  DPOCorrectnessTestBase.setUpClass()

  model_id = DPOCorrectnessTestBase.MODEL_ID
  ...
  for name, (max_prompt_len, dataset_filename, data_columns) in scenarios.items():
    print(f"\n--- Scenario: {name} ---")
    PyTorchSyncTrainingHooks.cleanup()
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
      config = base_test.build_tiny_qwen2_jax_config(
      ...
      jax_ref = run_jax_training(config, test_only_training_hooks_class=PyTorchSyncTrainingHooks)
