...
def get_tree_structure_info(tree: Dict[str, Any]):
    ...
    return {keystr(p): (getattr(l, 'shape', 'N/A'), str(getattr(l, 'dtype', type(l).__name__))) 
            for p, l in flat_with_path}
...
def print_structure_diff(params1, params2):
    ...
    for k in sorted(keys2 - keys1): print(f"  + Added in NNX: {k}")
    for k in sorted(keys1 - keys2): print(f"  - Missing in NNX: {k}")
