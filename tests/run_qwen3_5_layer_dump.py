...
from tests.unit.qwen3_5_layer_dump_test import (
    ...
    dump_tensors_to_npz,
    generate_comparison_markdown_table,
...
def benchmark_layer_on_tpu(
  ...
  print(
      f"\n>>> Running Qwen3.5 1-Layer Benchmark [{test_label}] in {dtype_str} on"
      " TPU..."
  )
  base_kwargs = {
  ...
  infer_mesh_shape = tuple(
      infer_tp_degree if axis == "model" else 1 for axis in cfg_infer.mesh_axes
  )
  ...
  # Bit-accurate weight copy from train to infer without submodule aliasing
  ...
  decoder_segment_ids_np = jnp.ones(
      (actual_batch_size, seq_len), dtype=jnp.int32
  )
  ...
  decoder_segment_ids = jax.device_put(
      decoder_segment_ids_np,
      NamedSharding(train_mesh, P(("data", "fsdp"), None)),
  )

  infer_replicated_sharding = NamedSharding(infer_mesh, P())
  infer_inputs = jax.device_put(inputs_np, infer_replicated_sharding)
  infer_decoder_positions = jax.device_put(
      decoder_positions_np, infer_replicated_sharding
  )
  infer_decoder_segment_ids = jax.device_put(
      decoder_segment_ids_np, infer_replicated_sharding
  )
  ...
  for name, t_train in train_tensors.items():
    ...
    m = metrics[name]
    print(
        f"     [{name:<25}] L_inf={m['max_abs_err']:.6e}, MAE={m['mae']:.6e},"
        f" CosSim={m['cos_sim']:.6f}"
    )
...
def main():
  ...
  print("=" * 80)
  print(
      "[Local TPU VM] Running directly on locally-attached TPU chips (no SPS"
      " proxy)."
  )
  print("=" * 80)
