...
def create_audio_in_video_sequence(
  ...
  expected_audio_tokens = int(multimodal_utils._get_feat_extract_output_lengths(jnp.array(audio_seqlens[0])).item()) # pylint: disable=protected-access
...
class GetRopeIndexComparisonTest(unittest.TestCase):
  ...
  def test_single_audio(self):
    ...
    jax_pos, torch_pos, jax_deltas, torch_deltas = self.compare_with_pytorch(input_ids, audio_seqlens=audio_seqlens)
