...
class UpdateStateParamTest(unittest.TestCase):
  ...
  def test_update_mode_add(self):
    ...
    new_state = maxtext_utils.update_state_param(self.state, target_path, update_value, mode="add")
  ...
  def test_update_mode_replace(self):
    target_path = ("layers", "layer_0", "bias")
    new_val = jnp.array([9.0, 9.0])
    new_state = maxtext_utils.update_state_param(self.state, target_path, new_val, mode="replace")

    actual = new_state.params["layers"]["layer_0"]["bias"]
    self.assertTrue(jnp.allclose(actual, new_val))

    # Other values are untouched
    original_gate = self.state.params["decoder"]["gate"]["bias"]
    new_gate = new_state.params["decoder"]["gate"]["bias"]
    self.assertTrue(jnp.allclose(original_gate, new_gate))
    original_layer_1 = self.state.params["layers"]["layer_1"]["bias"]
    new_layer_1 = new_state.params["layers"]["layer_1"]["bias"]
    self.assertTrue(jnp.array_equal(original_layer_1, new_layer_1))
