...
export EXTRA_FLAGS="sa_block_q=512 sa_block_kv=512 sa_block_kv_compute=512 sa_block_q_dkv=512 sa_block_kv_dkv=512 sa_block_kv_dkv_compute=512 sa_block_q_dq=512 sa_block_kv_dq=512 remat_policy=custom decoder_layer_input=offload float32_weight_sum=False use_tokamax_splash=True use_random_routing=True use_custom_sort_vjp=True attention=flash use_tokamax_gmm=False"

python3 -m maxtext.trainers.pre_train.train ${MAXTEXT_CONFIGS_DIR:-${MAXTEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}/base.yml base_output_directory=${BASE_OUTPUT_PATH} run_name=megablox_pre_training model_name=${MODEL_NAME} override_model_config=true use_indexer=false indexer_sparse_training=false tokenizer_type=huggingface tokenizer_path=${TOKENIZER_PATH} dataset_type=synthetic enable_checkpointing=false attention=flash use_tokamax_splash=True sparse_matmul=True megablox=True dtype=bfloat16 weight_dtype=bfloat16 per_device_batch_size=1 steps=5 max_target_length=4096 ici_expert_parallelism=4 ici_fsdp_parallelism=-1 opt_type=sgd ${EXTRA_FLAGS}
