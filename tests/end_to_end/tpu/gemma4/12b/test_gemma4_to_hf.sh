...
# Gemma4-12B MaxText checkpoint to Hugging Face format. Can be run on a v5p-8.
...
# 2. Provide a MaxText-format Gemma4-12B checkpoint via CKPT_PATH.
#    One can be produced with tests/end_to_end/tpu/gemma4/12b/convert_gemma4.sh.
# 3. Gemma 4 12B is the `gemma4_unified` HF architecture, which needs a
#    transformers build that ships `transformers.models.gemma4_unified`.
...
# Path to a pre-existing MaxText checkpoint for gemma4-12b. Must match USE_SCAN_LAYERS.
# Run tests/end_to_end/tpu/gemma4/12b/convert_gemma4.sh to produce one.
...
export HF_MODEL=google/gemma-4-12B
...
if [ "${USE_MULTIMODAL}" == true ]; then
    # Keep text after the image so the compared positions include ones that consume the
    # fused image representation; the placeholder positions themselves are masked out.
