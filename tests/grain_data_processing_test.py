...
from MaxText.globals import PKG_DIR, ASSETS_ROOT, REPO_ROOT
...
class GrainArrayRecordProcessingTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.config = pyconfig.initialize(
        ...
        tokenizer_path=os.path.join(ASSETS_ROOT, "tokenizer"),
...
def mount_gcsfuse():
  ...
  if not os.path.isdir(mount_path) or not os.listdir(mount_path):
    script_path = os.path.join(REPO_ROOT, "setup_gcsfuse.sh")
