...
def mount_gcsfuse():
  """
  Mounts a GCS bucket (gs://src/MaxText-dataset) to a local directory (/tmp/gcsfuse)
  ...
  if not os.path.isdir(mount_path) or not os.listdir(mount_path):
    ...
    exit_code = subprocess.call(
        ["bash", script_path, "DATASET_GCS_BUCKET=src/MaxText-dataset", f"MOUNT_PATH={os.path.join(temp_dir, 'gcsfuse')}"]
    )
