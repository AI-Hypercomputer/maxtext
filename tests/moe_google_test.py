...
class TokenDroppingTest(googletest.TestCase):
...
class DeepSeekRoutingTest(googletest.TestCase):
...
class RoutedMoeTest(googletest.TestCase):
  ...
  def get_moe_model_and_variables(self, variables, cfg, mesh):
    """retrieve expected model and variables from MoE"""
    ...
    return model, moe_variables

  def get_moe_output(self, variables, hidden_states, cfg, mesh):
    """retrieve expected outputfrom MoE"""
    model, moe_variables = self.get_moe_model_and_variables(variables, cfg, mesh)
    output = jax.jit(model.apply)(moe_variables, hidden_states)  # pylint: disable=not-callable
    return output

  def get_moe_output_and_grad(self, variables, hidden_states, cfg, mesh):
    """Computes output and gradients for MoE model."""
    model, moe_variables = self.get_moe_model_and_variables(variables, cfg, mesh)

    @jax.jit
    def loss_fn(moe_variables, hidden_states):
      output, _, _ = model.apply(moe_variables, hidden_states)  # pylint: disable=not-callable
      return jnp.sum(output), output

    (grad_variables, grad_states), output = jax.grad(loss_fn, argnums=(0, 1), has_aux=True)(moe_variables, hidden_states)  # pylint: disable=not-callable
    self.find_nans(grad_variables)
    self.find_nans(grad_states)
    return output, grad_variables, grad_states

  def find_nans(self, xs):
  ...
  def test_megablox_tpu_only(self):
    cfg = pyconfig.initialize(
        ...
        sparse_matmul=True,
        per_device_batch_size=1,
        max_target_length=128,  # Reduced to speed up test
        base_emb_dim=256,  # Reduced to speed up test
        base_mlp_dim=512,  # Reduced to speed up test
        base_moe_mlp_dim=512,  # Reduced to speed up test
    ...
    diff = jnp.abs(expected_output - actual_output)
    close_mask = diff <= (1e-02 + 1e-02 * jnp.abs(actual_output))
    self.assertGreater(jnp.mean(close_mask), 0.99)
  ...
  def test_megablox_expert_parallelism_tpu_only(self):
    cfg = pyconfig.initialize(
        ...
        megablox=True,
        sparse_matmul=True,
        per_device_batch_size=8,  # TODO(b/450900273): sharding error if pdbs=1
        max_target_length=128,  # Reduced to speed up test
        base_emb_dim=256,  # Reduced to speed up test
        base_mlp_dim=512,  # Reduced to speed up test
        base_moe_mlp_dim=512,  # Reduced to speed up test
  ...
  def test_megablox_ring_of_experts_tpu_only(self):
    cfg = pyconfig.initialize(
        [None, os.path.join(MAXTEXT_PKG_DIR, "configs", "base.yml")],
        run_name="megablox_ring_of_experts_tpu_only_test",
        enable_checkpointing=False,
        model_name="mixtral-8x7b",
        dtype="bfloat16",
        megablox=True,
        ...
        per_device_batch_size=8,  # TODO(b/450900273): sharding error if pdbs=1
        max_target_length=512,  # TODO(b/450900273): reduce length due to OOM
        base_emb_dim=256,  # Reduced to speed up test
        base_mlp_dim=512,  # Reduced to speed up test
        base_moe_mlp_dim=512,  # Reduced to speed up test
        override_model_config=True,
    ...
    with nn_partitioning.axis_rules(cfg.logical_axis_rules):
      ...
      self.assertGreater(
          jnp.mean(close_mask),
          0.99,
          msg=f"Expected output: {expected_output[..., 0]=}\nActual output: {actual_output[..., 0]=}",
      )

  def test_ring_of_experts_grad_equivalence(self):
    def get_value_and_grad(use_ring_of_experts):
      cfg = pyconfig.initialize(
          [None, os.path.join(MAXTEXT_PKG_DIR, "configs", "base.yml")],
          run_name="ring_of_experts_grad_equivalence_test",
          enable_checkpointing=False,
          model_name="mixtral-8x7b",
          dtype="bfloat16",
          megablox=True,
      ...
      with nn_partitioning.axis_rules(cfg.logical_axis_rules):
        ...
        return self.get_moe_output_and_grad(variables, hidden_states, cfg, mesh)

    output, var_grad, state_grad = get_value_and_grad(use_ring_of_experts=False)
    custom_output, custom_var_grad, custom_state_grad = get_value_and_grad(use_ring_of_experts=True)

    def assert_close(a, b, rtol=1e-02, atol=1e-02):
      diff = jnp.abs(a - b)
      close_mask = diff <= (atol + rtol * jnp.abs(b))
      fraction_close = jnp.mean(close_mask)
      self.assertGreater(
          fraction_close,
          0.99,
          msg=f"The arrays are not close. Only {fraction_close:.4f} of elements are close.\n"
          f"max absolute difference: {jnp.max(diff)=}\n"
          f"total difference: {jnp.sum(diff)=}",
      )
    ...
    def assert_norm_close(a, b, rtol=1e-02):
      self.assertLess(
          ...
          rtol,
          msg=f"The following two arrays are not close in norm\n" f"{jnp.linalg.norm(a - b)=}\n{jnp.linalg.norm(a)=}",
      )

    assert_close(output, custom_output)
    self.compare_gradients(var_grad, custom_var_grad, assert_norm_close)
    self.compare_gradients(state_grad, custom_state_grad, assert_close)

  def test_moe_fsdp_two_stage_parallelism_tpu_only(self):
    cfg = pyconfig.initialize(
        [None, os.path.join(MAXTEXT_PKG_DIR, "configs", "base.yml")],
        run_name="moe_block_fsdp_two_stage_test",
        enable_checkpointing=False,
        model_name="mixtral-8x7b",
        dtype="bfloat16",
        megablox=True,
  ...
  def test_custom_sort_vjp_grad_equivalence(self):
    def get_value_and_grad(use_custom_sort_vjp):
      cfg = pyconfig.initialize(
          [None, os.path.join(MAXTEXT_PKG_DIR, "configs", "base.yml")],
          run_name="moe_block_megablox_ep_grad_and_value_test",
          enable_checkpointing=False,
          model_name="mixtral-8x7b",
          dtype="bfloat16",
          megablox=True,
    ...
    output, var_grad, state_grad = get_value_and_grad(use_custom_sort_vjp=False)
    custom_output, custom_var_grad, custom_state_grad = get_value_and_grad(use_custom_sort_vjp=True)

    def assert_equal(a, b):
      self.assertTrue(jnp.array_equal(a, b), f"The following two arrays are not equal\n{a=}\n{b=}")

    def assert_close(a, b, rtol=1e-01, atol=1e-01):  # pylint: disable=unused-variable
      self.assertTrue(
          jax.numpy.allclose(
              a,
              b,
    ...
    assert_equal(output, custom_output)
    self.compare_gradients(state_grad, custom_state_grad, assert_equal)
    self.compare_gradients(var_grad, custom_var_grad, assert_equal)
    ...
    for global_expert_counts in [simple_group_sizes, manual_global_group_sizes]:
      for shard_id in range(expert_groups):
        ...
        local_group_sizes = jnp.sum(
            jnp.reshape(global_expert_counts, (expert_groups, experts_per_group)),
            axis=-1,
        )
        ...
        ), ("Local permuted rows do not match their unpermuted original rows" f" for shard_id={shard_id}")
  ...
  def test_get_all_to_all_params_unsharded_batch(self):
    ...
    for expert_shard_id in range(num_expert_parallelism_unsharded):
      ...
      self.assertTrue(
          jnp.array_equal(in_off, exp_in_off),
          "Unsharded Batch: Input offsets mismatch for shard" f" {expert_shard_id}",
      )
      self.assertTrue(
          jnp.array_equal(send_sz, exp_send_sz),
      ...
      self.assertTrue(
          jnp.array_equal(out_off, exp_out_off),
          "Unsharded Batch: Output offsets mismatch for shard" f" {expert_shard_id}",
      )
      self.assertTrue(
          jnp.array_equal(recv_sz, exp_recv_sz),
          "Unsharded Batch: Receive sizes mismatch for shard" f" {expert_shard_id}",
      )

  def test_get_all_to_all_params_with_truncation(self):
    """Tests get_all_to_all_params with truncation in forward pass."""
    # 4 shards, 4 experts.
    # Rows represent sender shards (batch shards).
    # Columns represent receiver experts.
    # e.g., Shard 0 wants to send: 1 token to Exp 0, 2 to Exp 1, 0 to Exp 2, 3 to Exp 3.
    num_expert_parallelism_sharded = 4
    all_group_sizes_sharded = jnp.array(
        [[1, 2, 0, 3], [4, 0, 1, 2], [0, 3, 2, 1], [2, 1, 4, 0]],
        dtype=jnp.int32,
    )
    # The receiver capacity for each expert is limited to 5 tokens.
    # Sum of columns in truncated matrix must be <= 5.
    buffer_size = 5

    # expected_input_offsets_sharded[i, j]: where shard i starts reading in its
    # local buffer when sending to expert j.
    # Sender buffers are NOT truncated, so offsets are based on original sizes.
    expected_input_offsets_sharded = jnp.array(
        [[0, 1, 3, 3], [0, 4, 4, 5], [0, 0, 3, 5], [0, 2, 3, 7]],
        dtype=jnp.int32,
    )
    # expected_send_sizes_sharded[i, j]: how many tokens shard i sends to expert j.
    # Truncated because we cannot send more than receiver capacity.
    # e.g. Exp 3 originally receives 3 (from S0) + 2 (from S1) + 1 (from S2) + 0 (from S3) = 6.
    # Truncated to 5: S0 sends 3, S1 sends 2, S2 sends 0 (truncated from 1), S3 sends 0.
    # So S2 sends [0, 3, 2, 0] (Row 2).
    expected_send_sizes_sharded = jnp.array(
        [[1, 2, 0, 3], [4, 0, 1, 2], [0, 3, 2, 0], [0, 0, 2, 0]],
        dtype=jnp.int32,
    )
    # expected_output_offsets_sharded[i, j]: where expert j starts writing in its
    # buffer when receiving from shard i.
    # Based on truncated sizes.
    expected_output_offsets_sharded = jnp.array(
        [[0, 0, 0, 0], [1, 2, 0, 3], [5, 2, 1, 5], [5, 5, 3, 5]],
        dtype=jnp.int32,
    )
    # expected_recv_sizes_sharded[i, j]: how many tokens expert i receives from shard j.
    # This is the transpose of truncated send_sizes.
    # e.g., Exp 0 receives: 1 (from S0), 4 (from S1), 0 (from S2), 0 (from S3, truncated from 2).
    # So Exp 0 row is [1, 4, 0, 0].
    expected_recv_sizes_sharded = jnp.array(
        [[1, 4, 0, 0], [2, 0, 3, 0], [0, 1, 2, 2], [3, 2, 0, 0]],
        dtype=jnp.int32,
    )
    ...
    for expert_shard_id in range(num_expert_parallelism_sharded):
      ...
      self.assertTrue(
          jnp.array_equal(in_off, exp_in_off),
          f"Truncated: Input offsets mismatch for shard {expert_shard_id}",
      )
      self.assertTrue(
          jnp.array_equal(send_sz, exp_send_sz),
          f"Truncated: Send sizes mismatch for shard {expert_shard_id}",
      )
      self.assertTrue(
          jnp.array_equal(out_off, exp_out_off),
          f"Truncated: Output offsets mismatch for shard {expert_shard_id}",
      )
      self.assertTrue(
          jnp.array_equal(recv_sz, exp_recv_sz),
          f"Truncated: Receive sizes mismatch for shard {expert_shard_id}",
      )

  def test_get_all_to_all_params_with_truncation_combine(self):
    """Tests get_all_to_all_params with truncation in combine stage."""
    # In combine stage, the communication is reversed.
    # Senders are experts (columns of original matrix), receivers are shards (rows).
    # We pass the same original all_group_sizes_sharded, but set is_dispatch=False.
    num_expert_parallelism_sharded = 4
    all_group_sizes_sharded = jnp.array(
        [[1, 2, 0, 3], [4, 0, 1, 2], [0, 3, 2, 1], [2, 1, 4, 0]],
        dtype=jnp.int32,
    )
    # Receiver capacity for each shard is limited to 5 tokens.
    buffer_size = 5

    # The effective routing matrix is transposed:
    # transposed = [[1, 4, 0, 2],
    #               [2, 0, 3, 1],
    #               [0, 1, 2, 4],
    #               [3, 2, 1, 0]]
    # Rows of transposed are sender experts.
    # Columns of transposed are receiver shards.
    # Truncation is applied to columns of transposed (receiver shards) to limit sum to buffer_size (5).
    # e.g., Shard 0 (Col 0 of transposed) originally receives: 1 (from E0) + 2 (from E1) + 0 (from E2) + 3 (from E3) = 6.
    # Truncated to 5: E0 sends 1, E1 sends 2, E2 sends 0, E3 sends 2 (truncated from 3).
    # So Col 0 of truncated_transposed is [1, 2, 0, 2].

    # expected_input_offsets_backward[i, j]: where expert i starts reading in its
    # local buffer when sending to shard j.
    # Based on truncated transposed matrix for sizes, but offsets are based on
    # the sender's buffer which is packed (forward receiver buffer was truncated).
    # So it uses truncated sizes for cumulative sum.
    expected_input_offsets_backward = jnp.array(
        [[0, 1, 5, 5], [0, 2, 2, 5], [0, 0, 1, 3], [0, 3, 5, 5]],
        dtype=jnp.int32,
    )
    # expected_send_sizes_backward[i, j]: how many tokens expert i sends to shard j.
    # This is transposed_truncated (transpose of forward-truncated matrix).
    # e.g. E0 (Row 0) sends: [1, 4, 0, 0] (since S2 receives 0 from E0, S3 receives 0 from E0).
    expected_send_sizes_backward = jnp.array(
        [[1, 4, 0, 0], [2, 0, 3, 0], [0, 1, 2, 2], [3, 2, 0, 0]],
        dtype=jnp.int32,
    )
    # expected_output_offsets_backward[i, j]: where shard j starts writing in its
    # buffer when receiving from expert i.
    # Receivers (shards) are NOT truncated (we write back to original untruncated positions).
    # So it uses untruncated sizes for cumulative sum along columns of transposed.
    expected_output_offsets_backward = jnp.array(
        [[0, 0, 0, 0], [1, 4, 0, 2], [3, 4, 3, 3], [3, 5, 5, 7]],
        dtype=jnp.int32,
    )
    # expected_recv_sizes_backward[i, j]: how many tokens shard i receives from expert j.
    # This is the columns of transposed_truncated.
    # e.g., S0 receives from E0 (1), E1 (2), E2 (0), E3 (3) -> [1, 2, 0, 3].
    expected_recv_sizes_backward = jnp.array(
        [[1, 2, 0, 3], [4, 0, 1, 2], [0, 3, 2, 0], [0, 0, 2, 0]],
        dtype=jnp.int32,
    )

    for expert_shard_id in range(num_expert_parallelism_sharded):
      exp_in_off = expected_input_offsets_backward[expert_shard_id]
      exp_send_sz = expected_send_sizes_backward[expert_shard_id]
      exp_out_off = expected_output_offsets_backward[expert_shard_id]
      exp_recv_sz = expected_recv_sizes_backward[expert_shard_id]
      ...
      self.assertTrue(
          jnp.array_equal(in_off, exp_in_off),
          "Truncated Combine: Input offsets mismatch for shard" f" {expert_shard_id}",
      )
      self.assertTrue(
          jnp.array_equal(send_sz, exp_send_sz),
          f"Truncated Combine: Send sizes mismatch for shard {expert_shard_id}",
      )
      self.assertTrue(
          jnp.array_equal(out_off, exp_out_off),
          "Truncated Combine: Output offsets mismatch for shard" f" {expert_shard_id}",
      )
      self.assertTrue(
          jnp.array_equal(recv_sz, exp_recv_sz),
          "Truncated Combine: Receive sizes mismatch for shard" f" {expert_shard_id}",
      )


class TruncateMatrixTest(googletest.TestCase):

  def test_no_truncation(self):
    all_shards = jnp.array([[10, 20], [30, 40]])
    buffer_size = 100
    truncated = moe._truncate_matrix(all_shards, buffer_size)  # pylint: disable=protected-access
    self.assertTrue(jnp.array_equal(all_shards, truncated))

  def test_truncation_col(self):
    # Column 1 sum: 20 + 90 = 110 > 100
    all_shards = jnp.array([[10, 20], [30, 90]])
    buffer_size = 100
    truncated = moe._truncate_matrix(all_shards, buffer_size)  # pylint: disable=protected-access
    expected = jnp.array([[10, 20], [30, 80]])
    self.assertTrue(jnp.array_equal(truncated, expected))

  def test_extreme_truncation(self):
    # Column 1 sum: 80 + 50 = 130 > 100
    all_shards = jnp.array([[10, 80], [30, 50]])
    buffer_size = 100
    truncated = moe._truncate_matrix(all_shards, buffer_size)  # pylint: disable=protected-access
    expected = jnp.array([[10, 80], [30, 20]])
    self.assertTrue(jnp.array_equal(truncated, expected))
