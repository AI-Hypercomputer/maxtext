...
"""Integration test validating DPO training step metrics against stored golden outputs.
...
class DPOTRLCorrectnessTest(DPOCorrectnessTestBase):
  ...
  def test_maxtext_dpo_correctness(self, name, max_prompt_len, dataset_filename, data_columns):
    ...
    golden_path = "tests/assets/golden_logits/golden_dpo_correctness.json"
    ...
    for key in ["chosen_logps", "rejected_logps"]:
      self.assertAlmostEqual(
          jax_metrics[key],
          golden[key],
          delta=self.LOG_PROBS_TOLERANCE,
          msg=f"Metric {key} diverges from golden: JAX {jax_metrics[key]:.6f} vs Golden {golden[key]:.6f}",
      )
