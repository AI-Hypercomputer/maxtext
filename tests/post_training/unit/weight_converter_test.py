...
# pylint: disable=protected-access,line-too-long,import-outside-toplevel
...
class TargetFreeConversionTest(unittest.TestCase):
  ...
  def test_case_5_target_free_conversion_large_layers(self):
    ...
    out_root = out["base"] if "base" in out else out
    for layer in range(num_layers):
      self.assertIn(f"layers_{layer}", out_root["decoder"])
      self.assertIn("moe_block", out_root["decoder"][f"layers_{layer}"])
      wi = getattr(
          out_root["decoder"][f"layers_{layer}"]["moe_block"]["wi"],
          "value",
          out_root["decoder"][f"layers_{layer}"]["moe_block"]["wi"],
      )
