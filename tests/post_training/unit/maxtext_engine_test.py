...
class MaxTextTrainingEngineTest(absltest.TestCase):
  ...
  def test_model_scope_yields_the_live_model_and_mutates_no_training_state(self):
    ...
    with t.model_scope(np.ones((2, 4), np.int32), pad_id=0) as (model, args, kwargs):
      out = jnp.sum(args[0])
  ...
  def test_model_scope_places_arrays_where_a_compiled_step_would(self):
    ...
    with mock.patch.object(maxtext_engine.sharding, "get_input_data_sharding", return_value=self._sharded_batch_spec(t)):
      with t.model_scope(np.ones((2, 4), np.int32), empty_prompt, mask=np.ones((2,), np.int32)) as (_, args, kwargs):
        tokens, prompt = args
    ...
    self.assertEqual(kwargs["mask"].sharding.spec, jax.sharding.PartitionSpec("data"))
    self.assertIs(prompt, empty_prompt)

  def test_model_scope_holds_the_logical_axis_rules_open_for_the_block(self):
    """The caller traces inside the block, so the rules must still be in force there."""
    t = maxtext_engine.MaxTextTrainingEngine(self.mock_config)

    with t.model_scope() as (_, _, _):
      inside = maxtext_engine.nn_partitioning.get_axis_rules()

    self.assertTrue(inside)
    # And released on exit: the scope is a loan, not a global mode switch.
    self.assertFalse(maxtext_engine.nn_partitioning.get_axis_rules())
