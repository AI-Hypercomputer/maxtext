...
class TrainSFTTest(unittest.TestCase):
  ...
  def test_validate_config_accepts_preaveraged_diffusion_evaluation(self):
    config = SimpleNamespace(
        optimizer_memory_host_offload=False,
        training_objective="block_diffusion",
        gradient_accumulation_steps=1,
        eval_interval=10,
        loss_is_preaveraged=True,
    )

    train_sft.validate_config(config)

  def test_setup_validates_before_initializing_model(self):
    config = SimpleNamespace(optimizer_memory_host_offload=True)
  ...
  def test_diffusion_objective_uses_weighted_tunix_trainer(self):
    config = SimpleNamespace(training_objective="block_diffusion")
