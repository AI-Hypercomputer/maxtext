...
def create_torch_vision_config(config, attn_implementation="eager"):
  """Create PyTorch vision encoder config from JAX config."""
  torch_config = Qwen3OmniMoeVisionEncoderConfig(
      ...
      rope_theta=config.rope_theta_for_vit,
      ...
      num_position_embeddings=config.num_position_embeddings_for_vit,
      out_hidden_size=config.out_hidden_size_for_vit,
      deepstack_visual_indexes=list(config.deepstack_visual_indexes_for_vit),
      hidden_act="gelu",
  )
  torch_config._attn_implementation = attn_implementation  # pylint: disable=protected-access
  return torch_config


def create_torch_encoder(config):
  """Create and configure PyTorch vision encoder."""
  torch_config = create_torch_vision_config(config)
  encoder = TorchQwen3OmniMoeVisionEncoder(torch_config)
  encoder.eval()
  return encoder
...
class TestQwen3OmniMoeVisionAttention(BaseVisionTestCaseWithMesh):
  ...
  def test_attention_output_matches_torch(self):
    ...
    torch_encoder = create_torch_encoder(self.config)
...
class TestQwen3OmniMoeVisionPatchMerger(BaseVisionTestCase):
  ...
  def _test_patch_merger_with_postshuffle(self, use_postshuffle_norm):
    ...
    torch_model = TorchQwen3OmniMoeVisionPatchMerger(torch_config, use_postshuffle_norm=use_postshuffle_norm)
    torch_model.eval()

    jax_model = JaxQwen3OmniMoeVisionPatchMerger(
        config=self.config, use_postshuffle_norm=use_postshuffle_norm, rngs=nnx.Rngs(42)
    )
...
class TestQwen3OmniMoeVisionPatchEmbed(BaseVisionTestCase):
  ...
  def test_patch_embed_output_matches_torch(self):
    ...
    jax_model = JaxQwen3OmniMoeVisionPatchEmbed(config=self.config, rngs=nnx.Rngs(42))
...
class TestQwen3OmniMoeVisionRotaryEmbedding(BaseVisionTestCase):
  ...
  def test_grid_based_embedding_matches_torch(self):
    ...
    jax_model = self._create_jax_rotary_model()
    torch_encoder = create_torch_encoder(self.config)
    ...
    cos_emb_jax, sin_emb_jax = jax_model.compute_cos_sin(num_frames, height, width)
  ...
  def test_rotation_application_matches_torch(self):
    ...
    q_rotated_jax = jax_model(q_jax, num_frames, height, width)
    k_rotated_jax = jax_model(k_jax, num_frames, height, width)
...
class TestQwen3OmniMoeVisionPosEmbedInterpolate(BaseVisionTestCase):
  ...
  def _copy_weights_and_test(self, num_frames, height, width):
    ...
    jax_model = self._create_jax_pos_embed_model()
    torch_encoder = create_torch_encoder(self.config)
    ...
    pos_embed_jax = jax_model(num_frames, height, width)
...
class TestQwen3OmniMoeVisionEncoderEndToEnd(BaseVisionTestCaseWithMesh):
  ...
  def test_vision_encoder_single_image(self):
    ...
    torch_hidden_states = split_into_patches(
        torch.from_numpy(np.array(jax_hidden_states)), temporal_patch_size, patch_size
    )
    ...
    jax_encoder_output, jax_deep_feats = jax_encoder(jax_hidden_states, 1, h, w)
    ...
    assert_all_close_jax_torch(
        jax_output, torch_output, rtol=1e-2, atol=1e-2, error_msg="Vision encoder final output differs"
    )

    # Compare deep features
    self.assertEqual(len(jax_deep_feats), len(torch_deep_feats), "Number of deep features should match")
    for i, (jax_feat, torch_feat) in enumerate(zip(jax_deep_feats, torch_deep_feats)):
      assert_all_close_jax_torch(jax_feat, torch_feat, rtol=1e-2, atol=1e-2, error_msg=f"Deep feature {i} differs")
