...
def _get_common_args(temp_dir, model_name, steps=1, overrides=None):
  ...
  tokenizer_path = os.path.join(MAXTEXT_ASSETS_ROOT, "tokenizers",
                                "tokenizer.llama2")
...
def _run_train_subprocess(args):
  ...
  cmd = [sys.executable, "-m", "maxtext.trainers.pre_train.train", args[1]
        ] + args[2:]
  env = os.environ.copy()
...
class PEFTIntegrationTest(unittest.TestCase):
  ...
  def test_native_sft_gemma4_lora(self):
    ...
    args = _get_common_args(temp_dir,
                            "gemma4-e2b",
                            steps=2,
                            overrides=overrides)
  ...
  def test_native_sft_gemma4_lora_checkpoint_save_and_restore(self):
    """Test native SFT with Gemma4-e2b, LoRA enabled, checkpoint save and subsequent restore."""
    temp_dir = self._create_temp_dir()
    metrics_file_saved = os.path.join(temp_dir, "saved_metrics_gemma4.txt")
    metrics_file_restored = os.path.join(temp_dir,
                                         "restored_metrics_gemma4.txt")
    ...
    # 1. Step 1 run with checkpointing enabled
    ...
    _run_sft_subprocess(args_save)

    checkpoint_dir = os.path.join(temp_dir, "test_peft_gemma4-e2b",
                                  "checkpoints", "0", "items")
    self.assertTrue(os.path.exists(checkpoint_dir),
                    f"Checkpoint directory {checkpoint_dir} was not created.")

    # 2. Step 2 run restoring from step 1 checkpoint
    ...
    _run_sft_subprocess(args_restore)
    ...
    self.assertEqual(len(restored_metrics), 1)
  ...
  def test_qlora_applied_verification(self):
    """Explicitly verify that QLoRA applies LoRAParam adapters and quantizes base weights."""
    from flax import nnx
    import jax
    from maxtext.utils import lora_utils, model_creation_utils
    from maxtext.configs import pyconfig

    cfg = pyconfig.initialize_pydantic(
        [sys.argv[0], get_test_config_path()],
        run_name="test_qlora_verify",
    ...
    # 1. Assert model is identified as LoRA-enabled
    self.assertTrue(lora_utils.is_lora_enabled(lora_model))

    # 2. Assert LoRAParam adapter variables exist
    ...
    # 3. Assert QLoRA quantization config is active
    self.assertEqual(cfg.lora.lora_weight_qtype, "int8")
  ...
  def test_pretrain_lora_checkpoint_save_and_restore(self):
    ...
    checkpoint_dir = os.path.join(temp_dir, "test_peft_default", "checkpoints",
                                  "0", "items")
    self.assertTrue(os.path.exists(checkpoint_dir),
                    f"Checkpoint directory {checkpoint_dir} was not created.")
    ...
    self.assertEqual(len(saved_metrics), 1,
                     "First run should have exactly 1 step.")
    self.assertEqual(len(restored_metrics), 1,
                     "Second run should have exactly 1 step.")
  ...
  def test_lora_optimizer_wrt_filtering(self):
    """Verify optimizer wrt filtering targets nnx.LoRAParam and base params remain frozen."""
    from flax import nnx
    import jax
    from maxtext.utils import lora_utils, model_creation_utils
    from maxtext.configs import pyconfig
    ...
    _, state = nnx.split(lora_model)
    lora_params = nnx.filter_state(state, nnx.LoRAParam)
    base_params = nnx.filter_state(state, nnx.Param)

    # Verify LoRAParam exists and base_params does not include LoRAParam
    self.assertGreater(len(jax.tree_util.tree_leaves(lora_params)), 0)
    self.assertGreater(len(jax.tree_util.tree_leaves(base_params)), 0)

    # Verify that wrt filtering works as expected for optax/nnx optimizer
    wrt_filter = nnx.LoRAParam
    filtered_state = nnx.filter_state(state, wrt_filter)
    self.assertEqual(len(jax.tree_util.tree_leaves(filtered_state)),
                     len(jax.tree_util.tree_leaves(lora_params)))

  def test_standalone_lora_adapter_restoration(self):
    """Test loading base parameters and separate LoRA adapter weights."""
    from flax import nnx
    from maxtext.utils import lora_utils, model_creation_utils
    ...
    # Extract target lora parameter structure
    target_state = nnx.state(lora_model, (nnx.Param, nnx.LoRAParam))
    has_pure = hasattr(target_state, "to_pure_dict")
    pure_target = target_state.to_pure_dict(
    ) if has_pure else checkpointing._tree_to_dict(target_state)

    self.assertIn("decoder", pure_target)

  def test_pretrain_gemma4_scanned_blocks_lora_resume(self):
    """Test Gemma4 pre-training with LoRA, checkpointing, and resume."""
    temp_dir = self._create_temp_dir()
    metrics_file_saved = os.path.join(temp_dir, "saved_metrics_gemma4_scan.txt")
    metrics_file_restored = os.path.join(temp_dir,
                                         "restored_metrics_gemma4_scan.txt")
    ...
    # 2. Step 2: Resume training from step 1 checkpoint
    args_restore = _get_common_args(
        temp_dir,
        "gemma4-e2b",
        steps=2,
