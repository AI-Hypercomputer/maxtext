...
class PEFTIntegrationTest(unittest.TestCase):
  ...
  def test_native_sft_gemma4_lora_checkpoint_save_and_restore(self):
    ...
    metrics_file_restored = os.path.join(temp_dir, "restored_metrics_gemma4.txt")
    ...
    checkpoint_dir = os.path.join(temp_dir, "test_peft_gemma4-e2b", "checkpoints", "0", "items")
  ...
  def test_lora_optimizer_wrt_filtering(self):
    ...
    self.assertEqual(len(jax.tree_util.tree_leaves(filtered_state)), len(jax.tree_util.tree_leaves(lora_params)))
  ...
  def test_standalone_lora_adapter_restoration(self):
    ...
    pure_target = target_state.to_pure_dict() if has_pure else checkpointing._tree_to_dict(target_state)
  ...
  def test_pretrain_gemma4_scanned_blocks_lora_resume(self):
    ...
    metrics_file_restored = os.path.join(temp_dir, "restored_metrics_gemma4_scan.txt")
