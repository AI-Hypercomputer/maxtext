...
def _get_peak_memory_gb(compiled) -> float:
  ...
  if hasattr(mem_obj, "peak_memory_in_bytes") and mem_obj.peak_memory_in_bytes:
    return float(mem_obj.peak_memory_in_bytes) / (1024.0**3)
  if hasattr(mem_obj, "cumulative_size_in_bytes") and mem_obj.cumulative_size_in_bytes:
    return float(mem_obj.cumulative_size_in_bytes) / (1024.0**3)
  if hasattr(mem_obj, "temp_size_in_bytes"):
    total_bytes = (
        int(getattr(mem_obj, "temp_size_in_bytes", 0))
        + int(getattr(mem_obj, "argument_size_in_bytes", 0))
        + int(getattr(mem_obj, "output_size_in_bytes", 0))
        - int(getattr(mem_obj, "alias_size_in_bytes", 0))
    )
