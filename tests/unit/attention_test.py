...
class ChunkedCausalMaskTest(unittest.TestCase):
    ...
    def test_basic_chunking(self):
        ...
        # pylint: disable=protected-access
        actual_mask = _generate_chunk_attention_mask(
            mask_shape=mask.shape, chunk_size=chunk_size
        )
        np.testing.assert_array_equal(actual_mask, expected_mask)
...
class AttentionTest(parameterized.TestCase):
    ...
    def test_tpu_kernel_attention_gqa_share_kv(self):
        self.tpu_kernel_attention_helper(
            self.num_kv_heads // 2, share_kv_projections=True
        )
    ...
    def test_share_kv_projections(self):
        ...
        self.assertTrue(
            jax.numpy.allclose(
                output_shared, output_no_share, rtol=1e-04, atol=1e-04, equal_nan=False
            )
        )
    ...
    def test_dot_product_cache_axis_order(self):
        ...
        for axis_order in random.choices(all_axis_orders, k=4):
            self.dot_product_attention_helper(
                prefill_cache_axis_order=axis_order, ar_cache_axis_order=axis_order
            )
            print(f"passed test for {axis_order=}")

    def dot_product_attention_helper(
        self, prefill_cache_axis_order, ar_cache_axis_order
    ):
    ...
    def _dot_product_attention(
        ...
        dummy_inputs_q = jnp.ones(
            (self.global_batch_size, config.max_target_length, config.base_emb_dim)
        )
        dummy_inputs_kv = jnp.ones(
            (self.global_batch_size, config.max_target_length, config.base_emb_dim)
        )
        ...
        for idx in range(prefill_length, decode_total_length):
            ...
            attention_w_layout_full_this_idx = attention_w_layout_full[
                :, idx : idx + 1, :
            ]
            self.assertTrue(
                attention_w_layout_full_this_idx.shape == attention_w_layout_idx.shape
            )
            self.assertTrue(
                jax.numpy.allclose(
    ...
    def _dot_product_attention_reshape_q(self, compute_axis_order):
        ...
        for idx in range(prefill_length, decode_total_length):
            ...
            attention_wo_reshape_q_full_this_idx = attention_wo_reshape_q_full[
                :, idx : idx + 1, :
            ]
            self.assertTrue(
                attention_wo_reshape_q_full_this_idx.shape
                == attention_wo_reshape_q_idx.shape
            )
            self.assertTrue(
                jax.numpy.allclose(
            ...
            attention_w_reshape_q_full_this_idx = attention_w_reshape_q_full[
                :, idx : idx + 1, :
            ]
            self.assertTrue(
                attention_w_reshape_q_full_this_idx.shape
                == attention_w_reshape_q_idx.shape
            )
            self.assertTrue(
                jax.numpy.allclose(
    ...
    @pytest.mark.skip(
        reason="Requires `vllm-tpu` package which is not yet a MaxText dependency."
    )
    @pytest.mark.tpu_only
    @mock.patch(
    ...
    def test_forward_serve_vllm(self, mock_sharded_ragged_paged_attention):
        ...
        mock_attention_metadata.query_start_loc = jnp.array(
            list(range(self.global_batch_size))
        )
        mock_attention_metadata.request_distribution = jnp.array(
        ...
        reshaped_mock_output = mock_output.reshape(
            self.global_batch_size, seq_len, self.num_query_heads, self.head_dim
        )
...
class MLATest(attention_test_util.MLATestBase):
    ...
    def test_projection_initialization(self):
        ...
        self.assertFalse(
            hasattr(mla_layer, "query"), "MLA should not have 'query' projection."
        )
        self.assertFalse(
            hasattr(mla_layer, "key"), "MLA should not have 'key' projection."
        )
        self.assertFalse(
            hasattr(mla_layer, "value"), "MLA should not have 'value' projection."
        )

        # 5. Assert that the MLA layer HAS all of its own specific projections AND the common 'out' projection
        self.assertTrue(
            hasattr(mla_layer, "wq_a"), "MLA should have 'wq_a' projection."
        )
        self.assertTrue(
            hasattr(mla_layer, "wq_b"), "MLA should have 'wq_b' projection."
        )
        self.assertTrue(
            hasattr(mla_layer, "wkv_a"), "MLA should have 'wkv_a' projection."
        )
        self.assertTrue(
            hasattr(mla_layer, "wkv_b"), "MLA should have 'wkv_b' projection."
        )
        self.assertTrue(
            hasattr(mla_layer, "q_norm"), "MLA should have 'q_norm' projection."
        )
        self.assertTrue(
            hasattr(mla_layer, "kv_norm"), "MLA should have 'kv_norm' projection."
        )
    ...
    def test_fused_mla_lora_proj_output_equivalence(self):
        ...
        cfg_unfused = pyconfig.initialize(
            [sys.argv[0], get_test_config_path()], **unfused_args
        )
        ...
        cfg_fused = pyconfig.initialize(
            [sys.argv[0], get_test_config_path()], **fused_args
        )
        mla_fused = MLA(
            config=cfg_fused,
        ...
        self.assertTrue(
            jax.numpy.allclose(
                output_unfused, output_fused, rtol=1e-05, atol=1e-05, equal_nan=False
            ),
    ...
    def test_indexer_loss(self):
        ...
        query, key, indexer_score = self.get_indexer_test_data(
            batch_size, q_len, kv_len, num_heads, head_dim
        )
...
class Qwen3NextGatedDeltaNetTest(unittest.TestCase):
    ...
    def test_autoregression(self):
        ...
        for idx in range(prefill_length, decode_total_length):
            ...
            self.assertTrue(
                jax.numpy.allclose(
                    gdn_full_this_idx, gdn_idx, rtol=1e-02, atol=1e-02, equal_nan=False
                )
            )
