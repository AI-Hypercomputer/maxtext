...
class Qwen3NextGatedDeltaNetTest(unittest.TestCase):
  ...
  def test_paged_state_uses_pinned_tpu_inference_gdn_api(self):
    """The MaxText paged-state bridge matches the pinned tpu-inference API."""
    ...
    attention_metadata = types.SimpleNamespace(
        padded_num_reqs=1,
        mamba_state_indices=jnp.array([1], dtype=jnp.int32),
        query_start_loc=jnp.array([0, 1], dtype=jnp.int32),
        request_distribution=jnp.array([0, 0, 1], dtype=jnp.int32),
        seq_lens=jnp.array([1], dtype=jnp.int32),
    )
    ...
    np.testing.assert_array_equal(mock_run_gdn.call_args.args[9], attention_metadata.mamba_state_indices)
    np.testing.assert_array_equal(mock_run_gdn.call_args.args[10], attention_metadata.query_start_loc)
