...
class TestQwen3Next(unittest.TestCase):
  ...
  def test_gated_delta_net_left_padding_invariance_and_gradients(self):
    ...
    unpadded_segment_ids = jnp.ones(
        (self.batch_size, valid_len), dtype=jnp.int32
    )
    ...
    unpadded_out, _ = jax_model(
        valid_hidden, decoder_segment_ids=unpadded_segment_ids
    )
    padded_out, _ = jax_model(
        padded_hidden, decoder_segment_ids=padded_segment_ids
    )

    # 1. Padding token outputs must be strictly zeroed out.
    zero_pad = np.zeros(
        (self.batch_size, pad_len, self.hidden_size), dtype=np.float32
    )
