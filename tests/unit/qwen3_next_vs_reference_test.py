...
class Qwen3NextExperts_PT(nn.ModuleList):
    ...
    def __init__(self, config):
        ...
        for _ in range(config.num_experts):
            self.append(
                Qwen3NextMLP_PT(config, intermediate_size=config.moe_intermediate_size)
            )
...
class Qwen3NextGatedDeltaNet_PT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.norm = Qwen3NextRMSNormGated_PT(
            self.head_v_dim, epsilon=self.layer_norm_epsilon
        )
...
class TestQwen3Next(unittest.TestCase):
    ...
    def test_rms_norm_gated(self):
        ...
        pt_model = Qwen3NextRMSNormGated_PT(
            self.hidden_size, epsilon=self.cfg.normalization_layer_epsilon
        )
    ...
    def test_gated_delta_net_structure(self):
        ...
        jax_model = qwen3.Qwen3NextGatedDeltaNet(
            config=self.cfg, rngs=self.nnx_rngs, inputs_shape=hidden_states_jax.shape
        )
    ...
    def test_qwen3_next_rms_norm(self):
        ...
        jax_model_wrapped = DummyModule(
            self.hidden_size, self.cfg.normalization_layer_epsilon, self.nnx_rngs
        )
    ...
    def test_gated_delta_net_full(self):
        ...
        jax_model = qwen3.Qwen3NextGatedDeltaNet(
            config=self.cfg, rngs=self.nnx_rngs, inputs_shape=hidden_states_pt.shape
        )
        ...
        v_w = qkvz_pt[:, 2 * key_dim : 2 * key_dim + value_dim].reshape(
            in_features, H_k, V_per_K * D_v
        )
        z_w = qkvz_pt[:, 2 * key_dim + value_dim :].reshape(
            in_features, H_k, V_per_K * D_v
        )

        reordered_qkvz = np.concatenate([q_w, k_w, v_w, z_w], axis=-1).reshape(
            in_features, -1
        )
        ...
        params = {
            ...
            "norm": {
                "weight": nnx.Param(jnp.array(pt_model.norm.weight.detach().numpy()))
            },
            "out_proj": {
                "kernel": nnx.Param(
