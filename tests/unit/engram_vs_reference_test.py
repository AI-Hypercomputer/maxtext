...
class MultiHeadEmbeddingTest(parameterized.TestCase):
  ...
  def test_mhe_match(self, vocab_sizes, head_dim=32):
    ...
    jax_model = MultiHeadEmbeddingJAX(cfg, vocab_sizes, head_dim, mesh, self.rngs)
    # weight transfer
...
class ShortConvTest(parameterized.TestCase):
  ...
  def test_shortconv_match(self, hc_mult, dilation=2, hidden_size=16, kernel_size=3):
    ...
    # 1. PyTorch
    pt_model = ShortConv(hidden_size, kernel_size, dilation, hc_mult=hc_mult, norm_eps=cfg.normalization_layer_epsilon)
    init_torch_weights(pt_model)
    pt_model.eval()
    with torch.no_grad():
      y_pt = pt_model(x_pt)
...
class EngramTest(parameterized.TestCase):
  ...
  def test_engram_match(self, hc_mult):
    # config
