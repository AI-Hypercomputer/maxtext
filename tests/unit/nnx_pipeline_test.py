...
class TestNonTrainablePipelineBackward(unittest.TestCase):
  ...
  Needs 4 devices: pipeline parallelism uses 1 device per stage (ici_pipeline_parallelism=4 -> 4). On
  CPU a single host is split into 4 simulated XLA devices via XLA_FLAGS=--xla_force_host_platform_device_count=4
  (see the module header); on TPU the chips are used directly. @_NEEDS_4_DEVICES skips when fewer than 4
