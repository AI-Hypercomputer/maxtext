...
"""End-to-end coverage of checkpoint_missing_param_policy on NNX restore.

Drives the real create_orbax_checkpoint_manager -> maybe_save_checkpoint ->
load_state_if_possible stack (no mocks). A weight the checkpoint is missing comes back as an
unmaterialized ShapeDtypeStruct and keeps its init value; `warn` (default) logs naming it,
`error` raises. The check filters by nnx.Param, so rngs/dropout absence never trips it.
"""
...
def _config(missing_param_policy="warn"):
...
class TestMissingParamRoundTrip(unittest.TestCase):
  ...
  def _load_overlay(self, manager, model_cls, policy="warn"):
  ...
  def test_missing_weight_raises_naming_the_path(self):
    ...
    with self.assertRaises(ValueError) as ctx:
      self._load_overlay(manager, _ModelExtraLayer, policy="error")
    msg = str(ctx.exception)
    self.assertIn("model/extra", msg)  # the exact offending parameter path
    self.assertIn("missing model weights", msg)

  def test_missing_weight_warn_keeps_init(self):
    """Under warn, restore succeeds and the missing weight stays an unmaterialized placeholder (init survives)."""
    manager = self._save_model()
    overlay = self._load_overlay(manager, _ModelExtraLayer, policy="warn")
    # The extra layer had no array on disk, so its leaves come back as unmaterialized
    # ShapeDtypeStructs -> the trainer fills them from its fresh init when it overlays this.
    extra = overlay["model"]["extra"]
    self.assertIsInstance(extra["kernel"], jax.ShapeDtypeStruct)
    self.assertIsInstance(extra["bias"], jax.ShapeDtypeStruct)
