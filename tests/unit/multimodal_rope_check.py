...
class GetRopeIndexComparisonTest(unittest.TestCase):
  ...
  def compare_with_pytorch(
    ...
    jax_attention_mask = jnp.array(attention_mask) if attention_mask is not None else None
    jax_image_grid_thw = jnp.array(image_grid_thw) if image_grid_thw is not None else None
    jax_video_grid_thw = jnp.array(video_grid_thw) if video_grid_thw is not None else None
    jax_audio_lengths = jnp.array(audio_lengths) if audio_lengths is not None else None
    jax_second_per_grids = jnp.array(second_per_grids) if second_per_grids is not None else None

    jax_position_ids, jax_deltas = multimodal_utils.get_rope_index(
        input_ids=jax_input_ids,
        image_grid_thw=jax_image_grid_thw,
        video_grid_thw=jax_video_grid_thw,
        attention_mask=jax_attention_mask,
        use_audio_in_video=use_audio_in_video,
        audio_lengths=jax_audio_lengths,
        second_per_grids=jax_second_per_grids,
    ...
    jax_position_ids_np = np.array(jax_position_ids)
    torch_position_ids_np = torch_position_ids.cpu().numpy()
    jax_deltas_np = np.array(jax_deltas)
