...
class MllogUtilsTest(unittest.TestCase):
  ...
  def test_full_lifecycle_ordering(self):
    ...
    mllog_utils.run_start()
    mllog_utils.block_start(config, step=0)
    mllog_utils.tracked_stats(config, step=100, step_time=0.5, loss=9.0)
    mllog_utils.eval_start(config, step=100)
    mllog_utils.eval_stop(config, step=100, eval_loss=9.0)
    mllog_utils.run_stop(current_epoch_num=100 * config.global_batch_size_to_train_on)
  ...
  def test_run_stop_closes_block_when_run_has_no_eval(self):
    ...
    self.assertEqual(
        ...
        {"samples_count": config.eval_interval * config.global_batch_size_to_train_on, "step": 6},
    )
