...
def _reference_partial_mrope(inputs: np.ndarray, positions: np.ndarray) -> np.ndarray:
  ...
  inv_freq = 1.0 / (
      _ROPE_THETA ** (np.arange(0, _ROTARY_DIM, 2, dtype=np.float32) / _ROTARY_DIM)
  )
...
class Qwen35PartialMropeTest(unittest.TestCase):
  ...
  def test_matches_reference_and_preserves_unrotated_suffix(self):
    inputs = np.linspace(-1.0, 1.0, num=2 * 4 * 3 * _HEAD_DIM, dtype=np.float32).reshape(
        2, 4, 3, _HEAD_DIM
    )
    text_positions = np.broadcast_to(np.arange(4, dtype=np.int32), (2, 4))
    multimodal_positions = np.stack(
        [text_positions, text_positions + 3, text_positions + 7], axis=0
    )
  ...
  def test_factor_one_preserves_full_width_mrope_behavior(self):
    inputs = np.linspace(-1.0, 1.0, num=2 * 4 * 3 * _ROTARY_DIM, dtype=np.float32).reshape(
        2, 4, 3, _ROTARY_DIM
    )
