...
class GdnBwdPallasTest(absltest.TestCase):
  ...
  def test_decoupled_conv1d_gdn_kernel_conv_bias_none(self):
    ...
    do = jax.random.normal(k7, (batch_size, seq_len, num_v_heads, head_v_dim), dtype=jnp.float32)
  ...
  def test_chunk_state_forward_with_cached_tinv_matches_chunk_forward(self):
    ...
    actual_state = gdn_bwd_pallas.chunk_state_forward_with_cached_tinv(
        k=k,
        v=v,
        b_val=b_val,
        a_val=a_val,
  ...
  def test_decoupled_conv1d_gdn_kernel_gradient_with_initial_states(self):
    ...
    dt_bias = jax.random.normal(k7, (num_v_heads,), dtype=jnp.float32)
    do = jax.random.normal(k8, (batch_size, seq_len, num_v_heads, head_v_dim), dtype=jnp.float32)
    conv_state = jax.random.normal(k9, (batch_size, conv_kernel_size - 1, dim_size), dtype=jnp.float32)
  ...
  def test_decoupled_conv1d_gdn_kernel_bwd_with_head_tile(self):
    ...
    t_inv = jax.random.normal(k10, (batch_size, num_chunks, num_v_heads, chunk_size, chunk_size), dtype=jnp.float32)
