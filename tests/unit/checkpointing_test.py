...
class FP8DequantizeOnLoadTest(parameterized.TestCase):
  ...
  def test_scale_sharding_derivation(self):
    ...
    moe_weight_sharding = jax.sharding.NamedSharding(
        mesh, jax.sharding.PartitionSpec(None, None, "fsdp", "tensor")
    )
    ...
    self.assertEqual(
        moe_scale_sharding.spec, jax.sharding.PartitionSpec(None, None, "fsdp", "tensor")
    )
