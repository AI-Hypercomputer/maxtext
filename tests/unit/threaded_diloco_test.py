...
from maxtext.trainers.diloco.threaded_fragmenter import FragmentedTreeManipulator
...
class ThreadedDilocoUnitTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.config = pyconfig.initialize(
        ...
        # main's DilocoParams validator requires num_diloco_fragments when
        # enable_streaming_diloco=True (default is None on main, was 16 on the branch).
...
class SyncerPathwaysBugReproTest(unittest.TestCase):
  ...
  def test_bare_take_on_scanned_fragment_fails_eagerly(self):
    ...
    with mock.patch("maxtext.trainers.diloco.threaded_fragmenter.jnp.take", self._pathways_take):
