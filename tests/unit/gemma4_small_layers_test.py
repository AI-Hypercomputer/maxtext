...
# ---------------------------------------------------------------------------
# Gemma-4 vision clipped-linears (PR #4790): pure jax-cpu mechanism + validation tests.
# Consolidated here from the standalone gemma4_clip_linears_test.py per review.
# ---------------------------------------------------------------------------
...
class SymbolsTest(unittest.TestCase):
  ...
  def test_public_symbols_present(self):
    for sym in (
        "validate_all_vision_clip_bounds",
        "clip_optimizer_freeze_mask",
        "Gemma4ClippedMlpBlock",
        "Gemma4Attention",
    ):
...
class OptionSInvariantsTest(unittest.TestCase):
  ...
  def test_other_negative_coord_fails(self):
    ...
    ok, _n_bad = gv.option_s_position_sentinel_ok(pos)
    self.assertFalse(ok)
...
# ---------------------------------------------------------------------------
# Gemma-4 E2B/E4B multimodal STATIC contract gate (configs/types.py).
# Consolidated here from the standalone gemma4_mm_contract_gate_test.py per review.
# ---------------------------------------------------------------------------


def _init_gate_config(overrides):
  """Initialize a gemma4-e2b multimodal MaxTextConfig for the contract-gate tests."""
  base_yml = os.path.join(MAXTEXT_REPO_ROOT, "src", "maxtext", "configs", "base.yml")
  argv = [
      "test",
      base_yml,
      "model_name=gemma4-e2b",
...
class Gemma4MmContractGateTest(unittest.TestCase):
  ...
  def test_valid_e2b_mm_config_builds(self):
    ...
    cfg = _init_gate_config(["use_clipped_linears_for_vit=true"])
  ...
  def test_unknown_ple_pad_mode_hard_fails(self):
    with self.assertRaises(Exception):
      _init_gate_config(["use_clipped_linears_for_vit=true", "ple_pad_mode=bogus"])

  def test_both_ple_pad_mode_is_valid_ablation_enum(self):
    cfg = _init_gate_config(["use_clipped_linears_for_vit=true", "ple_pad_mode=both"])
  ...
  def test_bidirectional_image_attn_hard_fails(self):
    ...
    with self.assertRaises(Exception):
      _init_gate_config(["use_clipped_linears_for_vit=true", "use_bidirectional_image_attn=true"])

  def test_fused_qkv_hard_fails(self):
    with self.assertRaises(Exception):
      _init_gate_config(["use_clipped_linears_for_vit=true", "fused_qkv=true"])

  def test_fused_mlp_hard_fails(self):
    with self.assertRaises(Exception):
      _init_gate_config(["use_clipped_linears_for_vit=true", "fused_mlp=true"])

  def test_clipped_linears_off_under_mm_hard_fails(self):
    with self.assertRaises(Exception):
      _init_gate_config(["use_clipped_linears_for_vit=false"])

  def test_packing_hard_fails(self):
    # Packed multimodal is unsupported for E2B/E4B in any mode (cross-doc image attention risk).
    with self.assertRaises(Exception):
      _init_gate_config(["use_clipped_linears_for_vit=true", "packing=true"])
