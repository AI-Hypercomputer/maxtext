...
Tests cover:
  ...
  - QK L2 norm: applied outside the kernel, as the layer documents
...
from maxtext.kernels.kda.tokamax import _MIN_TOKAMAX_VERSION, kda_api_unavailable, tokamax_chunk_kda
...
class TestKimiDeltaAttention:
  ...
  def test_a_log_init_follows_gate_path(self, mesh):
    """A_log init depends on the gate path, as in the reference layer.

    The softplus gate multiplies the decay by exp(A_log), so A_log is drawn as
    log(U(1, 16)); the safe gate already bounds the decay by `lower_bound` and
    A only sharpens the sigmoid, so it starts at A = 1.
    """
    safe = self._make_attn(mesh, use_kda_safe_gate=True)
    assert jnp.all(safe.A_log.value == 0.0)
    assert safe.A_log.value.shape == (4,)

    softplus = self._make_attn(mesh, use_kda_safe_gate=False)
    values = softplus.A_log.value
    assert values.shape == (4,)
    assert bool(jnp.all(values >= 0.0)), "log(U(1, 16)) must be non-negative"
    assert bool(jnp.all(values <= jnp.log(16.0) + 1e-6))
    assert bool(jnp.any(values > 0.0)), "A_log should be drawn, not all zeros"
...
def _kda_pyconfig(**kwargs):
  """Build a config selecting KDA, for exercising the config-time guards.

  `base.yml` defaults `packing=true` and `max_segments_per_seq=-1`, a
  combination KDA now rejects at config time, so the helper pins a valid bound
  by default; individual tests override it to exercise the guard.
  """
  defaults = {
      "attention_type": "kda",
      "scan_layers": False,
...
class TestKdaConfigGuards:
  ...
  def test_kda_cp_rejects_load_balance_at_config_time(self):
    """KDA under CP must reject load balancing when the config is parsed.

    `context_parallel_load_balance` defaults to true, so without the config
    guard a multi-node KDA run would only fail at its first forward step —
    after device allocation and weight initialization. The layer keeps its own
    runtime check as defense in depth.
    """
    with pytest.raises(ValueError, match="context_parallel_load_balance"):
      _kda_pyconfig(ici_context_parallelism=2, context_parallel_load_balance=True)
    cfg = _kda_pyconfig(ici_context_parallelism=2, context_parallel_load_balance=False)
    assert cfg.ici_context_parallelism == 2

  def test_kda_packing_requires_max_segments_at_config_time(self):
    """A packed KDA run needs a static segment bound before the data pipeline is built.

    The kernel derives per-rank segment metadata from segment_ids and needs
    `max_num_segments`; failing here beats failing after the pipeline and the
    weights are up.
    """
    with pytest.raises(ValueError, match="max_segments_per_seq"):
      _kda_pyconfig(packing=True, max_segments_per_seq=-1)
    cfg = _kda_pyconfig(packing=True, max_segments_per_seq=8)
    assert cfg.max_segments_per_seq == 8
  ...
  def test_use_kda_lora_true_rejected(self):
    """use_kda_lora=True selects an unimplemented path and must be rejected."""
...
class TestKdaKernelGuards:
  ...
  def test_missing_kda_api_error_names_required_version(self):
    """The unavailability error must say which tokamax release is needed."""
    with pytest.raises(ImportError) as excinfo:
      raise kda_api_unavailable()
    message = str(excinfo.value)
    assert _MIN_TOKAMAX_VERSION in message, "error must name the first release shipping the KDA API"
    assert "tokamax._src.ops.experimental.kda" in message
    assert "pip install" in message, "error must give an actionable upgrade command"

  def test_missing_kda_api_error_keeps_detail_and_cause(self):
    """Callers can explain what needed the API, and the cause stays chained."""
    cause = ImportError("no module named tokamax._src.ops.experimental.kda")
    with pytest.raises(ImportError) as excinfo:
      raise kda_api_unavailable(cause, detail="ContextParallelMetadata is unavailable.")
    assert str(excinfo.value).startswith("ContextParallelMetadata is unavailable.")
