...
class TestNnxConstructNamedSharding(unittest.TestCase):
  ...
  The helper resolves a NamedSharding for each NNX Variable inside an nnx.State and
  — unlike flax.nnx.spmd.get_var_pspec — also inserts the `nnx.PARTITION_NAME` axis at
  ...
  def test_composite_pytree_variable_resolved_to_replicated_shardings(self):
    """A Variable holding a composite pytree (e.g. tuple of arrays) is resolved to replicated NamedShardings."""
    with jax.set_mesh(self.mesh):
