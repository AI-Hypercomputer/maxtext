...
class RLDiffusionConfigTest(parameterized.TestCase):
  ...
  def test_inherits_complete_maxtext_schema(self):
    self.assertEmpty(set(types.MaxTextConfig.model_fields) - set(types.RLConfig.model_fields))

  def test_training_loop_owns_diffusion_objective_fields(self):
    diffusion_fields = {
        "training_objective",
        "block_diffusion_mask_id",
        "block_diffusion_min_noise",
        "block_diffusion_logit_alignment",
        "block_diffusion_canvas_policy",
    }

    self.assertEmpty(diffusion_fields - set(types.TrainingLoop.model_fields))
    self.assertEmpty(diffusion_fields - set(types.MaxTextConfig.model_fields))
    self.assertEmpty(diffusion_fields - set(types.RLConfig.model_fields))
