...
class UnscanLayersTest(absltest.TestCase):
  ...
  def test_inhomogeneous_cycle_interval_unscans_correctly(self):
    """Inhomogeneous scanned blocks unroll into interleaved per-layer entries."""
    num_layers = 4
    cycle_interval = 2
    repeats = num_layers // cycle_interval  # 2

    # slot 0 array (shape [_IN, 2, _OUT])
    k0 = jnp.arange(_IN * repeats * _OUT, dtype=jnp.float32).reshape(_IN, repeats, _OUT)
    # slot 1 array (shape [_IN, 2, _OUT])
    k1 = (jnp.arange(_IN * repeats * _OUT, dtype=jnp.float32) + 1000).reshape(_IN, repeats, _OUT)
    ...
    expected_keys = ["embed"] + [f"layers_{i}" for i in range(num_layers)]
    self.assertEqual(sorted(unscanned.keys()), sorted(expected_keys))

    # Check mapping: repeat i * cycle_interval + slot
    # layer 0: repeat 0, slot 0
    np.testing.assert_array_equal(np.asarray(_unwrap(unscanned["layers_0"]["kernel"])), np.asarray(k0[:, 0, :]))
    # layer 1: repeat 0, slot 1
    np.testing.assert_array_equal(np.asarray(_unwrap(unscanned["layers_1"]["kernel"])), np.asarray(k1[:, 0, :]))
    # layer 2: repeat 1, slot 0
    np.testing.assert_array_equal(np.asarray(_unwrap(unscanned["layers_2"]["kernel"])), np.asarray(k0[:, 1, :]))
    # layer 3: repeat 1, slot 1
    np.testing.assert_array_equal(np.asarray(_unwrap(unscanned["layers_3"]["kernel"])), np.asarray(k1[:, 1, :]))

  def test_inhomogeneous_indivisible_num_layers_raises(self):
    state = {
        "layers": {
            "layer_0": {"kernel": jnp.zeros((_IN, 2, _OUT))},
            "layer_1": {"kernel": jnp.zeros((_IN, 2, _OUT))},
        }
    }
    with self.assertRaisesRegex(ValueError, r"num_layers \(5\) must be cleanly divisible by cycle_interval \(2\)"):
      raiden_unscan.unscan_layers(state, num_layers=5, cycle_interval=2)

  def test_inhomogeneous_slot_exceeds_cycle_interval_raises(self):
    state = {
        "layers": {
            "layer_2": {"kernel": jnp.zeros((_IN, 2, _OUT))},
        }
    }
    with self.assertRaisesRegex(ValueError, r"extracted slot 2 from 'layer_2' exceeds cycle_interval 2"):
      raiden_unscan.unscan_layers(state, num_layers=4, cycle_interval=2)

  def test_inhomogeneous_missing_prefix_raises(self):
    state = {
        "layers": {
            "kernel": jnp.zeros((_IN, 2, _OUT)),
        }
    }
    with self.assertRaisesRegex(ValueError, r"does not have a 'layer_<slot>' cycle prefix"):
      raiden_unscan.unscan_layers(state, num_layers=4, cycle_interval=2)

  def test_inhomogeneous_zero_cycle_interval_raises(self):
    state = {"layers": {"layer_0": {"kernel": jnp.zeros((_IN, 2, _OUT))}}}
    with self.assertRaisesRegex(ValueError, r"cycle_interval must be >= 1"):
      raiden_unscan.unscan_layers(state, num_layers=4, cycle_interval=0)
