...
class UnscanLayersTest(absltest.TestCase):
  ...
  def test_inhomogeneous_cycle_interval_unscans_correctly(self):
    ...
    # Compare the names Raiden binds, not just the top-level keys: the `layer_<slot>` level
    # has to be gone from *inside* each layer, and the `nnx.Param` rewrap has to survive.
  ...
  def test_inhomogeneous_config_mismatches_raise(self):
    ...
    Each of these is otherwise silent: an out-of-range slot (or a `cycle_interval` the layer
    count does not divide) makes two slots claim the same physical layer, so the later write
    wins in `new_flat` and the overwritten weights never transfer; a missing `layer_<slot>`
    level means config and tree describe different models. No downstream check would notice.
    """
    two_slots = {
        "layers": {
    ...
    cases = [
        (
            "indivisible num_layers",
            two_slots,
            5,
            2,
            r"num_layers \(5\) must be cleanly divisible by cycle_interval \(2\)",
        ),
        (
            "slot at cycle_interval",
            {"layers": {"layer_2": {"kernel": jnp.zeros((_IN, 2, _OUT))}}},
            4,
            2,
            r"slot 2 parsed from key 'layers\.layer_2\.kernel' must be less than cycle_interval 2",
        ),
        (
            "missing layer_<slot> level",
            {"layers": {"kernel": jnp.zeros((_IN, 2, _OUT))}},
            4,
            2,
    ...
    for name, state, num_layers, cycle_interval, expected in cases:
      with self.subTest(name):
        with self.assertRaisesRegex(ValueError, expected):
          raiden_unscan.unscan_layers(state, num_layers=num_layers, cycle_interval=cycle_interval)
