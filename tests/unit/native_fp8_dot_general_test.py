...
def _load_real(key_w, key_s):
  """Loads or deterministically synthesizes weights and block scales."""
  w_path = os.path.join(FIXTURES_DIR, f"real_{key_w}.npy")
  s_path = os.path.join(FIXTURES_DIR, f"real_{key_s}.npy")
  if os.path.exists(w_path) and os.path.exists(s_path):
    w_u8 = np.load(w_path)
    w = w_u8.view(ml_dtypes.float8_e4m3fn)
    s = np.load(s_path).astype(ml_dtypes.bfloat16)
    return w, s
