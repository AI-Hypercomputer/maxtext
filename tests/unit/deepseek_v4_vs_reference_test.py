...
class DeepSeekV4AttentionMaskingTest(unittest.TestCase):
  ...
  def setUp(self):
    self.config = pyconfig.initialize([sys.argv[0], "src/maxtext/configs/base.yml"], run_name="test", skip_jax_distributed_system=True)
...
class DeepSeekV4ConversionMappingTest(unittest.TestCase):
  ...
  def _apply_param_mapping(self, mt_layer, pt_layer, l):
    ...
    HOOK_FNS = DEEPSEEKV4_MAXTEXT_TO_HF_PARAM_HOOK_FN(pt_config_dict, self.mx_config, scan_layers=False, saving_to_hf=False)
