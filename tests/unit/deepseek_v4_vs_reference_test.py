...
class DeepSeekV4ParityTest(unittest.TestCase):
  ...
  def test_unweighted_rms_norm_parity(self):
    ...
    jax_model = jax_norm_module.RMSNorm(num_features=512, epsilon=1e-6, with_scale=False, rngs=None)
  ...
  def test_rms_norm_parity(self):
    ...
    rngs = nnx.Rngs(42)
    jax_model = jax_norm_module.RMSNorm(num_features=512, epsilon=1e-6, rngs=rngs)
    jax_model.scale[...] = jnp.array(weight_np)
    out_jax = jax_model(x_jax)
  ...
  def test_rotary_embedding_parity(self):
    ...
    out_jax = jax_emb_module.apply_rotary_pos_emb(x_jax, cos_jax, sin_jax)
  ...
  def test_grouped_linear_parity(self):
    ...
    rngs = nnx.Rngs(42)
    config = PreTrainedConfig(matmul_precision="default")
    jax_model = jax_linear_module.DeepSeekV4GroupedLinear(
        config=config,
        in_features_per_group=i,
        out_features=o,
        n_groups=g,
