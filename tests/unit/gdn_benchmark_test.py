...
class GdnBenchmarkTest(absltest.TestCase):
  ...
  def test_benchmark_remat_full_and_custom(self):
    """Benchmarks GDN Kernel on TPU v7x/CPU under remat=full vs remat=custom."""
    backend = jax.default_backend()
    batch = 1
    ...
    model = qwen3.Qwen3NextGatedDeltaNet(config=cfg, rngs=nnx.Rngs(0))
    key = jax.random.PRNGKey(42)
    inputs = jax.random.normal(key, (batch, slen, hidden_size), dtype=dtype)

    # 1. Baseline uncheckpointed (remat=none)
    train_none, p_none = create_jitted_train_step(model, inputs.shape, remat=False)
    loss_none, out_none, grads_none = train_none(p_none, inputs)
    jax.block_until_ready(loss_none)

    # 2. Mode remat=full (policy=None, pure recompute)
    train_full, p_full = create_jitted_train_step(model, inputs.shape, remat="full", remat_policy=None)
    loss_full, out_full, grads_full = train_full(p_full, inputs)
    jax.block_until_ready(loss_full)

    # 3. Mode remat=custom (caching GDN residuals on device)
    custom_policy = jax.checkpoint_policies.save_only_these_names("gdn_core_attn_out", "gdn_chunk_states", "gdn_t_inv")
    train_custom, p_custom = create_jitted_train_step(model, inputs.shape, remat="custom", remat_policy=custom_policy)
    loss_custom, out_custom, grads_custom = train_custom(p_custom, inputs)
    jax.block_until_ready(loss_custom)

    # Check numerical parity
    np.testing.assert_allclose(np.asarray(loss_full), np.asarray(loss_none), rtol=1e-3, atol=1e-3)
    np.testing.assert_allclose(np.asarray(loss_custom), np.asarray(loss_none), rtol=1e-3, atol=1e-3)

    # Measure performance (timed steps)
    warmup_steps = 2
    bench_steps = 5
    for _ in range(warmup_steps):
      train_full(p_full, inputs)
      train_custom(p_custom, inputs)
    jax.effects_barrier()

    t0 = time.perf_counter()
    for _ in range(bench_steps):
      res = train_full(p_full, inputs)
      jax.block_until_ready(res)
    full_ms = (time.perf_counter() - t0) * 1000.0 / bench_steps

    t0 = time.perf_counter()
    for _ in range(bench_steps):
      res = train_custom(p_custom, inputs)
      jax.block_until_ready(res)
    custom_ms = (time.perf_counter() - t0) * 1000.0 / bench_steps

    print("\n" + "=" * 90)
    print(f">>> GHOSTFISH BENCHMARK: GDN KERNEL REMAT=FULL vs REMAT=CUSTOM (Backend={backend}, S={slen})")
    print(f"    remat=full   (pure recompute): {full_ms:.2f} ms/step")
    print(f"    remat=custom (residual cache): {custom_ms:.2f} ms/step")
    print(
        f"    delta from caching GDN:       {full_ms - custom_ms:+.2f} ms per step (speedup: {full_ms / custom_ms:.2f}x)"
    )
    print("=" * 90 + "\n")
