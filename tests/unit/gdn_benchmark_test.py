...
# pylint: disable=line-too-long,redefined-builtin,broad-exception-caught,protected-access,use-dict-literal,unused-variable,missing-function-docstring,import-outside-toplevel,chained-comparison,consider-using-enumerate,use-maxsplit-arg
...
FLAGS = flags.FLAGS
flags.DEFINE_bool(
    "enable_custom_call_tracing",
    False,
    "Whether to enable detailed LLO and custom call region tracing in XProf.",
)
flags.DEFINE_bool(
    "enable_xprof_trace",
    False,
    "Whether to capture XProf trace during benchmarks (disabled by default in automated tests to prevent Forge host OOM).",
)
flags.DEFINE_bool(
    "enable_64k_trace",
    False,
    "Whether to capture XProf trace at S=65536 (disabled by default to prevent b/523408873 Forge host OOM).",
)


def configure_custom_call_tracing():
  """Configures detailed LLO and custom call region tracing in XProf."""
  for flag_name in (
      "xla_xprof_register_llo_debug_info",
      "xla_xprof_enable_custom_call_tracing",
  ):
    try:
      FLAGS.set_default(flag_name, "True")
    except Exception:
      pass
    try:
  ...
  # Explicitly set xla_enable_custom_call_region_trace and xla_enable_transpose_trace
  # to False to prevent b/523408873 (134-million bundle interpolation loop in
  # convert_tpu_trace_to_xplane causing Forge OOM).
  for disable_flag in ("xla_enable_custom_call_region_trace", "xla_enable_transpose_trace"):
    try:
      FLAGS.set_default(disable_flag, "False")
    except Exception:
      pass
    try:
      if hasattr(FLAGS, disable_flag):
        setattr(FLAGS, disable_flag, False)
    except Exception:
      pass

  try:
    FLAGS.set_default("tpu_chip_config_name", "megachip")
  except Exception:
    pass
  try:
    FLAGS.set_default("deepsea_chip_config_name", "megachip")
  except Exception:
    pass

  os.environ["TPU_CHIP_CONFIG_NAME"] = "megachip"

  extra_args = [
      "--xla_xprof_register_llo_debug_info=true",
      "--xla_xprof_enable_custom_call_tracing=true",
      "--xla_enable_custom_call_region_trace=false",
      "--xla_enable_transpose_trace=false",
      "--deepsea_chip_config_name=megachip",
  ]
  cur_libtpu = os.environ.get("LIBTPU_INIT_ARGS", "")
  for arg in extra_args:
    arg_key = arg.split("=")[0]
    if arg_key not in cur_libtpu:
      cur_libtpu = f"{cur_libtpu} {arg}".strip()
  os.environ["LIBTPU_INIT_ARGS"] = cur_libtpu


# Early top-level configuration before TPU / JAX initialization
_enable_custom_call = any(
    arg in sys.argv
    for arg in (
        "--enable_custom_call_tracing",
        "--enable_custom_call_tracing=true",
        "--enable_custom_call_tracing=True",
        "--enable_custom_call_tracing=1",
    )
)
if _enable_custom_call:
  configure_custom_call_tracing()


def setUpModule():
  """Module-level setup for absltest."""
  if getattr(FLAGS, "enable_custom_call_tracing", False):
    configure_custom_call_tracing()
...
def create_model_configs(
    ...
    gdn_state_dtype: Any = jnp.float32,
    gdn_decay_dtype: Any = jnp.float32,
  ...
  if weight_dtype is None:
    ...
    weight_dtype = getattr(jnp, weight_dtype)
  if gdn_state_dtype is None:
    gdn_state_dtype = jnp.float32
  elif isinstance(gdn_state_dtype, str):
    gdn_state_dtype = getattr(jnp, gdn_state_dtype)
  if gdn_decay_dtype is None:
    gdn_decay_dtype = jnp.float32
  elif isinstance(gdn_decay_dtype, str):
    gdn_decay_dtype = getattr(jnp, gdn_decay_dtype)
  ...
  base_dict = dict(
      ...
      gdn_state_dtype=gdn_state_dtype,
      gdn_decay_dtype=gdn_decay_dtype,
...
def create_jitted_train_step(
  ...
  is_remat = remat.lower() in ("full", "custom", "true", "yes", "1") if isinstance(remat, str) else bool(remat)
    ...
    with jax.named_scope(step_scope):
      ...
      def loss_fn(m_inner):
        with jax.named_scope(fwd_scope):
          if is_remat:
            out = jax.checkpoint(lambda m, inp: m(inp), policy=remat_policy)(m_inner, x)
          else:
            out = m_inner(x)
...
def print_numerical_correctness_table(
  ...
  """Prints a numerical correctness comparison table with zero false-positive tolerance."""
  print("\n=========================================================================================")
  print(f">>> NUMERICAL CORRECTNESS: {comparison_name}")
  print("=========================================================================================")
  header = (
      f"  {'Tensor / Parameter':<40} | {'Max Abs Diff':<12} | {'Relative Diff':<13} |" f" {'Tolerance':<10} | {'Status'}"
  )
  sep = "  " + "-" * (len(header) - 2)
  print(sep)
  print(header)
  ...
  ref_mag_fwd = float(np.max(np.abs(ref_t)))
  abs_d_fwd = float(np.max(np.abs(ref_t - test_t)))
  rel_d_fwd = abs_d_fwd / (ref_mag_fwd + 1e-7)
  match_fwd = (abs_d_fwd <= 1e-6) if ref_mag_fwd < 1e-7 else (rel_d_fwd <= tolerance)
  ...
  # The loss is a mean over a large number of sign-randomized terms, so its
  # magnitude is dominated by cancellation and can be orders of magnitude
  # smaller than the individual summands. The relative delta then measures the
  # conditioning of the metric rather than the accuracy of the kernel, so the
  # loss row honours the absolute budget this function is already given: it
  # matches if either the absolute or the relative delta is within tolerance.
  # (The per-parameter gradient checks below stay strictly relative.)
  match_loss = (abs_d_loss <= abs_tolerance) or (rel_d_loss <= tolerance)
  rows.append(("Loss Scalar", abs_d_loss, rel_d_loss, match_loss))

  # 3. Parameter Gradients (Pytree Key Matching & Strict Non-Bypass Check)
  ...
  ref_dict = {_path_to_str(p): g for p, g in ref_leaves if hasattr(g, "shape")}
  test_dict = {_path_to_str(p): g for p, g in test_leaves if hasattr(g, "shape")}
  ...
  if missing_in_test or extra_in_test:
    print(f"  ❌ PYTREE MISMATCH: Missing in test: {missing_in_test}, Extra in test: {extra_in_test}")
    return True

  for name in sorted(ref_dict.keys()):
    g_ref_np = np.asarray(ref_dict[name])
    g_test_np = np.asarray(test_dict[name])
  ...
  for name, abs_d, rel_d, is_m in rows:
    ...
    print(f"  {name:<40} | {abs_d:<12.2e} | {rel_d:<13.2e} |" f" {tolerance:<10.2e} | {status}")
  print(sep)
  # The Tolerance column above only shows the relative budget, so state the
  # absolute budget explicitly whenever it is what admitted the loss row.
  # Without this the table reads as a contradiction (rel diff > tolerance, yet
  # MATCH) and an auditor cannot tell which gate fired.
  if match_loss and rel_d_loss > tolerance:
    print(
        f"  ℹ️  Loss Scalar matched on the absolute budget: |Δ| = {abs_d_loss:.2e} <= {abs_tolerance:.2e}."
        f" Its relative delta {rel_d_loss:.2e} exceeds {tolerance:.2e} because |loss| = {abs(lp):.2e}"
        " is dominated by cancellation."
    )
...
def run_memory_profile_analysis(
  ...
  print("\n=========================================================================================")
  print(f">>> HBM MEMORY PROFILING & COMPARATIVE ANALYSIS{title_suffix}")
  print("=========================================================================================")
  ...
  for name, fwd_fn, train_fn, p in zip(kernel_names, fwd_fns, train_fns, params_list):
    ...
    if fwd_analysis is not None:
      ...
      fwd_peak_compiled_mb = (
          fwd_analysis.argument_size_in_bytes + fwd_analysis.temp_size_in_bytes + fwd_analysis.output_size_in_bytes
      ) / (1024**2)
    else:
    ...
    if train_analysis is not None:
      train_peak_compiled_mb = (
          train_analysis.argument_size_in_bytes + train_analysis.temp_size_in_bytes + train_analysis.output_size_in_bytes
      ) / (1024**2)
    ...
    if fwd_analysis is not None and train_analysis is not None:
      ...
      breakdown_rows.append(
          (name, "Backward (Est.)", 0.0, bwd_peak_mb, 0.0, bwd_peak_mb, dev_in_use_train, dev_peak_train)
      )
      breakdown_rows.append(
          (name, "Train Step", 0.0, train_peak_mb, 0.0, train_peak_mb, dev_in_use_train, dev_peak_train)
      )
...
def print_latency_comparison(
  ...
  print("\n=========================================================================================")
  print(f">>> LATENCY & SPEEDUP: COMPARISON {policy_label}({kernel_names[0]} vs {kernel_names[1]})")
  print("=========================================================================================")
  header = f"  {'Pass / Step':<20} | {kernel_names[0]:<28} |" f" {kernel_names[1]:<32} | {'Speedup':<12} | {'Winner'}"
  sep = "  " + "-" * (len(header) - 2)
  print(sep)
  print(header)
  print(sep)
  ...
  for step_name, lats in passes:
    ...
    print(f"  {step_name:<20} | {p_str:>28} |" f" {k_str:>32} | {speedup_str:<12} | {winner}")
  print(sep)
...
def print_tradeoff_table(
  ...
  print("\n=========================================================================================")
  print(f">>> QUANTITATIVE TRADE-OFF {policy_label}: {kernel_name} vs {ref_name}")
  print("=========================================================================================")
  header = f"  {'Metric':<30} | {ref_name:<28} | {kernel_name:<32} |" f" {'Difference / Savings':<22} | {'Advantage'}"
  sep = "  " + "-" * (len(header) - 2)
  print(sep)
  print(header)
  print(sep)
  ...
  for label, val_ref, val_k, unit, lower_is_better in metrics:
    ...
    print(f"  {label:<30} | {val_ref_str:>28} | {val_k_str:>32} |" f" {diff_str:>22} | {advantage}")
  print(sep)
...
def print_cross_policy_summary(results: dict[str, dict[str, Any]]) -> None:
  ...
  print(">>> EXECUTIVE SUMMARY: CROSS-POLICY COMPARISON (remat=none vs remat=full)")
  print(
      "========================================================================================================================="
  )
  ...
  if has_diffs:
    ...
    for name in all_names:
      ...
      print(f"  {name:<40} | {none_str} | {full_str} |" f" {tol_val:<10.2e} | {status}")
    print(num_sep)
...
def print_3mode_memory_table(
  ...
  print(">>> HBM MEMORY PROFILING & COMPILATION ANALYSIS (3-WAY COMPARISON, FP32)")
  print(
      "========================================================================================================================="
  )
  header = (
...
def run_gdn_comparison(
    ...
    gdn_state_dtype_str: str | None = "float32",
    gdn_decay_dtype_str: str | None = "float32",
    hidden_size: int = 4096,
    num_key_heads: int = 16,
    num_value_heads: int = 64,
    head_dim: int = 128,
  ...
  print("Precision: jax_default_matmul_precision = highest (TPU MXU multi-pass" " FP32 simulation)")

  if backend == "cpu":
    gdn_bwd_pallas.ensure_cpu_interpret_registered()

  dtype = jnp.float32 if dtype_str is None else (getattr(jnp, dtype_str) if isinstance(dtype_str, str) else dtype_str)
  weight_dtype = (
      dtype
      if weight_dtype_str is None
      else (getattr(jnp, weight_dtype_str) if isinstance(weight_dtype_str, str) else weight_dtype_str)
  )
  gdn_state_dtype = (
      jnp.float32
      if gdn_state_dtype_str is None
      else (getattr(jnp, gdn_state_dtype_str) if isinstance(gdn_state_dtype_str, str) else gdn_state_dtype_str)
  )
  gdn_decay_dtype = (
      jnp.float32
      if gdn_decay_dtype_str is None
      else (getattr(jnp, gdn_decay_dtype_str) if isinstance(gdn_decay_dtype_str, str) else gdn_decay_dtype_str)
  )
  ...
  print(
      f"Config: Batch={batch}, SeqLen={slen}, Dtype={dtype},"
      f" WeightDtype={weight_dtype}, StateDtype={gdn_state_dtype},"
      f" DecayDtype={gdn_decay_dtype}"
  )
  print(
  ...
  print("\nInitializing models...")
  pure_jax_model = qwen3.Qwen3NextGatedDeltaNet(config=pure_jax_cfg, rngs=nnx.Rngs(0))
  gdn_kernel_model = qwen3.Qwen3NextGatedDeltaNet(config=gdn_kernel_cfg, rngs=nnx.Rngs(0))
  ...
  # =========================================================================
  print("\n=========================================================================================")
  print(f">>> STEP 1: PRE-WARMUP & JIT COMPILATION (STRICTLY OUTSIDE TRACE, {num_warmup} warmups each)")
  print("=========================================================================================")
  ...
  if diverged_remat:
    print("⚠️  Pure JAX (remat=full) differed from Pure JAX (remat=none) beyond tolerance (remat baseline noise).")
  overall_numerical_diverged = diverged_kernel
  ...
  if getattr(FLAGS, "enable_xprof_trace", False):
    try:
    ...
    print(
        f"[{time.strftime('%X')}] ℹ️ XProf tracing skipped (enable with" " --enable_xprof_trace to capture xplane trace)."
    )
    ...
    for xf in xplane_files:
      ...
      if os.path.abspath(xf) != os.path.abspath(target_named_copy):
        try:
          ...
          pass
      target_traced_copy = os.path.join(log_dir, "ghostlite_structured_3modes_custom_call_traced.xplane.pb")
      if os.path.abspath(xf) != os.path.abspath(target_traced_copy):
        try:
          shutil.copy(xf, target_traced_copy)
        except Exception:
          pass
      try:
        ...
        shutil.copy(
            xf,
            "/tmp/xprof_traces/ghostlite_structured_3modes_custom_call_traced.xplane.pb",
        )
      except Exception:
        pass
...
def print_standalone_kernel_memory_table(
    mem_fwd: Any | None,
    mem_train: Any | None,
    seq_len: int,
) -> None:
  """Prints compiled HBM memory usage for standalone GDN kernel."""
  print(
      "\n========================================================================================================================="
  )
  print(f">>> HBM MEMORY PROFILING & COMPILATION ANALYSIS (STANDALONE CANONICAL GDN KERNEL, S={seq_len})")
  print(
      "========================================================================================================================="
  )
  header = (
      f"  {'Step / Graph':<30} | {'Argument (MB)':<14} |"
      f" {'Temp/Scratch (MB)':<18} | {'Output (MB)':<14} | {'Peak HBM (MB)':<14}"
  )
  sep = "  " + "-" * (len(header) - 2)
  print(sep)
  print(header)
  ...
  def get_sizes(mem):
    if mem is not None:
      ...
      tmp_mb = getattr(mem, "temp_size_in_bytes", 0) / (1024**2)
      out_mb = getattr(mem, "output_size_in_bytes", 0) / (1024**2)
      pk_mb = getattr(mem, "peak_memory_in_bytes", 0) / (1024**2)
      if pk_mb == 0:
        pk_mb = arg_mb + tmp_mb + out_mb
      return arg_mb, tmp_mb, out_mb, pk_mb
    return 0.0, 0.0, 0.0, 0.0

  arg_f, tmp_f, out_f, pk_f = get_sizes(mem_fwd)
  arg_t, tmp_t, out_t, pk_t = get_sizes(mem_train)
  print(f"  {'Forward Pass Only':<30} | {arg_f:>11.2f} MB | {tmp_f:>15.2f} MB |" f" {out_f:>11.2f} MB | {pk_f:>11.2f} MB")
  print(
      f"  {'Training Step (Fwd + Bwd)':<30} | {arg_t:>11.2f} MB | {tmp_t:>15.2f} MB |"
      f" {out_t:>11.2f} MB | {pk_t:>11.2f} MB"
  )
  print(sep)


def run_standalone_kernel_profile(
    batch_size: int | None = None,
    seq_len: int | None = None,
    iters: int | None = None,
    warmup: int | None = None,
    dtype_str: str | None = "bfloat16",
    weight_dtype_str: str | None = "bfloat16",
    gdn_state_dtype_str: str | None = "float32",
    gdn_decay_dtype_str: str | None = "float32",
    hidden_size: int = 4096,
    num_key_heads: int = 16,
  ...
  backend = jax.default_backend()
  print(f"\nDevice: {jax.devices()[0]} ({backend})")
  print("Precision: Standalone Canonical GDN Kernel Profile (Hybrid Precision)")

  if backend == "cpu":
    gdn_bwd_pallas.ensure_cpu_interpret_registered()

  dtype = jnp.bfloat16 if dtype_str is None else (getattr(jnp, dtype_str) if isinstance(dtype_str, str) else dtype_str)
  weight_dtype = (
      dtype
      if weight_dtype_str is None
      else (getattr(jnp, weight_dtype_str) if isinstance(weight_dtype_str, str) else weight_dtype_str)
  ...
  if backend == "tpu":
    batch = 1 if batch_size is None else batch_size
    slen = 65536 if seq_len is None else seq_len
  ...
  print(
      f"Model: H={hidden_size}, K_Heads={num_key_heads},"
      f" V_Heads={num_value_heads}, HeadDim={head_dim}, ChunkSize={chunk_size}"
      f" ({slen // chunk_size} chunks)"
  )

  _, gdn_kernel_cfg = create_model_configs(
      hidden_size=hidden_size,
      num_key_heads=num_key_heads,
      num_value_heads=num_value_heads,
      head_dim=head_dim,
  ...
  print("\nInitializing Canonical GDN Kernel model...")
  gdn_kernel_model = qwen3.Qwen3NextGatedDeltaNet(config=gdn_kernel_cfg, rngs=nnx.Rngs(0))

  key = jax.random.PRNGKey(42)
  inputs = jax.random.normal(key, (batch, slen, hidden_size), dtype=dtype)

  print("\n--- Creating Functional JIT Training Steps (Forward & Train Step) ---")
  jit_fwd_kernel, params_fwd = create_jitted_forward(gdn_kernel_model, scope_name="GdnKernel_64k_Fwd")
  jit_train_kernel, params_train = create_jitted_train_step(
      gdn_kernel_model,
      inputs.shape,
      step_scope="GdnKernel_64k_TrainStep",
      fwd_scope="GdnKernel_64k_Fwd",
      bwd_scope="GdnKernel_64k_Bwd",
      remat=False,
  )

  print("\n--- Compiling and Warming Up ---")
  print(f"[{time.strftime('%X')}] Compiling Forward Pass (S={slen})...")
  compiled_fwd = jit_fwd_kernel.lower(params_fwd, inputs).compile()
  mem_fwd = compiled_fwd.memory_analysis() if hasattr(compiled_fwd, "memory_analysis") else None

  print(f"[{time.strftime('%X')}] Compiling Training Step (Forward + Backward," f" S={slen})...")
  compiled_train = jit_train_kernel.lower(params_train, inputs).compile()
  mem_train = compiled_train.memory_analysis() if hasattr(compiled_train, "memory_analysis") else None

  print(f"[{time.strftime('%X')}] Warming up ({num_warmup} steps)...")
  for _ in range(num_warmup):
    res_fwd = compiled_fwd(params_fwd, inputs)
    jax.block_until_ready(res_fwd)
    res_train = compiled_train(params_train, inputs)
    jax.block_until_ready(res_train)

  print(f"[{time.strftime('%X')}] ✅ Warmed up successfully.")

  # Print Memory Profile
  print_standalone_kernel_memory_table(mem_fwd, mem_train, slen)

  # Profiling with trace
  log_dir = os.environ.get("TEST_UNDECLARED_OUTPUTS_DIR", "/tmp/xprof_traces")
  os.makedirs(log_dir, exist_ok=True)
  tracing_active = False
  if getattr(FLAGS, "enable_64k_trace", False):
    try:
      jax.profiler.start_trace(log_dir)
      tracing_active = True
      print(f"[{time.strftime('%X')}] ✅ jax.profiler.start_trace active for 64k profile.")
    except Exception as e:
      print(f"⚠️ Failed to start JAX profiler trace: {e}")
  else:
    print(
        f"[{time.strftime('%X')}] ℹ️ 64k XProf tracing skipped (enable with"
        " --enable_64k_trace to avoid b/523408873 Forge host OOM)."
    )

  fwd_times = []
  train_times = []
  print(f"\n[{time.strftime('%X')}] >>> Executing {num_iters} iterations of" " 64k Canonical GDN Kernel...")
  for step_i in range(num_iters):
    # Fwd
    t_f = time.perf_counter()
    with jax.named_scope("GdnKernel_64k_Fwd_Step"):
      with jax.profiler.StepTraceAnnotation("GdnKernel_64k_Fwd_Step", step_num=step_i):
        res_f = compiled_fwd(params_fwd, inputs)
        jax.block_until_ready(res_f)
    fwd_dur = (time.perf_counter() - t_f) * 1000.0
    fwd_times.append(fwd_dur)

    # Train (Fwd + Bwd)
    t_t = time.perf_counter()
    with jax.named_scope("GdnKernel_64k_Train_Step"):
      with jax.profiler.StepTraceAnnotation("GdnKernel_64k_Train_Step", step_num=step_i):
        res_t = compiled_train(params_train, inputs)
        jax.block_until_ready(res_t)
    train_dur = (time.perf_counter() - t_t) * 1000.0
    train_times.append(train_dur)
    bwd_dur = max(0.0, train_dur - fwd_dur)
    print(
        f"    Step {step_i + 1}/{num_iters}: Fwd = {fwd_dur:.2f} ms, Bwd ="
        f" {bwd_dur:.2f} ms, Total Step = {train_dur:.2f} ms"
    )
  ...
  if tracing_active:
    ...
    gc.collect()
    print(f"\n[{time.strftime('%X')}] Stopping XProf trace for 64k profile...")
    try:
      jax.profiler.stop_trace()
      print(f"[{time.strftime('%X')}] ✅ Trace written to: {log_dir}")
    except Exception as e:
      print(f"⚠️ Failed to stop JAX profiler trace: {e}")

    # Check generated .xplane.pb
    xplane_files = glob.glob(os.path.join(log_dir, "**/*.xplane.pb"), recursive=True)
    print(f"\nDiscovered {len(xplane_files)} generated .xplane.pb file(s) in {log_dir}:")
    for xf in xplane_files:
      sz = os.path.getsize(xf)
      print(f"  📁 {xf} ({sz:,} bytes)")
      target_named_copy = os.path.join(log_dir, "ghostfish_64k_canonical_gdn.xplane.pb")
      if os.path.abspath(xf) != os.path.abspath(target_named_copy):
        try:
          shutil.copy(xf, target_named_copy)
        except Exception:
      ...
      try:
        os.makedirs("/tmp/xprof_traces", exist_ok=True)
        shutil.copy(xf, os.path.join("/tmp/xprof_traces", os.path.basename(xf)))
        shutil.copy(xf, "/tmp/xprof_traces/ghostfish_64k_canonical_gdn.xplane.pb")
      except Exception:
        pass

  # =========================================================================
  # Isolated Core Pallas Backward Kernel Call Profile (Hardware Pallas Call)
  # =========================================================================
  print("\n--- Benchmarking Isolated Core Pallas Backward Kernel (pallas_gdn_bwd_kernel) ---")
  num_chunks = slen // chunk_size
  q_size = num_key_heads * head_dim
  k_size = num_key_heads * head_dim
  v_size = num_value_heads * head_dim
  qkv_dim = q_size + k_size + v_size

  dummy_qkv_conv = jax.random.normal(key, (batch, slen, qkv_dim), dtype=dtype)
  dummy_b = jax.random.normal(key, (batch, slen, num_value_heads), dtype=dtype)
  dummy_a = jax.random.normal(key, (batch, slen, num_value_heads), dtype=dtype)
  dummy_a_log = jax.random.normal(key, (num_value_heads,), dtype=gdn_decay_dtype)
  dummy_dt_bias = jax.random.normal(key, (num_value_heads,), dtype=gdn_decay_dtype)
  dummy_do = jax.random.normal(key, (batch, slen, num_value_heads, head_dim), dtype=dtype)
  dummy_chunk_states = jax.random.normal(
      key, (batch, num_chunks, num_value_heads, head_dim, head_dim), dtype=gdn_state_dtype
  )
  dummy_t_inv = jax.random.normal(
      key, (batch, num_chunks, num_value_heads, chunk_size, chunk_size), dtype=gdn_state_dtype
  )

  @jax.jit
  def jit_isolated_pallas_bwd(qkv_c, b_in, a_in, al, dt, do_in, cs, ti):
    return gdn_bwd_pallas.pallas_gdn_bwd_kernel(
        ...
        kq_head_dim=head_dim,
        v_head_dim=head_dim,
        chunk_size=chunk_size,
        interpret=(backend == "cpu"),
    )

  print(f"[{time.strftime('%X')}] Compiling Isolated Pallas Backward Kernel (S={slen})...")
  compiled_pallas_bwd = jit_isolated_pallas_bwd.lower(
      dummy_qkv_conv,
      dummy_b,
      dummy_a,
      dummy_a_log,
  ...
  print(f"[{time.strftime('%X')}] Warming up Isolated Pallas Backward Kernel ({num_warmup} steps)...")
  for _ in range(num_warmup):
    res_pb = compiled_pallas_bwd(
        dummy_qkv_conv,
        dummy_b,
        dummy_a,
        dummy_a_log,
    ...
    jax.block_until_ready(res_pb)

  pallas_bwd_times = []
  print(f"[{time.strftime('%X')}] >>> Executing {num_iters} iterations of Isolated Pallas Backward Kernel...")
  for step_i in range(num_iters):
    t_pb = time.perf_counter()
    with jax.named_scope("Isolated_Pallas_GDN_Bwd"):
      with jax.profiler.StepTraceAnnotation("Isolated_Pallas_GDN_Bwd", step_num=step_i):
        res_pb = compiled_pallas_bwd(
            dummy_qkv_conv,
            dummy_b,
            dummy_a,
        ...
        jax.block_until_ready(res_pb)
    pb_dur = (time.perf_counter() - t_pb) * 1000.0
    pallas_bwd_times.append(pb_dur)
    print(f"    [Pallas Backward Call] Step {step_i + 1}/{num_iters}: {pb_dur:.2f} ms")

  avg_pallas_bwd = float(np.mean(pallas_bwd_times))

  # =========================================================================
  # Isolated Core Pallas Forward Kernel Call Profile (TPU Only)
  # =========================================================================
  avg_pallas_fwd = None
  if backend == "tpu":
    print("\n--- Benchmarking Isolated Core Pallas Forward Kernel (_run_local_gdn_decoupled_fwd) ---")
    dummy_conv_weight = jax.random.normal(key, (conv_kernel_dim, 1, qkv_dim), dtype=dtype)
    dummy_conv_state = jnp.zeros((batch, conv_kernel_dim - 1, qkv_dim), dtype=dtype)
    dummy_recurrent_state = jnp.zeros((batch, num_value_heads, head_dim, head_dim), dtype=gdn_state_dtype)

    @jax.jit
    def jit_isolated_pallas_fwd(qkv_in, b_in, a_in, cw, al, dt, cs_in, rs_in):
      return gdn_bwd_pallas._run_local_gdn_decoupled_fwd(
          qkv=qkv_in,
          b=b_in,
          a=a_in,
          conv_weight=cw,
    ...
    compiled_pallas_fwd = jit_isolated_pallas_fwd.lower(
        dummy_qkv_conv,
        dummy_b,
        dummy_a,
        dummy_conv_weight,
        ...
        dummy_dt_bias,
        dummy_conv_state,
        dummy_recurrent_state,
    ).compile()

    print(f"[{time.strftime('%X')}] Warming up Isolated Pallas Forward Kernel ({num_warmup} steps)...")
    for _ in range(num_warmup):
      res_pf = compiled_pallas_fwd(
          dummy_qkv_conv,
          dummy_b,
          dummy_a,
          dummy_conv_weight,
      ...
      jax.block_until_ready(res_pf)

    pallas_fwd_times = []
    print(f"[{time.strftime('%X')}] >>> Executing {num_iters} iterations of Isolated Pallas Forward Kernel...")
    for step_i in range(num_iters):
      t_pf = time.perf_counter()
      with jax.named_scope("Isolated_Pallas_GDN_Fwd"):
        with jax.profiler.StepTraceAnnotation("Isolated_Pallas_GDN_Fwd", step_num=step_i):
          res_pf = compiled_pallas_fwd(
              dummy_qkv_conv,
              dummy_b,
              dummy_a,
          ...
          jax.block_until_ready(res_pf)
      pf_dur = (time.perf_counter() - t_pf) * 1000.0
      pallas_fwd_times.append(pf_dur)
      print(f"    [Pallas Forward Call] Step {step_i + 1}/{num_iters}: {pf_dur:.2f} ms")

    avg_pallas_fwd = float(np.mean(pallas_fwd_times))

  # Print Summary Table
  avg_fwd = float(np.mean(fwd_times))
  avg_train = float(np.mean(train_times))
  avg_bwd = max(0.0, avg_train - avg_fwd)
  print("\n" + "=" * 85)
  print(f">>> 64K GDN KERNEL & LAYER PERFORMANCE BREAKDOWN (S={slen}, Chunks={slen // chunk_size})")
  print("=" * 85)
  print("  Isolated Hardware Pallas Kernel Calls:")
  if avg_pallas_fwd is not None:
    print(f"    Core Pallas Forward Kernel:   {avg_pallas_fwd:.2f} ms")
  print(
      f"    Core Pallas Backward Kernel:  {avg_pallas_bwd:.2f} ms  (Ref Prod Pretraining XProf: 31.93 ms across 4 head groups)"
  )
  print("")
  print("  Full Monolithic Decoder Layer on 1 Chip (Projections + Gating + Conv1D + Kernels):")
  print(f"    Forward Pass Latency:         {avg_fwd:.2f} ms")
  print(f"    Backward Pass Latency:        {avg_bwd:.2f} ms  (Includes ~150 ms linear GEMM backprop on 1 chip)")
  print(f"    Total Step Latency:           {avg_train:.2f} ms")
  print("=" * 85 + "\n")


class GdnBenchmarkTest(absltest.TestCase):
  ...
  def test_structured_profile_3modes(self):
    """Structured 3-mode profile benchmark on Cloud TPU."""
    if getattr(self.__class__, "_has_run_8k_fp32", False):
      print("\nSkipping duplicate test_structured_profile_3modes (already executed in test_benchmark_397b_8k_fp32).")
      return
    self.test_benchmark_397b_8k_fp32()

  def test_benchmark_8k_fp32(self):
    """Backwards compatibility alias."""
    if getattr(self.__class__, "_has_run_8k_fp32", False):
      print("\nSkipping duplicate test_benchmark_8k_fp32 (already executed in test_benchmark_397b_8k_fp32).")
      return
    self.test_benchmark_397b_8k_fp32()

  @staticmethod
  def _count_gdn_pallas_calls(hlo_text: str) -> dict[str, Any]:
    """Counts GDN Pallas ``tpu_custom_call`` instructions in compiled HLO text, split by kernel name."""
    counts = {"fwd_fused_conv1d_gdn": 0, "bwd_gdn_bwd_kernel": 0, "other_tpu_custom_call": 0, "lines": []}
    for line in hlo_text.splitlines():
      if 'custom_call_target="tpu_custom_call"' not in line:
        continue
      if "fused_conv1d_gdn" in line:
        counts["fwd_fused_conv1d_gdn"] += 1
      elif "gdn_bwd_kernel" in line:
        counts["bwd_gdn_bwd_kernel"] += 1
      else:
        counts["other_tpu_custom_call"] += 1
      # Keep a short evidence snippet (instruction name + kernel_name attribute if present).
      head = line.strip()[:120]
      kn = ""
      idx = line.find("kernel_name=")
      if idx >= 0:
        kn = line[idx : idx + 80]
      counts["lines"].append(f"{head} ... {kn}")
    counts["total_tpu_custom_call"] = (
        counts["fwd_fused_conv1d_gdn"] + counts["bwd_gdn_bwd_kernel"] + counts["other_tpu_custom_call"]
    )
    return counts

  def _run_gdn_remat_policy_sweep(self, dtype, tol: float, abs_tol: float) -> dict[str, dict[str, Any]]:
    """Sweeps GDN remat policies on one chip and compares each against a weight-synced pure-JAX reference.

    Modes (all use the Pallas GDN kernel unless stated):
      * ``remat=none``           : no jax.checkpoint (upper bound on memory, lower bound on time).
      * ``gdn=remat``            : jax.checkpoint(policy=None) -> the fwd Pallas kernel is replayed in bwd.
      * ``gdn_states=device``    : save_only_these_names(_expand_gdn_remat_names(["gdn_states"])).
      * ``gdn=device``           : save_only_these_names(_expand_gdn_remat_names(["gdn"])) (all 9 residuals).
    For every mode we check fwd/loss/grads against pure JAX, count the fwd/bwd GDN Pallas custom-calls in
    the compiled HLO, and record compiled HBM + step latency.
    """
    backend = jax.default_backend()
    batch = 1
    slen = 8192 if backend == "tpu" else 128
    hidden_size = 4096 if backend == "tpu" else 512
    num_key_heads = 16 if backend == "tpu" else 2
    num_value_heads = 64 if backend == "tpu" else 4
    head_dim = 128 if backend == "tpu" else 64
    chunk_size = 64 if backend == "tpu" else 32
    warmup_steps = 2 if backend == "tpu" else 1
    bench_steps = 5 if backend == "tpu" else 1

    pure_cfg, kernel_cfg = create_model_configs(
        hidden_size=hidden_size,
        num_key_heads=num_key_heads,
        num_value_heads=num_value_heads,
        head_dim=head_dim,
    ...
    kernel_model = qwen3.Qwen3NextGatedDeltaNet(config=kernel_cfg, rngs=nnx.Rngs(0))
    pure_model = qwen3.Qwen3NextGatedDeltaNet(config=pure_cfg, rngs=nnx.Rngs(0))
    _, params_state = nnx.split(kernel_model)
    nnx.update(pure_model, params_state)
    key = jax.random.PRNGKey(42)
    inputs = jax.random.normal(key, (batch, slen, hidden_size), dtype=dtype)

    print("\n" + "=" * 100)
    print(
        f">>> GDN REMAT POLICY SWEEP vs PURE JAX (backend={backend}, dtype={jnp.dtype(dtype).name}, "
        f"S={slen}, H={hidden_size}, Kh={num_key_heads}, Vh={num_value_heads}, tol={tol:.1e}/{abs_tol:.1e})"
    )
    print("=" * 100)

    # Reference: pure JAX, no remat.
    train_ref, p_ref = create_jitted_train_step(pure_model, inputs.shape, remat=False)
    compiled_ref = train_ref.lower(p_ref, inputs).compile()
    loss_ref, out_ref, grads_ref = compiled_ref(p_ref, inputs)
    jax.block_until_ready(grads_ref)

    gdn_states_names = maxtext_utils._expand_gdn_remat_names(["gdn_states"])
    gdn_all_names = maxtext_utils._expand_gdn_remat_names(["gdn"])
    print(f"gdn_states -> {gdn_states_names}")
    print(f"gdn        -> {gdn_all_names}")
    modes = [
        ("remat=none", False, None, (1, 1)),
        ("gdn=remat (policy=None)", "full", None, (2, 1)),
        ("gdn_states=device", "custom", jax.checkpoint_policies.save_only_these_names(*gdn_states_names), (1, 1)),
        ("gdn=device", "custom", jax.checkpoint_policies.save_only_these_names(*gdn_all_names), (1, 1)),
    ]

    results: dict[str, dict[str, Any]] = {}
    for label, remat, policy, expected_calls in modes:
      train_fn, p = create_jitted_train_step(kernel_model, inputs.shape, remat=remat, remat_policy=policy)
      compiled = train_fn.lower(p, inputs).compile()
      counts = self._count_gdn_pallas_calls(compiled.as_text())
      mem = compiled.memory_analysis() if hasattr(compiled, "memory_analysis") else None
      loss, out, grads = compiled(p, inputs)
      ...
      diverged = print_numerical_correctness_table(
          ...
          tolerance=tol,
          abs_tolerance=abs_tol,
          comparison_name=f"GDN kernel [{label}] vs Pure JAX (remat=none)",
          diff_records=diffs,
      )
      for _ in range(warmup_steps):
        jax.block_until_ready(compiled(p, inputs))
      t0 = time.perf_counter()
      for _ in range(bench_steps):
        jax.block_until_ready(compiled(p, inputs))
      ms = (time.perf_counter() - t0) * 1000.0 / bench_steps
      grad_rel = [r["rel_diff"] for k, r in diffs.items() if k not in ("Forward Output", "Loss Scalar")]
      results[label] = dict(
          counts=counts,
          expected_calls=expected_calls,
          arg_mb=(mem.argument_size_in_bytes / 2**20) if mem is not None else float("nan"),
          temp_mb=(mem.temp_size_in_bytes / 2**20) if mem is not None else float("nan"),
          out_mb=(mem.output_size_in_bytes / 2**20) if mem is not None else float("nan"),
          ms=ms,
          diverged=diverged,
          max_grad_rel=max(grad_rel) if grad_rel else float("nan"),
          fwd_rel=diffs.get("Forward Output", {}).get("rel_diff", float("nan")),
      )
      print(f"  [{label}] tpu_custom_call instructions in compiled HLO:")
      for ln in counts["lines"]:
        print(f"      {ln}")

    header = (
        f"  {'Mode':<26} | {'fwd pallas':<10} | {'bwd pallas':<10} | {'ms/step':<9} | "
        f"{'temp MB':<10} | {'arg MB':<9} | {'max grad rel':<13} | {'fwd rel':<9} | status"
    )
    print("\n" + "=" * len(header))
    print(
        f">>> SUMMARY: GDN remat policies ({jnp.dtype(dtype).name}, S={slen}) -- numerics vs pure JAX, HLO kernel counts"
    )
    print("=" * len(header))
    print(header)
    print("  " + "-" * (len(header) - 2))
    base_ms = results["gdn=remat (policy=None)"]["ms"]
    for label, r in results.items():
      c = r["counts"]
      status = "❌ DIVERGED" if r["diverged"] else "✅ MATCH"
      print(
          f"  {label:<26} | {c['fwd_fused_conv1d_gdn']:<10} | {c['bwd_gdn_bwd_kernel']:<10} | "
          f"{r['ms']:<9.2f} | {r['temp_mb']:<10.1f} | {r['arg_mb']:<9.1f} | {r['max_grad_rel']:<13.2e} | "
          f"{r['fwd_rel']:<9.2e} | {status}  (x{base_ms / r['ms']:.2f} vs gdn=remat)"
      )
    print("  " + "-" * (len(header) - 2))

    # Assertions: numerics for every mode; kernel replay counts on TPU only (CPU runs the pure-JAX fallback).
    for label, r in results.items():
      self.assertFalse(r["diverged"], f"[{label}] GDN kernel diverged from pure JAX beyond tolerance")
    if backend == "tpu":
      for label, r in results.items():
        c = r["counts"]
        exp_fwd, exp_bwd = r["expected_calls"]
        if c["fwd_fused_conv1d_gdn"] + c["bwd_gdn_bwd_kernel"] > 0:
          self.assertEqual(
              (c["fwd_fused_conv1d_gdn"], c["bwd_gdn_bwd_kernel"]),
              (exp_fwd, exp_bwd),
              f"[{label}] unexpected (fwd, bwd) GDN Pallas call count: {c}",
          )
        else:
          # kernel_name attribute not on the same HLO line; fall back to the total tpu_custom_call count.
          self.assertEqual(
              c["total_tpu_custom_call"], exp_fwd + exp_bwd, f"[{label}] unexpected total tpu_custom_call count: {c}"
          )
    return results

  def test_benchmark_remat_full_and_custom(self):
    """GDN kernel remat policies (gdn=remat / gdn_states=device / gdn=device) vs pure JAX on one chip.

    Verifies (1) grads match pure JAX for every policy, (2) `gdn_states=device` removes the forward Pallas
    kernel replay from the backward pass (1 fwd + 1 bwd custom-call in HLO, vs 2 fwd + 1 bwd for gdn=remat),
    and (3) reports the compiled HBM / latency trade-off.
    """
    backend = jax.default_backend()
    if backend == "tpu":
      self._run_gdn_remat_policy_sweep(jnp.float32, tol=1e-4, abs_tol=1e-5)
      self._run_gdn_remat_policy_sweep(jnp.bfloat16, tol=2.5e-2, abs_tol=3e-2)
    else:
      self._run_gdn_remat_policy_sweep(jnp.float32, tol=1e-3, abs_tol=1e-5)
  ...
  def test_benchmark_397b_8k_fp32(self):
    ...
    self.__class__._has_run_8k_fp32 = True
    backend = jax.default_backend()
    if backend == "tpu":
      print("\n=========================================================================================")
      print(">>> BENCHMARK: Dedicated 8k FP32 Comparison (Pure JAX vs Canonical GDN Kernel - Full Qwen3.5-397B Config)")
      print("=========================================================================================")
      bench_iters = 1
      gap_sec = 0.05
    ...
    self.assertFalse(diverged, "GDN Kernel gradients diverged beyond tolerance in FP32!")

  def test_benchmark_397b_8k_hybrid_precision(self):
    """Tier 1: Dedicated 8k comparison of Pure JAX vs Canonical GDN Kernel in Hybrid Precision (BF16 activations/weights, FP32 recurrent states/decay)."""
    backend = jax.default_backend()
    if backend == "tpu":
      print("\n=========================================================================================")
      print(
          ">>> TIER 1 BENCHMARK: Dedicated 8k Hybrid Precision Comparison (Pure"
          " JAX vs Canonical GDN Kernel - Full Qwen3.5-397B Config)"
      )
      print("=========================================================================================")
      bench_iters = 1
      gap_sec = 0.05
      diverged = run_gdn_comparison(
          ...
          gdn_state_dtype_str="float32",
          gdn_decay_dtype_str="float32",
          hidden_size=4096,
          num_key_heads=16,
          num_value_heads=64,
          head_dim=128,
      ...
      print("\n=========================================================================================")
      print(">>> CPU HERMETIC VERIFICATION: Tier 1 Hybrid Precision Comparison" " (S=128, B=1)")
      print("=========================================================================================")
      diverged = run_gdn_comparison(
          batch_size=1,
          seq_len=128,
    ...
    self.assertFalse(
        diverged,
        "GDN Kernel gradients diverged beyond tolerance in Hybrid Precision!",
    )

  def test_benchmark_397b_64k_kernel_profile(self):
    """Tier 2: Standalone Canonical GDN Kernel Profiling at S=65536 in Hybrid Precision on Cloud TPU v7x."""
    backend = jax.default_backend()
    if backend == "tpu":
      print("\n=========================================================================================")
      print(">>> TIER 2 BENCHMARK: 64k Standalone Kernel Profile (Canonical GDN" " Kernel - 1024 Chunks)")
      print("=========================================================================================")
      bench_iters = 1
      run_standalone_kernel_profile(
          batch_size=1,
          seq_len=65536,
          iters=bench_iters,
          warmup=1,
      ...
      print("\n=========================================================================================")
      print(">>> CPU HERMETIC VERIFICATION: Tier 2 Standalone Kernel Profile" " (S=128, B=1)")
      print("=========================================================================================")
      run_standalone_kernel_profile(
          batch_size=1,
          seq_len=128,
