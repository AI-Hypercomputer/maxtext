...
def _std_iter():
  return grain.MapDataset.range(10).to_iter_dataset().__iter__()
...
class TestGrainCheckpointable(unittest.TestCase):
  ...
  def test_standard_delegates_to_grain_native(self):
    with tempfile.TemporaryDirectory() as d:
      ...
      ocp_v1.save_checkpointables(str(path), {"iter": GrainCheckpointable(it)})
      # grain-native writes its own per-process file name.
      self.assertTrue((path / "iter" / "process_0-of-1.json").exists())
      restored = _std_iter()
      ocp_v1.load_checkpointables(str(path), {"iter": GrainCheckpointable(restored)})
  ...
  def test_scaled_list_explicit_index_count(self):
    with tempfile.TemporaryDirectory() as d:
      ...
      ocp_v1.save_checkpointables(str(path), {"iter": GrainCheckpointable([(a, 0, 2), (b, 1, 2)])})
      ...
      ocp_v1.load_checkpointables(
          str(path), {"iter": GrainCheckpointable([ra, rb], process_index=[0, 1], process_count=2)}
      )
  ...
  def test_scale_up_single_with_explicit_count(self):
    with tempfile.TemporaryDirectory() as d:
      ...
      for _ in range(7):
        next(it)
      expected = it.get_state()
      ocp_v1.save_checkpointables(str(path), {"iter": GrainCheckpointable([(it, 0, 1)])})
      restored = _std_iter()
      ocp_v1.load_checkpointables(
          str(path), {"iter": GrainCheckpointable(restored, process_index=0, process_count=1)}
      )
      self.assertEqual(restored.get_state(), expected)
...
class TestGrainUtilityFactories(unittest.TestCase):
  ...
  def test_for_save_remote(self):
    wrapper = _FakeRemote()
    c = grain_utility.for_save(None, 7, wrapper, 1)
  ...
  def test_for_save_elastic(self):
    ...
    mock_iter = unittest.mock.MagicMock()
    mock_iter.local_iterator = it
    c = grain_utility.for_save(None, 0, mock_iter, 1)
    self.assertIs(c._item, it)

  def test_for_save_standard(self):
    it = _std_iter()
    c = grain_utility.for_save(None, 0, it, 1)
    self.assertIs(c._item, it)

  def test_for_save_scaled_list(self):
    a, b = _std_iter(), _std_iter()
    c = grain_utility.for_save(None, 0, [a, b], 1)
