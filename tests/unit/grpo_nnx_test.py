...
class _MockLinenTransformer(nn.Module):
  """Tiny Linen module with the `model.apply(...)` signature that Linen `compute_log_probs` expects."""
...
class TestLinenGrpoRegression(unittest.TestCase):
  """Smoke tests that the Linen `grpo_loss_fn` and `compute_log_probs` still run on Linen-shaped inputs."""

  def setUp(self):
    self.config = _make_grpo_config()
    self.config.pure_nnx = False  # Force the Linen dispatch branch.
  ...
  def test_linen_grpo_loss_fn_still_runs(self):
    ...
    loss, aux = grpo_trainer.grpo_loss_fn(
        ...
        jax.random.key(1),
        self.params,
        self.reference_params["params"],  # On Linen, reference_params is the inner subtree.
        is_train=True,
    )
    self.assertTrue(jnp.isfinite(loss))
