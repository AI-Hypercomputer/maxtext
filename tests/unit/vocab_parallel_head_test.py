...
Getting that mesh takes a subprocess. The CPU backend reads
`--xla_force_host_platform_device_count` only when the backend initializes, which
pytest has already done by the time this file is imported, so the `setdefault`
below cannot take effect in the parent -- and under CI it does not even try, because
CI *assigns* `XLA_FLAGS=--xla_force_host_platform_device_count=1` and `setdefault`
leaves an assigned value alone. Everything here would then skip green.
`test_vocab_parallel_head_on_a_four_device_cpu_mesh` re-execs the module with the
flag appended, which wins because XLA takes the last occurrence of a repeated flag,
and it fails unless the child reports tests that actually ran.
"""
...
_DRIVER = "test_vocab_parallel_head_on_a_four_device_cpu_mesh"
_PASSED = re.compile(r"(\d+) passed")


def test_vocab_parallel_head_on_a_four_device_cpu_mesh():
  """Run every test below again in a child that actually has the four devices.

  See the module docstring for why re-exec is the only option. `-k` drops this driver
  from the child rather than `--deselect`, which would need a node id and so a path
  spelled relative to whatever pytest picked as the rootdir.
  """
  env = os.environ.copy()
  env["XLA_FLAGS"] = f"{env.get('XLA_FLAGS', '')} --xla_force_host_platform_device_count={_FSDP}".strip()
  env["JAX_PLATFORMS"] = "cpu"
  # The child imports `tests.utils.test_helpers`, which pytest puts on the path and a
  ...
  result = subprocess.run(
      [sys.executable, "-m", "pytest", __file__, "-q", "-k", f"not {_DRIVER}"],
      env=env,
      capture_output=True,
      text=True,
      check=False,
  ...
  passed = _PASSED.search(result.stdout)
  # Exit status 0 is also what a child that skipped everything produces, and a silently
  # empty run is the exact failure this driver exists to prevent.
  assert passed, f"the child did not report a completed run\n{report}"
  assert int(passed.group(1)) > 0, f"every test in the child skipped\n{report}"
  assert " skipped" not in result.stdout, f"the child still skipped tests\n{report}"
...
def _skip_unless_multi_device():
  if jax.device_count() < _FSDP:
    pytest.skip(f"needs {_FSDP} devices; {_DRIVER} re-execs this module with them")
