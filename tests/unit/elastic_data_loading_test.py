...
class ElasticDataLoadingTest(absltest.TestCase):
  ...
  def setUp(self):
    ...
    # 1. Global JAX patches
    self.mock_jax_devices = mock.patch.object(jax, "devices").start()
    self.mock_jax_process_count = mock.patch.object(jax, "process_count").start()
    self.mock_jax_process_index = mock.patch.object(jax, "process_index").start()
    self.mock_jax_local_devices = mock.patch.object(jax, "local_devices").start()
    self.mock_named_sharding = mock.patch.object(jax.sharding, "NamedSharding").start()
    self.mock_jax_device_put = mock.patch.object(jax, "device_put").start()
    self.mock_jax_device_put.side_effect = lambda x, *args, **kwargs: x

    # Default JAX values
    self.mock_jax_process_count.return_value = 4
    self.mock_jax_process_index.return_value = 0

    # 2. Other common patches
    self.mock_remote_iterator = mock.patch.object(multihost_dataloading, "RemoteIterator", FakeRemoteIterator).start()
    self.mock_colocated_class = mock.patch.object(
        multihost_dataloading.colocated_python,
        "colocated_python_class",
        new=lambda x: x,
    ).start()
    self.mock_get_pipeline_fn = mock.patch.object(grain_data_processing, "_get_pipeline_fn").start()
    self.mock_elastic_iterator = mock.patch.object(grain_data_processing, "ElasticIterator").start()
    self.mock_get_datasets = mock.patch.object(grain_data_processing, "get_datasets").start()
    self.mock_pathwaysutils = mock.patch.object(elastic_utils, "pathwaysutils").start()
    self.mock_colocated_cpu_devices = mock.patch.object(multihost_dataloading, "_colocated_cpu_devices").start()
    self.mock_colocated_cpu_mesh = mock.patch.object(multihost_dataloading, "_colocated_cpu_mesh").start()
  ...
  def test_elastic_data_loading_active_processes(self):
    ...
    # Configure global JAX mocks
    self.mock_jax_devices.return_value = devices
    self.mock_jax_local_devices.return_value = [devices[0]]

    # Mock pathwaysutils
    self.mock_pathwaysutils.is_pathways_backend_used.return_value = True
    ...
    self.mock_get_datasets.return_value = (mock.MagicMock(), mock.MagicMock())

    # Mock _get_pipeline_fn to return a dummy function
    self.mock_get_pipeline_fn.return_value = lambda dataset, *args, **kwargs: dataset
    ...
    self.mock_elastic_iterator.return_value = mock_dataloader

    # Mock NamedSharding.devices_indices_map
    mock_sharding_instance = self.mock_named_sharding.return_value
    ...
    # Initialize config (JAX is already mocked globally)
    ...
    self.mock_colocated_cpu_devices.return_value = [cpu_devices[0]]
    self.mock_colocated_cpu_mesh.return_value = cpu_mesh
    ...
    # shard_index=0
    self.mock_elastic_iterator.assert_called_once()
    _, called_kwargs = self.mock_elastic_iterator.call_args
  ...
  def test_elastic_data_loading_with_expansion_factor(self):
    # Setup 2 slices, 2 processes per slice, 1 device per process.
    devices = [
        FakeDevice(device_id=0, slice_index=0, process_index=0, task_id=0),
        FakeDevice(device_id=1, slice_index=0, process_index=1, task_id=1),
