...
class TestExpandReduce(unittest.TestCase):
  ...
  def test_expand_shape(self):
    """Verifies (B, L, D) -> (S, B, L, D)"""
    out = self.expand(self.x)
    expected_shape = (self.rate, self.batch, self.seq_len, self.dim)
    self.assertEqual(out.shape, expected_shape)

  def test_reduce_shape(self):
    """Verifies (S, B, L, D) -> (B, L, D)"""
    dummy_expanded = jnp.ones((self.rate, self.batch, self.seq_len, self.dim))
...
class TestMHC(unittest.TestCase):
  ...
  def test_moe_layer_output_shape(self):
    with nn_partitioning.axis_rules(self.config.logical_axis_rules):
      module = mhc.ManifoldConstrainedHyperConnections(self.config, "train", self.dim, self.mesh, self.rngs)
      layer = moe.RoutedMoE(
          config=self.config,
