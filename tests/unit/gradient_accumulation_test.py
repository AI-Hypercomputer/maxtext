...
from maxtext.utils import gradient_accumulation as ga_module
...
class TestNNXGradientAccumulation(unittest.TestCase):
  ...
  def test_rng_advancement_logic(self):
    ...
    def mock_loss_fn(m, config, data, dr_rng, params, is_train=True):
      ...
      return loss, {"total_loss": loss, "total_weights": 1.0, "moe_lb_loss": 0.0, "indexer_loss": 0.0, "mtp_loss": 0.0}
  ...
  def test_shard_optimizer_over_data(self):
    """Covers lines 87-92: shard_optimizer_over_data=True converts params to bf16."""
    config = self.MockConfig(shard_optimizer_over_data=True)
    rngs = nnx.Rngs(dropout=jax.random.key(10), params=jax.random.key(0))
    model = self.DropoutModel(rngs)

    # shard_optimizer_over_data requires an optimizer in the state for sharding extraction.
    tx = optax.adam(1e-3)
    optimizer = nnx.Optimizer(model, tx, wrt=nnx.Param)
    _, abstract_state = nnx.get_abstract_model(lambda: train_state_nnx.TrainStateNNX(model, optimizer), self.mesh)
    named_sharding = maxtext_utils_nnx.get_named_sharding_nnx(abstract_state)
    state_mesh_shardings = maxtext_utils_nnx.get_partition_spec_nnx(named_sharding)
    params_shardings, _ = sharding.maybe_update_params_sharding_with_opt(config, state_mesh_shardings)

    def loss_fn(m, config, data, dr_rng, params, is_train=True):
      logits = m(data["inputs"], is_train=False)
      loss = jnp.mean(logits**2)
      return loss, {"total_loss": loss, "total_weights": 1.0, "moe_lb_loss": 0.0, "indexer_loss": 0.0, "mtp_loss": 0.0}

    data = {"inputs": jnp.ones((2, 2))}
    with self.mesh:
      loss, _, _ = gradient_accumulation_loss_and_grad(loss_fn, config, model, None, params_shardings, data, None, [])
    self.assertIsInstance(float(loss), float)


class TestGradientAccumulationHelpers(unittest.TestCase):
  """Tests the sharding helper functions directly."""

  def test_update_sharding_for_reduced(self):
    """Covers line 170: update_sharding_for_reduced."""
    mock_sharding = MagicMock()
    ga_module.update_sharding_for_reduced(mock_sharding)
    mock_sharding.spec.update.assert_called_once_with(reduced={"data"})
    mock_sharding.update.assert_called_once()

  def test_update_sharding_for_unreduced(self):
    """Covers line 177: update_sharding_for_unreduced."""
    mock_sharding = MagicMock()
    ga_module.update_sharding_for_unreduced(mock_sharding)
    mock_sharding.spec.update.assert_called_once_with(unreduced={"data"})
    mock_sharding.update.assert_called_once()
