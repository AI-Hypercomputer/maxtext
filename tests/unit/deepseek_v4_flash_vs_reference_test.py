...
def hc_split_sinkhorn_cpu(
  ...
  comb = mixes[..., 2 * hc_mult :].view(*mixes.shape[:-1], hc_mult, hc_mult) * hc_scale[2] + hc_base[2 * hc_mult :].view(
      hc_mult, hc_mult
  )
...
class Indexer_PT(nn_pt.Module):
  ...
  def forward(self, x: torch.Tensor, q_latent: torch.Tensor, start_pos: int = 0) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    causal_mask = torch.arange(n_win, device=x.device).unsqueeze(0) >= (
        torch.arange(1, m + 1, device=x.device).unsqueeze(1) // 4
    )
  ...
  def forward(self, x: torch.Tensor, start_pos: int = 0) -> torch.Tensor:
    ...
    if self.rate == 0:
      ...
      if n_win > 0:
        ...
        scores[:, :, :, m:] = torch.where(
            (~causal_comp_mask).unsqueeze(0).unsqueeze(2), scores[:, :, :, m:], float("-inf")
        )
...
def _apply_layer_param_mapping(mt_layer, pt_model, layer_idx: int, pt_config_dict: dict, mx_config: Config):
  ...
  for mt_key, hf_key in mapping.items():
    ...
    if hf_key is None:
      ...
      print(
          f"HOOK CALL: mt_key={mt_key}, hf_key={hf_key}, pt_shape={pt_val.shape}, target_shape={target_shape}",
          flush=True,
      )
...
class DeepSeekV4FlashRMSNormTest(unittest.TestCase):
  ...
  def test_rmsnorm_parity(self):
    ...
    mt_norm = RMSNorm(
        num_features=self.dim, epsilon=self.eps, dtype=jnp.float32, weight_dtype=jnp.float32, rngs=self.rngs
    )
...
class DeepSeekV4FlashMoEBlockTest(unittest.TestCase):
  ...
  def test_hash_routing_moe_parity(self):
    ...
    _apply_layer_param_mapping(
        DummyLayer(mt_moe), DummyPTLayer(pt_moe), layer_idx=0, pt_config_dict=pt_config_dict, mx_config=self.mx_config
    )
...
class DeepSeekV4FlashDecoderLayerTest(unittest.TestCase):
  ...
  def _run_layer_test(self, layer_idx: int):
    ...
    x_np = np.random.uniform(0.1, 1.0, size=(self.batch_size, self.seq_len, self.args.hc_mult, self.dim)).astype(
        np.float32
    )
    pos_np = np.arange(self.seq_len)[None, :].repeat(self.batch_size, axis=0)
...
class DeepSeekV4FlashFullModelTest(unittest.TestCase):
  ...
  def test_full_model_parity(self):
    ...
    for i in range(self.args.n_layers):
      _apply_layer_param_mapping(
          getattr(mt_model.decoder, f"layers_{i}"), pt_model.layers[i], i, pt_config_dict, self.mx_config
      )
    ...
    np.testing.assert_allclose(np.array(mt_out), logits_pt.detach().numpy(), atol=1e-2, rtol=1e-2)
