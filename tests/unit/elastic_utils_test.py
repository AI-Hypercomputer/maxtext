...
class ElasticUtilsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.original_scale_up_signal_error = getattr(
            pathwaysutils.elastic.manager, "ScaleUpSignalError", None
        )

        # Initialize fakes as mocks
        self.fake_gcs_utils = create_autospec(gcs_utils)
        self.fake_gcs_utils.add_trailing_slash.side_effect = (
            gcs_utils.add_trailing_slash
        )
        ...
        pathwaysutils.elastic.manager.Manager = (
            lambda *args, **kwargs: self.fake_manager
        )
    ...
    def tearDown(self):
        ...
        pathwaysutils.elastic.manager.ScaleUpSignalError = (
    ...
    def test_clean_up_checkpoints_incomplete(self):
        ...
        self.fake_gcs_utils.gcs_delete_directory.assert_called_once_with(
            f"{checkpoint_dir}/10/"
        )
    ...
    def test_clean_up_checkpoints_complete(self):
        ...
        self.fake_gcs_utils.gcs_list_directories.return_value = ["1", "2", "10"]
        self.fake_gcs_utils.gcs_glob_pattern.return_value = [
            f"{checkpoint_dir}/10/commit_success_0"
        ]
        elastic_utils.clean_up_checkpoints(checkpoint_dir)
        self.fake_gcs_utils.gcs_delete_directory.assert_not_called()
    ...
    def test_get_local_batch_size_non_elastic(self):
        ...
        self.fake_jax.devices.return_value = [
            FakeDevice(slice_index=0, process_index=0, task_id=0) for _ in range(8)
        ]
        self.fake_pathwaysutils.is_pathways_backend_used.return_value = False
