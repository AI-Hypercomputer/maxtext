...
class MaxUtilsSummaryStats(unittest.TestCase):
    ...
    def test_l2norm_pytree(self):
        ...
        self.assertTrue(
            jax.numpy.allclose(pytree_l2_norm,
                               7,
                               rtol=1e-05,
                               atol=1e-08,
                               equal_nan=False))
...
class MaxUtilsPytree(unittest.TestCase):
    ...
    def test_calculate_num_params_from_pytree(self):
        ...
        self.assertEqual(
            max_utils.calculate_num_params_from_pytree(example_tree), 17)
        # Model params
        self.assertEqual(
            max_utils.calculate_num_params_from_pytree(self.params), 55)
...
class MaxUtilsT5XCrossEntropy(unittest.TestCase):
    ...
    def test_t5x_cross_entropy(self):
        ...
        targets = jax.random.randint(key,
                                     shape=(48, 2048),
                                     dtype=jax.numpy.int32,
                                     minval=1,
                                     maxval=10)
        logits = jax.random.uniform(key,
                                    shape=(48, 2048, 4096),
                                    dtype=jax.numpy.float32)

        # Calculate xent from optax implementation
        optax_xent = optax.softmax_cross_entropy_with_integer_labels(
            logits, targets)

        # Calculate xent from custom T5X implementation
        one_hot_targets = jax.nn.one_hot(targets, 4096)
        t5x_xent, _ = max_utils.cross_entropy_with_logits(
            logits, one_hot_targets, 0.0)
        t5x_xent = nn.with_logical_constraint(
            t5x_xent, ("activation_batch", "activation_length"))

        # Compare results
        self.assertTrue(
            jax.numpy.allclose(optax_xent,
                               t5x_xent,
                               rtol=1e-05,
                               atol=1e-08,
                               equal_nan=False))
    ...
    def test_cross_entropy_with_z_loss(self):
        ...
        total_loss_no_z, _ = max_utils.cross_entropy_with_logits(
            logits, one_hot_targets, z_loss=0.0)
        ...
        self.assertTrue(
            jnp.allclose(z_loss_only, expected_z_loss, rtol=1e-5, atol=1e-8))

        # Compare total loss aggregation
        self.assertTrue(
            jnp.allclose(total_loss_with_z,
                         total_loss_no_z + z_loss_only,
                         rtol=1e-5,
                         atol=1e-8))
...
class MaxUtilsCustomMesh(unittest.TestCase):
    ...
    def test_empty_value(self):
        self.assertFalse(
            max_utils.is_valid_custom_mesh([1, 1, 1, 1, 1, 64, 4, 1], ""))
    ...
    def test_valid_32x8(self):
        self.assertTrue(
            max_utils.is_valid_custom_mesh([1, 1, 32, 1, 1, 8, 1, 1],
                                           "hybrid_ring_32x8"))

    def test_invalid_64x4(self):
        with self.assertRaises(ValueError):
            max_utils.is_valid_custom_mesh([1, 1, 1, 1, 1, 16, 16, 1],
                                           "hybrid_ring_64x4")
...
class UnscanTest(unittest.TestCase):
    ...
    def test_unscan_train_state_params(self):
        ...
        if hasattr(state, "model"):
            ...
            params_to_unscan = jax.tree_util.tree_map(lambda x: x,
                                                      state.params)
        ...
        unstacked_shape = decoder_params["layers_5"]["mlp"]["wi_0"][
            "kernel"].shape
...
class TestGpuDistributedInitialization(unittest.TestCase):
    ...
    def test_initialize_jax_for_gpu_valid_devices(self, _mock_log,
                                                  _mock_devices, mock_init):
    ...
    @mock.patch("jax.devices")
    @mock.patch("maxtext.utils.max_logging.log")
    def test_initialize_jax_for_gpu_invalid_devices(self, _mock_log,
                                                    _mock_devices, mock_init):
    ...
    @mock.patch("jax.devices")
    @mock.patch("maxtext.utils.max_logging.log")
    def test_initialize_jax_for_gpu_no_devices(self, _mock_log, _mock_devices,
                                               mock_init, mock_config_update):
    ...
    @mock.patch("jax.devices")
    @mock.patch("maxtext.utils.max_logging.log")
    def test_initialize_jax_for_gpu_uses_slurm_when_cuda_unset(
            self, mock_log, _mock_devices, mock_init):
        ...
        mock_log.assert_any_call(
            "Using SLURM_STEP_GPUS to initialize JAX distributed system: 1,3")
    ...
    @mock.patch("jax.devices")
    @mock.patch("maxtext.utils.max_logging.log")
    def test_initialize_jax_for_gpu_prefers_cuda_visible_devices_in_loop(
            self, mock_log, _mock_devices, mock_init):
        ...
        mock_log.assert_any_call(
            "Using CUDA_VISIBLE_DEVICES to initialize JAX distributed system: 0,2"
        )
...
class TestMaybeInitializeJaxDistributedSystem(unittest.TestCase):
  ...
  """
    ...
    def setUp(self):
        patcher = mock.patch("jax.distributed.is_initialized",
                             return_value=False)
    ...
    def test_skip_flag_exits_early(self, mock_init):
        max_utils.maybe_initialize_jax_distributed_system(
            self._base_keys(skip_jax_distributed_system=True))
        mock_init.assert_not_called()

    @mock.patch("jax.distributed.initialize")
    def test_single_controller_exits_early(self, mock_init):
        max_utils.maybe_initialize_jax_distributed_system(
            self._base_keys(enable_single_controller=True))
        mock_init.assert_not_called()
    ...
    def test_inference_benchmark_exits_early(self, mock_init):
        max_utils.maybe_initialize_jax_distributed_system(
            self._base_keys(inference_benchmark_test=True))
        mock_init.assert_not_called()

    @mock.patch("jax.distributed.initialize")
    def test_compile_topology_exits_early(self, mock_init):
        max_utils.maybe_initialize_jax_distributed_system(
            self._base_keys(compile_topology=True))
        mock_init.assert_not_called()
    ...
    def test_gpu_multiprocess_no_emergency_calls_jax_init(self, mock_init):
        ...
        mock_init.assert_called_once_with(
            initialization_timeout=self._base_keys()
    ...
    @mock.patch(
        "orbax.checkpoint.multihost.initialize_distributed_to_device_ids",
        create=True)
    @mock.patch(
        "orbax.checkpoint.multihost.initialize_runtime_to_distributed_ids")
    @mock.patch("jax.distributed.initialize")
    def test_gpu_multiprocess_with_emergency_calls_ocp_multihost(
            self, mock_init, mock_runtime, mock_device):
        raw_keys = self._base_keys(hardware="gpu_multiprocess",
    ...
    def test_tpu_multi_tier_checkpointing(self, mock_mtc):
        ...
        mock_mtc.assert_called_once_with(
            local_checkpoint_directory=self._base_keys()
            ["local_checkpoint_directory"],
            backup_interval_minutes=self._base_keys()
            ["multi_tier_checkpointing_backup_interval_minutes"],
            run_name=self._base_keys()["run_name"],
            jax_initialization_timeout_seconds=self._base_keys()
            ["jax_distributed_initialization_timeout"],
    ...
    @mock.patch("maxtext.utils.max_utils.initialize_multi_tier_checkpointing")
    @mock.patch("jax.distributed.initialize")
    def test_single_controller_multi_tier_checkpointing_uses_colocated_python(
            self, mock_init, mock_mtc):
        raw_keys = self._base_keys(enable_single_controller=True,
    ...
    @mock.patch(
        "maxtext.utils.max_utils.elastic_utils.single_controller_mtc_init_kwargs"
    )
    @mock.patch("maxtext.utils.max_utils.initialize_multi_tier_checkpointing")
    @mock.patch("jax.distributed.initialize")
    ...
    def test_tpu_checkpointing_no_emergency_calls_jax_init(self, mock_init):
        raw_keys = self._base_keys(enable_checkpointing=True,
                                   compile_topology_num_slices=-1)
        max_utils.maybe_initialize_jax_distributed_system(raw_keys)
        mock_init.assert_called_once_with(
            initialization_timeout=self._base_keys()
    ...
    @mock.patch(
        "maxtext.utils.max_utils.initialize_jax_for_tpu_with_emergency_checkpointing"
    )
    ...
    def test_tpu_no_checkpointing_does_not_call_jax_init(self, mock_init):
        raw_keys = self._base_keys(enable_checkpointing=False,
                                   enable_multi_tier_checkpointing=False)
        max_utils.maybe_initialize_jax_distributed_system(raw_keys)
        mock_init.assert_not_called()
