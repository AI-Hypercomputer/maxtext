...
class BlockDiffusionSftMaskTest(unittest.TestCase):
  ...
  def test_nonzero_token_pad_id_does_not_mark_padding_valid(self):
    transform = SFTPromptMasking(
        text_column_name="messages",
        completion_only=False,
        max_target_length=8,
    ...
    clean = transform.map(element)
    padded = PadOrTrimToMaxLength(max_length=8, pad_id=7).map(clean)
    corrupted = BlockDiffusionCorruption(
        block_size=4,
        mask_id=99,
        min_noise=1.0,
        completion_only=False,
        axis=0,
    ).random_map(padded, np.random.default_rng(0))

    padding = np.arange(8) >= 4
    np.testing.assert_array_equal(padded["targets"][padding], 7)
    np.testing.assert_array_equal(padded["inputs_segmentation"][padding], 0)
    np.testing.assert_array_equal(padded["targets_segmentation"][padding], 0)
    np.testing.assert_array_equal(padded["inputs_position"][padding], 0)
    np.testing.assert_array_equal(padded["targets_position"][padding], 0)
