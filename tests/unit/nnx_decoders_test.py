...
from maxtext.src.maxtext.layers.embeddings import Embed
from maxtext.src.maxtext.layers.nnx_decoders import NNXDecoder, NNXDecoderLayer, deepstack_process
from maxtext.src.maxtext.layers.normalizations import RMSNorm
from maxtext.src.maxtext.models import gemma4, gemma4_small, qwen3
from maxtext.src.maxtext.models.gpt3 import Gpt3LayerNorm
from maxtext.src.maxtext.models.llama2 import LlamaDecoderLayer
from maxtext.src.maxtext.utils import maxtext_utils, maxtext_utils_nnx
...
class TestQwen3NextScannableBlock(unittest.TestCase):
  ...
  def test_updates_state_through_global_single_iteration_scan(self):
    with mock.patch.object(
        qwen3, "Qwen3NextDecoderLayer", _StatefulQwen3NextDecoderLayer
    ):
    ...
    np.testing.assert_array_equal(
        block.local_layers.call_count.value, jnp.ones(3, dtype=jnp.int32)
    )
  ...
  def test_restores_local_state_and_preserves_kv_order(self):
    ...
    np.testing.assert_array_equal(output, jnp.full((1, 1, 1), 13))
    np.testing.assert_array_equal(
        jnp.stack(updated_kvs), jnp.array([1, 2, 3, 13])
    )
    np.testing.assert_array_equal(
        block.local_layers.call_count.value, jnp.ones(3, dtype=jnp.int32)
    )
    ...
    np.testing.assert_array_equal(block.global_layer.call_count.value, 1)
    np.testing.assert_array_equal(
        block.global_layer.received_attention_metadata.value, True
    )
