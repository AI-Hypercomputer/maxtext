...
Qwen3-30B-A3B-base teacher to a pruned student (half query/KV heads, doubled
head_dim) in MaxText.

- **Teacher**: `qwen3-30b-a3b-base` (converted MaxText checkpoint)
- **Student**: pruned variant — `learn_to_init_mode: True`
...
`per_device_batch_size`, `gradient_accumulation_steps`, and
`max_target_length` MUST be set at the YAML top level (not in `*_overrides`
or via CLI). The trainer builds the teacher config from the YAML only,
ignoring CLI overrides. Mismatched shapes between student and teacher trip
a validator at startup.

Student shape fields (`base_num_query_heads`, `base_num_kv_heads`,
`head_dim`) behave similarly: when `override_model_config=True` is set in
`student_overrides`, the model config overrides any YAML top-level values,
and some YAML-block fields don't propagate (`head_dim` does, q/kv heads
don't). Verify the student shape in the second `Config param ...` block of
the log matches your intent.
...
## 4. Derive `distill_weights_copy_map`
...
## 5. CPU smoke test for the warm-start

A small CPU test verifies that the warm-start math is correct (~30 s):

```bash
JAX_PLATFORMS=cpu .venv/bin/python -m \
  maxtext.trainers.post_train.distillation.tools.test_lti_warmstart
```

The test builds a small student attention block with LTI bridges, runs a
forward pass on dummy input, and asserts:

1. The student kernel computed by `_calc_attn_weight(A, B, C)` matches the
   group-mean of the teacher Q kernel in the first 128 head_dim components.
2. The OUT projection forward matches the teacher's OUT applied to the
   group-expanded signal.
...
```bash
RUN_ID="qwen3-30b-lti-$(date +%Y%m%d-%H%M%S)"
nohup .venv/bin/python -m maxtext.trainers.post_train.distillation.train_distill \
  src/maxtext/configs/post_train/distillation_qwen3_30b_lti.yml \
  run_name=$RUN_ID \
  base_output_directory=<output_path> \
  > /tmp/distill.log 2>&1 &
...
Tail filtered progress:

```bash
tail -F /tmp/distill.log | awk '
  /Distillation metrics/{n++; if(n==1||n%500==0) print "step~"n": "$0; next}
  /Saving checkpoint|Finished save|Traceback|OOM|RESOURCE_EXHAUSTED|memory_space|ValueError|XlaRuntimeError|Killed|FAILED|abort_on_nan|abort_on_inf/{print}'
```
...
Configurations that don't fit at this scale:

| Config tried                                   | Outcome                                      |
| ---------------------------------------------- | -------------------------------------------- |
| per_device=2, grad_accum=8 (global=128)        | OOM at Adam `nu` init: `optax.MultiSteps` grad buffer (~120 GB fp32) overflows HBM |
| per_device=2, grad_accum=1, fp32 logits        | RuntimeProgramAllocationFailure at JIT load |
| **per_device=1, grad_accum=1, seq=4096, fp32 logits** | **Works.** Step time ~1.8 s |
...
If a run crashed or was killed:

```bash
# 1. Kill orphan grain workers (they hold GCS connections and starve relaunches)
ps -ef | grep -E '(grain|multiprocessing-fork)' | grep -v grep
# pkill -f multiprocessing-fork

# 2. Remove stale libtpu lockfile (only if it's yours)
ls -la /tmp/libtpu_lockfile && rm /tmp/libtpu_lockfile

# 3. Confirm no stray training process is still alive
ps -ef | grep train_distill | grep -v grep
```
