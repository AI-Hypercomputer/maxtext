...
python ../xpk/xpk.py workload create \
  ...
  --command "export LD_LIBRARY_PATH=/usr/local/cuda-12.6/compat:\$LD_LIBRARY_PATH; \
  cd /deps && bash gpu_multi_process_run.sh && \
  ...
  base_output_directory=gs://yujunzou-dev-supercomputer-testing/maxtext_gpu \
  dataset_type=synthetic \
  remat_policy=minimal_flash \
  logits_dot_in_fp32=false \
  dcn_fsdp_parallelism=$NUM_NODES \
