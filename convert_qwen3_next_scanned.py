...
python3 -m MaxText.utils.ckpt_scripts.convert_qwen3_next --base_model_path . --maxtext_model_path gs://<gcs_bucket>/<path/to/save/ckpt> --model_size qwen3-next-80b-a3b
"""
...
def hf_to_maxtext_mapping(layer_idx: int, num_experts: int, inhomogeneous_layer_cycle_interval: int) -> dict:
    ...
    maxtext_layer_prefix = f"decoder.layers.layer_{block_idx}"

    mapping.update({
        f"model.layers.{layer_idx}.input_layernorm.weight": f"{maxtext_layer_prefix}.input_layernorm.scale",
        f"model.layers.{layer_idx}.post_attention_layernorm.weight": f"{maxtext_layer_prefix}.post_attention_layernorm.scale",
    })

    is_full_attention_layer = (layer_idx + 1) % inhomogeneous_layer_cycle_interval == 0
    if is_full_attention_layer:
        mapping.update({
            f"model.layers.{layer_idx}.self_attn.q_proj.weight": f"{maxtext_layer_prefix}.attention.attention.query.kernel",
            f"model.layers.{layer_idx}.self_attn.k_proj.weight": f"{maxtext_layer_prefix}.attention.attention.key.kernel",
            f"model.layers.{layer_idx}.self_attn.v_proj.weight": f"{maxtext_layer_prefix}.attention.attention.value.kernel",
            f"model.layers.{layer_idx}.self_attn.o_proj.weight": f"{maxtext_layer_prefix}.attention.attention.out.kernel",
            f"model.layers.{layer_idx}.self_attn.q_norm.weight": f"{maxtext_layer_prefix}.attention.attention.query_norm.scale",
            f"model.layers.{layer_idx}.self_attn.k_norm.weight": f"{maxtext_layer_prefix}.attention.attention.key_norm.scale",
        })
    else:
        mapping.update({
            f"model.layers.{layer_idx}.linear_attn.in_proj_qkvz.weight": f"{maxtext_layer_prefix}.attention.in_proj_qkvz.kernel",
            f"model.layers.{layer_idx}.linear_attn.in_proj_ba.weight": f"{maxtext_layer_prefix}.attention.in_proj_ba.kernel",
            f"model.layers.{layer_idx}.linear_attn.conv1d.weight": f"{maxtext_layer_prefix}.attention.conv1d.kernel",
            f"model.layers.{layer_idx}.linear_attn.A_log": f"{maxtext_layer_prefix}.attention.A_log",
            f"model.layers.{layer_idx}.linear_attn.dt_bias": f"{maxtext_layer_prefix}.attention.dt_bias",
            f"model.layers.{layer_idx}.linear_attn.norm.weight": f"{maxtext_layer_prefix}.attention.norm.rms_norm.scale",
            f"model.layers.{layer_idx}.linear_attn.out_proj.weight": f"{maxtext_layer_prefix}.attention.out_proj.kernel",
        })

    mlp_prefix = f"{maxtext_layer_prefix}.mlp"
    mapping.update({
        f"model.layers.{layer_idx}.mlp.gate.weight": f"{mlp_prefix}.routed_experts.gate.kernel",
        f"model.layers.{layer_idx}.mlp.shared_expert.gate_proj.weight": f"{mlp_prefix}.shared_expert.wi_0.kernel",
        f"model.layers.{layer_idx}.mlp.shared_expert.up_proj.weight": f"{mlp_prefix}.shared_expert.wi_1.kernel",
        f"model.layers.{layer_idx}.mlp.shared_expert.down_proj.weight": f"{mlp_prefix}.shared_expert.wo.kernel",
        f"model.layers.{layer_idx}.mlp.shared_expert_gate.weight": f"{mlp_prefix}.shared_expert_gate.kernel",
    })
    for i in range(num_experts):
        mapping[f"model.layers.{layer_idx}.mlp.experts.{i}.gate_proj.weight"] = f"{mlp_prefix}.routed_experts.{i}.wi_0"
        mapping[f"model.layers.{layer_idx}.mlp.experts.{i}.up_proj.weight"] = f"{mlp_prefix}.routed_experts.{i}.wi_1"
        mapping[f"model.layers.{layer_idx}.mlp.experts.{i}.down_proj.weight"] = f"{mlp_prefix}.routed_experts.{i}.wo"
    ...
    for i in range(cycle):
        ...
        layer_struct = {
            ...
            "mlp": {
                "routed_experts": {
                    ...
                    "wi_0": np.zeros((num_experts_to_convert, num_stacked_layers, emb_dim, moe_intermediate_size), dtype=jnp.bfloat16),
                    "wi_1": np.zeros((num_experts_to_convert, num_stacked_layers, emb_dim, moe_intermediate_size), dtype=jnp.bfloat16),
                    "wo": np.zeros((num_experts_to_convert, num_stacked_layers, moe_intermediate_size, emb_dim), dtype=jnp.bfloat16),
                },
                "shared_expert": {
        ...
        if is_full_attention_layer:
            layer_struct["attention"] = {
             ...
             layer_struct["attention"] = {
                "in_proj_qkvz": {"kernel": np.zeros((emb_dim, num_stacked_layers, gdn_key_dim * 2 + gdn_value_dim * 2), dtype=jnp.bfloat16)},
                ...
                "norm": {"rms_norm": {"scale": np.zeros((model_params["gdn_value_head_dim"], num_stacked_layers), dtype=jnp.bfloat16)}},
...
def convert_hf_to_maxtext(base_model_path: str, model_params: dict, args) -> dict:
  ...
  if num_layers_to_convert % cycle != 0:
      raise ValueError(f"num_layers_to_convert ({num_layers_to_convert}) must be a multiple of the cycle length ({cycle})")
  num_stacked_layers = num_layers_to_convert // cycle
  ...
  for l in tqdm(range(num_layers_to_convert), desc="Processing Layers"):
    ...
    def get_hf_tensor(maxtext_key_suffix):
    ...
    ln["input_layernorm"]["scale"][:, stack_idx] = to_np_bfloat16(get_hf_tensor(f".input_layernorm.scale"))
    # HF: [emb_dim] -> slice of MaxText: [emb_dim, num_stacked_layers]
    ln["post_attention_layernorm"]["scale"][:, stack_idx] = to_np_bfloat16(get_hf_tensor(f".post_attention_layernorm.scale"))

    attn_block = ln["attention"]
    is_full_attention_layer = (l + 1) % cycle == 0
    if is_full_attention_layer:
        attn_params = attn_block["attention"]
        # HF: [8192, 2048] -> Transpose [2048, 8192] -> Reshape -> slice of MaxText: [emb_dim, num_stacked_layers, num_q_heads, head_dim * 2]
        ...
        # HF: [512, 2048] -> Transpose [2048, 512] -> Reshape -> slice of MaxText: [emb_dim, num_stacked_layers, num_kv_heads, head_dim]
        ...
        attn_params["out"]["kernel"][:, stack_idx, :] = to_np_bfloat16(get_hf_tensor(".attention.attention.out.kernel")).transpose()
        # HF: [256] -> slice of MaxText: [head_dim, num_stacked_layers]
        attn_params["query_norm"]["scale"][:, stack_idx] = to_np_bfloat16(get_hf_tensor(".attention.attention.query_norm.scale"))
        # HF: [256] -> slice of MaxText: [head_dim, num_stacked_layers]
        attn_params["key_norm"]["scale"][:, stack_idx] = to_np_bfloat16(get_hf_tensor(".attention.attention.key_norm.scale"))
    else: # Gated Delta Net
        # HF: [12288, 2048] -> Transpose -> slice of MaxText: [emb_dim, num_stacked_layers, 12288]
        attn_block["in_proj_qkvz"]["kernel"][:, stack_idx, :] = to_np_bfloat16(get_hf_tensor(".attention.in_proj_qkvz.kernel")).transpose()
        # HF: [64, 2048] -> Transpose -> slice of MaxText: [emb_dim, num_stacked_layers, 64]
        attn_block["in_proj_ba"]["kernel"][:, stack_idx, :] = to_np_bfloat16(get_hf_tensor(".attention.in_proj_ba.kernel")).transpose()
        ...
        attn_block["norm"]["rms_norm"]["scale"][:, stack_idx] = to_np_bfloat16(get_hf_tensor(".attention.norm.rms_norm.scale"))
        # HF: [2048, 4096] -> Transpose -> slice of MaxText: [4096, num_stacked_layers, 2048]
        attn_block["out_proj"]["kernel"][:, stack_idx, :] = to_np_bfloat16(get_hf_tensor(".attention.out_proj.kernel")).transpose()

    # MoE
    mlp_block = ln["mlp"]
    # HF: [512, 2048] -> Transpose -> slice of MaxText: [emb_dim, num_stacked_layers, 512]
    mlp_block["routed_experts"]["gate"]["kernel"][:, stack_idx, :] = to_np_bfloat16(get_hf_tensor(".mlp.routed_experts.gate.kernel")).transpose()
    # HF: [512, 2048] -> Transpose -> slice of MaxText: [emb_dim, num_stacked_layers, 512]
    mlp_block["shared_expert"]["wi_0"]["kernel"][:, stack_idx, :] = to_np_bfloat16(get_hf_tensor(".mlp.shared_expert.wi_0.kernel")).transpose()
    # HF: [512, 2048] -> Transpose -> slice of MaxText: [emb_dim, num_stacked_layers, 512]
    mlp_block["shared_expert"]["wi_1"]["kernel"][:, stack_idx, :] = to_np_bfloat16(get_hf_tensor(".mlp.shared_expert.wi_1.kernel")).transpose()
    # HF: [2048, 512] -> Transpose -> slice of MaxText: [512, num_stacked_layers, 2048]
    mlp_block["shared_expert"]["wo"]["kernel"][:, stack_idx, :] = to_np_bfloat16(get_hf_tensor(".mlp.shared_expert.wo.kernel")).transpose()
    # HF: [1, 2048] -> Transpose -> slice of MaxText: [emb_dim, num_stacked_layers, 1]
    mlp_block["shared_expert_gate"]["kernel"][:, stack_idx, :] = to_np_bfloat16(get_hf_tensor(".mlp.shared_expert_gate.kernel")).transpose()
    ...
    for i in range(num_experts_to_convert):
        ...
        mlp_block["routed_experts"]["wi_0"][i, stack_idx, :, :] = to_np_bfloat16(get_hf_tensor(f".mlp.routed_experts.{i}.wi_0")).transpose()
        # HF: [512, 2048] -> Transpose -> slice of MaxText: [num_experts, num_stacked_layers, emb_dim, moe_intermediate_size]
        mlp_block["routed_experts"]["wi_1"][i, stack_idx, :, :] = to_np_bfloat16(get_hf_tensor(f".mlp.routed_experts.{i}.wi_1")).transpose()
        # HF: [2048, 512] -> Transpose -> slice of MaxText: [num_experts, num_stacked_layers, moe_intermediate_size, emb_dim]
        mlp_block["routed_experts"]["wo"][i, stack_idx, :, :] = to_np_bfloat16(get_hf_tensor(f".mlp.routed_experts.{i}.wo")).transpose()
  gc.collect()
