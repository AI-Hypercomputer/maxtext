...
_MODEL_TO_BLOCKS = {
    ...
    # Stitched model
    "omni-gemma3-qwen3": ("gemma3", "qwen3"),
}
...
def get_image_offsets(config, processor_output: mm_utils.PreprocessorOutput | None):
  ...
  if vision_block == "gemma3" and decoder_block == "qwen3":
    from maxtext.experimental.omni_poc.utils import processor_omni_gemma3_qwen3  # pylint: disable=import-outside-toplevel

    return processor_omni_gemma3_qwen3.get_image_offsets_omni(processor_output)
...
def prepare_text_for_image_fusion(tokens, config, processor_output=None):
  ...
  if vision_block == "gemma3" and decoder_block == "qwen3":
    ...
    return processor_omni_gemma3_qwen3.add_extra_tokens_for_omni(tokens, config=config, processor_output=processor_output)
  elif vision_block in ["gemma3"]:
...
def get_bidirectional_mask_vision(config, decoder_input_tokens, is_video: bool = False):
  ...
  if vision_block == "gemma3" and decoder_block == "qwen3":
    ...
    bidirectional_mask_vision = decoder_input_tokens == processor_omni_gemma3_qwen3.IMAGE_PAD_ID
  elif decoder_block in ["gemma3"]:
