...
class TestCheckpointValidationAgent(unittest.TestCase):
  ...
  def test_missing_strict_architecture_flags(self):
    ...
    with self.assertRaisesRegex(ValueError, "REQUIRED: You must provide 'scan_layers'"):
      # Missing scan_layers
      validate_checkpoint(
          "", ["run_name=test-run", "model_name=qwen", "load_parameters_path=gs://fake", "tokenizer_path=fake/path"]
      )

    with self.assertRaisesRegex(ValueError, "REQUIRED: You must provide 'tokenizer_path'"):
      # Missing tokenizer_path
      validate_checkpoint(
          "", ["run_name=test-run", "model_name=qwen", "load_parameters_path=gs://fake", "scan_layers=false"]
      )

  @patch("maxtext.experimental.agent.ckpt_validation_pipeline.decode_validator.subprocess.Popen")
  @patch("os.makedirs")
  @patch("builtins.open")
  def test_successful_command_generation(self, _mock_open, _mock_makedirs, mock_popen):
  ...
  def test_upload_to_gcs(self, mock_upload_blob, _mock_open, _mock_makedirs, mock_popen):
