...
def main():
  ...
  for root, _, files in os.walk("/app/src/maxtext"):
    for file in files:
      ...
      rel_path = os.path.relpath(src_file, "/app")
      dest_file = os.path.join(repo_dir, rel_path)
  ...
  if (
      subprocess.run(["git", "remote", "set-url", "origin", remote_url], cwd=repo_dir, capture_output=True).returncode
      != 0
  ):
  ...
  push_res = subprocess.run(
      ["git", "push", "-uf", "origin", fork_branch], cwd=repo_dir, capture_output=True, text=True, check=False
  )
