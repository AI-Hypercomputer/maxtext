...
def _send_message_with_retry(chat, prompt, max_retries=5, base_sleep_seconds=15):
  """Sends a message to Gemini with exponential backoff on 429 rate-limit/quota errors."""
    ...
    try:
      ...
      if any(k in err_str for k in retry_keywords):
        if attempt < max_retries:
          sleep_time = base_sleep_seconds * (2 ** (attempt - 1)) # Exponential backoff algorithm
          logger.warning(
              f"Received API rate-limit/server error (attempt {attempt}/{max_retries}). Sleeping {sleep_time}s before retry..."
          )
...
def clear_failed_airflow_task(
    dag_id: str, task_id: str, new_branch: str = "", base_run_name: str = "", logical_date: str = ""
) -> str:
...
def run_agent_workflow(context: dict, failure_log: str):
  ...
  if remediation_level == "level_1_config":
    ...
    logger.info(
        "Phase 3.5: Overwatch Overseer inspecting Fixer output for hallucinations, API validity, and syntactic regressions..."
    )
    try:
      meta_template = _load_prompt_file("meta_agent.txt")
