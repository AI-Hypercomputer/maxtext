...
class TestCheckpointValidationAgent(unittest.TestCase):
    ...
    def test_missing_required_keys(self, mock_file, mock_exists):
        ...
        with self.assertRaisesRegex(
            KeyError, "CRITICAL ERROR: JSON config is missing required key"
        ):
    ...
    def test_missing_strict_architecture_flags(self, mock_file, mock_exists):
        ...
        with self.assertRaisesRegex(
            ValueError, "REQUIRED: You must provide 'scan_layers'"
        ):
            validate_checkpoint("fake_path.json")

    @patch("os.path.exists", return_value=True)
    @patch(
        "src.maxtext.experimental.agent.checkpoint_validation_agent.main.subprocess.run"
    )
    @patch("os.makedirs")
    def test_successful_command_generation(
        self, mock_makedirs, mock_subprocess, mock_exists
    ):
