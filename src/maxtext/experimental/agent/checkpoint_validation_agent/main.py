...
def validate_checkpoint(json_config_path):
    ...
    for key in required_keys:
        if key not in user_config:
            raise KeyError(
                f"CRITICAL ERROR: JSON config is missing required key '{key}'"
            )
    ...
    if "tokenizer_path" not in overrides:
        raise ValueError(
            "REQUIRED: You must provide 'tokenizer_path' in maxtext_overrides in your JSON config."
        )
    if "scan_layers" not in overrides:
        raise ValueError(
            "REQUIRED: You must provide 'scan_layers' (true/false) in maxtext_overrides in your JSON config."
        )
    tokenizer = overrides.pop("tokenizer_path")
    scan = overrides.pop("scan_layers")

    print(f"Validating {run_name}...")
    print(f"Reading weights from: {checkpoint_path}")
    ...
    if overrides:
        print("Applying additional flags from MaxText overrides...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Validate MaxText Checkpoints via JSON config"
    )
    parser.add_argument(
