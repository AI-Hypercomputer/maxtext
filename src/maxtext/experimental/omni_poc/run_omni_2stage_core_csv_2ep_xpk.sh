...
#
# Stage 1: ChartNet Core CSV Table Grounding (2 Epochs = 26,533 steps, Eval Disabled)
...
#     1,698,111 examples * 2 epochs / (128 devices * 1 per_device_batch_size = 128)
#     = 26,532.98 -> ceil = 26,533 steps (final checkpoint at step 26532).
...
EXP_NAME="${EXP_NAME:-omni_gemma3_qwen3_2stage_core_csv_2ep}"
...
# Stage 1: ChartNet Core CSV Table Grounding (2 Epochs = 26,533 steps on 128 devices, No Eval)
STAGE1_CONFIG="${STAGE1_CONFIG:-src/maxtext/experimental/omni_poc/pretrain-omni-gemma3-qwen3-chartnet-core-csv-xpk-128-2ep.yml}"
STAGE1_EPOCHS="${STAGE1_EPOCHS:-2}"
STAGE1_STEPS="${STAGE1_STEPS:-26533}"
STAGE1_CKPT_PERIOD="${STAGE1_CKPT_PERIOD:-2000}"
STAGE1_OUTPUT_DIR="${WORKING_DIR}/stage1_chartnet_core_csv_2ep"
STAGE1_RUN_NAME="${EXP_NAME}_stage1_core_csv_2ep"
...
STAGE2_CONFIG="${STAGE2_CONFIG:-src/maxtext/experimental/omni_poc/sft-omni-gemma3-qwen3-xpk-128.yml}"
...
# ------------------------------------------------------------------------------
# 2. Stage 1: ChartNet Core CSV Table Grounding on XPK (2 Epochs = 26,533 steps)
# ------------------------------------------------------------------------------
stage1_xpk() {
  local input_ckpt="${1:-${STITCHED_CKPT}}"
  local workload_name="${USER_PREFIX}-omni-s1-csv2ep-${TIMESTAMP}"
  workload_name="${workload_name:0:39}"
  workload_name="${workload_name%-}"

  echo "=================================================================="
  echo ">>> [XPK STAGE 1 - ChartNet Core CSV (2 Epochs)] Submitting Workload: ${workload_name}"
  ...
  (
    ...
    xpk workload create \
      ...
      --script-dir . \
      --command "export MEGASCALE_NUM_SLICES=${XPK_NUM_SLICES} && export TMPDIR=/dev/shm && export PYTHONPATH=src:\${PYTHONPATH:-} && export HF_HOME=/dev/shm/huggingface && export HF_TOKEN=${HF_TOKEN} && export HUGGING_FACE_HUB_TOKEN=${HF_TOKEN} && python3 -m pip install --no-cache-dir -U 'orbax-checkpoint>=0.12.4' && gcloud storage cp -r ${CHARTNET_CORE_DATASET_DIR} /dev/shm/ && python3 -m maxtext.experimental.omni_poc.train_sft_omni ${STAGE1_CONFIG} load_parameters_path=${input_ckpt} base_output_directory=${STAGE1_OUTPUT_DIR} run_name=${STAGE1_RUN_NAME} num_epoch=${STAGE1_EPOCHS} steps=${STAGE1_STEPS} eval_interval=-1 eval_steps=0 checkpoint_period=${STAGE1_CKPT_PERIOD} hf_access_token=${HF_TOKEN} scan_layers=true grain_worker_count=0"
  )
}
...
# ------------------------------------------------------------------------------
# 4. Full 2-Stage Pipeline on XPK (Stage 1 Core CSV 2ep -> Stage 2 ChartQA SFT)
# ------------------------------------------------------------------------------
pipeline_xpk() {
  local workload_name="${USER_PREFIX}-omni-core-csv2ep-${TIMESTAMP}"
  workload_name="${workload_name:0:39}"
  workload_name="${workload_name%-}"

  echo "=================================================================="
  echo ">>> [XPK FULL 2-STAGE PIPELINE: ChartNet Core CSV (2 Epochs) -> ChartQA SFT]"
  echo ">>> Submitting Workload: ${workload_name}"
  echo ">>> Base Working Dir:    ${WORKING_DIR}"
  echo ">>> Stage 1: ChartNet Core CSV Grounding (2 Epochs = ${STAGE1_STEPS} steps -> ${STAGE1_FINAL_CKPT})"
...
case "$ACTION" in
  ...
  delete)
    ...
    ;;
  help|*)
    echo "Usage: $0 <check_data|prepare_data|prepare_chartnet|prepare_chartqa|stitch|stage1|stage2|pipeline|eval|list|status|logs|delete> [checkpoint_path|workload_name] [eval_output_dir]"
    echo ""
    echo "Commands:"
    ...
    echo "  stage1 | csv      Launch Stage 1: ChartNet Core CSV pretraining (2 epochs = ${STAGE1_STEPS} steps, no eval)"
