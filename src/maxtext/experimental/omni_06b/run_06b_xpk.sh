#!/bin/bash
# ==============================================================================
# Omni Gemma3-4B Vision + Qwen3-0.6B LLM 3-Stage Training Pipeline on XPK
...
EXP_NAME="${EXP_NAME:-omni_gemma3_qwen3_06b_3stage}"
...
CKPT_DIR="${CKPT_DIR:-${GCS_BUCKET}/omni-gemma3-qwen3-0.6b/checkpoints}"
WORKING_DIR="${WORKING_DIR:-${GCS_BUCKET}/omni-gemma3-qwen3-0.6b/multimodal/3stage_pipeline/${EXP_NAME}}"

# Base Storage Buckets & Initial Checkpoints
VISION_SOURCE_CKPT="${VISION_SOURCE_CKPT:-${CKPT_DIR}/gemma3-4b_converted/0/items}"
LLM_SOURCE_CKPT="${LLM_SOURCE_CKPT:-${CKPT_DIR}/qwen3-0.6b_converted/0/items}"
STITCHED_CKPT="${STITCHED_CKPT:-${CKPT_DIR}/omni_stitched_gemma3-4b_qwen3-0.6b/0/items}"
...
prepare_ckpt() {
  echo "=================================================================="
  echo ">>> [PREPARE] Downloading & Converting Gemma3-4B + Qwen3-0.6B Checkpoints"
  echo "=================================================================="
...
stitch_ckpt() {
  echo "=================================================================="
  echo ">>> [STITCH] Stitching Gemma3-4B Vision + Qwen3-0.6B LLM"
  ...
  (
    ...
    HF_TOKEN="${HF_TOKEN}" HUGGING_FACE_HUB_TOKEN="${HF_TOKEN}" JAX_PLATFORMS=cpu python3 -m maxtext.experimental.omni_pipeline.utils.stitch_checkpoint \
      "${SCRIPT_DIR}/maxtext-omni-gemma3-qwen3-0.6b.yml" \
...
stage1_xpk() {
  ...
  local workload_name="${USER_PREFIX}-06b-s1-${TIMESTAMP}"

  echo "=================================================================="
  echo ">>> [XPK STAGE 1 - ChartNet Summary 0.6B] Submitting Workload: ${workload_name}"
...
stage2_xpk() {
  ...
  local workload_name="${USER_PREFIX}-06b-s2-${TIMESTAMP}"

  echo "=================================================================="
  echo ">>> [XPK STAGE 2 - ChartNet CSV 0.6B] Submitting Workload: ${workload_name}"
  echo ">>> Input Checkpoint: ${input_ckpt}"
  ...
  (
    ...
    xpk workload create \
      ...
      --base-docker-image "${XPK_BASE_DOCKER_IMAGE}" \
      --script-dir . \
      --command "export MEGASCALE_NUM_SLICES=${XPK_NUM_SLICES} && export TMPDIR=/dev/shm && export PYTHONPATH=src:\${PYTHONPATH:-} && export HF_HOME=/dev/shm/huggingface && export HF_TOKEN=${HF_TOKEN} && export HUGGING_FACE_HUB_TOKEN=${HF_TOKEN} && python3 -m pip install --no-cache-dir -U 'orbax-checkpoint>=0.12.4' && gcloud storage cp -r ${CHARTNET_DATASET_DIR} /dev/shm/ && python3 -m maxtext.experimental.omni_pipeline.train_sft_omni src/maxtext/experimental/omni_06b/pretrain-omni-gemma3-qwen3-0.6b-chartnet-xpk-128-csv.yml load_parameters_path=${input_ckpt} base_output_directory=${STAGE2_OUTPUT_DIR} run_name=${STAGE2_RUN_NAME} hf_access_token=${HF_TOKEN} scan_layers=true grain_worker_count=0"
  )
}
...
stage3_xpk() {
  ...
  local workload_name="${USER_PREFIX}-06b-s3-${TIMESTAMP}"

  echo "=================================================================="
  echo ">>> [XPK STAGE 3 - ChartQA SFT 0.6B (Projector Only)] Submitting Workload: ${workload_name}"
  echo ">>> Input Checkpoint: ${input_ckpt}"
...
# ------------------------------------------------------------------------------
pipeline_xpk() {
  local workload_name="${USER_PREFIX}-06b-3stage-${TIMESTAMP}"

  echo "=================================================================="
  echo ">>> [XPK FULL 3-STAGE PIPELINE 0.6B] Submitting Workload: ${workload_name}"
  ...
  (
    ...
    xpk workload create \
      ...
      --base-docker-image "${XPK_BASE_DOCKER_IMAGE}" \
      --script-dir . \
      --command "export MEGASCALE_NUM_SLICES=${XPK_NUM_SLICES} && export TMPDIR=/dev/shm && export PYTHONPATH=src:\${PYTHONPATH:-} && export HF_HOME=/dev/shm/huggingface && export HF_TOKEN=${HF_TOKEN} && export HUGGING_FACE_HUB_TOKEN=${HF_TOKEN} && python3 -m pip install --no-cache-dir -U 'orbax-checkpoint>=0.12.4' && gcloud storage cp -r ${CHARTNET_DATASET_DIR} /dev/shm/ && gcloud storage cp -r ${CHARTQA_DATASET_DIR} /dev/shm/ && echo '=== Stage 1: ChartNet Summary (0.6B) ===' && python3 -m maxtext.experimental.omni_pipeline.train_sft_omni src/maxtext/experimental/omni_06b/pretrain-omni-gemma3-qwen3-0.6b-chartnet-xpk-128.yml load_parameters_path=${STITCHED_CKPT} base_output_directory=${STAGE1_OUTPUT_DIR} run_name=${STAGE1_RUN_NAME} hf_access_token=${HF_TOKEN} scan_layers=true grain_worker_count=0 && echo '=== Stage 2: ChartNet CSV (0.6B) ===' && python3 -m maxtext.experimental.omni_pipeline.train_sft_omni src/maxtext/experimental/omni_06b/pretrain-omni-gemma3-qwen3-0.6b-chartnet-xpk-128-csv.yml load_parameters_path=${STAGE1_FINAL_CKPT} base_output_directory=${STAGE2_OUTPUT_DIR} run_name=${STAGE2_RUN_NAME} hf_access_token=${HF_TOKEN} scan_layers=true grain_worker_count=0 && echo '=== Stage 3: ChartQA SFT (0.6B) ===' && python3 -m maxtext.experimental.omni_pipeline.train_sft_omni src/maxtext/experimental/omni_06b/sft-omni-gemma3-qwen3-0.6b-xpk-128.yml load_parameters_path=${STAGE2_FINAL_CKPT} base_output_directory=${STAGE3_OUTPUT_DIR} run_name=${STAGE3_RUN_NAME} hf_access_token=${HF_TOKEN} scan_layers=true grain_worker_count=0"
  )

  echo ""
  echo ">>> 3-Stage Pipeline 0.6B Workload submitted: ${workload_name}"
  echo "=================================================================="
}
...
eval_sft() {
  ...
  echo "=================================================================="
  echo ">>> [EVAL] Evaluating Stage 3 SFT 0.6B Checkpoint: ${ckpt_path}"
  echo "=================================================================="
  ...
  MEGASCALE_NUM_SLICES=1 HF_TOKEN="${HF_TOKEN}" HUGGING_FACE_HUB_TOKEN="${HF_TOKEN}" python3 -m maxtext.experimental.omni_pipeline.eval_sft_omni \
    "${SCRIPT_DIR}/sft-omni-gemma3-qwen3-0.6b-xpk-128.yml" \
