...
class _ComputableMask(Mask):
  ...
  def __init__(
    ...
    if q_seq_len % (shard_count * shard_count) != 0:
      raise ValueError(
          f"Shard count squared ({shard_count * shard_count}) must" f" divide Q seq_len ({self.shape[0]}) evenly."
      )
...
class CausalMask(_ComputableMask):
  ...
  def __eq__(self, other: object):
    ...
    return (
        ...
        and np.array_equal(self.kv_sequence, other.kv_sequence)  # pyrefly: ignore[bad-argument-type]
    )
