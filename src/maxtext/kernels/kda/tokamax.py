...
def tokamax_chunk_kda(
  ...
  Args:
      ...
      implementation: tokamax backend to run — ``"mosaic"`` (Pallas TPU) or
          ``"xla"`` (tokamax's pure-JAX reference recurrence). The default is
          fixed on purpose: passing None would let tokamax pick its
          ``("mosaic", "xla")`` default, which silently falls back to the
          reference recurrence — a token-by-token triple ``fori_loop`` — when
          the Pallas kernel declines the shapes or hardware. Training would
          still be correct and unusably slow, so a fallback has to be asked
          for explicitly.
  ...
  o_h, _ = kimi_delta_attention(
      ...
      implementation=implementation,  # "mosaic" resolves to the mosaic_tpu Pallas kernel
