...
def chunk_kda(
  ...
  Args:
      ...
      implementation: which tokamax backend to run, ``"mosaic"`` (the Pallas
          TPU kernel) or ``"xla"`` (tokamax's pure-JAX reference recurrence).
          Defaults to ``"mosaic"`` and is not exposed as a config flag: asking
          tokamax to choose would let it fall back to the reference
          implementation silently, which is orders of magnitude slower. Pass
          ``"xla"`` explicitly from tests or debugging only.
