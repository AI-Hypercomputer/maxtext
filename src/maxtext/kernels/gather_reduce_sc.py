  ...
  with mlir.make_ir_context() as ctx, ir.Location.unknown():
    ...
    def _kernel_impl(
      ...
      def fill_load_offset_tile(offset_tile_local, idx_tile_local, col_pos):
        ...
        iota = arith.constant(
            ...
            ir.DenseIntElementsAttr.get(
                array.array(
                    "i", [i * mul_mem_layout for i in range(row_chunk_size)]
                ),
        ...
        loop_over_idx.attributes["sc.loop_unroll_factor"] = ir.IntegerAttr.get(
        ...
        with ir.InsertionPoint(loop_over_idx.body):
          ...
          if is_bf16:
            base_idx_rem = arith.divui(
                arith.remui(base_idx, const_lut(8, i32)), const_lut(2, i32)
            )
          else:
          ...
          loop_j.attributes["sc.loop_unroll_factor"] = ir.IntegerAttr.get(
          ...
          with ir.InsertionPoint(loop_j.body):
            ...
            idx_local = arith.addi(
                ...
                arith.muli(  # NOTYPO
                    loop_j.induction_variable, const_lut(vreg_size)
                ),
            )
            tpu.store(
                vec,
          ...
          last_full_vec = tpu.load(
              ...
              [
                  arith.subi(
                      arith.muli(  # NOTYPO
                          const_lut((col_chunk_size // 128)), i
                      ),
      ...
      def load_weights(lin_idx, dst_tile, sflag):
        ...
        """
        lin_idx = arith.divui(
            lin_idx, const_lut(16)
        )  # reading in increments of 16
      ...
      def perform_add(
        ...
        loop_pack_scratch = scf.ForOp(
            const_lut(0), const_lut(col_chunk_size), const_lut(vreg_size)
        )
        loop_pack_scratch.attributes["sc.loop_unroll_factor"] = (
            ir.IntegerAttr.get(i32, loop_unroll_factor_1)
        )
        ...
        with ir.InsertionPoint(loop_pack_scratch.body):
          ...
          def get_row_val(row_idx):
            if is_bf16:
              ...
              is_odd_scalar = arith.cmpi(
                  arith.CmpIPredicate.eq, parity_of_row, const_lut(0, i32)
              )
      ...
      def fill_out_offset_tile(offset_tile_out_local, col_pos, row_pos=None):
        ...
        if row_pos is not None:
          ...
          row_vec_offset = vector.broadcast(
              _I32[row_chunk_size], row_vec_offset
          )
        ...
        iota = arith.constant(
            ...
            ir.DenseIntElementsAttr.get(
                array.array(
                    "i", [i * 4 for i in range(row_chunk_size)]
        ...
        with ir.InsertionPoint(loop_over_idx.body):
          ...
          start_vec = arith.addi(
              vector.broadcast(
                  _I32[row_chunk_size], arith.index_cast(i32, loop_i)
              ),
              iota,
          )
          loop_j = scf.ForOp(
          ...
          with ir.InsertionPoint(loop_j.body):
            ...
            idx_local = arith.addi(
                arith.muli(  # NOTYPO
                    loop_i, const_lut((op.shape[1] // 128))
                ),
                arith.muli(  # NOTYPO
                    loop_j.induction_variable, const_lut(vreg_size)
                ),
        ...
        add_vec = vector.broadcast(
            _I32[vreg_size], const_lut(int(4 * rem_vreg * vreg_size), i32)
        )  # 4 comes from bf16
      ...
      scratch_out_0 = memref.reinterpret_cast(
          new_scratch_ref_ty, scratch_out_0, [], [], [],
          static_offsets=[0],
          static_sizes=new_scratch_shape,
          static_strides=[mem_num, 1]
      )
      ...
      scratch_out_1 = memref.reinterpret_cast(
          new_scratch_ref_ty, scratch_out_1, [], [], [],
          static_offsets=[0],
          static_sizes=new_scratch_shape,
          static_strides=[mem_num, 1]
      )
      ...
      new_input_ref_ty = ir.MemRefType.get(
          new_input_shape, bf16, memory_space=memoryspace_hbm
      )
      out_ref = tpu.reinterpret_cast(
          new_input_ref_ty, out_ref,
      )

      if topk_weights is not None:
        new_weights_shape = (topk_weights.size // 2, 2)
        new_weights_ref_ty = ir.MemRefType.get(
            new_weights_shape, bf16,
            memory_space=memoryspace_hbm
        )
        weights_ref = tpu.reinterpret_cast(
            new_weights_ref_ty, weights_ref
        )

      sflag_0 = tpu.sem_alloc(
          ir.MemRefType.get(
              (), dma_semaphore_type, memory_space=memory_space_semaphore
          )
      )
      ...
      global_chunk_to_process = arith.addi(
          arith.muli(  # NOTYPO
              current_sc_core, const_lut(num_sc_per_core, i32)
          ),
          current_local_core,
      )
      ...
      with ir.InsertionPoint(loop_col_chunk.body):
        ...
        source_slice = tpu.memref_slice(
            ir.MemRefType.get(
                (row_chunk_size,), i32, memory_space=memoryspace_hbm
            ),
            idx_ref,
        ...
        with ir.InsertionPoint(loop_over_row_chunks.body):
          ...
          tpu.wait_indirect_dma(
              semaphore=sflag_out_0, src=scratch_out_0, dst=out_ref
          )
          # wait stream #1 out
          tpu.wait_indirect_dma(
              semaphore=sflag_out_1, src=scratch_out_1, dst=out_ref
          )
        ...
        # setup stream for #1
        base_idx_val = arith.index_cast(
            i32, arith.addi(global_row_idx_start, const_lut(epi_idx_loc))
        )
        base_idx_val = tpu.assume_multiple(base_idx_val, 8)
        source_slice = tpu.memref_slice(
            ir.MemRefType.get(
    ...
    return mosaic.as_tpu_kernel(
        ...
        out_type=core.ShapedArray(
            (idx.shape[0] // reduce_group_size, op.shape[1]), jnp.bfloat16
        ),
    )(
        *(
            [
