...
def with_sequence_axis(axis_names: Any, sequence_axis: str, sequence_dim: int) -> Any:
  ...
  return context_parallel_utils.with_axis_on_dim(axis_names, sequence_axis, sequence_dim)
...
def validate_ulysses_mesh_axis(
  ...
  kv_sequence_axes = context_parallel_utils.mesh_axes_for_dim(axis_names_kv[sequence_dim_kv])
...
def validate_head_sharding(
  ...
  q_head_shards = context_parallel_utils.mesh_axes_size(mesh, q_head_axes, label="TPU Ulysses attention")
  kv_head_shards = context_parallel_utils.mesh_axes_size(mesh, kv_head_axes, label="TPU Ulysses attention")
