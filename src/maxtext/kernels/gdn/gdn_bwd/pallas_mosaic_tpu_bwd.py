...
def _bwd_gdn_pipeline_body(
  ...
  # TODO(b/562944370): Evaluate simplifying dS computation to eliminate 3 matmuls and intermediate dA VMEM buffer.
...
def _pallas_gdn_bwd_kernel_single_group(
  ...
  do_5d = do.reshape(batch_size, num_chunks, chunk_size, num_v_heads, cfg.v_head_dim)
  ...
  (
      ...
      scratch_shapes=[
          pltpu.VMEM((num_v_heads, cfg.kq_head_dim, cfg.v_head_dim), jnp.float32),
      ],
...
def pallas_gdn_bwd_kernel(
  ...
  group_dim_size = tile_q_size + tile_k_size + tile_v_heads * v_head_dim
