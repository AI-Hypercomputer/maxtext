...
def situ(
  ...
  out = beta * jnp.tanh(x_f32 / beta) * jax.nn.sigmoid(x_f32)
  return out.astype(x.dtype)


def linear_beta_tanh(
    x: jax.Array,
    linear_beta: float | None = 25.0,
) -> jax.Array:
  """Linear beta tanh activation function: linear_beta * tanh(x / linear_beta)."""
  if linear_beta is None:
    return x
  x_f32 = x.astype(jnp.float32)
  out = linear_beta * jnp.tanh(x_f32 / linear_beta)
  return out.astype(x.dtype)
...
def _convert_to_activation_function(fn_or_string: str | Callable[..., Any]) -> Callable[..., Any]:
  ...
  if fn_or_string == "linear":
    ...
    # Custom Situ activation used by Kimi K3
    return situ
  elif fn_or_string == "linear_beta_tanh":
    # Custom Linear Beta Tanh activation used by Kimi K3
    return linear_beta_tanh
