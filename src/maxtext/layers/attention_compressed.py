...
def csa_overlap_pooling(chunk_kv, chunk_gate, kv_norm, head_dim, prior_kv, prior_gate):
  ...
  Implements overlapping Ca/Cb pooling logic shared by both the CSA Compressor
  and the CSA Indexer.

  Args:
    chunk_kv: Input KV projection chunks.
    chunk_gate: Input gate projection chunks.
  ...
  def __call__(
    ...
    if model_mode == MODEL_MODE_AUTOREGRESSIVE and cache is not None:
      ...
      def indexer_compressor_fn(buf_kv, buf_gate):
        ...
        return (
            ...
            jnp.transpose(next_prior_kv, (0, 2, 1, 3)),
            jnp.transpose(next_prior_gate, (0, 2, 1, 3)),
        )
    ...
    # --- PREFILL CHUNKING & PRIMING ---
      ...
      if chunk_kv.shape[1] > 0:
        ...
        chunk_gate_reshaped = (
            chunk_gate.reshape((batch_size, n_windows, self.compress_rate, -1)) + self.position_bias.value
        )
