...
class Compressor(nnx.Module):
  ...
  def __call__(self, x, return_intermediates: bool = False):
    ...
    if self.overlap:
      ...
      # 8. Weighted Reduction
      # Multiply weights by the overlapped KV and reduce across the '2 * Ratio' dimension
      # Shape before sum: [Batch, Chunks, 2 * Ratio, compressed_dim]
      # Shape after sum: [Batch, Chunks, compressed_dim]
      compressed_x_prenorm = jnp.sum(weights * kv_overlapped, axis=2)
    ...
    # 5. Reduction (Weighted Sum)
    # Multiply the weights by the KV representation and sum across the 'Ratio' dimension
    # Shape before sum: [Batch, Chunks, Ratio, compressed_dim]
    # Shape after sum: [Batch, Chunks, compressed_dim]
    compressed_x_prenorm = jnp.sum(weights * x_kv_chunked, axis=2)
