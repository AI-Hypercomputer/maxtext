...
class Indexer(nnx.Module):
    ...
    def update_indexer_cache(
        self, kv_cache, k, decoder_segment_ids, model_mode, previous_chunk
    ):
    ...
    def apply_partial_rope(
        ...
        1. Split Order: Indexer splits the head dimension into [rope, nope], whereas
        MLA uses [nope, rope].
        2. Input Layout: Indexer uses concatenated layout (interleave=False),
        whereas MLA uses interleaved (interleave=True).

        Args:
          inputs: Input array of shape [batch, seqlen, indexer_n_heads,
            indexer_head_dim].
          positions: Position array of shape [batch, seqlen].

        Returns:
          Array with partial RoPE applied, with shape [batch, seqlen,
    ...
    def generate_mask(self, indexer_score: Array, topk_values: Array) -> Array:
        ...
        Args:
            indexer_score: [b, t, s] float - The computed relevance scores for all
              tokens.
            topk_values: [b, t, k] float - The top-k selected score values from
              jax.lax.top_k.

        Returns:
            mask: [b, t, s] - `0.0` for top-k selected elements,
            `DEFAULT_MASK_VALUE` elsewhere.
        """
    ...
    def __call__(
        ...
        This uses a ReLU-based similarity for QK with MQA-style broadcasting (shared
        K).
        It uses weighted aggregation over heads to produce a single score per token
        pair.
        ...
        Args:
          ...
          low_rank_q: Low-rank latent query representations of shape [b, t,
            q_lora_rank].
          inputs_kv: Input of shape [b, s, embed_dim], same as inputs_q
          inputs_positions: Position indices of shape [b, s].
          attention_mask: Optional attention mask of shape [b, t, s]. Positions with
            `0.0` allow attention, while positions with `DEFAULT_MASK_VALUE` (a
            large negative number) prevent it. Returns `None` if no masking is
            determined to be necessary based on the inputs and configuration.
        ...
        if model_mode != MODEL_MODE_TRAIN:
            k_cached, cached_s = self.update_indexer_cache(
                kv_cache, k, decoder_segment_ids, model_mode, previous_chunk
            )
        ...
        # Compute Index Scores safely by chunking the 'heads' dimension to prevent OOM
        ...
        if 0 < head_chunk_size <= h and h % head_chunk_size == 0:
            ...
            def scan_body_indexer(carry, xs):
                ...
                logits = jnp.einsum(
                    "bthd, bsd -> btsh", q_c, k, precision=self.config.matmul_precision
                )
            ...
            logits = jnp.einsum(
                "bthd, bsd -> btsh", q, k, precision=self.config.matmul_precision
            )
            logits = jax.nn.relu(logits)
            indexer_score = jnp.einsum(
                "btsh, bth -> bts",
                logits,
                weights,
                precision=self.config.matmul_precision,
        ...
        if self.config.indexer_use_approx_top_k:
            ...
            topk_values, topk_indices = jax.lax.top_k(
                indexer_score, k=self.indexer_topk
            )  # [b, t, k]
...
def mla_as_linen(
    *,
    ...
    prefill_input_axis_names: AxisNames = (
        PREFILL_KV_BATCH,
        PREFILL_LENGTH,
        EMBED,
    ),
    ...
    decode_out_axis_names: AxisNames = (
        DECODE_BATCH,
        DECODE_LENGTH,
        HEAD,
        D_KV,
...
class MLA(Attention):
    ...
    def __init__(
        ...
        Args:
          config: The model configuration. ... and other configuration parameters
            for MLA attention.
        ...
        if self.use_indexer:
            ...
            self.IndexerKVCache_0 = (
                self.init_indexer_cache(inputs_kv_shape)
                if model_mode != MODEL_MODE_TRAIN
                else None
            )
        else:
    ...
    def mla_query_projection(
        ...
        if self.q_lora_rank == 0:
            ...
            # LoRA path
            low_rank_q = self.wq_a(
                inputs_q, out_sharding=wqa_out_sharding
            )  # [B, L, q_lora_rank]
            ...
            q = self.wq_b(
                low_rank_q, out_sharding=query_sharding
            )  # [B, L, n_heads, qk_head_dim]
    ...
    def update_mla_kv_caches(
        ...
        Returns:
          ...
          - The autoregressive key-value cache, reconstructed from the MLA cache, or
          None.
        """
    ...
    def mla_kv_projection(
        ...
        key_rope = self.apply_rotary_embedding(
            key_rope, inputs_positions=inputs_positions
        )
        ...
        if self.config.attention != "paged" and model_mode != MODEL_MODE_TRAIN:
            if self.config.mla_naive_kvcache:
                cached_values = self.update_kv_caches(
                    key, value, decoder_segment_ids, model_mode, previous_chunk
                )
            else:
    ...
    def calculate_indexer_loss(
        ...
        This loss trains the indexer to predict which tokens are important by
        matching
        ...
        if 0 < head_chunk_size <= heads and heads % head_chunk_size == 0:
            ...
            q_h = query.transpose(2, 0, 1, 3).reshape(
                num_chunks, head_chunk_size, batch, q_len, dim
            )
            k_h = key.transpose(2, 0, 1, 3).reshape(
                num_chunks, head_chunk_size, batch, key.shape[1], dim
            )
            ...
            attention_probs, _ = jax.lax.scan(
                jax.checkpoint(scan_body_heads), init_probs, {"q": q_h, "k": k_h}
            )
            ...
            attention_probs = jnp.sum(
                jax.nn.softmax(attention_scores.astype(jnp.float32), axis=-1), axis=1
            )
            attention_probs = jax.lax.optimization_barrier(attention_probs)
        # L1 normalize aggregated target distribution
        attention_probs = attention_probs / (
            jnp.sum(attention_probs, axis=-1, keepdims=True) + EPS
        )
        ...
        *,
        ...
        if model_mode == MODEL_MODE_PREFILL:
            inputs_q = self._maybe_shard_with_logical(
                inputs_q, self.prefill_input_axis_names
            )
            inputs_kv = self._maybe_shard_with_logical(
                inputs_kv, self.prefill_input_axis_names
            )
        ...
        query, low_rank_q = self.mla_query_projection(
            inputs_q, inputs_positions, model_mode
        )
        ...
        if self.use_indexer:
            ...
            if (
                indexer_mask is not None
                and self.config.indexer_loss_scaling_factor > 0.0
            ):
