...
try:
    ...
    class MockLayer:
        ...
        def __call__(self, *args, **kwargs): return args[0] if args else jnp.zeros((1,))

    nnx = types.SimpleNamespace()
    nnx.Module = object
    nnx.Rngs = object
    nnx.Intermediate = object
    nnx.LayerNorm = MockLayer
...
try:
    ...
    DType = Any
    EMBED = "embed"
    EP_AS_CONTEXT = "ep_as_context"
    HEAD = "head"
    Q_LORA_UP_PROJ = "q_lora"
    KV_BATCH = "batch"
    KV_BATCH_NO_EXP = "batch"
    KV_HEAD = "head"
    KV_HEAD_DIM = "head_dim"
    KV_LORA_UP_PROJ = "kv_lora"
    LENGTH = "length"
    LENGTH_NO_EXP = "length"
    MODEL_MODE_PREFILL = "prefill"
    MODEL_MODE_TRAIN = "train"
    PREFILL_KV_BATCH = "batch"
...
try:
    ...
    class MockClass:
        def __init__(self, *args, **kwargs): pass
        def __call__(self, *args, **kwargs): return jnp.zeros((1,))
    
    nnx_wrappers = types.SimpleNamespace()
    nnx_wrappers.to_linen = lambda *args, **kwargs: None
    
    class MockAttention:
        def __init__(self, *args, **kwargs): pass
        def __call__(self, *args, **kwargs): return jnp.zeros((1,))
        def generate_attention_mask(self, *args, **kwargs): return None
    Attention = MockAttention
    
    nd_dense_init = lambda *args, **kwargs: lambda *a, **k: None
    NdInitializer = object
    variable_to_logically_partitioned = lambda *args, **kwargs: None
    DenseGeneral = MockClass
    RMSNorm = MockClass
    Quant = object
...
try:
    ...
    kvcache.MlaKVCache = lambda *args, **kwargs: lambda *a, **k: (None, None)
    page_manager = types.SimpleNamespace()
    page_manager.PageState = object
    paged_attention = types.SimpleNamespace()
    paged_attention.PagedAttentionOp = lambda *args, **kwargs: lambda *a, **k: (jnp.zeros((1,)), None, None)
    KVQuant = object
...
try:
    ...
    create_sharding = lambda *args, **kwargs: None
    EPS = 1e-9


def kernel(q_ref, k_ref, w_ref, mask_ref, o_score_ref, 
           k_scratch_ref, mask_scratch_ref, score_scratch_ref, 
           sem_k_0, sem_k_1, sem_mask_0, sem_mask_1, sem_score_0, sem_score_1, *, 
           has_mask, bS):
    """Pallas kernel for Indexer score computation with double buffering.
    ...
    Args:
        ...
        k_scratch_ref: Scratch buffer for K blocks. Shape: (2, bS, D). Memory: VMEM.
        mask_scratch_ref: Scratch buffer for Mask blocks. Shape: (2, bT, bS). Memory: VMEM.
        score_scratch_ref: Scratch buffer for Score blocks. Shape: (2, bT, bS). Memory: VMEM.
        sem_k_0, sem_k_1: Semaphores for K double buffering.
        sem_mask_0, sem_mask_1: Semaphores for Mask double buffering.
        sem_score_0, sem_score_1: Semaphores for Score double buffering.
    ...
    if has_mask:
        # Load Mask[0]
        pltpu.make_async_copy(
            mask_ref.at[b_idx, pl.ds(t_start, bT), pl.ds(0, bS)],
            mask_scratch_ref.at[0],
            sem_mask_0
        ).start()
    ...
    def body(i, _):
        ...
        # Select semaphores for current and next buffers
        sem_k_curr = jax.lax.cond(curr == 0, lambda: sem_k_0, lambda: sem_k_1)
        sem_k_next = jax.lax.cond(next_idx == 0, lambda: sem_k_0, lambda: sem_k_1)
        
        sem_mask_curr = jax.lax.cond(curr == 0, lambda: sem_mask_0, lambda: sem_mask_1)
        sem_mask_next = jax.lax.cond(next_idx == 0, lambda: sem_mask_0, lambda: sem_mask_1)
        
        sem_score_curr = jax.lax.cond(curr == 0, lambda: sem_score_0, lambda: sem_score_1)
        
        # 1. Wait for inputs for current iteration
        # We wait for the semaphore to reach (i // 2) + 1
        wait_val = (i // 2) + 1
        pltpu.semaphore_wait(sem_k_curr, wait_val)
        if has_mask:
            pltpu.semaphore_wait(sem_mask_curr, wait_val)
            
        # 2. Wait for output buffer availability
        # Ensure previous write from this buffer is done before we overwrite it.
        # The previous write was at step i-2, signaled sem_score_curr to i // 2.
        if i >= 2:
            pltpu.semaphore_wait(sem_score_curr, i // 2)
            
        # 3. Prefetch next block
        @pl.when(i < num_blocks - 1)
        def _prefetch():
            s_next = (i + 1) * bS
            # Load K[next]
            pltpu.make_async_copy(
                k_ref.at[b_idx, pl.ds(s_next, bS), :],
                k_scratch_ref.at[next_idx],
                sem_k_next
            ).start()
            ...
            if has_mask:
                # Load Mask[next]
                pltpu.make_async_copy(
                    mask_ref.at[b_idx, pl.ds(t_start, bT), pl.ds(s_next, bS)],
                    mask_scratch_ref.at[next_idx],
                    sem_mask_next
                ).start()
                
        # 4. Compute
        k_block = k_scratch_ref.at[curr] # Shape: (bS, D)
        ...
        if has_mask:
            mask_block = mask_scratch_ref.at[curr]
            score_block = score_block + mask_block
            
        # Store result in scratch
        score_scratch_ref.at[curr][...] = score_block
        
        # 5. Start writing output
        # Copy Score[curr] -> HBM
        pltpu.make_async_copy(
            score_scratch_ref.at[curr],
    ...
    # Epilogue: Wait for outstanding writes to ensure data consistency
    pltpu.semaphore_wait(sem_score_0, (num_blocks + 1) // 2)
    pltpu.semaphore_wait(sem_score_1, num_blocks // 2)


def backward_qw_kernel(q_ref, w_ref, k_ref, d_score_ref, d_q_ref, d_w_ref,
                       k_scratch_ref, d_score_scratch_ref, 
                       sem_k_0, sem_k_1, sem_ds_0, sem_ds_1, *,
                       bS):
    """Pallas kernel for d_q and d_w computation with double buffering.
    ...
    Args:
        ...
        k_scratch_ref: Scratch buffer for K blocks. Shape: (2, bS, D_padded). Memory: VMEM.
        d_score_scratch_ref: Scratch buffer for d_score blocks. Shape: (2, bT, bS). Memory: VMEM.
        sem_k_0, sem_k_1: Semaphores for K.
        sem_ds_0, sem_ds_1: Semaphores for d_score.
        bS: Block size for S dimension loop.
    """
    # Grid indices
    b_idx = pl.program_id(0)
    ...
    # Load d_score[0]
    pltpu.make_async_copy(
        d_score_ref.at[b_idx, pl.ds(t_start, bT), pl.ds(0, bS)],
        d_score_scratch_ref.at[0],
        sem_ds_0
    ).start()
    ...
    def body(i, _):
        ...
        sem_ds_curr = jax.lax.cond(curr == 0, lambda: sem_ds_0, lambda: sem_ds_1)
        sem_ds_next = jax.lax.cond(next_idx == 0, lambda: sem_ds_0, lambda: sem_ds_1)
        
        # 1. Wait for inputs
        wait_val = (i // 2) + 1
        pltpu.semaphore_wait(sem_k_curr, wait_val)
        pltpu.semaphore_wait(sem_ds_curr, wait_val)
        ...
        def _prefetch():
            ...
            pltpu.make_async_copy(
                d_score_ref.at[b_idx, pl.ds(t_start, bT), pl.ds(s_next, bS)],
                d_score_scratch_ref.at[next_idx],
                sem_ds_next
            ).start()
            
        # 3. Compute
        k_block = k_scratch_ref.at[curr] # (bS, D_padded)
        d_score_block = d_score_scratch_ref.at[curr] # (bT, bS)
...
def backward_k_kernel(k_ref, q_ref, w_ref, d_score_ref, d_k_ref,
                      q_scratch_ref, w_scratch_ref, d_score_scratch_ref, 
                      sem_q_0, sem_q_1, sem_w_0, sem_w_1, sem_ds_0, sem_ds_1, *,
                      bT):
    """Pallas kernel for d_k computation with double buffering.
    ...
    Args:
        ...
        q_scratch_ref: Scratch buffer for Q blocks. Shape: (2, bT, H, D_padded). Memory: VMEM.
        w_scratch_ref: Scratch buffer for W blocks. Shape: (2, bT, H_padded). Memory: VMEM.
        d_score_scratch_ref: Scratch buffer for d_score blocks. Shape: (2, bT, bS). Memory: VMEM.
        sem_q_0, sem_q_1: Semaphores for Q.
        sem_w_0, sem_w_1: Semaphores for W.
        sem_ds_0, sem_ds_1: Semaphores for d_score.
    ...
    # Load Q[0]
    pltpu.make_async_copy(
        q_ref.at[b_idx, pl.ds(0, bT), :, :],
        q_scratch_ref.at[0],
        sem_q_0
    ).start()
    # Load W[0]
    pltpu.make_async_copy(
        w_ref.at[b_idx, pl.ds(0, bT), :],
        w_scratch_ref.at[0],
        sem_w_0
    ).start()
    # Load d_score[0]
    pltpu.make_async_copy(
        d_score_ref.at[b_idx, pl.ds(0, bT), pl.ds(s_start, bS)],
        d_score_scratch_ref.at[0],
        sem_ds_0
    ).start()
    ...
    def body(i, _):
        ...
        sem_q_curr = jax.lax.cond(curr == 0, lambda: sem_q_0, lambda: sem_q_1)
        sem_q_next = jax.lax.cond(next_idx == 0, lambda: sem_q_0, lambda: sem_q_1)
        
        sem_w_curr = jax.lax.cond(curr == 0, lambda: sem_w_0, lambda: sem_w_1)
        sem_w_next = jax.lax.cond(next_idx == 0, lambda: sem_w_0, lambda: sem_w_1)
        ...
        # 1. Wait for inputs
        wait_val = (i // 2) + 1
        pltpu.semaphore_wait(sem_q_curr, wait_val)
        pltpu.semaphore_wait(sem_w_curr, wait_val)
        pltpu.semaphore_wait(sem_ds_curr, wait_val)
        ...
        def _prefetch():
            t_next = (i + 1) * bT
            pltpu.make_async_copy(
                q_ref.at[b_idx, pl.ds(t_next, bT), :, :],
                q_scratch_ref.at[next_idx],
                sem_q_next
            ).start()
            pltpu.make_async_copy(
                w_ref.at[b_idx, pl.ds(t_next, bT), :],
                w_scratch_ref.at[next_idx],
                sem_w_next
            ).start()
            pltpu.make_async_copy(
                d_score_ref.at[b_idx, pl.ds(t_next, bT), pl.ds(s_start, bS)],
                d_score_scratch_ref.at[next_idx],
                sem_ds_next
            ).start()
            
        # 3. Compute
        q_block = q_scratch_ref.at[curr] # (bT, H, D_padded)
        w_block_padded = w_scratch_ref.at[curr] # (bT, H_padded)
...
def backward_computation(q: jnp.ndarray, k: jnp.ndarray, w: jnp.ndarray, d_score: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    ...
    q_spec = pl.BlockSpec((None, bT, H, D_padded), lambda b, t: (b, t, 0, 0), memory_space=pltpu.VMEM)
    w_spec = pl.BlockSpec((None, bT, H_padded_w), lambda b, t: (b, t, 0), memory_space=pltpu.VMEM)
    k_spec_any = pl.BlockSpec(memory_space=None)
    d_score_spec_any = pl.BlockSpec(memory_space=None)
    
    # Outputs in VMEM (accumulators)
    d_q_spec = pl.BlockSpec((None, bT, H, D_padded), lambda b, t: (b, t, 0, 0), memory_space=pltpu.VMEM)
    d_w_spec = pl.BlockSpec((None, bT, H_padded_w), lambda b, t: (b, t, 0), memory_space=pltpu.VMEM)
    
    scratch_shapes_qw = [
        pltpu.VMEM((2, bS, D_padded), k.dtype),    # k_scratch
        pltpu.VMEM((2, bT, bS), d_score.dtype),    # d_score_scratch
        pltpu.SemaphoreType.DMA, # sem_k_0
        pltpu.SemaphoreType.DMA, # sem_k_1
        pltpu.SemaphoreType.DMA, # sem_ds_0
        pltpu.SemaphoreType.DMA, # sem_ds_1
    ]
    ...
    k_spec = pl.BlockSpec((None, bS, D_padded), lambda b, s: (b, s, 0), memory_space=pltpu.VMEM)
    q_spec_any = pl.BlockSpec(memory_space=None)
    w_spec_any = pl.BlockSpec(memory_space=None)
    # d_score_spec_any reused
    
    # Output in VMEM (accumulator)
    d_k_spec = pl.BlockSpec((None, bS, D_padded), lambda b, s: (b, s, 0), memory_space=pltpu.VMEM)
    
    scratch_shapes_k = [
        pltpu.VMEM((2, bT, H, D_padded), q.dtype), # q_scratch
        pltpu.VMEM((2, bT, H_padded_w), w.dtype),  # w_scratch (padded)
        pltpu.VMEM((2, bT, bS), d_score.dtype),    # d_score_scratch
        pltpu.SemaphoreType.DMA, # sem_q_0
        pltpu.SemaphoreType.DMA, # sem_q_1
        pltpu.SemaphoreType.DMA, # sem_w_0
        pltpu.SemaphoreType.DMA, # sem_w_1
        pltpu.SemaphoreType.DMA, # sem_ds_0
        pltpu.SemaphoreType.DMA, # sem_ds_1
    ]
...
class Indexer(nnx.Module):
  ...
  def kernel(self, q_ref, k_ref, w_ref, mask_ref, o_score_ref, 
            ...
            has_mask, bS):
      """Pallas kernel for Indexer score computation with double buffering."""
      # See implementation in module level function 'kernel'
      return kernel(q_ref, k_ref, w_ref, mask_ref, o_score_ref, 
                    k_scratch_ref, mask_scratch_ref, score_scratch_ref, 
                    sem_k_0, sem_k_1, sem_mask_0, sem_mask_1, sem_score_0, sem_score_1, 
                    has_mask=has_mask, bS=bS)
  ...
  def _computation_impl(self, q: jnp.ndarray, k: jnp.ndarray, w: jnp.ndarray, mask: Optional[jnp.ndarray], top_k: int) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
      ...
      w_spec = pl.BlockSpec((None, bT, H), lambda b, t: (b, t, 0), memory_space=pltpu.VMEM)
      ...
      scratch_shapes = [
          pltpu.VMEM((2, bS, D_padded), k.dtype),       # k_scratch
          pltpu.VMEM((2, bT, bS), jnp.float32),  # mask_scratch (assume float32)
          pltpu.VMEM((2, bT, bS), jnp.float32),  # score_scratch
          pltpu.SemaphoreType.DMA, # sem_k_0
          pltpu.SemaphoreType.DMA, # sem_k_1
          pltpu.SemaphoreType.DMA, # sem_mask_0
          pltpu.SemaphoreType.DMA, # sem_mask_1
          pltpu.SemaphoreType.DMA, # sem_score_0
          pltpu.SemaphoreType.DMA, # sem_score_1
      ]
