...
class NNXDecoder(nnx.Module):
  ...
  def __init__(
    ...
    if self.config.scan_layers:
      if self.is_deepseek:
        ...
        # The DeepSeek-V4 architecture implements a 3-Tier Split Scanning strategy.
        # This structure partitions the decoder layers to cleanly accommodate:
        # 1. Early prefix layers containing unique prefix MoE Hash Routing gate parameters (e.g., `gate.tid2eid`)
        #    which must reside outside JAX scan bounds due to non-uniform static routing metadata.
        # 2. Inhomogeneous compression rates and alternating attention topologies (CSA vs. HCA) across layers.
        # 3. Dynamic scan grouping of length 2 periodic cycle blocks to minimize compilation memory and overhead.
        #
        # The decoder stack is segmented as follows:
        # - Tier 1: Unrolled Prefix Segment (`pre_layers`) - Executes early hash layers containing
        #   static routing parameters.
        # - Tier 2: Middle Scanned blocks (`layers`) - Scanned in periodic cycles of 2 layers via
        #   `jax.lax.scan` for memory footprint savings.
        # - Tier 3: Unrolled Suffix Remainder (`post_layers`) - Executes any trailing layers that
        #   do not align with a full period cycle.
        scan_length = (config.num_decoder_layers - config.num_hash_layers) // 2
        num_remaining_layers = (config.num_decoder_layers - config.num_hash_layers) % 2
        layer_kwargs = {"num_of_layers": 2, "layer_offset": config.num_hash_layers}

        rem_layer_kwargs = {
            "num_of_layers": num_remaining_layers,
            "layer_offset": config.num_hash_layers + scan_length * 2,
        }

        RemattedDeepSeekV4Block = deepseek_v4.DeepSeekV4ScannableBlock

        if config.num_hash_layers > 0:
          self.pre_layers = RemattedDeepSeekV4Block(
              config=self.config,
              mesh=mesh,
              quant=self.quant,
              model_mode=self.model_mode,
              num_of_layers=config.num_hash_layers,
              layer_offset=0,
              rngs=rngs,
          )
        else:
        ...
        if num_remaining_layers > 0:
          self.post_layers = RemattedDeepSeekV4Block(
              config=self.config,
              mesh=mesh,
              quant=self.quant,
              model_mode=self.model_mode,
  ...
  def _apply_deepseek_v4_scanned_blocks(
    ...
    # Initial input tensor shape at entrance:
    # y: [B, S, D] where B = Batch size, S = Sequence length, D = Embedding dimension
    cfg = self.config
    scan_length = (cfg.num_decoder_layers - cfg.num_hash_layers) // 2
    ...
    # Apply the boundary prefix unrolled layers
    # Input shape: [B, S, D]
    # Output shape: [B, S, D]
    if cfg.num_hash_layers > 0:
      ...
      out_tuple = self._apply_layer_with_remat(self.pre_layers, y, policy, prevent_cse, **call_kwargs)
      y = out_tuple[0]  # Unrolled prefix execution: [B, S, D] -> [B, S, D]

    # Apply the main scan over the full blocks
    # This tier scans periodic cycles of block length 2 (CSA + HCA layers).
    # Input shape entering scan: [B, S, D]
    # Output shape exiting scan: [B, S, D] (via 20 step scans of length 2 cycle blocks)
    if scan_length > 0:
      y, self.layers, _ = self._apply_layers_sequentially(self.layers, y, *layer_args, length=scan_length, **layer_kwargs)

    # Apply any remaining layers that did not fit into a full scanned block
    # Input shape entering remainder tier: [B, S, D]
    # Output shape exiting remainder tier: [B, S, D] if remaining layers exist.
    ...
    if num_remaining_layers > 0:
      ...
      out_tuple = self._apply_layer_with_remat(self.post_layers, y, policy, prevent_cse, **call_kwargs)
      y = out_tuple[0]  # Remainder suffix execution: [B, S, D] -> [B, S, D]

    # Final return tensor shape:
    # y: [B, S, D]
    return y
