...
from maxtext.common.common_types import (ATTN_EMBED, ATTN_LENGTH, BATCH_ATTN, D_KV, DECODE_BATCH, DECODE_LENGTH, HEAD,
                                         KV_BATCH, KV_HEAD, KV_HEAD_DIM, MODEL_MODE_AUTOREGRESSIVE, MODEL_MODE_PREFILL,
                                         MODEL_MODE_TRAIN, PREFILL_KV_BATCH, PREFILL_LENGTH, Array, AttentionType, AxisIdxes,
                                         AxisNames, Config, DecoderBlockType, DType)
from maxtext.inference import kvcache
from maxtext.inference.kvcache import KVQuant
from maxtext.layers import nnx_wrappers
from maxtext.layers.attention_op import AttentionOp, _resolve_attention_type
from maxtext.layers.embeddings import (Gemma4PartialRotaryEmbedding, LLaMARotaryEmbedding, LlamaVisionRotaryEmbedding,
                                       PartialRotaryEmbedding, Qwen3OmniMoeThinkerTextRotaryEmbedding,
                                       Qwen3OmniMoeVisionRotaryEmbedding, RotaryEmbedding, YarnRotaryEmbedding)
...
from maxtext.layers.normalizations import GlobalRMSNorm, Qwen3NextRMSNorm, RMSNorm
from maxtext.layers.quantizations import AqtQuantization as Quant
from maxtext.utils.sharding import create_sharding, logical_to_mesh_axes, maybe_shard_with_logical
...
def l2_norm_as_linen(self, eps: float = 1e-6):
    ...
    return nnx_wrappers.to_linen(
        L2Norm, eps=eps, metadata_fn=variable_to_logically_partitioned
    )
...
class Attention(nnx.Module):
    ...
    def __init__(
        ...
        self.rope_max_timescale = (
            rope_max_timescale
            if rope_max_timescale is not None
        ...
        self.is_qwen3_hybrid = (
            ...
            in (DecoderBlockType.QWEN3_NEXT, DecoderBlockType.QWEN3_5)
    ...
    def query_projection(
        self, inputs_q: Array, out_sharding: NamedSharding | None = None
    ) -> Array:
        """Query projection."""
    ...
    def qkv_projection(
        self, inputs: Array, proj_name: str, out_sharding: NamedSharding | None = None
    ):
        ...
        @jax.shard_map(
            mesh=self.mesh, in_specs=(q_bshd,), out_specs=(q_bshd, k_bshd, v_bshd)
        )
        def split_qkv(qkv_proj: Array) -> tuple[Array, Array, Array]:
            num_local_heads = qkv_proj.shape[2]
            num_query_heads = (num_local_heads * self.num_query_heads) // (
                self.num_query_heads + 2 * self.num_kv_heads
            )
            ...
            return tuple(
                jnp.split(
                    qkv_proj, [num_query_heads, num_query_heads + num_kv_heads], axis=2
                )
            )
    ...
    def out_projection(
        self, out: Array, out_sharding: NamedSharding | None = None
    ) -> Array:
        """out projection"""
    ...
    def compute_shared_kv(
        ...
        if self.share_kv_layer:
            raise ValueError(
                "compute_shared_kv cannot be called on a share_kv_layer=True layer."
            )
        if self.config.fused_qkv:
        ...
        value = (
            ...
            else self.kv_projection(
                inputs_kv, proj_name="value", out_sharding=qkv_sharding
            )
        )
        is_llama4_decoder_block = self.config.decoder_block == DecoderBlockType.LLAMA4
        ...
        if not self.is_nope_layer:
            key = self.apply_rotary_embedding(
                key, inputs_positions=inputs_positions, rope_kwargs=rope_kwargs
            )
    ...
    def init_rotary_embedding(self):
        ...
        if self.is_vision:
            if self.config.model_name.startswith("qwen3"):
                ...
                raise ValueError(
                    f"Unsupported model type for vision rotary embedding: {self.config.model_name}"
                )
        ...
        elif self.config.model_name.startswith("llama3.1") or rope_type.startswith(
            "llama3.1"
        ):
            ...
            self.partial_rotary_factor is not None and self.partial_rotary_factor < 1.0
        ):
            ...
            if (
                ...
                and self.config.local_rope_max_timescale > 0
            ):
            ...
            if (
                self.config.model_name.startswith("gemma3")
    ...
    def update_kv_caches(
        self, key, value, decoder_segment_ids, model_mode, previous_chunk
    ):
    ...
    def __call__(
        ...
        *,
        ...
        if self.share_kv_layer and not use_shared_kv:
            raise ValueError(
                "share_kv_layer=True requires both shared_key and shared_value to be provided."
            )
        if use_shared_kv and self.config.fused_qkv:
            raise ValueError(
                "shared_key / shared_value are incompatible with fused_qkv."
            )
        ...
        if self.config.fused_qkv:
            ...
            query = self.query_projection(inputs_q, out_sharding=qkv_sharding)
            key = self.kv_projection(
                inputs_kv, proj_name="key", out_sharding=qkv_sharding
            )
            if self.share_kv_projections:
                value = key
            else:
                value = self.kv_projection(
                    inputs_kv, proj_name="value", out_sharding=qkv_sharding
                )
        ...
        if self.is_qwen3_hybrid:
            ...
            gate = gate.reshape(
                batch_size, seq_len, self.config.num_query_heads * self.config.head_dim
            )
        ...
        if use_rope:
            query = self.apply_rotary_embedding(
                query, inputs_positions=inputs_positions, rope_kwargs=rope_kwargs
            )
            if not use_shared_kv:
                key = self.apply_rotary_embedding(
                    key, inputs_positions=inputs_positions, rope_kwargs=rope_kwargs
        ...
        if self.temperature_tuning and not use_rope:
            attn_scales = (
                jnp.log(
                    jnp.floor(
                        (inputs_positions.astype(self.dtype) + 1.0)
            ...
            query = (query * attn_scales[:, :, jnp.newaxis, jnp.newaxis]).astype(
                self.dtype
            )
        ...
        if (
            self.config.attention in ("vllm_rpa", "vllm_batched_rpa")
