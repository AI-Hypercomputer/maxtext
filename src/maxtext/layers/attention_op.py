...
class DeepSeekV4AsymmetricMask(splash_attention_mask._ComputableMask):  # pylint: disable=protected-access
  """Pallas-optimized asymmetric mask for DeepSeek-V4 dual-branch attention pre-training.

  Instructs the Splash/Tokamax attention TPU kernels how to block SRAM memory tiles.
  First `seq_comp` keys (compressed history) are fully visible, while the remaining
  local keys are causally local-sliding masked.
  """
  ...
  def __init__(
    ...
    self.seq_comp = seq_comp

    def deepseek_v4_mask_function(q_ids, kv_ids):
      """Computes the block-level boolean mask for Pallas SRAM tiling.

      Instead of materializing a massive sequence-length boolean matrix in global memory,
      this function is evaluated on-the-fly for each block/tile of query and key indices
      loaded into TPU SRAM during the FlashAttention forward pass.

      Args:
          q_ids: Query indices array representing positions in the local sequence: [seq_local]
          kv_ids: Key/Value indices array representing positions in the concatenated
                 history: [seq_comp + seq_local]
      """
      if q_ids.size == 0 or kv_ids.size == 0:
        return np.empty((q_ids.shape[0], kv_ids.shape[1]), dtype=np.bool_)

      # Coordinate Condition 1: Compressed past history is fully attendable.
      # If a key index lies within the first `seq_comp` columns, it represents apooled historical
      # token generated by the outer Compressor. Local queries can fully attend to these without causal blocking.
      is_comp = kv_ids < self.seq_comp

      # Coordinate Condition 2: Local sliding history is causally masked.
      # If a key index lies beyond `seq_comp`, it represents a local token in the current window.
      # Queries can only attend causally to past and present local tokens, satisfying:
      # query_index >= (key_index - seq_comp offset).
      is_causal = q_ids >= (kv_ids - self.seq_comp)

      # Combine: a local query can attend if the key is compressed history OR causal local.
      return is_comp | is_causal

    super().__init__(
        shape=shape,
        mask_function=deepseek_v4_mask_function,
        shard_count=shard_count,
    )

  def materialize(self) -> jnp.ndarray:
    """Fallback to materialize dense float mask for golden parity testing."""
    mask_comp = jnp.zeros((self.seq_local, self.seq_comp), dtype=jnp.float32)
    mask_local = jnp.triu(jnp.full((self.seq_local, self.seq_local), -jnp.inf, dtype=jnp.float32), k=1)
    return jnp.concatenate([mask_comp, mask_local], axis=1)
...
def deepseek_v4_attention_forward(q_local, k_local, v_local, k_compressed, v_compressed, cos_unrotate, sin_unrotate):
  ...
  # 2. Instantiate Pallas computable mask and materialize it for dot product computation
  mask_obj = DeepSeekV4AsymmetricMask(shape=(S_local, S_comp + S_local), seq_local=S_local, seq_comp=S_comp)
  attn_mask = mask_obj.materialize()[None, None, :, :]  # [1, 1, S_local, S_comp + S_local]
