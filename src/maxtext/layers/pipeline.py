...
class PipelineBaseLinen(nn.Module, PipelineSharedMixin):
  """Base module that implements shared pipelining logic across stages for Linen."""
  ...
  def get_weight_sharding(self, *init_args):
    ...
    return {"params": partition_spec_with_extra_layer["params"]["layers"]}
...
class Pipeline(PipelineBaseLinen):
  ...
  def run_one_iteration(
    ...
    return self.advance_circular_buffers(stages_output, loop_state)
...
class CircularPipeline(PipelineBaseLinen):
  ...
  *current* repeat is executing.
  ...
  def init_states(self, inputs):
    ...
    """
    init_loop_state = super().init_states(inputs)
  ...
  def gather_microbatch_inputs_vmap(self, xs, ids, ids_dim):
    ...
    xs = jnp.asarray(xs)  # Safe casting for non-JAX arrays
    ndim = xs.ndim
  ...
  def get_current_weights_from_bsw(self, bsw, loop_iteration, physical_partition_spec, is_initializing=None):
    ...
    return meta.remove_axis(weights, 0, circular_metadata_params)
  ...
  def __call__(
    ...
    def run_iteration_scannable(model, loop_state, bsw, weights):
      ...
      return (new_loop_state, bsw, weights), None
...
def create_pipeline(
    config: Config, layers: nn.Module, mesh: Mesh, remat_policy: Any = None
