...
def unscan_layers(state: Any, num_layers: int, layer_container: str = "layers") -> Any:
  """Splits `state`'s scanned `layer_container` axis into per-layer entries.

  Args:
    state: An `nnx.State` (or any pytree exposing `to_pure_dict`/`to_dict`,
      or a plain nested dict) of MaxText params, scanned along axis 1 under
      a `layer_container` key (MaxText's default scan layout).
    num_layers: Number of layers the scanned axis must have. Used both to
      validate the input and to bound the unscan loop.
    layer_container: The pytree key holding the scanned per-layer params
      (MaxText's decoder body uses "layers").

  Returns:
    A nested dict with `layer_container` keys replaced by
    `f"{layer_container}_{i}"` for each layer `i`, each holding the
    corresponding axis-1 slice of the original array, wrapped in `nnx.Param`
    -- matching `nnx.state(..., nnx.Param)`'s leaf type, so downstream
    consumers (`raiden_synchronizer.flatten_weights`, which unwraps `.value`)
    see the same leaf shape whether or not this transform ran. Non-scanned
    entries (e.g. embeddings, final norm) pass through unchanged, also
    rewrapped.
  """
  ...
  for key, value in flat.items():
    ...
    if not hasattr(arr, "shape") or arr.ndim <= _LAYER_AXIS:
      # Not a per-layer leaf (shouldn't happen for real params under
      # `layers`, but don't silently drop anything unexpected).
      new_flat[key] = value
      continue

    if arr.shape[_LAYER_AXIS] != num_layers:
      raise ValueError(
          f"unscan_layers: {jax.tree_util.keystr(key)!r} has "
          f"shape {arr.shape}, expected axis {_LAYER_AXIS} to be "
          f"num_layers={num_layers}."
      )

    for i in range(num_layers):
      sliced = jax.lax.index_in_dim(arr, i, axis=_LAYER_AXIS, keepdims=False)
