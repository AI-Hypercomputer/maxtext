...
def _validate_batch_masks(
  ...
  shapes = {
      ...
      "targets_segmentation": tuple(targets_segmentation.shape),
  ...
  if any(value is None for value in concrete):
    return
  concrete_positions = np.asarray(concrete[0], dtype=np.int32)
  validity = np.asarray(concrete[1]) != 0
  completion, corruption = (np.asarray(value, dtype=bool) for value in concrete[2:4])
  weights = np.asarray(concrete[4], dtype=np.float32)
  weighted = weights != 0
  ...
  if completion_only:
    block_ids = concrete_positions // block_size
    unseen_block = np.iinfo(block_ids.dtype).min
    seen_completion_blocks = np.maximum.accumulate(np.where(validity & completion, block_ids, unseen_block), axis=-1)
    if np.any(validity & ~completion & (seen_completion_blocks == block_ids)):
      raise ValueError(
  ...
  supervision = completion if completion_only else validity
  if np.any(corruption & ~supervision):
    raise ValueError("corruption_mask must be a subset of the configured supervision scope")
  if np.any(weighted & ~supervision):
    raise ValueError("targets_loss_mask must be a subset of the configured supervision scope")
  ...
  if np.any(weighted & ~allowed):
    raise ValueError("diffusion SFT loss weights must own corrupted targets or shifted block anchors")
...
def create_batch_adapter(config):
  ...
  block_size = int(config.causal_block_size)
  completion_only = bool(config.sft_train_on_completion_only)

  def adapt(raw_batch: Mapping[str, Any]) -> diffusion_types.DiffusionTokenBatch:
    unavailable = sorted(name for name in _REQUIRED_FIELDS if name not in raw_batch or raw_batch[name] is None)
    if unavailable:
      raise ValueError("block-diffusion SFT requires non-None batch fields; " f"unavailable {unavailable}")
    _validate_batch_masks(
        raw_batch["targets_position"],
        raw_batch["targets_segmentation"],
        raw_batch["completion_mask"],
        raw_batch["corruption_mask"],
        raw_batch["targets_loss_mask"],
        alignment=alignment,
    ...
    raw_loss_weights = jnp.asarray(raw_batch["targets_loss_mask"], dtype=jnp.float32)
    supervision_mask = completion_mask if completion_only else validity_mask
    allowed = corruption_mask
    if alignment == "shifted":
      allowed |= (positions > 0) & (positions % block_size == 0)
    loss_weights = jnp.where(validity_mask & supervision_mask & allowed, raw_loss_weights, 0.0)
    return diffusion_types.DiffusionTokenBatch.create(
        model_inputs={
            "input_tokens": jnp.asarray(raw_batch["inputs"]),
            "input_positions": jnp.asarray(raw_batch["inputs_position"], dtype=jnp.int32),
