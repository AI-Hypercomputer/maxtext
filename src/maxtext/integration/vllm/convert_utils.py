...
def tunix_compat_context():
  ...
  def _compat_bulk(arr, scan_axis, per_layer, key_path):
    if hasattr(arr, "shape"):
      if scan_axis < 0:
        scan_axis = len(arr.shape) + scan_axis
      if len(arr.shape) <= scan_axis:
  ...
  def _compat_unstack(src_val, tgt_val, key_path, scan_axis=None):
    if scan_axis is not None and hasattr(src_val, "shape"):
      if scan_axis < 0:
        scan_axis = len(src_val.shape) + scan_axis
      if len(src_val.shape) <= scan_axis:
