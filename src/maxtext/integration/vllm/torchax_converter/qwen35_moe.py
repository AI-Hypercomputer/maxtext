    ...
    for slot in range(self.num_slots):
      ...
      for rep in range(self.num_reps):
        ...
        if has_shared:
          ...
          self.vllm_state[f"{p}.mlp.shared_expert.down_proj.weight"] = jnp.transpose(
              sh_down_layers[rep], (1, 0)
          )

          if "shared_expert_gate" in mlp_block:
            self.vllm_state[f"{p}.mlp.shared_expert_gate.weight"] = jnp.transpose(
                sh_gate_router_layers[rep], (1, 0)
            )
