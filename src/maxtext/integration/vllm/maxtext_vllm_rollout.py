...
class MaxTextVllmSampler(VllmSampler):
  ...
  def __init__(
    ...
    self._model_name = model_name or getattr(model_config, "model", "") or engine_kwargs.get("model", "") or ""

    hf_config = getattr(model_config, "hf_config", None)
    model_type = getattr(hf_config, "model_type", "") or ""
    architectures = getattr(model_config, "architectures", []) or []
    model_str = f"{self._model_name} {model_type} {' '.join(str(a) for a in architectures)}".lower()
    self._is_gemma = "gemma" in model_str
