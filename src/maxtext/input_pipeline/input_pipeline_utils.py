...
class PadOrTrimToMaxLength(grain.MapTransform):
  ...
  def map(
    ...
    for key, _ in element.items():
      ...
      elif key == "completion_mask" or key.endswith(("_segmentation", "_position")):
...
class BlockDiffusionCorruption(grain.RandomMapTransform):
  ...
  def random_map(self, element, rng: np.random.Generator):
    ...
    if self.select_last_completion_suffix:
      ...
      targets_segmentation = np.where(validity_mask, targets_segmentation, targets_segmentation.dtype.type(0))
    ...
    if self.select_last_completion_suffix:
      ...
      output["inputs_segmentation"] = np.where(validity_mask, inputs_segmentation, inputs_segmentation.dtype.type(0))
