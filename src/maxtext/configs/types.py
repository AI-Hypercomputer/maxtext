...
class MaxTextConfig(
  ...
  def set_derived_and_validate_values(self) -> "MaxTextConfig":
    ...
    if self.enable_multi_tier_checkpointing:
      ...
      if (self.multi_tier_checkpointing_backup_interval_minutes is None) == (self.multi_tier_checkpointing_backup_interval_steps is None):
        raise ValueError("Exactly one of `multi_tier_checkpointing_backup_interval_minutes` or `multi_tier_checkpointing_backup_interval_steps` must be specified.")
      if self.multi_tier_checkpointing_backup_interval_steps is not None and self.multi_tier_checkpointing_backup_interval_steps < self.local_checkpoint_period:
        raise ValueError(f"`multi_tier_checkpointing_backup_interval_steps` ({self.multi_tier_checkpointing_backup_interval_steps}) must be >= `local_checkpoint_period` ({self.local_checkpoint_period}).")
