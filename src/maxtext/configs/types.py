...
class Checkpointing(BaseModel):
    ...
    source_checkpoint_layout: Literal["orbax", "safetensors", "safetensors_dynamic"] = (
...
class Quantization(BaseModel):
    ...
    use_te_comm_gemm_overlap: bool = Field(
        False,
        description="If True, uses Transformer Engine's collective GEMM overlap algorithm.",
    )
...
class Attention(BaseModel):
    ...
    attention_type: Literal[
        "global", "local_sliding", "chunk", "mla", "full", "compressed"
...
class AttentionIndexer(BaseModel):
    ...
    use_indexer: bool = Field(
        False, description="Whether to use sparse indexer for MLA."
    )
    ...
    indexer_use_approx_top_k: bool = Field(
        False,
        description="Whether to use approximate top-k selection for the indexer on TPU.",
    )
...
class HardwareAndMesh(BaseModel):
    ...
    enable_nnx: bool = Field(
        True, description="Whether to use NNX for model definition."
    )
    ...
    pure_nnx_decoder: bool = Field(
        True, description="Whether to enable pure NNX decoder."
    )
...
class PipelineParallelism(BaseModel):
    ...
    scan_pipeline_repeats: bool = Field(
        True, description="Use jax.lax.scan over pipeline repeats."
    )
...
class RLHardware(BaseModel):
    ...
    inference_replicas: int = Field(
        1,
        description="Legacy experimental GRPO: number of inference (sampler) replicas.",
    )
...
class VLLM(BaseModel):
    ...
    hbm_utilization_vllm: float = Field(
        0.72, description="Target HBM utilization for vLLM."
    )
...
class RL(BaseModel):
    ...
    num_iterations: int = Field(
        1, description="Number of iterations per batch (μ in GRPO paper)."
    )
...
class RLDataset(BaseModel):
    ...
    train_fraction: float = Field(
        1.0, description="Fraction of the dataset to be used for training."
    )
...
class MaxTextConfig(
    ...
    def _validate_use_te_comm_gemm_overlap(self):
        ...
        tsp_size = (
            self.ici_tensor_sequence_parallelism * self.dcn_tensor_sequence_parallelism
        )
    ...
    def set_derived_and_validate_values(self) -> "MaxTextConfig":
            ...
            if os.path.exists(custom_mesh_path):
                with open(custom_mesh_path, "r") as f:  # pylint: disable=unspecified-encoding
                    ...
                    if "logical_axis_rules" in custom_mesh_config:
                        self.logical_axis_rules = custom_mesh_config[
                            "logical_axis_rules"
                        ]
        ...
        if self.run_name and self.base_output_directory:
            ...
            self.managed_mldiagnostics_dir = os.path.join(
                output_dir, "managed-mldiagnostics"
            )
        else:
        ...
        if self.distill_beta > 0.0:
            if not self.scan_layers:
                raise ValueError(
                    "a value of self.distill_beta > 0.0 requires self.scan_layers = True"
                )
            if not self.enable_nnx:
                raise ValueError(
                    "a value of self.distill_beta > 0.0 requires self.enable_nnx = True"
                )
        ...
        if (
            ...
            raise ValueError(
                f"distill_temperature_end must be > 0, got {self.distill_temperature_end}"
            )
        ...
        if self.enable_multi_tier_checkpointing:
            if not self.local_checkpoint_directory:
                raise ValueError(
                    "`local_checkpoint_directory` must be set for multi-tier checkpointing."
                )
            if self.local_checkpoint_period <= 0:
                raise ValueError(
                    "`local_checkpoint_period` must be > 0 for multi-tier checkpointing."
                )
            if self.multi_tier_checkpointing_backup_interval_minutes <= 0:
                raise ValueError(
                    "`multi_tier_checkpointing_backup_interval_minutes` must be > 0."
                )
        ...
        if self.enable_emergency_checkpoint:
            if not self.local_checkpoint_directory:
                raise ValueError(
                    "`local_checkpoint_directory` must be set for emergency checkpointing."
                )
            if self.local_checkpoint_period <= 0:
                raise ValueError(
        ...
        if (
            ...
            and self.attention != "dot_product"
        ):
            raise ValueError(
                "DeepSeek4 decoder block currently only supports dot_product attention."
            )
        if self.use_indexer:
        ...
        if self.attention_type == AttentionType.CHUNK.value and (
            ...
            raise ValueError(
                "`chunk_attn_window_size` must be an integer > 0 for 'chunk' attention."
            )
        ...
        if self.context_sharding not in ("context", "expert"):
            raise ValueError(
                f"Assigned context_sharding f{self.context_sharding} is not supported."
            )
        if (
        ...
        if (
            ...
            if self.dq_reduction_steps not in (0, 3):
                raise ValueError(
                    "TPU Tokamax ring attention requires dq_reduction_steps to be 0 or 3."
                )
            if (
                self.max_target_length % (context_parallel_size * context_parallel_size)
                != 0
            ):
                raise ValueError(
            ...
            if self.packing:
                raise ValueError(
                    "TPU Tokamax ring attention does not support packing yet."
                )
            ...
            if self.use_indexer:
                raise ValueError(
                    "TPU Tokamax ring attention does not support sparse indexer masks."
                )
            ...
            if self.use_qk_clip:
                raise ValueError(
                    "TPU Tokamax ring attention does not support QK-Clip statistics yet."
                )
        ...
        if self.decoder_block in (
            ...
            if rotary_dim % 2 != 0:
                raise ValueError(
                    f"Calculated rotary dimension ({rotary_dim}) must be a multiple of 2."
                )
        else:
            if (
        ...
        if self.dataset_type == DatasetType.HF:
            ...
            if self.eval_interval > 0 and not self.grain_eval_files:
                raise ValueError(
                    "Please specify grain_eval_files or set eval_interval to <=0."
                )
        ...
        if self.share_kv_projections and self.attention_type == "mla":
            raise ValueError(
                "`share_kv_projections` is not compatible with `attention_type='mla'`."
            )
