  ...
  def validate_lineage(self) -> "DeepSeekMoE":
    ...
    if self.use_lineage:
      ...
      if decoder_block is not None and decoder_block != DecoderBlockType.DEEPSEEK:
        raise ValueError("use_lineage=True requires decoder_block='deepseek', got" f" decoder_block={decoder_block!r}.")
    return self
...
class MaxTextConfig(
  ...
  def set_derived_and_validate_values(self) -> "MaxTextConfig":
    ...
    if self.use_lineage:
      ...
      if self.decoder_block != DecoderBlockType.DEEPSEEK:
        raise ValueError(
            "use_lineage=True requires decoder_block='deepseek', got" f" decoder_block={self.decoder_block!r}."
        )
      if self.attention_type == "global":
        self.attention_type = "mla"
      elif self.attention_type != "mla":
        raise ValueError(
            "use_lineage=True requires attention_type='mla', got" f" attention_type={self.attention_type!r}."
        )
      ...
      if rope_type == RopeType.DEFAULT:
        ...
        raise ValueError("use_lineage=True requires rope_type='yarn', got" f" rope_type={rope_type!r}.")
      if self.capacity_factor <= 0:
        self.capacity_factor = 0.5
      if self.param_scan_axis not in (0, 1):
        raise ValueError("use_lineage=True requires param_scan_axis in (0, 1), got" f" {self.param_scan_axis}.")
