...
def _prepare_for_pydantic(raw_keys: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in raw_keys.items():
        ...
        if key == "model_name" and "-Instruct" in new_value:
            new_value = new_value.replace("-Instruct", "")

        if key == "tokenizer_path" and new_value is None:
            if "-Instruct" in raw_keys["model_name"]:
                model_name_original = raw_keys["model_name"].replace("-Instruct", "")
                new_value = HF_IDS.get(model_name_original) + "-Instruct"
            else:
                model_name_original = raw_keys["model_name"]
                new_value = HF_IDS.get(model_name_original)
        ...
        if key == "muon_consistent_rms":
            if value in ["None", "none"]:
                ...
                try:
                    ...
                    raise ValueError(
                        "muon_consistent_rms should be None or float"
                    ) from e
...
def initialize_pydantic(argv: list[str], **kwargs) -> MaxTextConfig:
    ...
    model_name = (
        model_name.replace("-Instruct", "") if "-Instruct" in model_name else model_name
    )
    ...
    if pydantic_kwargs.get("use_tokamax_splash") and pydantic_kwargs.get(
        ...
        raise ValueError(
            "At most one of `use_tokamax_splash` and `use_jax_splash` can be set to True."
        )
