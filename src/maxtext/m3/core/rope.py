...
def apply_rope(
  ...
  # Geometric series of rotation periods, one per dimension pair. Matches the
  # legacy `RotaryEmbedding.timescale` expression term for term.
  fraction = 2 * jnp.arange(0, half_dim) / head_dim
  ...
  # Rotate each (x1[i], x2[i]) pair by its angle. Equivalent to the legacy
