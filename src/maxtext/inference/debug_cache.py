...
def print_cache_index(decode_state):
  cache = decode_state["cache"]
  try:
    dense_idx = cache['decoder']['dense_layers']['self_attention']['KVCache_0']['cache_ar_index']
    print(f"  dense_layers cache_ar_index: {dense_idx}")
  except KeyError as e:
    print(f"  dense_layers cache_ar_index not found: {e}")
  
  try:
    moe_idx = cache['decoder']['moe_layers']['self_attention']['KVCache_0']['cache_ar_index']
    print(f"  moe_layers cache_ar_index: {moe_idx}")
  except KeyError as e:
    print(f"  moe_layers cache_ar_index not found: {e}")
...
def main(argv: Sequence[str]) -> None:
  ...
  # Monkeypatch AutoTokenizer to avoid Illegal header value b'Bearer ' when token is empty
  try:
  ...
  for i in range(_NUM_STREAMS):
    ...
    print("Prefill logits shape:", prefill_result["logits"].shape)
  ...
  print("After insert:")
  print_cache_index(decode_state)
  ...
  for i in range(_NUM_STREAMS):
    ...
    print(f"Results token IDs: {results}")
