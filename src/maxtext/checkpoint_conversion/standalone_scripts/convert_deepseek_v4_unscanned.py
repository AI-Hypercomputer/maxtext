...
def convert_weights(abstract_params, weight_dict, config, block_to_convert=None):
  ...
  def map_fn(path, abstract_var):
    ...
    # 2. Unscanned Layer params
    if block_prefix.startswith("decoder-layers_"):
      layer_idx = int(block_prefix.split("-")[1].split("_")[-1])
      suffix = path_str[len(block_prefix)+1:] # Strip prefix (e.g. "decoder-layers_0-mhc_attention-")
      ...
      if w.shape != target_shape:
        if w.size == np.prod(target_shape):
          ...
          raise ValueError(f"Shape mismatch for {path_str}. Weight: {w.shape}, Target: {target_shape}")
      
      return w

    raise ValueError(f"CRITICAL: Unmapped MaxText prefix '{block_prefix}' in '{path_str}'.")
...
def main():
  ...
  if os.path.exists(index_path):
    ...
    with open("unscanned_ckpt_param_names.txt", "w") as f:
