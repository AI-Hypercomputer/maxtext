...
def main():
  ...
  if len(argv) < 2:
    # Default arguments updated for a scanned conversion run for deepseek4-tiny
    argv = ['', 'src/maxtext/configs/base.yml', 'model_name=deepseek4-tiny', 'override_model_config=True', 'attention=dot_product', 'skip_jax_distributed_system=True', 'weight_dtype=bfloat16', 'scan_layers=True', 'base_output_directory=gs://snehalv-data/deepseek_v4-flash/scanned-tiny/']

  print("Initializing configuration...")
  # Initialize without heavyweight runtime
  config = pyconfig.initialize(argv)
  
  # Assert that we are only converting for 4 experts
  assert config.num_experts == 4, f"This script only supports converting checkpoints for configurations with exactly 4 experts, but got {config.num_experts}."
