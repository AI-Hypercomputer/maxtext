...
Example Usage:
  To convert HF LoRA to MaxText adapter:
    ...
    python src/maxtext/checkpoint_conversion/hf_lora_to_maxtext.py \
...
def load_hf_lora_adapter(adapter_path: str, hf_model_id: str) -> dict:
  ...
  adapter_file_path = str(adapter_file)

  # Load the adapter weights
  if adapter_file_path.endswith(".safetensors"):
    with safe_open(adapter_file_path, framework="numpy") as f:
      lora_weights = {k: f.get_tensor(k) for k in f.keys()}
  else:
    raise ValueError(f"Unsupported adapter file format: {adapter_file_path}")
...
def convert_hf_lora_key_to_maxtext(hf_key: str, param_mapping: dict) -> str:
  ...
  # Handle prefix. Expected target is usually "model.layers..."
...
def convert_lora_to_maxtext_adapter(config, lora_weights: dict, output_path: str, hf_model_id: str):
  ...
  # Map HF LoRA weights to MaxText keys
  ...
  # Save as a standalone adapter checkpoint
...
def main(args: Sequence[str]) -> None:
  ...
  adapter_name = os.path.basename(os.path.normpath(config.hf_lora_adapter_path))
  ...
  outputpath = epath.Path(full_output_path)
