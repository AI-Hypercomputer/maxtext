...
def convert(argv):
  ...
  max_logging.log(f"Starting conversion for model: {model_name}")
  max_logging.log(f"Path: {maxtext_ckpt_path}")
    ...
    for module_name, content in current_dict.items():
      ...
      if isinstance(content, dict) and "kernel_lora_a" in content:
        ...
        if lookup_key in mapping:
          ...
          max_logging.log(f"Mapped {lookup_key} to {len(hf_paths)} HF layers")
  ...
  max_logging.log(f"Saved weights to: {adapter_file}")
  max_logging.log(f"Saved config to: {config_file}")
  max_logging.log(f"Target modules detected: {list(found_hf_modules)}")
