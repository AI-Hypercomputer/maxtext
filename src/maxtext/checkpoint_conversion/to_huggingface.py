...
def _transform_and_save_weights(
  ...
  if lora_restore_path and not load_parameters_path:
    ...
    # CRITICAL: Free up the old MaxText checkpoint from memory now that we have the HF array copies!
    del maxtext_state_dict
    import gc; gc.collect()

    max_logging.log("\nSaving HuggingFace model...")
    save_model_files(transformed_hf_weights, hf_config_obj, tokenizer, processor, output_directory, parallel_threads=8)
