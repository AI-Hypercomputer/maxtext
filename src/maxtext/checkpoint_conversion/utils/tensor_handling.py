    ...
    for lyr_idx, hf_key_single in enumerate(layer_keys_for_expert):
      ...
      if target_sharding is not None:
        ...
        if hasattr(target_sharding, "spec"):
          ...
          slice_sharding = jax.sharding.NamedSharding(
              target_sharding.mesh,
              jax.sharding.PartitionSpec(*spec_list)
          )
        else:
...
def _get_hf_loading_function(hf_source_keys_or_key, tensor_getter, hook_fn, mt_target_leaf, config):
