...
# from https://huggingface.co/Qwen/Qwen3.8-27B/blob/main/config.json
# The dense member of the Qwen3.5 family: `model_type` is `qwen3_5` (not `qwen3_5_moe`)
# and the text config carries `intermediate_size` instead of the MoE keys
...
try:
  # Will execute successfully if Transformers is updated with Qwen3.5 support.
  # Separate from the MoE block above: the dense model uses `Qwen3_5Config`, and a
  # transformers build that has one class but not the other should still work.
  qwen3_5_27b_config = transformers.Qwen3_5Config(**qwen3_5_27b_dict)  # pyrefly: ignore[missing-attribute]
except AttributeError:
