...
def DEEPSEEK_V4_MAXTEXT_TO_HF_PARAM_MAPPING(config, maxtext_config, scan_layers=False):
  ...
  for i in range(n_layers):
    ...
    # HC Norms (DeepSeek V4) mapped to standard MaxText Layer Norms
    mapping[f"{prefix}-pre_self_attention_layer_norm-scale"] = f"layers.{i}.attn_norm.weight"
    mapping[f"{prefix}-post_self_attention_layer_norm-scale"] = f"layers.{i}.ffn_norm.weight"
    mapping[f"{prefix}-mhc_attention-mhc_norm-scale"] = f"layers.{i}.attn_norm.weight"
    mapping[f"{prefix}-mhc_mlp-mhc_norm-scale"] = f"layers.{i}.ffn_norm.weight"
    
    # MHC Attention Alpha (Atomic)
    mapping[f"{prefix}-mhc_attention-pre_alpha"] = f"layers.{i}.hc_attn_fn"
    mapping[f"{prefix}-mhc_attention-post_alpha"] = f"layers.{i}.hc_attn_fn"
    mapping[f"{prefix}-mhc_attention-res_alpha"] = f"layers.{i}.hc_attn_fn"
    
    # MHC Attention Beta (Atomic)
    mapping[f"{prefix}-mhc_attention-pre_beta"] = f"layers.{i}.hc_attn_base"
    mapping[f"{prefix}-mhc_attention-post_beta"] = f"layers.{i}.hc_attn_base"
    mapping[f"{prefix}-mhc_attention-res_beta"] = f"layers.{i}.hc_attn_base"
    
    # MHC Attention Scale (Atomic)
    mapping[f"{prefix}-mhc_attention-pre_alpha_scale"] = f"layers.{i}.hc_attn_scale"
    mapping[f"{prefix}-mhc_attention-post_alpha_scale"] = f"layers.{i}.hc_attn_scale"
    mapping[f"{prefix}-mhc_attention-res_alpha_scale"] = f"layers.{i}.hc_attn_scale"
    
    # MHC MLP Alpha (Atomic)
    mapping[f"{prefix}-mhc_mlp-pre_alpha"] = f"layers.{i}.hc_ffn_fn"
    mapping[f"{prefix}-mhc_mlp-post_alpha"] = f"layers.{i}.hc_ffn_fn"
    mapping[f"{prefix}-mhc_mlp-res_alpha"] = f"layers.{i}.hc_ffn_fn"
    
    # MHC MLP Beta (Atomic)
    mapping[f"{prefix}-mhc_mlp-pre_beta"] = f"layers.{i}.hc_ffn_base"
    mapping[f"{prefix}-mhc_mlp-post_beta"] = f"layers.{i}.hc_ffn_base"
    mapping[f"{prefix}-mhc_mlp-res_beta"] = f"layers.{i}.hc_ffn_base"
    
    # MHC MLP Scale (Atomic)
    mapping[f"{prefix}-mhc_mlp-pre_alpha_scale"] = f"layers.{i}.hc_ffn_scale"
    mapping[f"{prefix}-mhc_mlp-post_alpha_scale"] = f"layers.{i}.hc_ffn_scale"
    mapping[f"{prefix}-mhc_mlp-res_alpha_scale"] = f"layers.{i}.hc_ffn_scale"

    # Attention Core
    mapping[f"{prefix}-self_attention-q_norm-scale"] = f"layers.{i}.attn.q_norm.weight"
    mapping[f"{prefix}-self_attention-kv_norm-scale"] = f"layers.{i}.attn.kv_norm.weight"
    mapping[f"{prefix}-self_attention-wq_a-kernel"] = f"layers.{i}.attn.wq_a.weight"
    mapping[f"{prefix}-self_attention-wq_b-kernel"] = f"layers.{i}.attn.wq_b.weight"
    mapping[f"{prefix}-self_attention-wkv-kernel"] = f"layers.{i}.attn.wkv.weight"
    mapping[f"{prefix}-self_attention-sinks"] = f"layers.{i}.attn.attn_sink"
    
    # Output projection
    mapping[f"{prefix}-self_attention-o_a_proj-kernel"] = f"layers.{i}.attn.wo_a.weight"
    mapping[f"{prefix}-self_attention-o_b_proj-kernel"] = f"layers.{i}.attn.wo_b.weight"
    
    # Compressors CSA and HCA
    mapping[f"{prefix}-self_attention-csa_compressor-gate_proj-kernel"] = f"layers.{i}.attn.compressor.wgate.weight"
    mapping[f"{prefix}-self_attention-csa_compressor-kv_proj-kernel"] = f"layers.{i}.attn.compressor.wkv.weight"
    mapping[f"{prefix}-self_attention-csa_compressor-kv_norm-scale"] = f"layers.{i}.attn.compressor.norm.weight"
    mapping[f"{prefix}-self_attention-csa_compressor-position_bias"] = f"layers.{i}.attn.compressor.ape"

    mapping[f"{prefix}-self_attention-hca_compressor-gate_proj-kernel"] = f"layers.{i}.attn.compressor.wgate.weight"
    mapping[f"{prefix}-self_attention-hca_compressor-kv_proj-kernel"] = f"layers.{i}.attn.compressor.wkv.weight"
    mapping[f"{prefix}-self_attention-hca_compressor-kv_norm-scale"] = f"layers.{i}.attn.compressor.norm.weight"
    mapping[f"{prefix}-self_attention-hca_compressor-position_bias"] = f"layers.{i}.attn.compressor.ape"

    mapping[f"{prefix}-self_attention-csa_compressor-indexer-gate_proj-kernel"] = f"layers.{i}.attn.indexer.compressor.wgate.weight"
    mapping[f"{prefix}-self_attention-csa_compressor-indexer-kv_proj-kernel"] = f"layers.{i}.attn.indexer.compressor.wkv.weight"
    mapping[f"{prefix}-self_attention-csa_compressor-indexer-kv_norm-scale"] = f"layers.{i}.attn.indexer.compressor.norm.weight"
    mapping[f"{prefix}-self_attention-csa_compressor-indexer-position_bias"] = f"layers.{i}.attn.indexer.compressor.ape"
    mapping[f"{prefix}-self_attention-csa_compressor-indexer-weights_proj-kernel"] = f"layers.{i}.attn.indexer.weights_proj.weight"
    mapping[f"{prefix}-self_attention-csa_compressor-indexer-q_proj-kernel"] = f"layers.{i}.attn.indexer.wq_b.weight"
    
    # MoE
    num_experts = config.get("n_routed_experts", 0)
    if num_experts == 0:
        # Failsafe if not found in config
        num_experts = 8
    ...
    if i < num_hash_layers:
      mapping[f"Tid2EidVar-decoder-layers_{i}-mlp-MoeBlock_0-tid2eid"] = f"layers.{i}.ffn.gate.tid2eid"
    else:
      mapping[f"{prefix}-mlp-MoeBlock_0-gate-bias"] = f"layers.{i}.ffn.gate.bias"
    
    mapping[f"{prefix}-mlp-shared_experts-wi_0-kernel"] = f"layers.{i}.ffn.shared_experts.w1.weight"
    mapping[f"{prefix}-mlp-shared_experts-wi_1-kernel"] = f"layers.{i}.ffn.shared_experts.w3.weight"
    mapping[f"{prefix}-mlp-shared_experts-wo-kernel"] = f"layers.{i}.ffn.shared_experts.w2.weight"
    
    mapping[f"{prefix}-mlp-MoeBlock_0-wi_0"] = [
        f"layers.{i}.ffn.experts.{e}.w1.weight" for e in range(num_experts)
    ]
    mapping[f"{prefix}-mlp-MoeBlock_0-wi_1"] = [
        f"layers.{i}.ffn.experts.{e}.w3.weight" for e in range(num_experts)
    ]
    mapping[f"{prefix}-mlp-MoeBlock_0-wo"] = [
        f"layers.{i}.ffn.experts.{e}.w2.weight" for e in range(num_experts)
    ]
...
def DEEPSEEK_V4_MAXTEXT_TO_HF_PARAM_HOOK_FN(config, maxtext_config, scan_layers=False, saving_to_hf=False):
  ...
  def mhc_dummy_norm(input_tensor, target_shape=None):
      import numpy as np
      return np.ones(target_shape, dtype=input_tensor.dtype)
  ...
  for i in range(n_layers):
    ...
    if saving_to_hf:
      # Use composite tuple hooks for saving
