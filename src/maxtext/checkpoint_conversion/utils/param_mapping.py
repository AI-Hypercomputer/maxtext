  ...
  def reshape_kernel(input_tensor, target_shape):
    if saving_to_hf:
      ...
      try:
        ...
        raise ValueError(f'RESHAPE FAILED: {str(e)} for target {target_shape} with input {input_tensor.shape}')
...
def DEEPSEEK_MAXTEXT_TO_HF_PARAM_MAPPING(config, maxtext_config, scan_layers=False):
  ...
  num_experts = config.get("n_routed_experts", config.get("num_experts", maxtext_config.num_experts))
...
def DEEPSEEK_V4_MAXTEXT_TO_HF_PARAM_MAPPING(config, maxtext_config, scan_layers=False):
  ...
  num_experts = maxtext_config.num_experts if getattr(maxtext_config, "num_experts", 0) > 0 else _get(config, "n_routed_experts", 256)
  ...
  if not scan_layers:
    for i in range(n_layers):
      ...
      if i < num_hash_layers:
        mapping[f"{prefix.replace('params-', 'Tid2EidVar-')}-mlp-MoeBlock_0-tid2eid"] = f"{hf_prefix}.ffn.gate.tid2eid"
      else:
    ...
    prefix = "params-decoder-scanned_blocks-layers_0"
    mapping[f"{prefix}-pre_self_attention_layer_norm-scale"] = [f"layers.{i}.attn_norm.weight" for i in hca_layers]
    mapping[f"{prefix}-post_self_attention_layer_norm-scale"] = [f"layers.{i}.ffn_norm.weight" for i in hca_layers]
    mapping[f"{prefix}-mhc_attention-mhc_norm-scale"] = None
    mapping[f"{prefix}-mhc_mlp-mhc_norm-scale"] = None

    mapping[f"{prefix}-mhc_attention-pre_alpha"] = [f"layers.{i}.hc_attn_fn" for i in hca_layers]
    ...
    mapping[f"{prefix}-mhc_attention-res_alpha_scale"] = [f"layers.{i}.hc_attn_scale" for i in hca_layers]
    ...
    mapping[f"{prefix}-self_attention-q_norm-scale"] = [f"layers.{i}.attn.q_norm.weight" for i in hca_layers]
    mapping[f"{prefix}-self_attention-kv_norm-scale"] = [f"layers.{i}.attn.kv_norm.weight" for i in hca_layers]
    mapping[f"{prefix}-self_attention-wq_a-kernel"] = [f"layers.{i}.attn.wq_a.weight" for i in hca_layers]
    mapping[f"{prefix}-self_attention-wq_b-kernel"] = [f"layers.{i}.attn.wq_b.weight" for i in hca_layers]
    mapping[f"{prefix}-self_attention-wkv-kernel"] = [f"layers.{i}.attn.wkv.weight" for i in hca_layers]
    mapping[f"{prefix}-self_attention-sinks"] = [f"layers.{i}.attn.attn_sink" for i in hca_layers]

    mapping[f"{prefix}-self_attention-o_a_proj-kernel"] = [f"layers.{i}.attn.wo_a.weight" for i in hca_layers]
    mapping[f"{prefix}-self_attention-o_b_proj-kernel"] = [f"layers.{i}.attn.wo_b.weight" for i in hca_layers]

    mapping[f"{prefix}-self_attention-hca_compressor-gate_proj-kernel"] = [f"layers.{i}.attn.compressor.wgate.weight" for i in hca_layers]
    mapping[f"{prefix}-self_attention-hca_compressor-kv_proj-kernel"] = [f"layers.{i}.attn.compressor.wkv.weight" for i in hca_layers]
    mapping[f"{prefix}-self_attention-hca_compressor-kv_norm-scale"] = [f"layers.{i}.attn.compressor.norm.weight" for i in hca_layers]
    mapping[f"{prefix}-self_attention-hca_compressor-position_bias"] = [f"layers.{i}.attn.compressor.ape" for i in hca_layers]

    mapping[f"{prefix}-mlp-MoeBlock_0-gate-kernel"] = [f"layers.{i}.ffn.gate.weight" for i in hca_layers]
    mapping[f"{prefix.replace('params-', 'MoEBiasVar-')}-mlp-MoeBlock_0-gate-bias"] = [f"layers.{i}.ffn.gate.bias" for i in hca_layers]

    mapping[f"{prefix}-mlp-shared_experts-wi_0-kernel"] = [f"layers.{i}.ffn.shared_experts.w1.weight" for i in hca_layers]
    mapping[f"{prefix}-mlp-shared_experts-wi_1-kernel"] = [f"layers.{i}.ffn.shared_experts.w3.weight" for i in hca_layers]
    mapping[f"{prefix}-mlp-shared_experts-wo-kernel"] = [f"layers.{i}.ffn.shared_experts.w2.weight" for i in hca_layers]

    mapping[f"{prefix}-mlp-MoeBlock_0-wi_0"] = [
        [f"layers.{i}.ffn.experts.{e}.w1.weight" for i in hca_layers] for e in range(num_experts)
    ]
    mapping[f"{prefix}-mlp-MoeBlock_0-wi_1"] = [
        [f"layers.{i}.ffn.experts.{e}.w3.weight" for i in hca_layers] for e in range(num_experts)
    ]
    mapping[f"{prefix}-mlp-MoeBlock_0-wo"] = [
        [f"layers.{i}.ffn.experts.{e}.w2.weight" for i in hca_layers] for e in range(num_experts)
    ]

    # Layer 1 in Scanned Blocks (CSA)
    prefix = "params-decoder-scanned_blocks-layers_1"
    mapping[f"{prefix}-pre_self_attention_layer_norm-scale"] = [f"layers.{i}.attn_norm.weight" for i in csa_layers]
    mapping[f"{prefix}-post_self_attention_layer_norm-scale"] = [f"layers.{i}.ffn_norm.weight" for i in csa_layers]
    mapping[f"{prefix}-mhc_attention-mhc_norm-scale"] = None
    mapping[f"{prefix}-mhc_mlp-mhc_norm-scale"] = None

    mapping[f"{prefix}-mhc_attention-pre_alpha"] = [f"layers.{i}.hc_attn_fn" for i in csa_layers]
    mapping[f"{prefix}-mhc_attention-post_alpha"] = [f"layers.{i}.hc_attn_fn" for i in csa_layers]
    mapping[f"{prefix}-mhc_attention-res_alpha"] = [f"layers.{i}.hc_attn_fn" for i in csa_layers]

    mapping[f"{prefix}-mhc_attention-pre_beta"] = [f"layers.{i}.hc_attn_base" for i in csa_layers]
    mapping[f"{prefix}-mhc_attention-post_beta"] = [f"layers.{i}.hc_attn_base" for i in csa_layers]
    mapping[f"{prefix}-mhc_attention-res_beta"] = [f"layers.{i}.hc_attn_base" for i in csa_layers]

    mapping[f"{prefix}-mhc_attention-pre_alpha_scale"] = [f"layers.{i}.hc_attn_scale" for i in csa_layers]
    mapping[f"{prefix}-mhc_attention-post_alpha_scale"] = [f"layers.{i}.hc_attn_scale" for i in csa_layers]
    mapping[f"{prefix}-mhc_attention-res_alpha_scale"] = [f"layers.{i}.hc_attn_scale" for i in csa_layers]

    mapping[f"{prefix}-mhc_mlp-pre_alpha"] = [f"layers.{i}.hc_ffn_fn" for i in csa_layers]
    mapping[f"{prefix}-mhc_mlp-post_alpha"] = [f"layers.{i}.hc_ffn_fn" for i in csa_layers]
    mapping[f"{prefix}-mhc_mlp-res_alpha"] = [f"layers.{i}.hc_ffn_fn" for i in csa_layers]

    mapping[f"{prefix}-mhc_mlp-pre_beta"] = [f"layers.{i}.hc_ffn_base" for i in csa_layers]
    mapping[f"{prefix}-mhc_mlp-post_beta"] = [f"layers.{i}.hc_ffn_base" for i in csa_layers]
    mapping[f"{prefix}-mhc_mlp-res_beta"] = [f"layers.{i}.hc_ffn_base" for i in csa_layers]

    mapping[f"{prefix}-mhc_mlp-pre_alpha_scale"] = [f"layers.{i}.hc_ffn_scale" for i in csa_layers]
    mapping[f"{prefix}-mhc_mlp-post_alpha_scale"] = [f"layers.{i}.hc_ffn_scale" for i in csa_layers]
    mapping[f"{prefix}-mhc_mlp-res_alpha_scale"] = [f"layers.{i}.hc_ffn_scale" for i in csa_layers]

    mapping[f"{prefix}-self_attention-q_norm-scale"] = [f"layers.{i}.attn.q_norm.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-kv_norm-scale"] = [f"layers.{i}.attn.kv_norm.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-wq_a-kernel"] = [f"layers.{i}.attn.wq_a.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-wq_b-kernel"] = [f"layers.{i}.attn.wq_b.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-wkv-kernel"] = [f"layers.{i}.attn.wkv.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-sinks"] = [f"layers.{i}.attn.attn_sink" for i in csa_layers]

    mapping[f"{prefix}-self_attention-o_a_proj-kernel"] = [f"layers.{i}.attn.wo_a.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-o_b_proj-kernel"] = [f"layers.{i}.attn.wo_b.weight" for i in csa_layers]

    mapping[f"{prefix}-self_attention-csa_compressor-gate_proj-kernel"] = [f"layers.{i}.attn.compressor.wgate.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-csa_compressor-kv_proj-kernel"] = [f"layers.{i}.attn.compressor.wkv.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-csa_compressor-kv_norm-scale"] = [f"layers.{i}.attn.compressor.norm.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-csa_compressor-position_bias"] = [f"layers.{i}.attn.compressor.ape" for i in csa_layers]

    mapping[f"{prefix}-self_attention-csa_compressor-indexer-gate_proj-kernel"] = [f"layers.{i}.attn.indexer.compressor.wgate.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-csa_compressor-indexer-kv_proj-kernel"] = [f"layers.{i}.attn.indexer.compressor.wkv.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-csa_compressor-indexer-kv_norm-scale"] = [f"layers.{i}.attn.indexer.compressor.norm.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-csa_compressor-indexer-position_bias"] = [f"layers.{i}.attn.indexer.compressor.ape" for i in csa_layers]
    mapping[f"{prefix}-self_attention-csa_compressor-indexer-weights_proj-kernel"] = [f"layers.{i}.attn.indexer.weights_proj.weight" for i in csa_layers]
    mapping[f"{prefix}-self_attention-csa_compressor-indexer-q_proj-kernel"] = [f"layers.{i}.attn.indexer.wq_b.weight" for i in csa_layers]

    mapping[f"{prefix}-mlp-MoeBlock_0-gate-kernel"] = [f"layers.{i}.ffn.gate.weight" for i in csa_layers]
    mapping[f"{prefix.replace('params-', 'MoEBiasVar-')}-mlp-MoeBlock_0-gate-bias"] = [f"layers.{i}.ffn.gate.bias" for i in csa_layers]

    mapping[f"{prefix}-mlp-shared_experts-wi_0-kernel"] = [f"layers.{i}.ffn.shared_experts.w1.weight" for i in csa_layers]
    mapping[f"{prefix}-mlp-shared_experts-wi_1-kernel"] = [f"layers.{i}.ffn.shared_experts.w3.weight" for i in csa_layers]
    mapping[f"{prefix}-mlp-shared_experts-wo-kernel"] = [f"layers.{i}.ffn.shared_experts.w2.weight" for i in csa_layers]

    mapping[f"{prefix}-mlp-MoeBlock_0-wi_0"] = [
        [f"layers.{i}.ffn.experts.{e}.w1.weight" for i in csa_layers] for e in range(num_experts)
    ]
    mapping[f"{prefix}-mlp-MoeBlock_0-wi_1"] = [
        [f"layers.{i}.ffn.experts.{e}.w3.weight" for i in csa_layers] for e in range(num_experts)
    ]
    mapping[f"{prefix}-mlp-MoeBlock_0-wo"] = [
...
def DEEPSEEK_V4_MAXTEXT_TO_HF_PARAM_HOOK_FN(config, maxtext_config, scan_layers=False, saving_to_hf=False):
  ...
  # Alpha Hooks
  def reshape_mhc_pre(input_tensor, target_shape):
      transposed = input_tensor.T
      mix_hc = transposed.shape[-1]
      k = int(np.sqrt(1 + mix_hc) - 1)
      return transposed[..., :k]
  
  def reshape_mhc_post(input_tensor, target_shape):
      transposed = input_tensor.T
      mix_hc = transposed.shape[-1]
      k = int(np.sqrt(1 + mix_hc) - 1)
      return transposed[..., k:2*k]
      
  def reshape_mhc_res(input_tensor, target_shape):
      transposed = input_tensor.T
      mix_hc = transposed.shape[-1]
      k = int(np.sqrt(1 + mix_hc) - 1)
      return transposed[..., 2*k:]
      
  def composite_mhc_alpha(weights, target_shape=None):
      pre_alpha, post_alpha, res_alpha = weights
      return np.concatenate([pre_alpha, post_alpha, res_alpha], axis=-1).T

  # Beta Hooks
  def reshape_mhc_beta_pre(input_tensor, target_shape):
      mix_hc = input_tensor.shape[-1]
      k = int(np.sqrt(1 + mix_hc) - 1)
      return input_tensor[:k]

  def reshape_mhc_beta_post(input_tensor, target_shape):
      mix_hc = input_tensor.shape[-1]
      k = int(np.sqrt(1 + mix_hc) - 1)
      return input_tensor[k:2*k]

  def reshape_mhc_beta_res(input_tensor, target_shape):
      mix_hc = input_tensor.shape[-1]
      k = int(np.sqrt(1 + mix_hc) - 1)
      return input_tensor[2*k:].reshape(k, k)

  def composite_mhc_beta(weights, target_shape=None):
      pre_beta, post_beta, res_beta = weights
      return np.concatenate([pre_beta, post_beta, res_beta.flatten()], axis=-1)

  # Scale Hooks
  def reshape_mhc_scale_pre(input_tensor, target_shape):
      return input_tensor[0:1]
      
  def reshape_mhc_scale_post(input_tensor, target_shape):
      return input_tensor[1:2]
      
  def reshape_mhc_scale_res(input_tensor, target_shape):
      return input_tensor[2:3]

  def composite_mhc_scale(weights, target_shape=None):
      pre_scale, post_scale, res_scale = weights
      return np.concatenate([pre_scale, post_scale, res_scale], axis=-1)
  ...
  def _attach_layer_hooks(prefix):
    ...
    if saving_to_hf:
      mapping[(f"{prefix}-mhc_attention-pre_alpha", f"{prefix}-mhc_attention-post_alpha", f"{prefix}-mhc_attention-res_alpha")] = composite_mhc_alpha
      mapping[(f"{prefix}-mhc_mlp-pre_alpha", f"{prefix}-mhc_mlp-post_alpha", f"{prefix}-mhc_mlp-res_alpha")] = composite_mhc_alpha
      mapping[(f"{prefix}-mhc_attention-pre_beta", f"{prefix}-mhc_attention-post_beta", f"{prefix}-mhc_attention-res_beta")] = composite_mhc_beta
      mapping[(f"{prefix}-mhc_mlp-pre_beta", f"{prefix}-mhc_mlp-post_beta", f"{prefix}-mhc_mlp-res_beta")] = composite_mhc_beta
      mapping[(f"{prefix}-mhc_attention-pre_alpha_scale", f"{prefix}-mhc_attention-post_alpha_scale", f"{prefix}-mhc_attention-res_alpha_scale")] = composite_mhc_scale
      ...
      mapping[f"{prefix}-mhc_attention-pre_alpha"] = reshape_mhc_pre
      mapping[f"{prefix}-mhc_attention-post_alpha"] = reshape_mhc_post
      mapping[f"{prefix}-mhc_attention-res_alpha"] = reshape_mhc_res
      mapping[f"{prefix}-mhc_mlp-pre_alpha"] = reshape_mhc_pre
      mapping[f"{prefix}-mhc_mlp-post_alpha"] = reshape_mhc_post
      mapping[f"{prefix}-mhc_mlp-res_alpha"] = reshape_mhc_res
      mapping[f"{prefix}-mhc_attention-pre_beta"] = reshape_mhc_beta_pre
      mapping[f"{prefix}-mhc_attention-post_beta"] = reshape_mhc_beta_post
      mapping[f"{prefix}-mhc_attention-res_beta"] = reshape_mhc_beta_res
      mapping[f"{prefix}-mhc_mlp-pre_beta"] = reshape_mhc_beta_pre
      mapping[f"{prefix}-mhc_mlp-post_beta"] = reshape_mhc_beta_post
      mapping[f"{prefix}-mhc_mlp-res_beta"] = reshape_mhc_beta_res

    mapping[f"{prefix}-mhc_attention-pre_alpha_scale"] = reshape_mhc_scale_pre
    mapping[f"{prefix}-mhc_attention-post_alpha_scale"] = reshape_mhc_scale_post
    mapping[f"{prefix}-mhc_attention-res_alpha_scale"] = reshape_mhc_scale_res
    mapping[f"{prefix}-mhc_mlp-pre_alpha_scale"] = reshape_mhc_scale_pre
    mapping[f"{prefix}-mhc_mlp-post_alpha_scale"] = reshape_mhc_scale_post
    mapping[f"{prefix}-mhc_mlp-res_alpha_scale"] = reshape_mhc_scale_res
