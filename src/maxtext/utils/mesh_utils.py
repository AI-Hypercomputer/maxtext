...
def _expand_tree_on_mesh(
  ...
  def _leaf_in_sharding(leaf):
    if isinstance(leaf, jax.Array):
      named_sh = sharding.NamedSharding(
          mesh=mesh,
          spec=_get_spec(leaf),
      )
      if hasattr(leaf, "layout") and leaf.layout is not None:
        return Format(layout=leaf.layout, sharding=named_sh)
      return named_sh
    return leaf
...
def _force_leaf_to_tiled(arr: Any) -> Any:
  if not isinstance(arr, jax.Array):
    return arr
  untiled_layout = Layout(major_to_minor=tuple(range(arr.ndim)), tiling=None)
  in_format = Format(layout=untiled_layout, sharding=arr.sharding)

  @jax.jit(in_shardings=(in_format,), out_shardings=arr.sharding)
  def _to_tiled(x):
    return x

  return _to_tiled(arr)


def force_tree_to_tiled(tree: Any) -> Any:
  return jax.tree.map(_force_leaf_to_tiled, tree)
...
def stack_across_meshes_pytree(
  ...
  stacked_untiled = concatenate_by_mesh_axis(
      expanded_trees_on_expanded_mesh, axis_name
  )
  return force_tree_to_tiled(stacked_untiled)
