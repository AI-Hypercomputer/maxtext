...
def validate_train_config(config):
  ...
  if getattr(config, "use_indexer", False) and not getattr(config, "indexer_sparse_training", False):
    if getattr(config, "indexer_loss_scaling_factor", 0.0) <= 0.0:
      raise ValueError(
          "use_indexer=True with indexer_sparse_training=False and "
          "indexer_loss_scaling_factor<=0.0 zeroes the entire training objective. "
          "Enable indexer_sparse_training or set a positive indexer_loss_scaling_factor."
      )
