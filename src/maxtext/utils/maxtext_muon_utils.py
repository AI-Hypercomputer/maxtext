...
"""Utilities for MaxText Muon optimizer integration and dimension/sharding generation.

This module provides functions to automatically generate
ShardedMuonDimensionNumbers with physical NamedSharding for various MaxText
models.
"""
...
def get_maxtext_muon_weight_dimension_numbers(model, config=None, mesh=None, verbose=False):
  """Extracts a matching pytree of ShardedMuonDimensionNumbers with physical shardings from a model."""
  if mesh is None:
    if config is None:
      raise ValueError("Either mesh or config must be provided to get_maxtext_muon_weight_dimension_numbers.")
...
get_sharded_muon_weight_dimension_numbers = get_maxtext_muon_weight_dimension_numbers
