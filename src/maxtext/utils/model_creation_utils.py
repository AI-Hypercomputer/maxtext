...
def _repeat_checkpoint_to_model_shapes(ckpt_arr, model_arr):
  """Repeat ckpt_arr along any dimension where model_arr is larger.
  ...
  max_logging.log(f"Repeating checkpoint array from {ckpt_shape} to {model_shape} with repeats {repeats}")
  out_arr = ckpt_arr
  for axis, rep in enumerate(repeats):
      if rep > 1:
        out_arr = jnp.repeat(out_arr, rep, axis=axis)
    ...
    if config.load_parameters_path:
      try:
        ...
        if checkpoint:
          ...
          checkpoint = jax.tree.map(_repeat_checkpoint_to_model_shapes, checkpoint, model_arrays)
