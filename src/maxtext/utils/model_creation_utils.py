...
def from_pretrained(
  ...
  with mesh:
    if config.load_parameters_path:
      ...
      # Check if the path points to a Tunix composite checkpoint containing 'model_params'.
      # On remote backends (e.g. GCS), accessing a non-existent or mock bucket (like 'gs://fake/...'
      # in unit tests) causes epath/gfile to raise an OSError/PermissionDeniedError rather than
      # returning False. We catch this safely and fall back to the original path.
      try:
