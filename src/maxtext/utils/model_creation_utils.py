...
def from_pretrained(
  ...
  with mesh:
    if config.load_parameters_path:
      # If the checkpoint is Tunix format, Checkpointer metadata read will fail because it's a CheckpointManager output.
      # We tolerate metadata being None here, as the downstream `load_params_from_path` logic uses a proper CheckpointManager
      # to read the metadata for Tunix checkpoints.
