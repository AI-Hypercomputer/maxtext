...
def record_slice_state(recorder, active_slices_override: int | None = None) -> None:
    ...
    active_slices = (
        active_slices_override
        if active_slices_override is not None
        else len(elastic_manager.active_slice_indices)
    )
...
def record_elastic_event_start(recorder, config) -> None:
    ...
    event_type = (
        "elastic_scale_up" if is_scale_up_event(config) else "elastic_slice_down"
    )
    pending_elastic_event_type = event_type
    if recorder:
        recorder.record_custom_badput_event_start_time(
...
def record_elastic_wait_end_and_reinit_start(recorder) -> None:
    ...
    pending_elastic_event_type = None
    if recorder:
        recorder.record_custom_badput_event_end_time(
            custom_badput_event_type=event_type
        )
        recorder.record_custom_badput_event_start_time(
...
def clean_up_checkpoints(checkpoint_dir: str):
    ...
    max_logging.log(
        "Elastic utils: Checking for incomplete checkpoint after an elastic event..."
    )
    ...
    success_markers = gcs_utils.gcs_glob_pattern(
        f"{latest_checkpoint_path}commit_success*"
    )

    if not success_markers:
        max_logging.log(
            f"No commit_success file found. Deleting {latest_checkpoint_path}..."
        )
...
def live_devices(config=None):
    ...
    if should_use_elastic(config):
        ...
        return [
            ...
            if d is not None
            and getattr(d, "slice_index", 0) in elastic_manager.active_slice_indices
        ]
    return jax.devices()
