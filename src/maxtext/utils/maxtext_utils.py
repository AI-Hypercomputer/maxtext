...
def get_functional_train_with_signature(
  ...
  out_state_mesh_shardings = state_mesh_shardings

  if config.pure_nnx:
    in_shardings = (state_mesh_shardings, data_sharding)  # State, batch
  else:
    in_shardings = (state_mesh_shardings, data_sharding, None)  # State, batch, rng
  out_shardings = (out_state_mesh_shardings, None)  # State, metrics
...
def get_abstract_state_nnx(config, mesh, nnx_init_trainstate_fn, is_training=True):
  ...
  if hasattr(abstract_state, "optimizer") and hasattr(abstract_state.optimizer, "opt_state"):
    def _extract_opt_sharding(x):
      return getattr(x, "sharding", x)
    state_mesh_shardings.optimizer.opt_state = jax.tree.map(
        _extract_opt_sharding,
        abstract_state.optimizer.opt_state,
        is_leaf=lambda x: isinstance(x, (jax.ShapeDtypeStruct, jax.sharding.Sharding)),
    )

  if is_training and config.shard_optimizer_over_data:
    # Add data to sharding for optimizer state
    state_mesh_shardings.optimizer = jax.tree_util.tree_map_with_path(
        functools.partial(sharding.add_data_to_sharding, mesh),
  ...
  if is_training:
    opt_mk = "pinned_host" if config.optimizer_memory_host_offload else "device"
    def move_opt_sharding(path, leaf_sharding):
      return leaf_sharding.with_memory_kind(opt_mk)
  ...
  if is_training and (config.parameter_memory_host_offload or getattr(config, "parameter_memory_two_layer_buffer", False)):
    assert config.param_scan_axis == 0, "You must set the scan axis 0 to enable parameter offloading."

    def move_param_sharding(path, leaf_sharding):
      if sharding.is_scanned_block_param_path(path):
        return leaf_sharding.with_memory_kind("pinned_host")
      return leaf_sharding.with_memory_kind("device")

  abstract_sharded_state = maxtext_utils_nnx.set_named_sharding_nnx(abstract_state, state_mesh_shardings)
  state_mesh_shardings = maxtext_utils_nnx.nnx_extract_named_sharding(abstract_sharded_state)
  if hasattr(abstract_sharded_state, "optimizer") and hasattr(abstract_sharded_state.optimizer, "opt_state"):
    def _extract_opt_sharding(x):
      return getattr(x, "sharding", x)
    state_mesh_shardings.optimizer.opt_state = jax.tree.map(
        _extract_opt_sharding,
        abstract_sharded_state.optimizer.opt_state,
        is_leaf=lambda x: isinstance(x, (jax.ShapeDtypeStruct, jax.sharding.Sharding)),
    )

  if is_training and (config.parameter_memory_host_offload or getattr(config, "parameter_memory_two_layer_buffer", False)):
    state_mesh_shardings.model = jax.tree_util.tree_map_with_path(
        move_param_sharding,
        state_mesh_shardings.model,
        is_leaf=lambda x: isinstance(x, NamedSharding),
    )

    def move_abstract_var_sharding(path, leaf_var):
      val = getattr(leaf_var, "value", leaf_var)
      if hasattr(val, "sharding"):
        new_sharding = move_param_sharding(path, val.sharding)
        new_val = jax.ShapeDtypeStruct(val.shape, val.dtype, sharding=new_sharding)
        if isinstance(leaf_var, nnx.Variable):
          return leaf_var.replace(value=new_val)
        return new_val
      return leaf_var

    abstract_sharded_state.model = jax.tree_util.tree_map_with_path(
        move_abstract_var_sharding,
        abstract_sharded_state.model,
...
def layerwise_scanned_optimizer_update(optimizer: nnx.Optimizer, model: nnx.Module, grads, scanned_leading_dim: int, **kwargs):
  ...
  layer-by-layer during the forward pass (see `parameter_memory_two_layer_buffer`).
