...
class Qwen3NextGatedDeltaNet(nnx.Module):
  ...
  def __call__(
    ...
    # STEP B: 1D Convolution
    # =========================================================================
    cp_axes = gdn_context_axes(self.config)
    cp_axis_name = cp_axes[0] if cp_axes else None
    cp_size = (
        self.mesh.shape[cp_axis_name] if (self.mesh and cp_axis_name and cp_axis_name in self.mesh.axis_names) else 1
    )

    use_head_sharded_cp = (
        1 < cp_size <= self.num_k_heads
        and getattr(cfg, "use_gdn_kernel", False)
        and model_mode != MODEL_MODE_AUTOREGRESSIVE
    )

    if use_head_sharded_cp:
      if self.num_k_heads % cp_size != 0:
        raise ValueError(
    ...
    if getattr(cfg, "use_gdn_kernel", False):
      try:
        from maxtext.kernels.gdn.gdn_bwd_pallas import gdn_decoupled_conv1d  # pylint: disable=import-outside-toplevel
      except ImportError:
        try:
          from maxtext.kernels.gdn import gdn_decoupled_conv1d  # pylint: disable=import-outside-toplevel
        except ImportError:
          try:
            from maxtext.src.maxtext.kernels.gdn.gdn_bwd_pallas import gdn_decoupled_conv1d  # pylint: disable=import-outside-toplevel
          except ImportError as e:
            raise ImportError(
