...
class Qwen3NextGatedDeltaNet(nnx.Module):
  ...
  def __init__(self, config: Config, inputs_shape: tuple, dtype: DType = jnp.float32, model_mode: str = MODEL_MODE_TRAIN, *, rngs: nnx.Rngs):
    """
    Args:
    ...
    if model_mode != MODEL_MODE_TRAIN:
      ...
      print(f"[DEBUG GDN] Initializing KVCache with batch={runtime_batch_size}", flush=True)
  ...
  def __call__(
    ...
    if model_mode != MODEL_MODE_TRAIN and active_cache is not None:
      ...
      print(f"\n[DEBUG CALL] Executing __call__ in mode: {model_mode}", flush=True)
      print(f"[DEBUG CALL] Incoming hidden_states shape: {hidden_states.shape}", flush=True)
      print(f"[DEBUG CALL] Incoming qkv shape: {qkv.shape}", flush=True)
      print(f"[DEBUG CALL] Retrieved conv_state shape: {conv_state.shape}", flush=True)
      print(f"[DEBUG CALL] Retrieved recurrent_state shape: {recurrent_state.shape}\n", flush=True)
    ...
    if model_mode != MODEL_MODE_TRAIN and active_cache is not None:
      if next_conv_state.shape[0] != orig_cache_batch:
        if next_conv_state.shape[0] == 1:
          ...
          next_recurrent_state = jnp.broadcast_to(next_recurrent_state, (orig_cache_batch,) + next_recurrent_state.shape[1:])
  ...
  def init_kv_caches(self, batch_size: int):
    ...
    return kvcache.KVCache(
        max_prefill_length=cfg.max_prefill_predict_length,
        max_target_length=cfg.max_target_length,
        batch=batch_size,  # Injected directly from the JAX tracer
        key_seq_len=1,
        value_seq_len=1,
        key_heads=self.num_v_heads,
        value_heads=self.num_v_heads,
