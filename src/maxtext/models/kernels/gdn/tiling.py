...
def get_vmem_estimate_bytes(
  ...
  conv_state_buffer_bytes = 2 * (tile_b * max(0, kernel_size - 1) * conv_state_dim_size * conv_state_bytes)
  recurrent_state_buffer_bytes = 2 * (tile_b * n_v * d_v * d_k * rec_state_bytes)
...
def calculate_mixed_tile_size(
  ...
  prefill_candidates = [c for c in (128, 64, 32, 16, 8, 4, 2, 1) if c <= seq_len and c <= max_chunk_cap]
