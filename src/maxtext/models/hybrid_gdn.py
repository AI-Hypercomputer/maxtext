...
def _pallas_gdn_bwd_kernel(
  ...
  def chunk_forward(q, k, v, b_val, a_val, a_log_val, dt_bias_val, state_prev):
    ...
    mask_cumsum = jnp.tril(
        jnp.ones((chunk_size, chunk_size), dtype=log_g.dtype)
    )
  ...
  def fetch_inputs(chunk_idx, slot):
    ...
    pltpu.make_async_copy(
        qkv_ref.at[seq_idx, chunk_idx], qkv_vmem.at[slot], sem_qkv.at[slot]
    ).start()
    pltpu.make_async_copy(
        b_ref.at[seq_idx, chunk_idx], b_vmem.at[slot], sem_b.at[slot]
    ).start()
    pltpu.make_async_copy(
        a_ref.at[seq_idx, chunk_idx], a_vmem.at[slot], sem_a.at[slot]
    ).start()
    pltpu.make_async_copy(
        do_ref.at[seq_idx, chunk_idx], do_vmem.at[slot], sem_do.at[slot]
    ).start()
    pltpu.make_async_copy(
        chunk_states_ref.at[seq_idx, chunk_idx],
        chunk_states_vmem.at[slot],
        sem_chunk_states.at[slot],
    ).start()
  ...
  def wait_inputs(slot):
    ...
    pltpu.make_async_copy(
        qkv_vmem.at[slot], qkv_vmem.at[slot], sem_qkv.at[slot]
    ).wait()
    pltpu.make_async_copy(
        b_vmem.at[slot], b_vmem.at[slot], sem_b.at[slot]
    ).wait()
    pltpu.make_async_copy(
        a_vmem.at[slot], a_vmem.at[slot], sem_a.at[slot]
    ).wait()
    pltpu.make_async_copy(
        do_vmem.at[slot], do_vmem.at[slot], sem_do.at[slot]
    ).wait()
    pltpu.make_async_copy(
        chunk_states_vmem.at[slot],
        chunk_states_vmem.at[slot],
  ...
  def store_outputs(chunk_idx, slot):
    pltpu.make_async_copy(
        d_qkv_vmem.at[slot],
        d_qkv_ref.at[seq_idx, chunk_idx],
        sem_d_qkv.at[slot],
    ).start()
    pltpu.make_async_copy(
        d_b_vmem.at[slot], d_b_ref.at[seq_idx, chunk_idx], sem_d_b.at[slot]
    ).start()
    pltpu.make_async_copy(
        d_a_vmem.at[slot], d_a_ref.at[seq_idx, chunk_idx], sem_d_a.at[slot]
    ).start()

  def wait_outputs(slot):
    pltpu.make_async_copy(
        d_qkv_vmem.at[slot], d_qkv_vmem.at[slot], sem_d_qkv.at[slot]
    ).wait()
    pltpu.make_async_copy(
        d_b_vmem.at[slot], d_b_vmem.at[slot], sem_d_b.at[slot]
    ).wait()
    pltpu.make_async_copy(
        d_a_vmem.at[slot], d_a_vmem.at[slot], sem_d_a.at[slot]
    ).wait()
  ...
  pltpu.make_async_copy(
      a_log_ref.at[seq_idx], a_log_vmem, sem_a_log.at[0]
  ).start()
  pltpu.make_async_copy(
      dt_bias_ref.at[seq_idx], dt_bias_vmem, sem_dt_bias.at[0]
  ).start()
  ...
  def loop_body(i, carry):
    ...
    k = qkv_val[:, q_size : q_size + k_size].reshape(
        (chunk_size, num_kq_heads, kq_head_dim)
    )
    v = qkv_val[:, q_size + k_size :].reshape(
        (chunk_size, num_v_heads, v_head_dim)
    )
    ...
    (
        ...
        d_dt_bias_val,
        d_state_prev,
    ) = vjp_fn((do_val.astype(jnp.float32), d_state))
  ...
  d_state, d_conv_weight_acc, d_a_log_acc, d_dt_bias_acc = jax.lax.fori_loop(
      ...
      num_chunks,
      loop_body,
      (d_state, d_conv_weight_acc, d_a_log_acc, d_dt_bias_acc),
  )
  ...
  d_conv_weight_scratch[...] = d_conv_weight_acc.astype(
      d_conv_weight_scratch.dtype
  )
  ...
  pltpu.make_async_copy(
      d_conv_weight_scratch,
      d_conv_weight_ref.at[seq_idx, ...],
      sem_d_conv_weight.at[0],
  ).start()
  pltpu.make_async_copy(
      d_a_log_scratch, d_a_log_ref.at[seq_idx, ...], sem_d_a_log.at[0]
  ).start()
  pltpu.make_async_copy(
      d_dt_bias_scratch, d_dt_bias_ref.at[seq_idx, ...], sem_d_dt_bias.at[0]
  ).start()

  pltpu.make_async_copy(
      d_conv_weight_scratch, d_conv_weight_scratch, sem_d_conv_weight.at[0]
  ).wait()
  pltpu.make_async_copy(
      d_a_log_scratch, d_a_log_scratch, sem_d_a_log.at[0]
  ).wait()
  pltpu.make_async_copy(
      d_dt_bias_scratch, d_dt_bias_scratch, sem_d_dt_bias.at[0]
  ).wait()
...
def pallas_fused_conv1d_gdn_bwd_computation(
  ...
  pre_conv_qkv_4d = pre_conv_qkv.reshape(
      batch_size, num_chunks, chunk_size, dim_size
  )
  ...
  d_conv_weight_shape = jax.ShapeDtypeStruct(
      (batch_size, kernel_size, dim_size), conv_weight_2d.dtype
  )
  d_a_log_shape = jax.ShapeDtypeStruct(
      (batch_size, num_v_heads), a_log_2d.dtype
  )
  d_dt_bias_shape = jax.ShapeDtypeStruct(
      (batch_size, num_v_heads), dt_bias_2d.dtype
  )
  ...
  scratch_shapes = (
      pltpu.VMEM(
          (2, chunk_size + pad_len, dim_size), padded_pre_conv_qkv.dtype
      ),
...
def pure_jax_fused_conv1d_gdn(
  ...
  from maxtext.src.maxtext.models.qwen3 import jax_chunk_gated_delta_rule
  ...
  if next_recurrent_state is None:
    next_recurrent_state = jnp.zeros(
        (batch, num_v_heads, head_k_dim, head_v_dim), dtype=compute_dtype
    )
...
def _compute_forward_conv_and_states(
  ...
  q = qkv_conv[:, :, :q_size].reshape(
      batch_size, num_chunks, chunk_size, num_kq_heads, head_k_dim
  )
  k = qkv_conv[:, :, q_size : q_size + k_size].reshape(
      batch_size, num_chunks, chunk_size, num_kq_heads, head_k_dim
  )
  v = qkv_conv[:, :, q_size + k_size :].reshape(
  ...
  scale = jax.lax.rsqrt(jnp.array(head_k_dim, dtype=jnp.float32)).astype(
      compute_dtype
  )
  q = q * scale
  ...
  def scan_fn(carry_state, chunk_inputs):
    ...
    cumsum_log_g = jnp.einsum(
        "cd,bhd->bhc", mask_cumsum, log_g.swapaxes(1, 2)
    ).swapaxes(1, 2)
...
def _run_tokamax_fused_fwd(
  ...
  (new_conv_state, new_recurrent_state), core_attn_out_flat = (
      tokamax_gdn_wrapper.fused_conv1d_gdn(
          qkv=qkv_flat,
          b=b_flat,
          a=a_flat,
...
def _hybrid_fused_conv1d_gdn_bwd(
  ...
  d_qkv, d_b, d_a, d_conv_weight, d_a_log, d_dt_bias = (
...
hybrid_fused_conv1d_gdn.defvjp(
    _hybrid_fused_conv1d_gdn_fwd, _hybrid_fused_conv1d_gdn_bwd
)
