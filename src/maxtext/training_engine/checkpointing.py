  ...
  def save_checkpoint(
    ...
    if self.get_latest_step() == step:
      if not self._supersedes_saved_checkpoint(step, checkpoint_state.micro_step_count):
        ...
        return False
      logging.info("Replacing intra-step checkpoint at step %d with a more complete one.", step)
      # Drain in-flight saves before Orbax deletes the old checkpoint to overwrite it.
      self._checkpointer.wait()
      # `overwrite` lifts Orbax's existing-step check (deleting the old step in place);
      # `force` is still needed because the save-interval policy declines a step it has
      # already saved.
