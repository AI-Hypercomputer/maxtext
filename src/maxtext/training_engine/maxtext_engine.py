...
class MaxTextTrainingEngine(abstract_engine.AbstractTrainingEngine):
  ...
  def __init__(
    ...
    self._use_weight_converter = bool(
        self._config.use_weight_converter or os.environ.get("USE_WEIGHT_CONVERTER", "0").lower() in ("1", "true", "yes")
    )
    self._rollout_backend = self._config.rollout_backend or os.environ.get("ROLLOUT_BACKEND", "maxtext")
  ...
  def prepare_weight_sync(
    ...
    if staging_transport == "raiden":
      ...
      if self._config.use_raiden_ffi is True and not ffi_available:
        raise RuntimeError(
      ...
      use_raiden_ffi = raiden_synchronizer.resolve_use_ffi(self._config.use_raiden_ffi, is_proxy=is_pathways)
      ...
      host_stage = raiden_synchronizer.normalize_host_stage(None, use_ffi=use_raiden_ffi, is_proxy=is_pathways)
      ...
      if verify_weights:
        if is_pathways and not host_stage:
          logging.info("Skipping host-side checksum calculation in FFI mode to preserve 0 MB host memory staging.")
        else:
