...
class MaxTextTrainingEngine(abstract_engine.AbstractTrainingEngine):
  ...
  def _needs_recompile(self, signature: Any) -> bool:
    ...
    correctness -- reusing a kernel whose closed-over statistics changed is a silently wrong
  ...
  def prepare_weight_sync(
    ...
    Args:
      ...
      sync_request: Optional synchronization request parameters.
    ...
    if staging_transport == "raiden":
      ...
      if getattr(self._config, "scan_layers", False):
        params_state = raiden_unscan.unscan_layers(
            params_state, num_layers=self._config.num_decoder_layers
        )

      # 3. Bind parameters to the Raiden transport
      if not hasattr(self, "_raiden_sync") or self._raiden_sync is None:
        self._raiden_sync = raiden_synchronizer.RaidenSynchronizer(
            job_name="trainer",
            worker_index=getattr(self, "worker_index", 0),
            auto_h2d=False,
  ...
  def release_weight_sync(self, sync_request: Any = None, **kwargs: Any) -> Any:
    """Releases staged weight buffers after transfer completion."""
    if hasattr(self, "_raiden_sync") and self._raiden_sync is not None:
