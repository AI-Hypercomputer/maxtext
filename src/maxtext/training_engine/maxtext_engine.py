...
class MaxTextTrainingEngine(abstract_engine.AbstractTrainingEngine):
  ...
  def __init__(
    ...
    kv_tp = getattr(self._config, "kv_tp_size", 1) or rollout_tp
    moe_tp = getattr(self._config, "moe_mlp_tp_size", 1) or rollout_tp
  ...
  def close(self) -> None:
    ...
    if getattr(self._config, "enable_checkpointing", True) and self._checkpoint_dir() and self._checkpoint_manager:
