...
def _is_jax_dynamic(value: Any) -> bool:
  ...
  if not leaves:
    ...
    return True
  return all(leaf is None or isinstance(leaf, (float, int, jax.Array, np.generic, np.ndarray)) for leaf in leaves)
...
class MaxTextTrainingEngine(abstract_engine.AbstractTrainingEngine):
  ...
  def _split_into_chunks(self, nested_state: Any, num_chunks: int) -> list[Any]:
    ...
    """
    flat = flatten_dict(nested_state)
  ...
  def prepare_weight_sync(
    ...
    if staging_transport == "raiden":
      ...
      if self._raiden_syncs is None:
        ...
        self._raiden_syncs = [
            raiden_synchronizer.RaidenSynchronizer(
                job_name="trainer",
                worker_index=jax.process_index() * num_chunks + i + 1,
                auto_h2d=False,
      ...
      chunks = (
          self._split_into_chunks(params_state, num_chunks)
          if num_chunks > 1
          else [params_state]
      )
      del params_state
