...
class MaxTextTrainingEngine(abstract_engine.AbstractTrainingEngine):
  ...
  def _batch_data_shardings(self, dynamic_batch: Any) -> Any:
    ...
    The `in_shardings` half of what `_compile_for_batch` needs; `model_scope` places its
  ...
  def _place_read_only_inputs(self, inputs: Any) -> Any:
    ...
    `model_scope` does not own the jit -- the caller does.
  ...
  @contextlib.contextmanager
  def model_scope(self, *args: Any, **kwargs: Any) -> Iterator[tuple[nnx.Module, tuple[Any, ...], dict[str, Any]]]:
    ...
    live model off the train state, the inputs on `_leaf_data_sharding`, and the block
    held open under `_sharding_ctx` so MaxText's logical axis rules are active while the
    caller traces. The caller owns the computation and its own `jax.jit`, so nothing here
    is compiled or cached, and none of the engine's kernels, accumulators or metrics are
    touched.

    The block is where tracing has to happen: `jax.jit` is lazy, and the axis rules are
    gone once the block exits, so a call built inside it but traced afterwards loses both
    them and the placement above.

    Usage:

        with engine.model_scope(tokens, pad_id=0) as (model, args, kwargs):
    ...
    Yields:
      A `(model, args, kwargs)` tuple whose array inputs are committed to this engine's
      mesh.
