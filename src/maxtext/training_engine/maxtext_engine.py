...
def _row_and_mask(payload: abstract_engine.RLTrainerPayload) -> tuple[jax.Array, jax.Array]:
  """Returns the `[batch, seq]` model row and its mask, prompt half then completion half.

  `RLTrainerPayload` is prompt/completion-split, and concatenating is uniform across both
  of Tunix's assemblers rather than a special case for either: `PaddedBatchAssembler`
  fills `[B, P]` and `[B, C]`, while `SequencePackedBatchAssembler` puts the entire packed
  row in the completion half and leaves the prompt zero-width (`to_rl_trainer_payload` in
  `tunix/experimental/orchestrator/batch_assembly.py`).
  """
  halves = [jnp.asarray(payload.prompt_ids), jnp.asarray(payload.completion_ids)]
  masks = [
      jnp.ones_like(half) if mask is None else jnp.asarray(mask).astype(jnp.int32)
      for half, mask in zip(halves, (payload.prompt_mask, payload.completion_mask))
  ]
  return jnp.concatenate(halves, axis=-1), jnp.concatenate(masks, axis=-1).astype(jnp.int32)
...
def _aligned(field: Any, name: str, width: int) -> jax.Array | None:
  ...
  the packed assembler ever sets them -- where the prompt half is zero-width, so the two
...
class MaxTextTrainingEngine(abstract_engine.AbstractTrainingEngine):
  ...
  def _batch_data_shardings(self, dynamic_batch: Any) -> Any:
    ...
    afterwards) but wastes compute across the axis for that micro-batch. Sequence packing
    reaches this from the distributed packer, whose row count is tunix's
