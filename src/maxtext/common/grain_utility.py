...
class GrainCheckpointable(ocp.StatefulCheckpointable):
  ...
  def __init__(self, item, *, process_index=None, process_count=None, step=None):
  ...
  async def load(self, directory):
    ...
    if isinstance(item, list):
      ...
      specs = list(zip(item, self._process_index))  # pyrefly: ignore[bad-argument-type]

      async def _read_list():
        for iterator, idx in specs:
          filename = directory / _process_filename(idx, self._process_count)  # pyrefly: ignore[bad-argument-type]
          if not await asyncio.to_thread(filename.exists):
            raise ValueError(f"File {filename} does not exist.")
...
def for_restore(
  ...
  if process_count_stored > process_count_jax:
    ...
    return GrainCheckpointable(local_iterators, process_index=process_index, process_count=process_count_stored)
