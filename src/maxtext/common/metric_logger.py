...
class MetricLogger:
  ...
  def record_train_metrics(self, metrics, step, step_time):
    ...
    if getattr(self.config, "enable_non_spmd_diloco", False):
      # Non-SPMD DiLoCo learner threads each run under their own submesh; evaluating the
      # jnp-based schedule here would dispatch an unplaced JAX op from the logging thread.
