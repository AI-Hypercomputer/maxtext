...
def load_params_from_path(
  ...
  with context:
    ...
    restored = ocp.load(
        path, {restore_key: params_collection}, checkpointable_name=checkpointable_name  # pyrefly: ignore[bad-argument-type]
    )
...
def save_params_to_path(checkpoint_dir, params, use_ocdbt=True, use_zarr3=True):
  ...
  with context:
    ocp.save(
        checkpoint_dir, {"params": params}, checkpointable_name="items", overwrite=True  # pyrefly: ignore[bad-argument-type]
    )
