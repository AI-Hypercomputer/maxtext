...
def create_rl_components(  # pylint: disable=too-many-positional-arguments
  ...
  if trainer_config.rl.use_agentic_rollout:
    ...
    grpo_config = AgenticGrpoConfig(
        ...
        # Tuples for both: bucket edges and band bounds are formatted into
        # metric names, so they must be hashable and stable across the step.
        sampler_is_length_buckets=(
            tuple(trainer_config.rl.sampler_is_length_buckets)
            if trainer_config.rl.sampler_is_length_buckets
            else None
        ),
        sampler_is_report_bands=(
            tuple(tuple(band) for band in trainer_config.rl.sampler_is_report_bands)
            if trainer_config.rl.sampler_is_report_bands
            else ()
        ),
