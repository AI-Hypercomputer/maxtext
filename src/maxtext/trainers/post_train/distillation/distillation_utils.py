...
class CombinedDistillationStrategy(DistillationStrategy):
  ...
  def _record_router_similarity_metrics(
    ...
    """Helper to record router similarity metrics per layer."""
    if not (
    ...
    # router_selections shape: [L, B, T, K]
    # Calculate Top-1 Similarity
    s_top1 = student_output.router_selections[..., 0]  # [L, B, T]
    t_top1 = teacher_output.router_selections[..., 0]  # [L, B, T]
    top1_match = (s_top1 == t_top1).astype(jnp.float32)

    # Calculate Top-K Similarity
    s_expanded = jnp.expand_dims(student_output.router_selections, axis=-1)  # [L, B, T, K, 1]
    t_expanded = jnp.expand_dims(teacher_output.router_selections, axis=-2)  # [L, B, T, 1, K]
    matches = (s_expanded == t_expanded)
    overlap_counts = jnp.sum(jnp.any(matches, axis=-1), axis=-1).astype(jnp.float32)  # [L, B, T]
    ...
    for layer_idx in range(num_layers):
      # Apply mask per-layer
      layer_top1_match = top1_match[layer_idx]  # [B, T]
      layer_overlap_counts = overlap_counts[layer_idx]  # [B, T]
      layer_mask = mask

      top1_similarity = jnp.sum(layer_top1_match * layer_mask) / jnp.maximum(jnp.sum(layer_mask), 1.0)
      metrics[f"{prefix}/layer_{layer_idx}_router_similarity_top1"] = (top1_similarity, one)

      topk_similarity = jnp.sum((layer_overlap_counts / k_val) * layer_mask) / jnp.maximum(jnp.sum(layer_mask), 1.0)
      metrics[f"{prefix}/layer_{layer_idx}_router_similarity_top{k_val}"] = (topk_similarity, one)
  ...
  def _record_expert_fractions(
    ...
    """Helper to record expert selection fractions across all layers."""
    # router_selections shape: [L, B, T, K]
    L, B, T, K = router_selections.shape
    # Broadcast mask [B, T] to [L, B, T, K]
    flat_mask = jnp.broadcast_to(mask[None, :, :, None], (L, B, T, K)).reshape(-1)
    flat_indices = router_selections.reshape(-1)

    # Count occurrences, ignoring masked tokens by mapping them to an extra bin
    valid_indices = jnp.where(flat_mask > 0, flat_indices, self.num_experts)
    counts = jnp.bincount(valid_indices, length=self.num_experts + 1)[:self.num_experts]

    fractions = counts.astype(jnp.float32) / jnp.maximum(jnp.sum(counts), 1.0)
    one = jnp.array(1.0, dtype=jnp.float32)
    for i in range(self.num_experts):
      metrics[f"{prefix}_expert_{i}_fraction"] = (fractions[i], one)
  ...
  def compute_eval_loss(
    ...
    metrics = {
        ...
        "eval/student_perplexity": (jnp.exp(jnp.minimum(task_loss, _PPL_CE_CAP)), one),
    }
