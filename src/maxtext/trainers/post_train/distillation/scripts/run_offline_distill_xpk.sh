...
#   echo "== layering on ${XPK_BASE_IMAGE} =="
#   echo "  tunix   : ${TUNIX_SOURCE}"
#   echo "  flax    : git+https://github.com/google/flax@1fcc2a5" # Pinning Flax
#   echo "  jax     : ${JAX_PIN}"
#   echo "  jaxlib  : ${JAXLIB_PIN}"
#   echo "  libtpu  : ${LIBTPU_PIN}"
#   if ! sudo docker image inspect "$XPK_BASE_IMAGE" >/dev/null 2>&1; then
...
# # 2. Install the specific Flax version compatible with JAX $JAX_PIN
# RUN pip install --no-cache-dir --force-reinstall "git+https://github.com/google/flax@1fcc2a5"

# # 3. Repin jax/libtpu so the image's libtpu and the installed jax stay compatible.
# RUN pip install --no-cache-dir --force-reinstall --no-deps \\
#       "jax==$JAX_PIN" "jaxlib==$JAXLIB_PIN" "libtpu==$LIBTPU_PIN"

# # 4. The Goldilocks pin: satisfies both TensorFlow and TFDS
...
# ${gcsfuse_prelude} \
# python3 src/maxtext/trainers/post_train/distillation/save_top_k_teacher_logits.py ${XPK_DISTILL_CONFIG} \
#   --gcs_upload_path=\$BASE_OUTPUT_DIRECTORY/logits \
