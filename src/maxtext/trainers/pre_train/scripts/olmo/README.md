...
export TOTAL_DEVICES=512 # set to match device-type * 2 (Ironwood has 2 JAX devices/chip)
...
For new deployments, use the Cluster Toolkit version above. The XPK wrapper
...
Orbax picks up the latest checkpoint under `${BASE_OUTPUT_DIRECTORY?}/${RUN_NAME?}/checkpoints/`;
