...
def loss_fn(model, config, data, dropout_rng, params, sparsity_state=None, is_train=True):
  ...
  if config.routed_bias and config.routed_bias_update_rate > 0.0:
    if isinstance(model, nn.Module):
      ...
      # so collect every decoder match keyed by its path, joined into a single string --
      # the path is used downstream (maxtext_utils_nnx.apply_moe_bias_updates) to locate
      ...
      # leaves, so they pass through untouched. MTP updates are kept separate: each MTP
      # layer is un-scanned with a 1-D bias, applied below by index. Unlike
      # collect_intermediates_by_suffix we must not ravel: each decoder update is a 2-D
      # matrix transposed at the apply site.
      moe_bias_updates = {}
  ...
  if isinstance(model, nn.Module):
    ...
    if config.routed_bias and config.routed_bias_update_rate > 0.0:
      if getattr(config, "model_name", "").startswith("deepseek4"):
        ...
        # 1. Update main decoder MoE layers. Each sow is located generically by its path,
        # so this covers both DeepSeek's single scanned collection and Qwen3-Next's
        # per-layer-position sows.
