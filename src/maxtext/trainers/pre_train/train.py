...
def scale_up(
  ...
  # Inline delta check
  claimed = elastic_manager.available_inactive_slices
  newly_claimed = claimed - elastic_manager.active_slice_indices
  if not newly_claimed:
    _logger.info(
        "[*] No new slices to claim (already claimed). Skipping recovery."
    )
    elastic_manager.available_inactive_slices = frozenset()
    return

  # Claim the available slices by updating active_slice_indices
  elastic_manager.active_slice_indices = (
      elastic_manager.active_slice_indices | newly_claimed
  )
  elastic_manager.available_inactive_slices = frozenset()
...
def train_loop(config, recorder, state=None):
    ...
    while True:
      ...
      if elastic_manager and elastic_utils.elastic_enabled(config):
        ...
        if new_slice and not init_done:
          ...
          raise elastic_utils.manager.ScaleUpSignalError("Scale up during initialization")

        if init_done and new_slice:
          raise elastic_utils.manager.ScaleUpSignalError("Both events set during initialization")
  ...
  # Initialize host snapshot manager only in elastic mode (moved here so start_step is defined)
  if config.elastic_enabled:
  ...
  try:
    ...
    while python_vars["step"] < immutable_data["steps"]:
      with (
        ...
        try:
          if elastic_manager is not None and elastic_manager.available_inactive_slices:
            scale_up(
                jax_device_state,
                python_vars,
                immutable_data,
          ...
          if config.elastic_enabled and elastic.is_error_due_to_slice_down(e):
            _logger.exception("[!] Elastic event detected around step %d", python_vars["step"])
            recover(jax_device_state, python_vars, immutable_data)
          else:
            _logger.exception("[!] JAX Runtime Error detected around step %d. Re-raising.", python_vars["step"])
            raise
