  ...
  if config.gradient_accumulation_steps > 1:
    ...
    if isinstance(model, nn.Module):
      ...
      if config.parameter_memory_host_offload:
        # Params are kept on host (pinned_host) in in_shardings. Move them to
        # device before the forward/backward pass so that all dot_general
        # operands share the same memory space (XLA on GPU requires this).
        device_model_shardings = jax.tree_util.tree_map_with_path(
            maxtext_utils_nnx.move_memory_to_device,
            state_mesh_shardings.model,
            is_leaf=lambda x: isinstance(x, NamedSharding),
        )
        model_state = nnx.state(state.model)
        new_model_state = jax.device_put(model_state, device_model_shardings)
        nnx.update(state.model, new_model_state)
