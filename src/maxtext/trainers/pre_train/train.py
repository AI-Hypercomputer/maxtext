...
def train_step(model, config, state_mesh_shardings, params_shardings, state, data, dropout_rng=None):
  ...
  if isinstance(model, nn.Module):
    ...
    # Capture fp8 OWG stats before apply_gradients.
    # The optimizer (Adam) accumulates mu/nu moments for OWG entries too; apply_gradients
    # produces wrong values for them. We overwrite with the correct values after.
    # In FSDP (ici_fsdp_parallelism > 1), Flax's fp8 backward can produce NaN OWG grads
    # because `g` at out_qdq_bwd may be NaN from per-device partial gradient issues. This
    # NaN also propagates to regular param grads via the straight-through estimator. Fix:
    # replace NaN OWG entries with current state values (skip the amax update), and zero
    # out any remaining NaN in float grads so the optimizer receives finite values.
    fp8_stats = grads.get(maxtext_utils.OVERWRITE_WITH_GRADIENT, None)
    ...
    # Zero out any remaining NaN in float gradients (NaN from straight-through estimator
    # would produce NaN params; zeroing gives a zero gradient contribution instead).
    ...
    # Overwrite fp8 OWG params with the NaN-sanitized values,
    # bypassing whatever the optimizer wrote for these entries.
    if fp8_stats is not None:
    ...
    if config.gradient_clipping_threshold > 0:
      ...
      grads = raw_grads
    # Zero out any NaN in float gradients (fp8 straight-through estimator can produce NaN
    # under FSDP at step 0, corrupting params via the optimizer update).
    grads = jax.tree_util.tree_map(
        lambda x: jnp.nan_to_num(x, nan=0.0) if jnp.issubdtype(x.dtype, jnp.floating) else x,
        grads,
    )
  ...
  if not config.optimizer_memory_host_offload:
    ...
    if isinstance(model, nn.Module):
      ...
      _, model_params, _ = nnx.split(new_state.model, nnx.Param, ...)
