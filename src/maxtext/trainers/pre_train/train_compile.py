...
def get_shaped_inputs(topology_mesh, config):
  ...
  if config.pure_nnx:
    ...
    # Keep axis_rules active for model init logic but omit jax.set_mesh: Flax 0.12.6
    # _to_variable calls var.shape for all variables when a global mesh is set, which
    # crashes on masked optimizer state vars (MaskedNode has no .shape).  We only need
    # the graphdef here, so sharding auto-assignment is not required.
