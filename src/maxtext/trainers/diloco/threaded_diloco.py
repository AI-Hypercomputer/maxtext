...
def get_first_step(model, state):
  """Extracts step integer safely from the NNX/Optax train state (state.optimizer.step).

  Prevents AttributeError and multislice host transfer errors.
  """
  del model  # NNX-only (Linen was removed on main); kept for call-site signature parity
  try:
...
def _run_learner_loop(
  ...
  with jax.set_mesh(submesh), submesh, nn_partitioning.axis_rules(learner_config.logical_axis_rules):
    ...
    params_template = nnx.state(state.model, nnx.Param)
    ...
    try:
      ...
      for step in range(start_step, learner_config.steps):
        ...
        with jax.profiler.StepTraceAnnotation(f"train_learner_{learner_idx}", step_num=step):
          ...
          # NNX train/eval steps take no rng (Linen was removed on main).
          step_rng_args = ()
    ...
    try:
      with fragment_locks[frag_idx]:
        ...
        if config.enable_checkpointing and checkpoint_manager is not None and (step % config.checkpoint_period == 0 or step == sync_steps[-1]):
          ...
          # On main, `save_checkpoint` routes `enable_diloco=True` states through the SPMD
          # `to_diloco_checkpoint_dict` (DiLoCoTrainState layout); SyncerState is a plain
          # PyTreeNode and must be saved as-is. The branch achieved this with
          # `pure_nnx=False`, a flag main no longer has. deepcopy (not copy.copy) so the
          # override does not leak into the shared global config's `_flat_config`.
          syncer_ckpt_config = copy.deepcopy(config)
          syncer_ckpt_config._flat_config["enable_diloco"] = False
