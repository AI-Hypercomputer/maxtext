...
def pre_process_qwen3_image(image: np.ndarray | list[np.ndarray]):
  ...
  for img in images_in:
    ...
    pil_img = pil_img.resize((644, 532))
    ...
    resized_img_pil = pil_img.resize((resized_width, resized_height), resample=resample_method)
    resized_img_np = np.array(resized_img_pil).astype(np.float32)
    # print(f"Resized image shape: {resized_img_np.shape}")
...
def smart_nframes(
  ...
  if "nframes" in ele:
    ...
    if nframes > total_frames:
      max_logging(f"smart_nframes: nframes[{nframes}] > total_frames[{total_frames}]")
...
def _read_video_decord_no_torch(
    ele: dict,
...
def preprocess_video(video):
  ...
  with jax.default_device(jax.devices("cpu")[0]):
    ...
    print(f"JAX resized video shape: {jax_resized_video.shape}")
    print(f"resized dtype: {jax_resized_video.dtype}")
    jax_resized_video = jax_resized_video.astype(np.uint8)
    resized_height, resized_width = smart_resize(
        height,
        width,
        factor=image_factor_2,
        min_pixels=VIDEO_MIN_PIXELS,
    ...
    print(f"JAX resized video2 shape: {jax_resized_video.shape}")
  ...
  # print(f"mean after normalization: {np.mean(resized_video)}, std after normalization: {np.std(resized_video)}")
...
def pre_process_audio_qwen3_omni(audio_array):
  ...
  audio_mask = np.ones((audio_features.shape[0], audio_features.shape[1]), dtype=np.int32)
  print(f"Original audio shape: {audio_features.shape}")
  print(f"Original audio mask shape: {audio_mask.shape}")
  ...
  print(f"Extracted audio features mask shape: {audio_features_mask.shape}")
...
def preprocess_mm_data_qwen3_omni(config):
  ...
  if config.image_path is not None:
    ...
    processed_images = pre_process_qwen3_image(images)
    processor_outputs.pixel_values = processed_images

  if config.video_path is not None:
    new_config = {
        "video": "/home/hengtaoguo_google_com/projects/test_video.mp4",
        "fps": 2.0,
        "video_start": 0.0,
        "video_end": 10.0,
    }
    video_array, fps = _read_video_decord_no_torch(new_config)
    video_processed = preprocess_video(video_array)
    processor_outputs.mm_data["pixel_values_video"] = video_processed
  ...
  if config.audio_path is not None or (config.video_path is not None and config.use_audio_in_video):
    ...
    processor_outputs.mm_data["audio_features"] = mt_audio
    processor_outputs.mm_data["audio_mask"] = mt_audio_mask
