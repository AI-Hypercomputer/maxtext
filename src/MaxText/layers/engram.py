...
class CompressedTokenizer:
  ...
  def _build_lookup_table(self, tokenizer: HFTokenizer, normalizer: normalizers.Sequence):
    """
...
class NgramHashMapping:
  ...
  def __init__(
    ...
    """
    Args:
      engram_vocab_size: List of minimum head vocab sizes for each n-gram order.
  ...
  def _calculate_vocab_size_across_layers(self):
    ...
    def find_next_unseen_prime(start: int, seen_primes: set):
      candidate = start + 1
...
class Engram(nnx.Module):
  ...
  def __init__(
    ...
    self.multi_head_embedding = MultiHeadEmbedding(
        config=config, vocab_sizes=vocab_sizes, head_dim=engram_head_dim, mesh=mesh, rngs=rngs
    )
