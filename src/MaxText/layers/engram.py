...
class ShortConv(nnx.Module):
  ...
  def __init__(
    ...
    self.norms = nnx.List([nnx.RMSNorm(num_features=hidden_size, rngs=rngs) for _ in range(hc_mult)])
...
#     self.embedding = Embed(num_embeddings=total_N, num_features=D, config=config, mesh=mesh, rngs=rngs)
...
class Engram(nnx.Module):
  ...
  def __init__(self, layer_id: int, config, backbone_cfg, rngs: nnx.Rngs):
    ...
    self.norm1 = nnx.List([nnx.RMSNorm(backbone_cfg.hidden_size, rngs=rngs) for _ in range(backbone_cfg.hc_mult)])

    self.norm2 = nnx.List([nnx.RMSNorm(backbone_cfg.hidden_size, rngs=rngs) for _ in range(backbone_cfg.hc_mult)])
