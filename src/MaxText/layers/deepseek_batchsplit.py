...
class DeepSeekMoELayer(DeepSeekGenericLayer):
  ...
  def __call__(
    ...
    def attn(x, decoder_segment_ids, decoder_positions):
    ...
    x = split(x)
    dpos = split(decoder_positions)
    dseg = split(decoder_segment_ids)

    # Attention.
    x = [xi + attn(xi, yi, zi) for xi, yi, zi in zip(x, dseg, dpos)]

    # Mixture-of-experts.
    x = [xi + moe(xi) for xi in x]
