...
class DecoderLayer(nnx.Module):
    ...
    def __init__(
        ...
        # Note: rms_norm is likely a Linen Module or function. We wrap it for NNX.
        self.pre_self_attention_norm = nnx_wrappers.ToNNX(
            rms_norm(
                num_features=cfg.emb_dim, # Input shape is dynamic, usually emb_dim
                dtype=cfg.dtype,
        ...
        # 2. Self Attention
        # attention_as_linen returns a Module. We wrap it.
        # We need a sample input shape to initialize attention if it's strictly shape-dependent
        # For simplicity here, we assume lazy init or that attention_as_linen handles it via ToNNX compat.
        attention_cls = attention_as_linen(
            config=self.config,
            ...
            inputs_q_shape=(1, 1, cfg.emb_dim), # Placeholder shape for init
        ...
        self.self_attention = nnx_wrappers.ToNNX(attention_cls, rngs=rngs)

        # 3. MLP Block
        mlp_cls = linears.mlp_block(
        ...
        self.mlp = nnx_wrappers.ToNNX(mlp_cls, rngs=rngs)
        
        # 4. Dropout
        self.dropout = nnx.Dropout(rate=cfg.dropout_rate, rngs=rngs, broadcast_dims=(-2,))
    ...
    def __call__(
        ...
        # --- Attention ---
        # Note: ToNNX wrappers call the underlying Linen module. 
        # We need to match the signature of the wrapped linen module.
        ...
        if cfg.record_internal_nn_metrics:
            # NNX Sow API equivalent (using context metric collection if needed, or simple print for debug)
            # nnx.add_summary is available in newer versions, or manual hooks.
            pass 
...
class Decoder(nnx.Module):
        ...
        def create_layer_fn(rng):
            if issubclass(decoder_layer_class, nnx.Module):
                layer = decoder_layer_class(
                    ...
                    model_mode=self.model_mode, rngs=rng, **layer_kwargs
                )
            else:
                layer_linen = decoder_layer_class(
                    config=self.config, mesh=self.mesh, quant=self.quant, 
    ...
    def apply_output_head(self, shared_embedding, y, deterministic, model_mode):
        ...
        if cfg.logits_via_embedding:
            ...
            # Apply sharding manually if needed, or rely on internal mesh/sharding
    ...
    def __call__(
        ...
        if cfg.scan_layers:
            ...
            for i, layer in enumerate(self.layers):
                ...
                # Call the layer
                # Note: layer_kwargs now correctly contains/excludes bidirectional_mask
                out = layer(
                    y,
                    *layer_args,
                    kv_cache=kv_cache,
        ...
        if cfg.num_vocab_tiling > 1 and self.model_mode == MODEL_MODE_TRAIN:
            logits = None
            # If using NNX graph metrics, you might use nnx.add_summary here 
            # instead of Linen's sow.
        else:
