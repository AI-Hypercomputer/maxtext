...
class Qwen3NextFullAttention(nnx.Module):
  ...
  It wraps the main `attentions.attention_op` class, which is used to perform the core attention operation:
  ...
  Attributes:
    ...
    q_proj: Dense layer for query and gate.
    k_proj: Dense layer for key.
    v_proj: Dense layer for value.
    o_proj: Dense layer for output projection.
    q_norm: RMSNorm for queries.
    k_norm: RMSNorm for keys.
    rotary_embedding: Partial rotary embedding module.
    attention_op: The core attention calculation module.
  """
  def __init__(
