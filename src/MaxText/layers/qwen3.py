...
from MaxText.common_types import AttentionType, Config, DType, Array, BATCH, LENGTH_NO_EXP, EMBED, AttentionType, MODEL_MODE_TRAIN
...
from MaxText.layers.linears import DenseGeneral, MlpBlock, DenseGeneral
...
class Qwen3OmniMoeVisionBlock(nnx.Module):
  ...
  def __call__(
    ...
    Returns:
        Output tensor of shape (batch, T*H*W, hidden_size)
    """
    attn_output = self.attn(self.ln1(x), num_frames=num_frames, height=height, width=width)
    x = x + attn_output
    y = self.ln2(x)
    y = self.mlp(y)
    y = jax.nn.gelu(y)
