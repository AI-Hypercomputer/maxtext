    ...
    def __init__(
        ...
        *,
        ...
        batch_size, seq_len = max_utils.get_batch_seq_len_for_mode(
            config=cfg, model_mode=MODEL_MODE_TRAIN
        )
        self.transformer_layer.lazy_init(
            inputs=jnp.zeros(
                (batch_size, seq_len, self.config.emb_dim), dtype=self.config.dtype
            ),
        ...
        *,
        ...
        for k in range(1, cfg.mtp_num_layers + 1):
            ...
            mtp_logits = self.decoder.apply_output_head(
                shared_embedding, mtp_hidden_state, deterministic, model_mode
            )
            ...
            if cfg.mtp_eval_target_module == k:
                ...
                mtp_preds_list.append(
                    jnp.argmax(mtp_logits, axis=-1).astype(jnp.float32)
                )
...
def calculate_mtp_acceptance_rate(intermediate_outputs, config):
    ...
    mtp_preds = (
        mtp_preds_raw[0]
        if isinstance(mtp_preds_raw, (tuple, list)) and mtp_preds_raw
        else mtp_preds_raw
    )
    valid_mask = (
        valid_mask_raw[0]
        if isinstance(valid_mask_raw, (tuple, list)) and valid_mask_raw
        else valid_mask_raw
    )
