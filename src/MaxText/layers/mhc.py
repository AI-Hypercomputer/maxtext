...
class ManifoldConstrainedHyperConnections(nnx.Module):
  ...
  def __call__(
    ...
    Args:
        ...
        residual_x: Input tensor of shape `(batch, seq, expansion_rate, dim)`, generally before pre-normalization.
    ...
    layer_input = jnp.einsum("bskd,bsk -> bsd", layer_x, pre_mapping, precision=self.matmul_precision)
    ...
    res_out = jnp.einsum("bskd,bskm -> bsmd", residual_x, res_mapping, precision=self.matmul_precision)
