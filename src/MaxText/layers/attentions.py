...
class Attention(nnx.Module):
  ...
  def __call__(
    ...
    if model_mode == MODEL_MODE_PREFILL:
      ...
      query = self._maybe_shard_with_logical(query, (DECODE_BATCH, DECODE_LENGTH, HEAD, D_KV), sharding_desc="attention/query")
      key = self._maybe_shard_with_logical(key, (DECODE_BATCH, DECODE_LENGTH, KV_HEAD, D_KV), sharding_desc="attention/key")
      value = self._maybe_shard_with_logical(value, (DECODE_BATCH, DECODE_LENGTH, KV_HEAD, D_KV), sharding_desc="attention/value")
    elif model_mode == MODEL_MODE_TRAIN and self.config.expert_shard_attention_option == EP_AS_CONTEXT:
