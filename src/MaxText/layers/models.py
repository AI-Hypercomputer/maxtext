...
from MaxText.layers.decoders import Decoder, decoder_as_linen 
...
class TransformerLinenPure(nn.Module):
  ...
  def setup(self):
    ...
    self.decoder = decoder_as_linen(config=cfg, mesh=mesh, quant=self.quant, model_mode=self.model_mode, rngs=nnx.Rngs(0))
