...
class YarnRotaryEmbedding(nnx.Module):
  ...
  def __call__(self, inputs: Array, position: None | Array = None) -> Array:
    ...
    rotated_sharding = (
      NamedSharding(self.mesh, nn.logical_to_mesh_axes(
        ('activation_batch', 'activation_length_no_exp', None, None)
      )) if self.shard_mode == ShardMode.EXPLICIT else None
    )
    rotated = jnp.einsum('ijkl, ijml->ijkl', inputs_complex, freqs, out_sharding=rotated_sharding)  # shape: [B, S, N, half_dim]
