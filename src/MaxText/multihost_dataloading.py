    ...
    if self.out_of_data and self.generate_padding_batch:
      ...
      if not loaded_data_success:
        try:
          ...
          if isinstance(e, StopIteration) and self.generate_padding_batch:
            max_logging.log(
                f"MultiHostDataLoadIterator: host {jax.process_index()} failed to load data with {type(e)} error: ({e}). It may reached end of data, generating padding batch as generate_padding_batch=True."
            )
