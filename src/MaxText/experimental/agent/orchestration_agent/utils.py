...
def check_github_file_exists(blob_url):
  ...
  Returns:
    ...
    str
      The raw URL of the file if it exists, or an error message.
  """
...
def get_github_file_content(blob_url):
  ...
  Returns:
    tuple: A tuple containing
      ...
      bool
        True if the content was retrieved successfully, False otherwise.
      str
        The content of the file if successful, or an error message if not.
  """
...
def remove_local_imports(source_code, filepath=None):
  ...
  Returns:
    tuple: A tuple containing
      ...
      str
        The modified source code with local imports removed.
      str
        A newline-separated string of the names of the removed imports.
  """
...
def have_module(target_name, file_url):
  ...
  Returns:
    ...
    tuple
      ``(ImportFrom, full_module)`` if the ``target_name`` is an alias from an
      import statement.
  """
...
def resolve_complex_import(module_path_base_url, importPackage, base_url, current_dir_url, num_try=0, Message=""):
  ...
  Args:
    ...
    importPackage (str): The specific name being imported (e.g.,
      ``modeling_llama``, ``configuration_llama``).
    base_url (str): The base URL of the repository.
    current_dir_url (str): The URL of the directory containing the original
      import statement.
