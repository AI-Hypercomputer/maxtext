...
# Assuming these utilities are available in the target environment
...
if TYPE_CHECKING:
    # JAX equivalent for type checking
...
class PretrainedConfig(PushToHubMixin):
  ...
  r"""
    ...
    Arg:
        ...
        torch_dtype (`str`, *optional*):
            ...
            floating type string. For example, for `jnp.float16` ``torch_dtype` is the
            `"float16"` string.
  ...
  def __init__(
      ...
      *,
      ...
      torch_dtype: Optional[Union[str, jnp.dtype]] = None,
      # Common arguments
    ...
    if torch_dtype is not None and isinstance(torch_dtype, str) and is_jax_available():
      # we will start using self.torch_dtype in v5, but to be consistent with
      # from_pretrained's torch_dtype arg convert it to an actual jnp.dtype object
      torch_dtype = getattr(jnp, torch_dtype)
...
from maxtext import __version__
from maxtext.constants import CONFIG_NAME


def get_configuration_file(configuration_files: List[str]) -> str:
  """Get the configuration file to use for this version of MaxText.

  Args:
    configuration_files: The list of available configuration files.
  ...
  maxtext_version = version.parse(__version__)
  for v in available_versions:
    if version.parse(v) <= maxtext_version:
...
class PretrainedConfig(PushToHubMixin):
  ...
  r"""
    ...
    Arg:
        ...
        tie_word_embeddings (`bool`, *optional*, defaults to `True`):
            Whether the model's input and output word embeddings should be tied. Note that this is only relevant if the
            model has a output word embedding layer.
        jax_dtype (`str`, *optional*):
            The `dtype` of the weights. This attribute can be used to initialize the model to a non-default `dtype`
            (which is normally `float32`) and thus allow for optimal storage allocation. For example, if the saved
            model is `bfloat16`, ideally we want to load it back using the minimal amount of memory needed to load
            `bfloat16` weights. Since the config object is stored in plain text, this attribute contains just the
            floating type string. For example, for `jnp.bfloat16` ``jax_dtype` is the
            `"bfloat16"` string.
  ...
  def __init__(
      ...
      *,
      ...
      output_attentions: bool = False,
      return_dict: bool = True,
      jax_dtype: Optional[Union[str, jnp.dtype]] = None,
      # Common arguments
      pruned_heads: Optional[Dict[int, List[int]]] = None,
      tie_word_embeddings: bool = True,
      chunk_size_feed_forward: int = 0,
    ...
    if jax_dtype is not None and isinstance(jax_dtype, str):
      jax_dtype = getattr(jnp, jax_dtype)
  ...
  def dict_jax_dtype_to_str(self, d: Dict[str, Any]) -> None:
    """
    Checks whether the passed dictionary and its nested dicts have a *jax_dtype* key and if it's not None,
    converts jnp.dtype to a string of just the type. For example, `jnp.bfloat16` get converted into *"bfloat16"*
    string, which can then be stored in the json format.
    """
    if d.get("jax_dtype") is not None:
      if isinstance(d["jax_dtype"], dict):
        d["jax_dtype"] = {k: v.__name__ if not isinstance(v, str) else v for k, v in d["jax_dtype"].items()}
      elif not isinstance(d["jax_dtype"], str):
        d["jax_dtype"] = d["jax_dtype"].__name__
  ...
  def _remove_keys_not_serialized(self, d: Dict[str, Any]) -> None:
    ...
    """
    if hasattr(self, "quantization_config"):
      # Pop the `_pre_quantization_dtype` as jnp.dtypes are not serializable.
...
# The rope_config_validation function is assumed to exist in the MaxText ecosystem.
from MaxText.layers.rope_utils import rope_config_validation
