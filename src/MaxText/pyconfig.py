...
def pyconfig_to_pydantic(raw_keys: dict[str, Any]) -> types.MaxTextConfig:
  """
  Converts a raw dictionary from pyconfig_deprecated into a pydantic MaxTextConfig object.
  ...
  if "decoder_block" in dehydrated and isinstance(dehydrated["decoder_block"], DecoderBlockType):
    dehydrated["decoder_block"] = dehydrated["decoder_block"].value

  # Convert tuples (from pyconfig_deprecated) back to lists for Pydantic validation.
