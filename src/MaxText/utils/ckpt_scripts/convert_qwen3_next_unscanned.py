...
MODEL_PARAMS_DICT = {
    "qwen3-next-80b-a3b": {
        ...
        "num_blocks": 12,  # 48 layers / 4 types
        ...
        "gdn_in_proj_ba_dim": 64,
        "gdn_in_proj_qkvz_dim": 12288,
        "gdn_norm_dim": 128,
...
# These will host the simple 1 to 1 mappings
def _hf_to_maxtext_mapping(layer_idx: int = -1, num_experts: int = 512) -> dict:
  """
  ...
  mapping = {
      ...
      # moe
      # shared
      f"model.layers.{layer_idx}.mlp.shared_expert.down_proj.weight": f"decoder.layers_{layer_idx}.mlp.shared_expert.wo.kernel",
      f"model.layers.{layer_idx}.mlp.shared_expert.gate_proj.weight": f"decoder.layers_{layer_idx}.mlp.shared_expert.wi_0.kernel",
      f"model.layers.{layer_idx}.mlp.shared_expert.up_proj.weight": f"decoder.layers_{layer_idx}.mlp.shared_expert.wi_1.kernel",
      f"model.layers.{layer_idx}.mlp.shared_expert_gate.weight": f"decoder.layers_{layer_idx}.mlp.shared_expert_gate.kernel",
      # routed
      f"model.layers.{layer_idx}.mlp.gate.weight": f"decoder.layers_{layer_idx}.mlp.routed_experts.gate.kernel",
  }

  # Gated attention
  if layer_idx % 4 == 3:
    # Gated Attention (GA) Layer
    mapping.update(
        {
            f"model.layers.{layer_idx}.self_attn.k_norm.weight": (
                f"decoder.layers_{layer_idx}.attention.attention.key_norm.scale"
            ),
            f"model.layers.{layer_idx}.self_attn.k_proj.weight": (
                f"decoder.layers_{layer_idx}.attention.attention.key.kernel"
            ),
            f"model.layers.{layer_idx}.self_attn.o_proj.weight": (
                f"decoder.layers_{layer_idx}.attention.attention.out.kernel"
            ),
            f"model.layers.{layer_idx}.self_attn.q_norm.weight": (
                f"decoder.layers_{layer_idx}.attention.attention.query_norm.scale"
            ),
            f"model.layers.{layer_idx}.self_attn.q_proj.weight": (
                f"decoder.layers_{layer_idx}.attention.attention.query.kernel"
            ),
            f"model.layers.{layer_idx}.self_attn.v_proj.weight": (
                f"decoder.layers_{layer_idx}.attention.attention.value.kernel"
            ),
        }
    )
  else:
    # Gated DeltaNet (GDN) Layer | Linear Attention
    mapping.update(
        {
            f"model.layers.{layer_idx}.linear_attn.A_log": f"decoder.layers_{layer_idx}.attention.A_log",
            f"model.layers.{layer_idx}.linear_attn.conv1d.weight": f"decoder.layers_{layer_idx}.attention.conv1d.kernel",
            f"model.layers.{layer_idx}.linear_attn.dt_bias": f"decoder.layers_{layer_idx}.attention.dt_bias",
            f"model.layers.{layer_idx}.linear_attn.in_proj_ba.weight": f"decoder.layers_{layer_idx}.attention.in_proj_ba.kernel",
            f"model.layers.{layer_idx}.linear_attn.in_proj_qkvz.weight": f"decoder.layers_{layer_idx}.attention.in_proj_qkvz.kernel",
            f"model.layers.{layer_idx}.linear_attn.norm.weight": f"decoder.layers_{layer_idx}.attention.norm.rms_norm.scale",
            f"model.layers.{layer_idx}.linear_attn.out_proj.weight": f"decoder.layers_{layer_idx}.attention.out_proj.kernel",
        }
    )
  return mapping
