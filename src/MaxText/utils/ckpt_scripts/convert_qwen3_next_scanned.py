    ...
    for hf_key, mt_key in hf_map.items():
      if mt_key.endswith(maxtext_key_suffix):
        if hf_key in chkpt_vars:
          ...
          raise ValueError(
              f"HF Key {hf_key} not found in chkpt_vars for MaxText suffix: {maxtext_key_suffix} in layer {l}"
          )
...
if __name__ == "__main__":
  ...
  # Dry run options
  parser.add_argument(
      "--num_layers_to_convert", type=int, default=-1, help="Number of layers to convert for a dry run. -1 for all."
  )
  parser.add_argument(
      "--num_experts_to_convert", type=int, default=-1, help="Number of experts to convert for a dry run. -1 for all."
  )
