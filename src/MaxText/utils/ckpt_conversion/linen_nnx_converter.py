...
def _transpose_layers_axes(tree: Any, src_axis: int = 1, dst_axis: int = 0) -> Any:
  """Transpose the layers dimension in arrays within a tree.

  Linen stores stacked layers as (d0, num_layers, d1, ...) with layers at axis 1.
  NNX stores stacked layers as (num_layers, d0, d1, ...) with layers at axis 0.

  This swaps axis 0 and axis 1 for all arrays with ndim >= 2.
  """
  if isinstance(tree, dict):
...
def _stack_layers(decoder: dict) -> dict:
  ...
  To:
    decoder/layers/mlp/wi/kernel -> [num_layers, embed, mlp]
  """
  import re
  ...
  # Get the structure from the first layer to use as template
  first_layer = layer_indices[sorted_indices[0]]
...
def convert_linen_to_nnx(state: dict) -> dict:
  ...
  if "params" in state:
    ...
    if "decoder" in stripped and isinstance(stripped["decoder"], dict):
      stripped["decoder"] = _stack_layers(stripped["decoder"])
    if "encoder" in stripped and isinstance(stripped["encoder"], dict):
      stripped["encoder"] = _stack_layers(stripped["encoder"])

    # Transpose layers dimension: Linen (d0, num_layers, ...) -> NNX (num_layers, d0, ...)
    for component in ("decoder", "encoder"):
      if component in stripped and isinstance(stripped[component], dict):
        if "layers" in stripped[component]:
          log(f"  Transposing {component}/layers axes: (d0, layers, ...) -> (layers, d0, ...)")
          stripped[component]["layers"] = _transpose_layers_axes(
              stripped[component]["layers"], src_axis=1, dst_axis=0
          )
...
def convert_nnx_to_linen(state: dict) -> dict:
  ...
  if "params" in state:
    ...
    # Transpose layers dimension: NNX (num_layers, d0, ...) -> Linen (d0, num_layers, ...)
    for component in ("decoder", "encoder"):
      if component in stripped and isinstance(stripped[component], dict):
        if "layers" in stripped[component]:
          log(f"  Transposing {component}/layers axes: (layers, d0, ...) -> (d0, layers, ...)")
          stripped[component]["layers"] = _transpose_layers_axes(
              stripped[component]["layers"], src_axis=0, dst_axis=1
          )
