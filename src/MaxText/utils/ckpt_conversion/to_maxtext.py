...
class LazyHFLoader:
  ...
  def _initialize_index(self):
    ...
    if "model.safetensors.index.json" in files:
      ...
      if self.is_local:
        raise ValueError("Could not find recognized model weights (safetensors) in local directory.")
...
def _build_single_axis_stacked_tensor(
  ...
  """
  tensors_to_stack = []
  # it's a standard scanned layer, and we use the configured param_scan_axis.
  axis_to_stack = config.param_scan_axis
  # Heuristic to determine if we are stacking layers or experts.
  # Applicable: qwen3, mixtral (no expert stacking: gpt-oss)
  # If the number of items to stack equals the number of layers, it is standard scanned layer.
  # Otherwise, if an unscanned MoE layer, and we stack along the expert axis (0).
  if (
      config.num_experts > 1
      and any(config.model_name.startswith(prefix) for prefix in ["qwen3", "mixtral"])
      and len(hf_source_keys) == config.num_experts
  ):
    axis_to_stack = 0
  # The hook function needs the shape of an individual slice, not the full stacked tensor.
...
def get_maxtext_model_info(config):
  ...
  for idx, (path_tuple, abstract_leaf_value) in enumerate(abstract_params_flat):
    ...
    maxtext_abstract_dict[mt_param_key] = (idx, mt_target_shape)
