...
```bash
python -m MaxText.utils.ckpt_conversion.to_maxtext src/MaxText/configs/base.yml \
...
```bash
python -m MaxText.utils.ckpt_conversion.to_huggingface src/MaxText/configs/base.yml \
...
To ensure the conversion was successful, you can use the `src/MaxText/tests/forward_pass_logit_checker.py` script. It runs a forward pass on both the original and converted models and compares the output logits to verify conversion. It is used to verify the bidirectional conversion. 

### Usage

```bash
python3 -m maxtext_tests.forward_pass_logit_checker src/MaxText/configs/base.yml \
