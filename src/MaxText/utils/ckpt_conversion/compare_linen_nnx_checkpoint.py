...
def detect_format(state: dict) -> str:
  ...
  raise ValueError(f"Could not detect checkpoint format. params keys: {list(params.keys()) if isinstance(params, dict) else type(params)}")
...
def load_checkpoint(checkpoint_path: str) -> dict:
  ...
  try:
    ...
    unsharded = jax.sharding.NamedSharding(
        single_device_mesh, jax.sharding.PartitionSpec()
    )

    # Build restore args that restore arrays without original sharding
    restore_args = jax.tree_util.tree_map(
        lambda x: ocp.ArrayRestoreArgs(sharding=unsharded)
        if hasattr(x, "shape")
        else None,
