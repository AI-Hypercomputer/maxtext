...
from MaxText.diloco import (
    build_diloco_state,
    build_diloco_train_step,
...
def train_loop(config, recorder, state=None):
  ...
  if not config.enable_diloco:
    raise TypeError("Set enable_diloco to True to run DiLoCo training.")
  ...
  state = build_diloco_state(config, lambda: state)
  diloco_train_step = build_diloco_train_step(config, train_step)
  ...
  with mesh, nn_partitioning.axis_rules(config.logical_axis_rules):
    ...
    shaped_batch = reshape_first_axis_with_diloco(config.num_diloco_replicas, shaped_batch)
  ...
  try:
    ...
    for step in np.arange(start_step, config.steps):
      ...
      with jax.profiler.StepTraceAnnotation("train", step_num=step):
        ...
        example_batch = reshape_first_axis_with_diloco(config.num_diloco_replicas, example_batch)
