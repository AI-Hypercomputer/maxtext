...
def create_audio_in_video_sequence(
  ...
  Args:
    ...
    audio_seqlens: Raw audio sequence lengths. Shape: (num_audios,).
  ...
  expected_audio_tokens = int(
      multimodal_utils._get_feat_extract_output_lengths(jnp.array(audio_seqlens[0])).item()
  )
  ...
  total_video_tokens = video_tokens_per_frame * num_frames
...
class GetRopeIndexComparisonTest(unittest.TestCase):
  ...
  def compare_with_pytorch(
    ...
    Args:
      ...
      audio_seqlens: Optional (num_audios,)
    ...
    jax_audio_seqlens = jnp.array(audio_seqlens) if audio_seqlens is not None else None
    ...
    jax_position_ids, jax_deltas = multimodal_utils.get_rope_index(
        ...
        audio_seqlens=jax_audio_seqlens,
    ...
    torch_audio_seqlens = torch.from_numpy(audio_seqlens).long() if audio_seqlens is not None else None
    ...
    torch_position_ids, torch_deltas = self.pytorch_model.get_rope_index(
        ...
        use_audio_in_video=use_audio_in_video,
        audio_seqlens=torch_audio_seqlens,
  ...
  def test_text_only(self):
    ...
    np.testing.assert_allclose(jax_pos, torch_pos, rtol=1e-5,
                               err_msg="Single sequence positions don't match PyTorch")
    np.testing.assert_allclose(jax_deltas, torch_deltas, rtol=1e-5,
                               err_msg="Single sequence deltas don't match PyTorch")
    ...
    np.testing.assert_allclose(jax_pos, torch_pos, rtol=1e-5,
                               err_msg="Padded sequence positions don't match PyTorch")
    np.testing.assert_allclose(jax_deltas, torch_deltas, rtol=1e-5,
                               err_msg="Padded sequence deltas don't match PyTorch")
    ...
    np.testing.assert_allclose(jax_pos, torch_pos, rtol=1e-5,
                               err_msg="Batched sequence positions don't match PyTorch")
    np.testing.assert_allclose(jax_deltas, torch_deltas, rtol=1e-5,
                               err_msg="Batched sequence deltas don't match PyTorch")

  def test_single_image(self):
    """Test single image sequence against PyTorch."""
    # Sequence: <|vision_start|> <|image_pad|> x4 <|vision_end|>
    input_ids = np.array([[
        VISION_START, IMAGE_TOKEN, IMAGE_TOKEN,
        IMAGE_TOKEN, IMAGE_TOKEN, VISION_END
    ]])
  ...
  def test_multiple_images(self):
    ...
    input_ids = np.array([[
        VISION_START, IMAGE_TOKEN, IMAGE_TOKEN, VISION_END,
        100,  # text token
        VISION_START, IMAGE_TOKEN, IMAGE_TOKEN, VISION_END,
    ]])
  ...
  def test_single_video_temporal_spacing(self):
    ...
    input_ids = np.array([[
        VISION_START,
        VIDEO_TOKEN, VIDEO_TOKEN, VIDEO_TOKEN, VIDEO_TOKEN,  # frame 1
        VIDEO_TOKEN, VIDEO_TOKEN, VIDEO_TOKEN, VIDEO_TOKEN,  # frame 2
        VISION_END,
    ]])
  ...
  def test_single_audio(self):
    ...
    audio_seqlens = np.array([1600])
    expected_tokens = int(
        multimodal_utils._get_feat_extract_output_lengths(jnp.array(1600)).item()
    )
    ...
    jax_pos, torch_pos, jax_deltas, torch_deltas = self.compare_with_pytorch(
        input_ids, audio_seqlens=audio_seqlens
    )
  ...
  def test_video_with_different_fps(self):
    ...
    input_ids = np.array([[
        VISION_START, *([VIDEO_TOKEN] * num_tokens), VISION_END
    ]])
  ...
  def test_audio_in_video(self):
    ...
    token_sequence = create_audio_in_video_sequence(
        video_grid_thw, audio_seqlens, second_per_grids, spatial_merge_size, position_id_per_seconds
    )
    ...
    np.testing.assert_allclose(jax_pos, torch_pos, rtol=1e-5,
                               err_msg="Audio-in-video positions don't match PyTorch")
    np.testing.assert_allclose(jax_deltas, torch_deltas, rtol=1e-5,
                               err_msg="Audio-in-video deltas don't match PyTorch")
...
class MRoPEComparisonTest(unittest.TestCase):
  ...
  def test_mrope_text_only_1d(self):
    ...
    assert_mrope_matches_pytorch(
        query_states, position_ids_3d,
        err_msg="MRoPE text-only output doesn't match PyTorch"
    )
  ...
  def test_mrope_vision_3d(self):
    ...
    assert_mrope_matches_pytorch(
        query_states, position_ids_3d,
        err_msg="MRoPE vision 3D output doesn't match PyTorch"
    )
  ...
  def test_mrope_mixed_sequence(self):
    ...
    assert_mrope_matches_pytorch(
        query_states, position_ids_3d,
        err_msg="MRoPE mixed sequence output doesn't match PyTorch"
    )
  ...
  def test_mrope_batch(self):
    ...
    assert_mrope_matches_pytorch(
        query_states, position_ids_3d,
        rtol=1e-3, atol=1e-3,
        err_msg="MRoPE batched output doesn't match PyTorch"
    )
...
class ComputeQwen3OmniPositionsTest(unittest.TestCase):
  ...
  def test_transform_wrapper(self):
    ...
    element = {
        "inputs": np.array([[VISION_START, IMAGE_TOKEN, IMAGE_TOKEN, IMAGE_TOKEN, IMAGE_TOKEN, VISION_END, 100]], dtype=np.int32),
