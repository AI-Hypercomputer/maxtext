...
def setup_initial_state(
  ...
  if config.pure_nnx:
    train_state_graphdef, _ = nnx.get_abstract_model(init_state_fn, mesh)
  ...
  with nn_partitioning.axis_rules(config.logical_axis_rules):
    ...
    if restored:
      ...
      _train_state = jax.jit(
          init_state_partial,
          in_shardings=None,
          out_shardings=state_mesh_shardings,
      )()
      if config.pure_nnx:
        state = nnx.State(_train_state)
      else:
        state = _train_state
      if raw_params:  # If we loaded a partial state, we need to merge it.
        if config.pure_nnx:
          ...
          state = _train_state.replace(params=raw_params)

  if config.pure_nnx:
    # Return a TrainStateNNX module, instead of pure state
    train_state = nnx.merge(train_state_graphdef, state)

  else:
    train_state = max_utils.unbox_logicallypartioned(state)

  return train_state, state_mesh_annotations, state_mesh_shardings, data_iterator
