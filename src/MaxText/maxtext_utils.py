...
def maybe_shard(inputs, named_sharding, shard_mode):
  ...
  """
  is_named_sharding = isinstance(named_sharding, jax.sharding.NamedSharding)
  is_single_device_sharding = isinstance(named_sharding, jax.sharding.SingleDeviceSharding)

  if not is_named_sharding and not is_single_device_sharding:
    raise AssertionError(f"Expected NamedSharding or SingleDeviceSharding, found {named_sharding}")
