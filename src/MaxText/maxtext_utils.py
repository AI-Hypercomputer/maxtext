...
def calculate_qwen3_next_ffn_tflops_per_device(config):
  """Helper function to calculate Qwen3-Next MoE FFN TFLOPs.
  Qwen3-Next uses 1 shared expert and N routed experts in every layer.
  """
  gate_flops = 2 * config.per_device_batch_size * config.max_target_length * config.emb_dim * config.num_experts
  num_moe_layers = config.num_decoder_layers
  shared_experts_flops = calculate_ffn_mamtul_tflops_per_device(config, config.moe_mlp_dim) * 1
  routed_experts_flops = calculate_ffn_mamtul_tflops_per_device(config, config.moe_mlp_dim) * config.num_experts_per_tok

  moe_ffn_flops = (gate_flops + shared_experts_flops + routed_experts_flops) * num_moe_layers
  return moe_ffn_flops
...
def calculate_gated_delta_net_flops_per_device(config):
  ...
  flops_intra = 2 * B * S * H_v * C * (2 * D_k + D_v)
