...
def get_vocab_tiling_loss_linen(
  ...
  # sharding_axes = ('data', 'stage', 'fsdp', 'fsdp_transpose', 'sequence', 'context', 'tensor', 'tensor_transpose', 'expert')
  ...
  def _chunked_cross_entropy_loss_fwd(gathered_params, hidden_states, labels, segmentation):
      ...
      initial_loss = 0.0 #jax.lax.pvary(0.0, sharding_axes)
      total_loss, _ = jax.lax.scan(
          fwd_scan_body,
          initial_loss,
          (reshaped_hidden_states, reshaped_labels, reshaped_segmentation)
      ...
      # replicated_loss = jax.lax.psum(total_loss, axis_name=sharding_axes)
  ...
  def _chunked_cross_entropy_loss_bwd(residuals, loss_cotangent):
      ...
      # grad_params = jax.lax.psum(grad_params, axis_name=sharding_axes)
