...
"""Mapping MaxText Deepseek (MoE) weights to intermediate vLLM-like keys."""
...
class DEEPSEEK_VLLM_MAPPING:
  """Mapping MaxText Deepseek (MoE) weights to intermediate vLLM-like keys.
  Source uses explicit 'dense_layers'/'moe_layers'. Target uses generic 'layers.*'.
  """
  ...
  def to_hf_transpose_keys():
    """Returns a dictionary of keys to be transposed in MaxText weights."""
    return {}

  @staticmethod
  def lora_to_hf_mappings():
    """Returns a dictionary of LoRA keys to HF keys."""
    return None

  @staticmethod
  def to_hf_mapping():
    """Mapping from MaxText model to intermediate target format."""
    return {
        ...
        # MLA Norms
        "base.decoder.dense_layers.self_attention.kv_norm.scale": ("layers.*.attn.kv_rms_norm.scale", (None, "layer")),
        "base.decoder.dense_layers.self_attention.q_norm.scale": ("layers.*.attn.q_rms_norm.scale", (None, "layer")),
        # --- Dense MLP ---
        ...
        "base.decoder.moe_layers.post_self_attention_layer_norm.scale": ("layers.*.pre_mlp_norm.scale", (None, "layer")),
        # --- Attention (MLA + Decoupled RoPE) for MoE Layers ---
        # --- Attention (MLA) for MoE layers ---
        ...
        "base.decoder.moe_layers.self_attention.kv_norm.scale": ("layers.*.attn.kv_rms_norm.scale", (None, "layer")),
        "base.decoder.moe_layers.self_attention.q_norm.scale": ("layers.*.attn.q_rms_norm.scale", (None, "layer")),
        # --- DeepSeek MoE Blocks ---
        # Shared Experts
