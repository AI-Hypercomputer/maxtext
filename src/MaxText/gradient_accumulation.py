...
def gradient_accumulation_loss_and_grad(
  ...
  def _maybe_shard(inputs, sharding_names):
    """Wrapper of maybe_shard with fixed shard_mode"""
    return maybe_shard(inputs, sharding_names, config.shard_mode)
  ...
  ga_params = jax.tree.map(_maybe_shard, ga_params, ga_params_shardings)
  ...
  init_grad = jax.tree.map(_maybe_shard, init_grad, grad_shardings)
  init_grad_and_loss = {
      "loss": 0.0,
      "grad": init_grad,
      "total_weights": 0,
  ...
  raw_grads = jax.tree.map(_maybe_shard, raw_grads, params_shardings)
