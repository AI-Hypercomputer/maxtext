...
def gradient_accumulation_loss_and_grad(
  ...
  def accumulate_gradient(acc_grad_and_loss, data):
    ...
    if is_nnx:
      ...
      model = nnx.merge(graphdef, ga_params, acc_grad_and_loss["rest_state"])
      (_, aux), cur_batch_gradient = grad_func(model, config, data, None, None, *extra_dpo_args, is_train=True)
      _, _, next_rest_state = nnx.split(model, nnx.Param, ...)
