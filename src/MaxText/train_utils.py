...
def create_training_tools(config, model, mesh):
  ...
  if config.enable_multi_tier_checkpointing:
    ...
    abstract_state, _, _ = src/MaxText_utils.get_abstract_state(
...
def jit_train_step(
  ...
  if config.compiled_trainstep_file != "":
    ...
    p_train_step = src/MaxText_utils.load_compiled(config, functional_train, state)
...
def jit_eval_step(
  ...
  (
      ...
      static_argnums,
      donate_argnums,
  ) = src/MaxText_utils.get_functional_eval_with_signature(
...
def setup_train_loop(config, recorder, devices=None):
  ...
  with maybe_record_goodput(recorder, GoodputEvent.TRAINING_PREPARATION):
    ...
    state, _, state_mesh_shardings, data_iterator = (
        src/MaxText_utils.setup_training_state(
    ...
    if not config.using_pipeline_parallelism and not config.use_multimodal:
      ...
      src/MaxText_utils.assert_params_sufficiently_sharded(
