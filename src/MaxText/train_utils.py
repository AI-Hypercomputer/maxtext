...
def setup_train_loop(config, recorder, devices=None):
  ...
  with maybe_record_goodput(recorder, GoodputEvent.TRAINING_PREPARATION):
    ...
    train_state, _, state_mesh_shardings, data_iterator = maxtext_utils.setup_training_state(
        data_iterator, config, mesh, checkpoint_manager, init_state_fn
    )
    if config.pure_nnx:
      # train_state is instance of TrainStateNNX
      train_state_graphdef, state = nnx.split(train_state)
    ...
    if config.debug_sharding:
      ...
      maxtext_utils.print_shardings_params(state_params, state_mesh_shardings_params, model.mesh)
