...
class RunInfo(BaseModel):
  ...
  model_name: str = Field("default", description="The name of the model configuration to use.")
...
class Checkpointing(BaseModel):
  ...
  source_checkpoint_layout: str = Field("orbax", description="Optional checkpoint context to use for loading.")
...
class MoEKernels(BaseModel):
  ...
  tile_activation_dim: int = Field(1024, description="Tunable tiling dimension for activation in Megablox.")
  tile_weight_dim: int = Field(1024, description="Tunable tiling dimension for weight in Megablox.")
...
class DatasetGeneral(BaseModel):
  ...
  train_data_columns: list[str] = Field(["text"], description="Columns to use from the training data.")
  train_image_column: str = Field("image", description="Column name for images in the training data.")
  eval_data_columns: list[str] = Field(["text"], description="Columns to use from the evaluation data.")
  eval_image_column: str = Field("image", description="Column name for images in evaluation data.")
  packing: bool = Field(
      True,
...
class MaxTextConfig(
  ...
  def set_derived_values(self) -> "MaxTextConfig":
    ...
    if getattr(self, "decode_sampling_nucleus_p", None) == -1.0:
