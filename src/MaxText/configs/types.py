...
class Attention(BaseModel):
    ...
    enable_padding_causal_mask: bool = Field(
        True, description="Temporary flag for TE padding."
    )
...
class PagedAttention(BaseModel):
    ...
    pagedattn_num_pages: int = Field(
        64, description="Total number of pages to allocate for paged attention."
    )
    pagedattn_tokens_per_page: int = Field(
        32, description="Number of tokens each page can hold."
    )
    pagedattn_pages_per_compute_block: int = Field(
        4, description="Number of pages processed together in pallas kernels."
    )
    pagedattn_max_pages_per_group: int = Field(
        -1, description="Max pages per request; -1 defaults to max_target_length."
    )
    ...
    pagedattn_head_dim_alignment: int = Field(
        128, description="Alignment of head_dim to the nearest multiple."
    )
...
class MoEGeneral(BaseModel):
    ...
    load_balance_loss_weight: NonNegativeFloat = Field(
        0.01, description="Weight for the load balancing auxiliary loss."
    )
    use_custom_sort_vjp: bool = Field(
        True, description="Whether to use a custom sort VJP for sparse matmul ops."
    )
...
class DeepSeekMoE(BaseModel):
    ...
    base_moe_mlp_dim: int = Field(
        7168, description="Intermediate dimension at MoE layer (DeepSeek style)."
    )
    ...
    mlp_bias: bool = Field(
        False, description="Whether to add a learnable bias for MLP matmul."
    )
...
class DcnParallelism(BaseModel):
    ...
    dcn_tensor_transpose_parallelism: int = Field(
        1, description="DCN axis for tensor transpose parallelism."
    )
...
class IciParallelism(BaseModel):
    ...
    ici_tensor_transpose_parallelism: int = Field(
        1, description="ICI axis for tensor transpose parallelism."
    )
...
class TfdsDataset(BaseModel):
    ...
    eval_dataset_name: str = Field(
        "c4/en:3.0.1", description="Name of the TFDS eval dataset."
    )
...
class Rope(BaseModel):
    ...
    rope_max_timescale: int = Field(
        10_000, description="The maximum timescale for global attention RoPE."
    )
...
class StackTrace(BaseModel):
    ...
    collect_stack_trace: bool = Field(
        False, description="Enable periodic stack trace collection."
    )
    stack_trace_to_cloud: bool = Field(
        False, description="Upload stack traces to cloud logging instead of console."
    )
    stack_trace_interval_seconds: int = Field(
        600, description="Frequency of stack trace collection in seconds."
    )
...
class GRPO(BaseModel):
    ...
    loss_algo: str = Field(
        "grpo", description="Loss algorithm, e.g., 'grpo' or 'gspo-token'."
    )
...
class RLDataset(BaseModel):
    ...
    micro_batch_size: float = Field(
        -1, description="Micro batch size for rollout and training."
    )
    ...
    def set_derived_and_validate_values(self) -> "MaxTextConfig":
        ...
        if (
            ...
            tokenizer_path = next(
                filter(
                    ...
                    (
                        os.path.join(
                            MAXTEXT_ASSETS_ROOT, os.path.basename(tokenizer_path)
                        ),
        ...
        if self.shard_mode == ShardMode.EXPLICIT:
            ...
            if self.quantization:
                raise ValueError(
                    "Quantization is not supported with 'explicit' sharding."
                )
        if (
        ...
        if (
            ...
            and self.context_parallel_strategy.lower() == "ring"
        ):
