...
QWEN3_OMNI_IMAGE_TOKEN = 151655  # <|image_pad|> token ID
QWEN3_OMNI_VIDEO_TOKEN = 151656  # <|video_pad|> token ID
QWEN3_OMNI_AUDIO_TOKEN = 151657  # <|audio_pad|> token ID
QWEN3_OMNI_VISION_START_TOKEN = 151652  # <|vision_start|> token ID
QWEN3_OMNI_AUDIO_START_TOKEN = 151654  # <|audio_start|> token ID
...
def get_llm_pos_ids_for_vision(
  ...
  Args:
    ...
    spatial_merge_size: Number of patches merged spatially (e.g., 2 means 2×2 patches → 1 token).
  ...
  Example:
    If spatial_merge_size=2, grid_h=4, grid_w=4, num_frames=2:
      - After merge: 2×2 grid per frame
      - Total tokens: 2 frames × 2 × 2 = 8 tokens
      - Output shape: (3, 8)
  ...
  h_index = (
      jnp.arange(llm_grid_h).reshape(1, -1, 1).repeat(len(t_index), axis=0).repeat(llm_grid_w, axis=2).flatten()
  )

  # Create width indices: [0, 1, 2, ..., W-1, 0, 1, 2, ..., W-1, ...]
  # Shape: (num_frames, 1, llm_grid_w) -> expand -> (num_frames, llm_grid_h, llm_grid_w) -> flatten
  w_index = (
      jnp.arange(llm_grid_w).reshape(1, 1, -1).repeat(len(t_index), axis=0).repeat(llm_grid_h, axis=1).flatten()
  )

  # Create temporal indices: [t0, t0, ..., t0 (H×W times), t1, t1, ..., t1 (H×W times), ...]
...
def get_chunked_index(
    token_indices: jax.Array, tokens_per_chunk: int, remove_index: int
) -> list[tuple[int, int]]:
...
def get_rope_index(
    ...
    audio_seqlens: jax.Array | None = None,
  ...
  For multimodal sequences with vision:
    ...
    - Example with video (3 temporal patches, 2×2 spatial):
  ...
  Args:
    ...
    audio_seqlens: Audio sequence lengths. Shape: (num_audios,).
  ...
  for i in range(batch_size):
    ...
    video_nums = (
        (jnp.sum(vision_tokens == QWEN3_OMNI_AUDIO_START_TOKEN).item() if use_audio_in_video else jnp.sum(vision_tokens == QWEN3_OMNI_VIDEO_TOKEN).item())
    ...
    for _ in range(multimodal_nums):
      ...
      if min_ed == ed_audio_start:
        audio_len = _get_feat_extract_output_lengths(audio_seqlens[audio_idx]).item()
