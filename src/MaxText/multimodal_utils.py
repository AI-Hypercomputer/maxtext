...
def pad_to_max_tiles(images: np.ndarray, max_num_tiles: int = 20) -> tuple[np.ndarray, np.ndarray]:
  """
...
def pre_process_llama4_image(image: np.ndarray | list[np.ndarray]) -> PreprocessorOutput:
  ...
  for img in images_in:
    ...
    image_tiles, image_mask = pad_to_max_tiles(image_tiles, max_num_tiles=20)
...
def get_dummy_image_shape_for_init(model_name, batch_size=1, num_image_per_sequence=1, num_tiles_per_image=20):
