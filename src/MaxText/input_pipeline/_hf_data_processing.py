...
def vision_sft_preprocessing_pipeline(
  ...
  data_source = _input_pipeline_utils.HFDataSource(
      dataset=dataset,
      dataloading_host_index=dataloading_host_index,
      dataloading_host_count=dataloading_host_count,
      num_threads=1,
      streaming=config.hf_data_streaming,
  )
  operations = []
  operations.append(
  ...
  index_sampler = grain.IndexSampler(
      ...
      num_epochs=1 if config.hf_data_streaming else config.num_epoch,
...
def preprocessing_pipeline(
  ...
  index_sampler = grain.IndexSampler(
      num_records=len(data_source),
      num_epochs=1 if streaming else num_epochs,
      shard_options=grain.ShardOptions(
          shard_index=dataloading_host_index, shard_count=dataloading_host_count, drop_remainder=False
      ),
      shuffle=False,
