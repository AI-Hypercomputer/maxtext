...
def pre_process_image_sft(example, image_column, model_name):
  ...
  def _process_image_fn(image):
    if isinstance(image, list):
      output_images = [np.array(multimodal_utils.convert_to_RGB(img)) for img in image]
      image = np.stack(output_images)
    else:
...
class PadOrTrimToMaxLength(grain.MapTransform):
  ...
  def _pad_image_and_mask(
    ...
    max_num_items = (self.max_length - 1) // image_offsets
    ...
    if image_tensor is not None:
      preprocessed_image.pixel_mask = _pad(image_tensor)
...
class ExtractImagesAndMasks(grain.MapTransform):
  ...
  def map(self, element: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    ...
    output = {
        **element,
        "image_masks": preprocessed_image.pixel_mask,
    }
