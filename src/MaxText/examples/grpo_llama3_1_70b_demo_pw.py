...
if num_vms >= 2:
  ...
  llama3_1_70b, MESH = get_ref_maxtext_model(config_ref, jax_devices)
  actor_mesh = MESH
  reference_mesh = MESH
  rollout_mesh = MESH
...
if num_vms >= 2:
  ...
  llama3_1_70b_policy, policy_mesh = get_ref_maxtext_model(config_policy, jax_devices)
...
def generate_responses(
  ...
  for pass_num in range(num_passes):
    ...
    if DEBUG:
      print(f"Pass {pass_num+1}/{num_passes}, responses: {responses}")
...
cluster_config = rl_cluster_lib.ClusterConfig(
    ...
    rollout_vllm_swap_space_size_gb=SWAP_SPACE_VLLM_GB,
)
