...
"""
Thi script performs SFT training and evaluation workflow on OpenAI's GSM8K dataset.
...
REASONING_START = instruction_data_processing.REASONING_START
REASONING_END = instruction_data_processing.REASONING_END
ANSWER_START = instruction_data_processing.ANSWER_START
ANSWER_END = instruction_data_processing.ANSWER_END
# Regex to check the full format (reasoning + answer markers)
MATCH_FORMAT = re.compile(
    rf"^[\s]{{0,}}" rf"{REASONING_START}.+?{REASONING_END}.*?" rf"{ANSWER_START}(.+?){ANSWER_END}" rf"[\s]{{0,}}$",
    flags=re.MULTILINE | re.DOTALL,
)
# Regex to extract the final numerical answer
MATCH_ANSWER = re.compile(rf"{ANSWER_START}.*?([\d\.\,]{{1,}}){ANSWER_END}", flags=re.MULTILINE | re.DOTALL)
          ...
          lambda x: {
              ...
              "prompt": tokenizer.apply_chat_template(
                  [
                      {
                          "role": "user",
                          "content": instruction_data_processing.QA_PROMPT_TEMPLATE.format(question=x["question"]),
                      }
                  ],
                  tokenize=False,
                  add_generation_prompt=True,
              ),
              "target_answer": instruction_data_processing.extract_reasoning_and_answer(x["answer"])[1],
          }
      )
  )
  pipeline = pipeline[:NUM_TEST_SAMPLES]
  return pipeline.to_iter_dataset().batch(BATCH_SIZE, drop_remainder=True)


def evaluate_model(dataset, vllm_rollout):
  ...
  for batch in tqdm(dataset, total=total_batches):
    ...
    for i, question in enumerate(batch["question"]):
      ...
      is_correct, is_partially_correct, has_correct_format = score_response(
          target=batch["target_answer"][i], prediction=batch_response.text[i]
      )
      if is_correct:
        total_correct += 1
...
def score_response(target, prediction):
  is_correct, is_partially_correct, has_correct_format = False, False, False
  extracted_response = guess.group(1) if (guess := MATCH_ANSWER.search(prediction)) is not None else "-1000000"
  try:
