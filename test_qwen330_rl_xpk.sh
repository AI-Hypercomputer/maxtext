...
export RUN_ID="2026-06-15-22-21-28"
export PROJECT_ID="${PROJECT_ID:-cloud-tpu-multipod-dev}" # GCP project ID where the Ironwood cluster is deployed
export BASE_OUTPUT_DIRECTORY="${BASE_OUTPUT_DIRECTORY:-gs://yixuannwang-maxtext-dataset/rl/weightsync}" # GCS bucket path for outputs (e.g., gs://my-bucket/outputs)
export MAXTEXT_CKPT_PATH="${MAXTEXT_CKPT_PATH:-gs://runner-maxtext-logs/qwen3-30b-a3b-base/to_maxtext/scanned/2026-06-15-22-21-28/0/items}" # GCS path of the MaxText checkpoint you want to fine-tune from (e.g., gs://my-bucket/checkpoints/maxtext-ckpt)
export WORKLOAD_NAME="rl-22-21-28-r48"
export DOCKER_IMAGE="gcr.io/tpu-prod-env-multipod/maxtext_post_training_nightly:latest"

export CLUSTER_NAME="${CLUSTER_NAME:-mlperf-v5p}" # Name of the cluster
export ZONE="${ZONE:-europe-west4}" # Zone where your Ironwood cluster is deployed
...
# MaxText command
MAXTEXT_COMMAND="PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python \
...
vllm_additional_config='{"maxtext_config": {"model_name": "qwen3-30b-a3b", "model_call_mode": "inference", "enable_dp_attention": false, "allow_split_physical_axes": true, "log_config": false, "weight_dtype": "bfloat16", "prefuse_moe_weights": true, "scan_layers": true}}' \
&& python3 -m maxtext.inference.vllm_decode \
model_name=qwen3-30b-a3b-base \
load_parameters_path=$BASE_OUTPUT_DIRECTORY/$WORKLOAD_NAME/checkpoints/actor/4/model_params \
vllm_hf_overrides='{"architectures": ["MaxTextForCausalLM"]}' \
...
# Workload Creation
# Workload Creation
rm -rf /tmp/xpk_workspace
mkdir -p /tmp/xpk_workspace
rsync -a --exclude=".git" --exclude=".venv" --exclude="venv" --exclude="maxtext_venv" --exclude="myenv" --exclude="__pycache__" --exclude=".agent" ../maxtext /tmp/xpk_workspace/
rsync -a --exclude=".git" --exclude=".venv" --exclude="venv" --exclude="__pycache__" ../tunix /tmp/xpk_workspace/
find /tmp/xpk_workspace -type l -delete

xpk workload create-pathways \
  --script-dir=/tmp/xpk_workspace \
  ...
  --command="export PYTHONPATH=/app/maxtext:/app/maxtext/src:\$PYTHONPATH && pip install -e /app/maxtext && pip install -e /app/tunix && cd /app/maxtext && ${MAXTEXT_COMMAND}"