...
python3 MaxText/train.py MaxText/configs/base.yml run_name=$RUN_NAME steps=500 dcn_data_parallelism=1 ici_fsdp_parallelism=8\
    metrics_file='saved_metrics.txt' save_period=20 base_output_directory=$OUTPUT_PATH dataset_path=$DATASET_PATH

python3 MaxText/train.py MaxText/configs/base.yml run_name=$RUN_NAME steps=501 dcn_data_parallelism=1 ici_fsdp_parallelism=8\
