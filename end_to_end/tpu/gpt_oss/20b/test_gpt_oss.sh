...
# SCANNED_CKPT_PATH=gs://shuningjin-multipod-dev/gpt-oss-20b/scan-bf16-v2-2025-09-08-06-52-03/0/items
# UNSCANNED_CKPT_PATH=gs://runner-maxtext-logs/2025-09-19-21-02/unscanned/0/items
...
# GOLD=/tmp/golden_data
# gcloud storage cp gs://maxtext-test-assets/golden_data_${MODEL_NAME}.jsonl $GOLD
...
attention=dot_product sparse_matmul=True megablox=True per_device_batch_size=1 max_target_length=4 max_prefill_predict_length=4 dtype=float32 \
...
base_output_directory=${BASE_OUTPUT_PATH} run_name=megablox_pre_training model_name=${MODEL_NAME} \
tokenizer_type=huggingface tokenizer_path=${TOKENIZER_PATH} dataset_type=synthetic \
...
# python3 -m MaxText.train "${MAXTEXT_PKG_DIR:-${MAXTEXT_REPO_ROOT:-$PWD}/src/MaxText}/"configs/base.yml \
# base_output_directory=${BASE_OUTPUT_PATH} run_name=megablox_fine_tuning model_name=${MODEL_NAME} \
# tokenizer_type=huggingface tokenizer_path=${TOKENIZER_PATH} dataset_path=${DATASET_PATH} \
...
base_output_directory=${BASE_OUTPUT_PATH} run_name=megablox_supervised_fine_tuning model_name=${MODEL_NAME} \
...
base_output_directory=${BASE_OUTPUT_PATH} run_name=decode model_name=${MODEL_NAME} \
