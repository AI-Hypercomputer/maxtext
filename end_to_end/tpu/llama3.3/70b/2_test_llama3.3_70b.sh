...
python3 -m MaxText.train MaxText/configs/base.yml model_name=${MODEL_VARIATION} run_name=${FINETUNE_RUN_NAME} base_output_directory=${BASE_OUTPUT_PATH} dataset_path=${DATASET_PATH} tokenizer_path=assets/tokenizer_llama3.tiktoken tokenizer_type=tiktoken steps=10 per_device_batch_size=1 load_parameters_path=${CONVERTED_CHECKPOINT}

# We also test whether the forward pass logits match the golden logits for Llama3.3-70B-Instruct
python3 -m MaxText.tests.forward_pass_logit_checker MaxText/configs/base.yml base_output_directory=${BASE_OUTPUT_PATH} tokenizer_path=assets/tokenizer_llama3.tiktoken load_parameters_path=${CONVERTED_CHECKPOINT} run_name=forward_pass_test per_device_batch_size=1 model_name=${MODEL_VARIATION} max_prefill_predict_length=4 max_target_length=4 dataset_type=synthetic dtype=float32 async_checkpointing=false dtype=float32 activations_in_float32=true matmul_precision=float32 --max_kl_div=1e-4
