...
export LIBTPU_INIT_ARGS=' --xla_tpu_scoped_vmem_limit_kib=98304 --xla_tpu_enable_async_collective_fusion=true \
    --xla_tpu_enable_async_collective_fusion_fuse_all_gather=true --xla_tpu_enable_async_collective_fusion_multiple_steps=true \
    --xla_tpu_overlap_compute_collective_tc=true --xla_enable_async_all_gather=true --xla_tpu_enable_all_experimental_scheduler_features=true \
    --xla_tpu_enable_scheduler_memory_pressure_tracking=true --xla_tpu_host_transfer_overlap_limit=24 \
    --xla_tpu_aggressive_opt_barrier_removal=ENABLED --xla_lhs_prioritize_async_depth_over_stall=ENABLED \
    --xla_tpu_enable_ag_backward_pipelining=true --xla_should_allow_loop_variant_parameter_in_chain=ENABLED \
    --xla_should_add_loop_invariant_op_in_chain=ENABLED --xla_max_concurrent_host_send_recv=100 --xla_tpu_scheduler_percent_shared_memory_limit=100 \
    --xla_latency_hiding_scheduler_rerun=2 --xla_tpu_scheduler_percent_shared_memory_limit=90' 
    
python3 MaxText/train.py MaxText/configs/base.yml per_device_batch_size=2 ici_fsdp_parallelism=-1 remat_policy=full \
    optimizer_memory_host_offload=True gradient_clipping_threshold=0 max_target_length=8192 gcs_metrics=True \
    use_iota_embed=True dataset_path=gs://max-datasets-rogue dataset_type=synthetic reuse_example_batch=1 \
    profiler=xplane sa_block_q=1024 sa_block_q_dkv=2048 sa_block_q_dq=2048 steps=20 enable_checkpointing=false \
    model_name=llama3-70b base_output_directory=gs://runner-maxtext-logs run_name=llama3_1_70b_opt_off
