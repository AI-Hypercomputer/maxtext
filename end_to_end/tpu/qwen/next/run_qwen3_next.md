...
This model implementation supports both **Token Dropping** and **Dropless** strategies for Mixture of Experts routing.

* **Token Dropping**: Enforces a capacity limit per expert. Excess tokens assigned to a busy expert are dropped (skipped) to maintain constant memory and throughput. 
    - Dropping implementation with flag `sparse_matmul=False` and reasonable `capacity_factor`, commonly used from 1 to 1.25.
* **Dropless**: Ensures all tokens are processed by their assigned experts without dropping, prioritizing accuracy.
...
**Configuration Note:**
To enable the efficient Dropless implementation, ensure the `use_tokamax_gmm` flag is configured correctly by overriding the value in base.yml (default is False). 

- [Tokamax ragged_dot](https://github.com/openxla/tokamax/tree/main/tokamax/_src/ops/ragged_dot) implementation with flag `sparse_matmul=True use_tokamax_gmm=True`. 
