...
export MODEL_BUCKET=gs://hengtaoguo-maxtext-logs/checkpoints/gemma3-${MODEL_VARIATION}
...
# python MaxText/convert_gemma3_chkpt.py --base_model_path ${CHKPT_BUCKET}/${MODEL_VARIATION} --maxtext_model_path ${MODEL_BUCKET} --model_size ${MODEL_VARIATION}
...
# export BASE_OUTPUT_DIRECTORY=gs://hengtaoguo-maxtext-logs
...
export OUTPUT_PATH=gs://hengtaoguo-maxtext-logs/1-correctness/gemma3-${MODEL_VARIATION}/${idx}
export UNSCANNED_CKPT_PATH=${BASE_OUTPUT_DIRECTORY}/${RUN_NAME}/checkpoints/0/items
export DATASET_PATH=gs://maxtext-dataset

# python MaxText/train.py MaxText/configs/base.yml base_output_directory=${OUTPUT_DIRECTORY} dataset_path=${DATASET_PATH} tokenizer_path=assets/tokenizer.gemma per_device_batch_size=1 run_name=runner_pretrain_${idx} max_target_length=8192 steps=5 enable_checkpointing=false model_name=gemma2-2b
python MaxText/train.py MaxText/configs/base.yml base_output_directory=${OUTPUT_DIRECTORY} dataset_path=${DATASET_PATH} tokenizer_path=assets/tokenizer.gemma3 per_device_batch_size=1 run_name=runner_pretrain_${idx} max_target_length=8192 steps=5 enable_checkpointing=false model_name=gemma3-$MODEL_VARIATION


# python3 MaxText/train.py MaxText/configs/base.yml model_name=gemma3-4b\
#   steps=15 per_device_batch_size=1 enable_checkpointing=false run_name=$idx\
#   remat_policy=full dataset_path=$DATASET_PATH\
#   max_target_length=8192 base_output_directory=$OUTPUT_PATH profiler=xplane
#   ici_fsdp_transpose_parallelism=256 ici_fsdp_parallelism=-1\
