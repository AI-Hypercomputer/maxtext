...
if [ -z "${BASE_OUTPUT_PATH}" ]; then
    ...
    export BASE_OUTPUT_PATH=gs://hengtaoguo-maxtext-logs/1-correctness/$(date +%Y-%m-%d-%H-%M)
...
python MaxText/train_compile.py MaxText/configs/base.yml model_name=$MODEL_NAME \
    base_output_directory=$BASE_OUTPUT_PATH run_name=compile dataset_path=$DATASET_PATH \
    tokenizer_path=assets/tokenizer_llama3.tiktoken \
    enable_checkpointing=false per_device_batch_size=$PER_DEVICE_BATCH_SIZE \
    save_config_to_gcs=true \
    dump_hlo=true dump_hlo_gcs_dir=$BASE_OUTPUT_PATH/compile/xla_dump
...
python MaxText/train.py MaxText/configs/base.yml model_name=${MODEL_NAME} \
    base_output_directory=$BASE_OUTPUT_PATH run_name=train dataset_path=$DATASET_PATH \
    tokenizer_path=assets/tokenizer_llama3.tiktoken \
    enable_checkpointing=false per_device_batch_size=${PER_DEVICE_BATCH_SIZE} \
    save_config_to_gcs=true steps=2 \
    dump_hlo=true dump_hlo_gcs_dir=$BASE_OUTPUT_PATH/train/xla_dump

# save_config_to_gcs: False # If true, {base_output_directory}/{run_name}/
