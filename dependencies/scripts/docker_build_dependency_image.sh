...
build_tpu_image() {
  if [[ -n "$LIBTPU_GCS_PATH" ]]; then
    docker_build_args+=("LIBTPU_GCS_PATH=${LIBTPU_GCS_PATH}")
    run_docker_build "$MAXTEXT_REPO_ROOT/dependencies/dockerfiles/maxtext_tpu_dependencies.Dockerfile" "${docker_build_args[@]}"
    run_docker_build "$MAXTEXT_REPO_ROOT/dependencies/dockerfiles/maxtext_libtpu_path.Dockerfile" "CUSTOM_LIBTPU=true"
  else
    docker_build_args+=("LIBTPU_GCS_PATH=NONE")
