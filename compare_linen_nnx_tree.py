...
def compare_tree_structures(linen_vars, nnx_state):
    ...
    if shape_mismatches:
        ...
        for path, linen_shape, nnx_shape in shape_mismatches[:10]:
...
def print_tree_structure(paths_dict, name, max_depth=3):
    ...
    tree = {}
    for norm_path, (orig_path, shape, dtype) in paths_dict.items():
...
def main():
    ...
    parser.add_argument("--model", type=str, default="gemma2-2b",
                        help="Model config to use (e.g., gemma2-2b, llama2-7b)")
    parser.add_argument("--depth", type=int, default=4,
                        help="Max depth for tree structure printout")
    parser.add_argument("--verbose", action="store_true",
                        help="Print detailed path information")
    ...
    if linen_only:
        ...
        print("="*60)
        for path in sorted(linen_only)[:20]:
            orig_path, shape, dtype = linen_normalized[path]
            print(f"  {path}: {shape} {dtype}")
        if len(linen_only) > 20:
            print(f"  ... and {len(linen_only) - 20} more")

    if nnx_only:
        print(f"\n{'='*60}")
        print("Paths ONLY in NNX:")
        print("="*60)
        for path in sorted(nnx_only)[:20]:
            orig_path, shape, dtype = nnx_normalized[path]
            print(f"  {path}: {shape} {dtype}")
        if len(nnx_only) > 20:
            print(f"  ... and {len(nnx_only) - 20} more")
