...
# Qwen3-Next-80B on v6e-256 with optimizer state offloaded to host, and XLA's
# rematerialization host-spill DISABLED.
#
# Diffs vs run_qwen3_80b_xpk.sh (AOT-validated before launching -- compiles clean,
# no HBM error):
#   1. REMOVED --xla_tpu_rematerialization_use_host_memory_offload=true
#      That flag is what produced host_temp = 26.24 GiB/step of PCIe traffic.
#      Removing it alone OOMs by 5.37 MB, so it is paired with (2).
#   2. optimizer_memory_host_offload=True  +  scanned_optimizer_update=True
#      Moves 2.47 GiB of f32 AdamW moments (the f32[32,13,2,48,1536] expert
#      tensors dominating the HBM allocation dump) to host. scanned_optimizer_update
#      is REQUIRED: the whole-tree nnx.Optimizer.update path leaves opt_state
#      host-resident and optax fails with a memory_space ValueError. The scanned
#      path runs tx.update per decoder layer under lax.scan with max_utils.to_device
#      on each slice. Verified for adamw/adam_pax -- NOT safe for muon.
#   3. Profiler: profiler_steps=1 and tpu_num_sparse_core_tiles_to_trace=0, so the
#      capture is not truncated. The run-0812030226 trace died at 69% of the step
#      because SparseCore TEC tile tracing emitted 4.82M of 5.0M events.
#
# NOT changed (deliberately): num_moe_token_chunks=2 (chunks=4 costs ~640 MB HBM
# by deepening the ring-of-experts pipeline; chunks=1 overflows SparseCore SPMEM),
# ragged_buffer_factor=1.5 (1.1 is not tile-aligned and fails to compile),
# context=offload, per_device_batch_size=8.
#
# AOT prediction for this config:  argument 3.71 -> 1.24 GiB device,
#                                  host_temp   26.24 -> 13.96 GiB/step.
...
export WORKLOAD_NAME="mohit-qn80b-optoff-${TIMESTAMP}"
export DEVICE_TYPE="v6e-256"
export NUM_SLICES=1
export PRIORITY="very-high"
export MAX_RESTARTS=0
export MODEL_NAME="qwen3-next-80b-a3b"
export BASE_OUTPUT_DIR="gs://runner-maxtext-logs/qwen3-next-80b-profiles/optoff-noflag-${TIMESTAMP}"
...
XLA_FLAGS_ARRAY=(
  ...
  # --xla_tpu_rematerialization_use_host_memory_offload=true  <-- REMOVED
)
...
MAXTEXT_ARGS_ARRAY=(
  ...
  "use_gmm_v2=True"
  "optimizer_memory_host_offload=True"   # <-- CHANGED
  "scanned_optimizer_update=True"        # <-- REQUIRED companion for the NNX path
...
echo "CLOUD_LOGGING_URL=${LOGS_URL}"
