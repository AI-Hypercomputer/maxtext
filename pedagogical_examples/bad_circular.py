...
# w is [layers, features, features]
# x is really state=[stages, micro_batch, features]
def stage(w, x, iter_index):
  def get_circular_index(stage_index, iter_index):
    return (stage_index + iter_index) // n_stages
  
  def get_layer_index(stage_index, iter_index):
    circ_index = get_circular_index(stage_index, iter_index)
    layer_index = circ_index * n_stages + stage_index
    return layer_index

  for stage_index in range(n_stages):
    layer_index = get_layer_index(stage_index, iter_index)
    x = layer(w[layer_index], x)
  return x
...
def spmd_pipeline(w, x):
  ...
  for iter_index in range(n_microbatches + n_stages - 1):
    state = shift_right_and_insert_input(state, x[iter_index])
    state = jax.vmap(stage, in_axes=(0,0,None), out_axes=0,
                     spmd_axis_name='stage')(w, state, iter_index)
    outputs = outputs.at[iter_index].set(state[-1])  # last layer output
...
print("Running one iter of output...", flush=True)
output_pipeline = output_jit(weight1, input)
print("One iter of output succeeded!!!", flush=True)
