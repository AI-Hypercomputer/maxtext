...
def get_paraminfos(tree: PyTree, directory):
...
class SingleReplicaArrayHandler(
  ...
  async def async_save(self,
      ...
      print('===== Seral vals', serial_values)
      print('===== Seral infos', serial_infos)
      print('===== Seral args', serial_args)
...
def main(args):
  ...
  train_state, mesh_tree, axes_tree, _ = setup_sharded_pytree(
    args.tree_size,
    args.mesh_shape
    )
  ...
  param_infos = get_paraminfos(train_state, path)
  values, _ = jax.tree_util.tree_flatten(train_state)
  flat_infos, _ = jax.tree_util.tree_flatten(param_infos)
  print('values',  len(values), values)
  print(print(type(values[0])))
  # print('train_state', len(train_state), train_state)
  # print('params infos', len(param_infos), param_infos)
  print('flat infos', len(flat_infos), flat_infos)
  asyncio.run(handler.serialize(values, flat_infos))
  # asyncio.run(handler.serialize(train_state, param_infos))
  ...
  restore_args, _ = jax.tree_util.tree_flatten(restore_args)

  # print(len(param_infos), len(restore_args))
  # print(restore_args)

  restored = asyncio.run(handler.deserialize(flat_infos, restore_args))
  ...
  # assert jnp.allclose(restored, values, rtol=1e-4, atol=1e-4)
