...
parser.add_argument(
    "--embedding_dimension", "-d", required=False, default=2048, type=int
)
parser.add_argument(
    "--batch_size", "-b", required=False, default=131072, type=int
)
...
def main(_argv: Sequence[str]) -> None:
  ...
  dcn_parallelism = [
      args.dcn_data_parallelism,
      args.dcn_fsdp_parallelism,
      args.dcn_tensor_parallelism,
  ]
  ici_parallelism = [
      args.ici_data_parallelism,
      args.ici_fsdp_parallelism,
  ...
  print(
      f"total {memory_bytes/1e9} GB, parameters {parameter_bytes/1e9} GB, activations {activation_bytes/1e9} GB"
  )
  ...
  def gen_layer(random_key):
    ...
    return {
        "EMB2FF": 1e-4
        * jax.random.normal(keys[0], (D_FF, D_EMB), dtype=jax.numpy.bfloat16),
        "FF2EMB": 1e-4
        * jax.random.normal(keys[1], (D_FF, D_EMB), dtype=jax.numpy.bfloat16),
    }
  ...
  def gen_data(random_key):
    return jax.random.uniform(
        random_key, (BATCH, D_EMB), dtype=jax.numpy.bfloat16
    )
  ...
  jit_gen_layers = jax.jit(
      gen_layers, in_shardings=None, out_shardings=parameter_mesh_shardings
  )
  ...
  with Mesh(mesh.devices, mesh.axis_names):
    ...
    time = simple_timeit(
        lambda: jax.block_until_ready(jit_func(presharded_X, presharded_layers))
    )
    print(
