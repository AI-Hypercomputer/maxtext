...
def gen_data_fn():
  ...
  activations = jax.random.normal(
      key, shape=(batch_size, seq_len, emb_dim), dtype=jnp.bfloat16
  )
  weights = jax.random.normal(
      key, shape=(emb_dim, n_heads, head_dim), dtype=jnp.bfloat16
  )
...
jit_matmul = pjit(
    matmul, out_shardings=P(MESH_FSDP_AXIS, None, MESH_TENSOR_AXIS, None)
)
...
def collective_matmul(activations, weights):
  ...
  def scanned_call(i, carrys):
    ...
    accum = jax.lax.dynamic_update_slice(
        accum, update_forward, (0, forward_update_index, 0, 0)
    )
    accum = jax.lax.dynamic_update_slice(
        accum, update_backward, (0, backward_update_index, 0, 0)
    )
...
with global_mesh:
  ...
  print(
      f"input {activations.shape=} {activations.addressable_shards[0].data.shape=}"
  )
  print(f"input {weights.shape=} {weights.addressable_shards[0].data.shape=}")
  print(
      f"naive_outputs {naive_outputs.shape=} {naive_outputs.addressable_shards[0].data.shape=}"
  )
  print(
      f"collective_outputs {collective_outputs.shape=} {collective_outputs.addressable_shards[0].data.shape=}"
  )

  assert jnp.allclose(
      naive_outputs, collective_outputs
