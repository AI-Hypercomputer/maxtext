...
def main() -> int:
  ...
  for model in ucfg.list_of_models:
    ...
    model.tuning_params["checkpoint_storage_use_ocdbt"] = True
    model.tuning_params["checkpoint_storage_use_zarr3"] = True
  ...
  for model in ucfg.list_of_models:
    for num_slices in ucfg.num_slices_list:
      ...
      wl_config = mxr.WorkloadConfig(
          ...
          device_type=ucfg.cluster_config.device_type,
      ...
      command, name = mxr.generate_xpk_workload_cmd(
          cluster_config=ucfg.cluster_config, wl_config=wl_config
      )
