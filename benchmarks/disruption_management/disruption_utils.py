...
def get_pod_name_from_regex(workload_name: str, pod_regex: str) -> str | None:
  ...
  print(f"Workload '{workload_name}': Getting pod name matching '{pod_regex}'...")
  pod_name_command = [
    "kubectl",
    "get",
    "pods",
  ...
  try:
    ...
    if pod_names:
      ...
      print(f"Workload '{workload_name}': No pod found matching regex '{pod_regex}'.")
  except subprocess.CalledProcessError as e:
    print(f"Workload '{workload_name}': Error getting pod information: {e}")
  return None
...
def wait_for_pod_to_start(workload_name: str, pod_regex: str) -> str | None:
  ...
  print(f"Workload '{workload_name}': Waiting for pod matching '{pod_regex}' to be in 'Running' state...")
  while True:
    pod_name = get_pod_name_from_regex(workload_name, pod_regex)
    if pod_name:
      pod_status = get_pod_status(workload_name, pod_name)
      if pod_status == "Running":
        print(f"Workload '{workload_name}': Step pod '{pod_name}' is now in 'Running' state.")
        return pod_name
    time.sleep(POLLING_INTERVAL_SECONDS)

  print(f"Workload '{workload_name}': Timed out waiting for step pod matching '{pod_regex}' to reach 'Running' state.")
  return None
