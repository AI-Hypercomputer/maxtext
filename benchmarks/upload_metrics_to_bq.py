...
# It is important to parse the last n steps avoiding steps that have tracing turned on,
# by default last_n_steps should be total_steps - 10 to avoid initial tracing settings.
...
def parse_metrics(local_metrics_file, total_steps, last_n_steps=10) -> Metrics:
  """Parses key performance metrics from a MaxText metrics file.

  This function calculates stable performance metrics by averaging over the last
  'n' steps of a run, which helps to exclude initial compilation and warmup
  phases. It also calculates the total end-to-end step time across all steps.

  Args:
    local_metrics_file: The local path to the metrics file, which contains
      JSON objects, one per line.
    total_steps: The total number of steps the training run was configured for.
      This is used to calculate the total end-to-end time.
    last_n_steps: The number of final steps to use for calculating stable
      averages and medians. Defaults to 10.

  Returns:
    A `Metrics` object containing the parsed performance data.
  """
