...
async def _queue_and_wait_for_response(request: Union[CompletionRequest, ChatCompletionRequest]):
    """Puts a request on the queue and waits for the response."""
...
def _build_chat_completion_response(request, completion_result, llm):
    ...
    if "gpt-oss" in request.model and harmony_enc:
        try:
            parsed_messages = harmony_enc.parse_messages_from_completion_tokens(
                completion_result.tokens, role=Role.ASSISTANT
            )
            user_visible = "".join(
    ...
    want_top_logprobs = (request.top_logprobs or 0) > 0 if isinstance(request, ChatCompletionRequest) else (request.logprobs or 0) > 0
    lp_payload = server_utils.to_openai_logprobs(
        getattr(completion_result, "logprobs", None), llm, want_top=want_top_logprobs
    )
    text_out, lp_payload, finish_reason = server_utils.apply_stops_to_text_and_logprobs(
...
def _build_completion_response(request, completions, prompts, llm):
    ...
    for idx, _ in enumerate(prompts):
        ...
        text_out, lp_payload, stop_reason = server_utils.apply_stops_to_text_and_logprobs(
            text_out, lp_payload, request.stop
        )
...
def _prepare_batch_for_broadcast(batched_items):
    ...
    first_request_id, first_request = batched_items[0]
    ...
    params = {
        "max_tokens": first_request.max_tokens, "logprobs": logprobs_param,
        "echo": getattr(first_request, "echo", False), "stop": first_request.stop,
        "temperature": first_request.temperature, "seed": first_request.seed,
        "top_k": first_request.top_k, "top_p": first_request.top_p,
    }
