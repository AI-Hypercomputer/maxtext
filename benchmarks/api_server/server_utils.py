...
"""
This module provides utility functions for the MaxText API server.

It includes helpers for processing requests, formatting responses to be
OpenAI-compatible, handling log probabilities, counting tokens, and managing
debug logging.
"""
...
def log_debug_event(request_id: str, event_type: str, content: dict):
  ...
  try:
    ...
    logger.error("Error writing to debug log file '%s': %s", DEBUG_LOG_FILE, e)
...
def count_tokens(s: str, llm: MaxTextGenerator) -> int:
  ...
  try:
    ...
    logger.warning("Could not count tokens for string '%s...': %s", s[:50], e)
    return 0
