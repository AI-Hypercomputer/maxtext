...
DEFAULT_PROMPT_TEMPLATE = """You are an expert at answering questions based on provided charts.
...
def main(config, local_args):
  ...
  for idx, example in enumerate(tqdm(test_ds, desc=f"Evaluating {hf_path} dataset")):
    ...
    prefill_result, first_token = engine.prefill(
        ...
        image_masks=getattr(processor_output, "pixel_mask", None) if config.use_multimodal and "llama4" in config.model_name else None,
