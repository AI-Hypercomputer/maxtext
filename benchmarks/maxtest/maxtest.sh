...
export DOCKER_IMAGE="gcr.io/tpu-prod-env-one-vm/ran-test-image-1"
export NODEPOOL
export TPU_TOPOLOGY
...
if [[ $STATUS =~ "unchanged"|"created" ]]; then
  echo ""
  echo "Job started, visit https://pantheon.corp.google.com/logs/query;query=resource.labels.pod_name%3D~%22${JOB_NAME}%22;duration=PT1H?project=${GKE_PROJECT} to view logs"
