...
def wait_for_xpk_workloads_completion_async(
    ...
    while completed_count < len(workload_names):
        try:
            ...
            workload_name, return_code = result_queue.get(
...
def _build_args_from_config(wl_config: WorkloadConfig) -> dict:
    ...
    return {
        ...
        * wl_config.num_devices_per_slice
        * wl_config.num_slices,
        "precision": precision,
        "optimizer": optimizer,
...
def generate_xpk_workload_cmd(
    ...
    is_pathways_headless_enabled = (
        wl_config.pathways_config and wl_config.pathways_config.headless
    )
    ...
    wl_config.metrics_gcs_file = os.path.join(
        wl_config.base_output_directory, wl_config.run_name, "metrics"
    )
    ...
    if (
        wl_config.generate_metrics_and_upload_to_big_query
        and not is_pathways_headless_enabled
    ):
        ...
        for k, v in args.items():
            args_str += f"--{k}={v} "
        upload_metrics_to_bq_cmd = (
            f"&& python3 -m benchmarks.upload_metrics_to_bq {args_str}"
        )

    print(f"User command: {user_command}")
    all_xpk_storage = ""
    if wl_config.xpk_storage:
        all_xpk_storage = " ".join(
            f"--storage={storage_test}" for storage_test in wl_config.xpk_storage
        )
