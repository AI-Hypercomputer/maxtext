...
def build_user_command(
    model: maxstar_model_configs.MaxTextModel,
    num_slices: int,
    num_steps: int,
  ...
  install_libtpu_cmd = ''
  if libtpu_type == LibTpuType.V6E_NIGHTLY_WHEEL:
    install_libtpu_cmd += f' pip install *${libtpu_date}*.whl &&'
...
def run_xpk_workload(
  ...
  command, _ = generate_xpk_workload_cmd(
      model, cluster_config, num_slices, libtpu_type, libtpu_version, buffer_size=buffer_size
  )
