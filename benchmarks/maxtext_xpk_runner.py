...
class LibTpuType(enum.Enum):
  """Specifies the source of the libtpu.so library for the workload.

  Attributes:
    NIGHTLY: Use a nightly build of libtpu from the public GCS bucket.
    CUSTOM: Use a custom libtpu.so file. The user must place this file in the
      working directory from which the script is run.
    MAXTEXT: Use the libtpu library that is pre-installed in the MaxText
      base docker image.
  """
...
class PathwaysConfig:
  """Configuration for running workloads with the Pathways backend.

  Attributes:
    server_image: The Docker image for the Pathways server.
    proxy_server_image: The Docker image for the Pathways proxy server.
    runner_image: The Docker image for the main workload runner.
    colocated_python_sidecar_image: The Docker image for the Python sidecar.
    server_flags: Command-line flags for the Pathways server.
    proxy_flags: Command-line flags for the Pathways proxy.
    worker_flags: Command-line flags for the Pathways worker.
    headless: If True, runs in headless mode without a user-specified command.
  """
...
class WorkloadConfig:
  ...
  Attributes:
    model: The MaxTextModel configuration object.
    num_slices: The number of TPU slices to use for the workload.
    device_type: The type of TPU device (e.g., 'v5litepod-256').
    base_output_directory: The base GCS path for storing logs and checkpoints.
    base_docker_image: The name of the base Docker image for the workload.
    libtpu_type: The source of the libtpu library to use.
    libtpu_nightly_version: The specific version for nightly libtpu builds (e.g.,
      '20241201').
    num_steps: The number of training steps to run.
    max_restarts: The maximum number of times XPK should restart a failed job.
    priority: The priority for the XPK workload (e.g., 'medium').
    xpk_path: The local path to the XPK script.
    pathways_config: Configuration for running with Pathways, if applicable.
    run_name: A specific name for the run; if None, one is generated.
    generate_metrics_and_upload_to_big_query: If True, generates metrics and
      uploads them to a BigQuery database.
    hardware_id: The hardware identifier for metrics reporting (e.g., 'v6e').
    metrics_gcs_file: The GCS path where the metrics file will be stored.
    base_config: The path to the base MaxText YAML configuration file.
    topology: The TPU topology, derived in `__post_init__`.
    num_devices_per_slice: The number of devices per slice, derived in
      `__post_init__`.
    db_project: The Google Cloud project for the BigQuery database.
    db_dataset: The BigQuery dataset for storing metrics.
    db_is_test: If True, uses the test BigQuery table instead of production.
    disruption_configs: Configuration for injecting disruptions during the run.
    xpk_storage: A list of GCS buckets to mount as storage for the workload.
    hlo_dump: If True, enables HLO dumping for the workload.
  """
...
def _get_config_tuning_params(wl_config: WorkloadConfig):
  ...
  if is_pw_enabled:
    ...
    print(
        f"Pathways tuning params for model: {wl_config.model.model_name} are:" f" {wl_config.model.pathways_tuning_params}"
    )
...
def _build_args_from_config(wl_config: WorkloadConfig) -> dict:
  """Builds a dictionary of arguments for the metrics upload script.

  Args:
    wl_config: The workload configuration.

  Returns:
    A dictionary of arguments for the `upload_metrics_to_bq` script.
  """
  ...
  if (
    ...
    max_logging.log(f"using steps=({num_steps}) in model convergence test setup")
...
def build_user_command(
  ...
  """Constructs the main command to be executed inside the workload's container.

  This function assembles the complete shell command that sets up the
  environment and runs the MaxText training script.

  Args:
    name: The name of the run.
    wl_config: The workload configuration object.

  Returns:
    A string containing the complete user command.
  """
  is_pw_enabled = wl_config.pathways_config is not None
...
def _get_pathways_proxy_flags(wl_config: WorkloadConfig):
  """Gets the pathways proxy flags for the workload and removes any extras.

  Args:
    wl_config: The workload configuration.

  Returns:
    A string of proxy flags.
  """
...
def _combine_flag_strings(base_flags: str, flags_to_add: str) -> str:
  ...
  Args:
    base_flags: The base string of flags.
    flags_to_add: The string of flags to add.

  Returns:
    A combined string of flags.
  """
...
def _get_pathways_worker_flags(wl_config: WorkloadConfig):
  """Gets the pathways worker flags for the workload and removes any extras.

    Args:
      wl_config: The workload configuration.

    Returns:
      A string of worker flags.
  """
...
def _get_pathways_server_flags(wl_config: WorkloadConfig):
  """Gets the pathways server flags for the workload and removes any extras.

  Args:
    wl_config: The workload configuration.

  Returns:
    A string of server flags.
  """
...
def _get_pathways_specific_flags(wl_config: WorkloadConfig):
  """Assembles the command-line flags specific to a Pathways run.

    Args:
      wl_config: The workload configuration.

    Returns:
      A string containing all Pathways-specific flags for the XPK command.
  """
  pw_config = wl_config.pathways_config
...
def generate_xpk_workload_cmd(
  ...
  """Generates a command to run a maxtext model on XPK.
    ...
    Args:
      cluster_config: The configuration of the target XPK cluster.
      wl_config: The configuration of the workload to run.
      workload_name: An optional name for the workload. If None, a name is
        generated.
      exp_name: An optional experiment name for tracking.

    Returns:
      A tuple containing:
        - The full XPK workload creation command string.
...
def xpk_benchmark_runner(
  ...
  """Orchestrates running a list of benchmark workloads using XPK.

  This function generates commands for each workload, optionally sets up a
  DisruptionManager for fault tolerance tests, and then starts the workloads
  asynchronously.

  Args:
    cluster_config: The configuration of the target XPK cluster.
    workload_configs: A list of workload configurations to run.
    disruption_manager: An optional DisruptionManager to handle disruptions.
    exp_name: An optional experiment name for tracking.

  Returns:
    The DisruptionManager instance, which may contain active disruptions.
  """
  xpk_workload_names = []
  xpk_workload_cmds = []
...
def on_device_benchmark_runner(
  ...
  """Runs a list of workloads directly on the current device, without XPK.

  This is useful for local testing or environments where XPK is not available.
  ...
  for wl_config in workload_configs:
    ...
    print(f"User command: {user_command}")
    subprocess.run(user_command, shell=True, text=True, check=False)
...
def main() -> int:
  """Main entry point for running benchmarks from the command line.
    ...
    This function defines example configurations and demonstrates how to launch
    workloads programmatically. It can be customized to run different models,
    clusters, and configurations.

    Returns:
        An exit code, `os.EX_OK` on successful completion.
  """
