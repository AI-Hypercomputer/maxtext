...
from benchmarks.convergence.c4_exp import e2e_model_dict
...
def add_xpk_runner_arguments(custom_parser: argparse.ArgumentParser):
  ...
  custom_parser.add_argument(
      '--model_name',
      type=str,
      choices=list(trillium_model_dict.keys()) + list(v5p_model_dict.keys()) + list(v5e_model_dict.keys()) + list(e2e_model_dict.keys()),
...
def main() -> None:
  ...
  duplicates = (trillium_model_dict.keys() & v5p_model_dict.keys() & v5e_model_dict.keys() & e2e_model_dict.keys())
  ...
  if model is None:
    model = e2e_model_dict.get(options.model_name)
  libtpu_type = None
