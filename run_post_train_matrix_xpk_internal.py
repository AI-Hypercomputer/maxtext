...
def last_error(log_path):
  ...
  last_marker = max((i for i, ln in enumerate(raw) if any(m in ln for m in chain_markers)), default=-1)
  after_marker = [ln for ln in raw[last_marker + 1 :] if any(w in ln for w in wanted) and not any(n in ln for n in noise)]
  lines = after_marker or [ln for ln in raw if any(w in ln for w in wanted) and not any(n in ln for n in noise)]
